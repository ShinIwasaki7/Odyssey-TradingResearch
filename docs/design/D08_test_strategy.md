...
| 各契約の意味論（何が正しい振る舞いか） | D02〜D07 の各節 | 参照のみ。本書は**どのテストがどの契約行を押さえているか**の対応だけを書く（第4〜8節） |
...
| 2 | 意味論テストの対応 | D02〜D07 の各文書が挙げた**意味論の契約行**と実在するテストの対応、および未整備の名指し（第4〜8節） | **契約行そのものの内容**（D02〜D07 が正本）。段階3 の契約行（待機・追い越し・後続確認）との対応（**段階3・D05 v0.2 の承認後に本書 v0.2**） |
...
| D02 §11・D03 §11・D04 §12・D05 §11・D06 §11・D07 §11 | 各文書が挙げたテストの種別と項目（第4〜8節の対応表の左列） |
