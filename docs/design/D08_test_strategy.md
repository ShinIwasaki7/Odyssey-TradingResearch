...
作成日: 2026-09-22
状態: **v0.1（起草、承認待ち）**。全体計画書 §8.4「テスト戦略（D08 で具体化）」を、**段階1〜2 で実際に書かれたテスト資産を正本として**具体化する。新しいテストを書く文書ではなく、**すでにあるものを記述し、足りないところを名指しする**文書である。第14節に要決定3件、第13節に未整備20件の一覧を置く。
...
対応段階: 段階1〜。段階3（遅延シナリオ4ケース・検証戦略 B）の拡張方針は第9.5節に置き、確定は D05 v0.2 の承認後。

## 0. 本書の位置付けと凡例

テストの**層をいくつ持ち、どの層に何を書き、どこに置き、どう名付けるか**を決める。個々のテストの中身は決めない（それは各設計文書の契約が決める）。

本書の特徴は、**記述の根拠を「設計文書がこう書いたから」ではなく「実際に `tests/` にこう書かれているから」に置く**ことである。段階1〜2 の実装（PR #14〜#20）で 1,100 件を超えるテストが既に書かれており、そこに現れた規約が事実上の正本になっている。本書はそれを言葉にし、**設計文書の契約と実際のテストの対応が切れている箇所を名指しする**（第8節）。
...
| 【提案】 | 本書が推奨する設計。承認で確定 |
| 【要決定】 | 承認時にユーザーが選択する事項。**3件**（第14節） |

## 1. 責務と境界

| 項目 | 内容 | 印 |
|---|---|---|
| 提供するもの | テストの層の責務・命名・書き分けの判断（第2〜3節）、意味論テストと契約行の対応表（第4〜8節）、人工データ生成仕様（第9節）、golden の運用（第10節）、決定論の検証（第11節）、CI の並び（第12節） | 【提案】 |
...
| 事項 | 正本 | 本書の扱い |
|---|---|---|
| テストの配置（ディレクトリと層の対応） | **D01 §9**（本書は決めない） | 参照のみ。本書 §2.1 は既存の7層を**記述**するだけで、置き場所を新たに定めない。**D01 §9 に行が無い2層**（統合・受入）を第14節 Q2 で問う |
| 依存規則の契約名（L1・L2a〜L2c・F1a〜F8） | D01 §6 | 参照のみ。検査の置き場所だけを第7節で決める |
| 各契約の意味論（何が正しい振る舞いか） | **上位設計書 §7.2・§4.7.15 E**（起点）と、それを受けた D02〜D07 の各節 | 参照のみ。本書は**どのテストがどの契約行を押さえているか**の対応だけを書き、契約を足さない（第4〜8節） |
| trace の15表・保存形式・平坦化の規則 | D06 §9.1・§9.2 | 参照のみ。golden の期待値の形式はこれに従う（第10節） |
| 結果ダイジェストと決定論の条件 | D07 §9 | 参照のみ。検証の仕方だけを第11節で決める |
| 期間のアクセス分類 | ADR-0014・D03 §3.8 | 参照のみ。**人工データの日付がこの表の外にある**ことを第9.5節で挙げる（第14節 Q3） |
...
|---|---|---|---|
| 1 | 層の定義 | 7層（単体・意味論・プロパティ・golden・統合・受入・依存規則）の**責務・命名・どの層に書くかの判断**（第2.2節・第3節）。**置き場所（ディレクトリと層の対応）の正本は D01 §9 であり、本書は決めない**。本書 §2.1 は既存の7層を記述し、D01 §9 に行が無い2層を第14節 Q2 で問う | **置き場所の正本を7層に広げること**（D01 §9 の改訂。Q2 で選択肢1 が選ばれた場合に同じ PR で行う）。性能テスト・負荷テスト・変異テストを層として持つか（**本書 v0.2・段階4以降**） |
| 2 | 意味論テストの対応 | **棚卸しの起点である上位設計書 §7.2（7項目）と §4.7.15 E（8項目）**、および D03〜D07 の各文書が挙げた**意味論の契約行**と、実在するテストの対応、および未整備の名指し（第4〜8節）。**下位文書に再掲されていない上位の契約も対象内**である（D01 §9・全体計画 §8.4 が意味論層の対象を上位2節と定義しているため） | **契約行そのものの内容**（上位設計書と D02〜D07 が正本。本書は対応を書くだけで契約を足さない）。段階3 の契約行（待機・追い越し・後続確認・再発火）との対応（**段階3・D05 v0.2 の承認後に本書 v0.2**）。評価窓外の採点と選定（**段階5・D09**） |
...
**D01 §9 のテスト配置表には、統合（`tests/integration/`）と受入（`tests/acceptance/`）の行が無い**【要決定 Q2】。この2層は段階2 の実装で必要になって追加されたもので、D01 §9 は6行のままである。本書はこの2層を**既にある事実として記述**し、D01 §9 の改訂の要否を第14節 Q2 で問う。
...
この1行が**第4〜8節の対応表の機械可読な根拠**になる。意味論テストに出どころの1行が無いものは、対応表で追えないため**未整備として扱う**【提案】。他の層の docstring は任意とし、性質の定義や手計算の根拠を書きたいときに書く。
...
| 状態 | 意味 |
|---|---|
| **対応あり** | `tests/semantics/` に、その契約行を押さえるテストが実在する |
| **未整備（層違い）** | テスト自体は実在するが、**意味論以外の層にある**（単体・統合など）。契約行としての固定になっていない |
| **未整備（無し）** | その契約行を押さえるテストが見当たらない |
...
| # | 契約行（上位 §7.2） | 状態 | テスト |
|---|---|---|---|
| 1 | 将来のデータを追加しても、過去時点の部品出力・意思決定が変わらない | **未整備（無し）** | **市場データの見え方までは対応あり**: `tests/property/marketdata/test_asof_properties.py::test_adding_future_bars_does_not_change_the_latest_available`、`::test_every_returned_bar_was_already_available`。**部品の出力と意思決定まで通した先読み不変のテストが無い**（同じ run を、未来の足を足した入力でもう一度回して trace が一致することを確かめるもの） |
...
7項目のうち**対応あり3件、一部が対象外2件、未整備（無し）2件**（#1 の部品出力まで通した先読み不変、#5 の gap）。

**D02（共通カーネル）には意味論の行が無い**【合意済み】D02 §11。同書のテストは単体・プロパティ・アーキテクチャの3種で、値型と演算規則しか持たないため意味論の対象になる「複数部品をまたぐ振る舞い」が存在しない。これは欠落ではなく設計どおりである。対応は第7節（層ごとの置き場所）で示す。
...
**`tests/semantics/` に D04 に対応するファイルが無い**。3行すべて未整備（層違い）である。

| # | 契約行（D04 §12） | 状態 | 実在するテスト（別の層） |
|---|---|---|---|
| 1 | 未宣言キーの拒否 | **未整備（層違い）** | `tests/unit/strategy/test_declarations.py`（33件）に含まれる。例: `::test_an_unregistered_data_type_is_rejected`、`::test_an_allowed_input_event_must_name_a_delivered_event_input` |
| 2 | 役割の型不一致の拒否 | **未整備（層違い）** | `tests/unit/strategy/test_compiler.py`（33件）に含まれる |
| 3 | 能力検査の各拒否が理由コード付きで返ること | **未整備（層違い）** | `tests/unit/marketdata/test_schedule.py::test_a_seeded_random_delay_is_rejected_by_the_capability_check` ほか |
...
| 2 | 受付と約定の原子性 | **未整備（無し）** | 見当たらない。近いのは `tests/semantics/backtest/…::test_a_failed_run_records_the_ledger_state_after_the_cancellations`（失敗時の台帳）だが、**1判断時点の中で受付と約定が分割不能であること**は押さえていない |
| 3 | 同じ `event_id` の再配送 | **未整備（無し）** | `tests/integration/strategy/test_strategy_a_trace.py::test_a_batch_is_not_processed_twice` がバッチ単位の重複を押さえるが、**`event_id` 単位ではなく、層も統合**である |
| 4 | 終端後の別イベント | **未整備（層違い）** | `tests/semantics/strategy/test_opportunity_transitions.py::test_nothing_is_accepted_after_the_run_end_batch`、`::test_an_admission_notice_for_an_unknown_opportunity_is_rejected` が**戦略側**にある。バックテスト側の意味論としては無い |
| 5 | 予約移管の二重計上 | **未整備（層違い）** | `tests/unit/backtest/test_portfolio.py::test_a_transferred_reservation_is_not_counted_twice`、`::test_the_consumed_budget_counts_held_reservations`（**単体層**） |
| 6 | 期限と始値の同時刻 | **未整備（無し）** | `tests/unit/backtest/test_orders.py::test_transition3_expiry_records_the_reason` は期限切れそのもの。**期限到来と候補の始値が同時刻に並んだときの順序**は押さえていない。D06 §5.1 は「遷移3 はどの設定でも発生しない」と書いており、§6.2 の表現可能性テストの形で書くのが筋 |
...
**9行のうち対応ありは5行**。未整備（無し）3行（#2・#3・#6）、未整備（層違い）2行（#4・#5）。
...
**`tests/semantics/evaluation/` が存在しない**。5行すべて未整備（層違い）である。テストは実在し、`tests/unit/evaluation/test_evaluate_run.py`（51件）にある。

| # | 契約行（D07 §11） | 状態 | 実在するテスト（単体層） |
|---|---|---|---|
| 1 | `REJECTED` の run で指標を出さない | **未整備（層違い）** | `tests/unit/evaluation/test_evaluate_run.py::test_a_run_that_did_not_complete_is_rejected_without_metrics` |
| 2 | 致命検査の不合格で指標を出さず検査表だけを出す | **未整備（層違い）** | 同ファイル `::test_a_missing_table_is_a_fatal_check_not_an_exception`、`::test_a_trade_count_mismatch_is_fatal`、`::test_a_broken_id_chain_is_fatal`、`::test_a_close_side_chain_break_is_fatal` |
| 3 | 必須列が欠けた表と0行の表を区別する | **未整備（層違い）** | 同ファイル `::test_a_missing_column_is_distinguished_from_an_empty_table` |
| 4 | 0件の鍵が行として出る | **未整備（層違い）** | 同ファイル `::test_every_category_key_gets_a_row_even_at_zero`、`::test_the_rejection_categories_split_entry_from_close` |
| 5 | 通貨違いを拒否する | **未整備（層違い）** | 同ファイル `::test_a_foreign_currency_column_is_fatal`、`::test_a_foreign_ledger_currency_is_reported_not_raised` |

**5行すべてにテストは存在する**。欠けているのは層の配置と契約行との1対1の対応だけである。
...
**ここに食い違いがある**【要決定 Q3】。ADR-0014 のアクセス分類の表は **2016〜2023 = 研究履歴 / 2024〜2025 = 旧基盤の封印 / 2026年分 = 未分類の隔離**の3行で、**2016年より前の期間について何も言っていない**。T01 再現生成器の 2015年のデータは、受入テストの中で snapshot の partition を**研究履歴として作って**通している（`tests/fixtures/synthetic/snapshots.py` が `AccessClass.RESEARCH_HISTORY` を使う）。分類の表に無い期間を研究履歴として扱ってよいかは、どの文書も決めていない。

### 9.6 段階3 の遅延シナリオ4ケースへの拡張方針【提案】
...
3. **比較の置き場所は意味論層**【提案】。1つの run では遅延シナリオ別の比較ができない（run manifest は `delay_scenario_ref` を1件しか持たない。D07 §6.3）ため、4ケースそれぞれで run を回して**4つの trace を1つのテストの中で突き合わせる**。golden にはしない（4ケース分の固定出力は差分が読みにくく、遅延が変わるたびに4本とも書き換わる）。

段階3 の着手時に、この3点を D08 v0.2 として確定させる。
...
| 区分 | 件数 | 内訳 |
|---|---|---|
| **未整備（無し）** | 3 | D06 §11 の #2 受付と約定の原子性、#3 同じ `event_id` の再配送、#6 期限と始値の同時刻 |
| **未整備（層違い）** | 10 | D04 §12 の3行すべて（単体層にある）、D06 §11 の #4 終端後の別イベント（戦略側の意味論層）・#5 予約移管の二重計上（単体層）、D07 §11 の5行すべて（単体層にある） |
...
| 4 | D01 §9 のテスト配置表に統合（`tests/integration/`）と受入（`tests/acceptance/`）の行が無い | D01 §9（第14節 Q2） |
| 5 | D03 §11 の意味論の行が実態より少ない（承認済み snapshot の関門 27件が契約行に挙がっていない） | D03 §11・§3.7.1 |
...
**未整備は合計 20件**（意味論の対応 13件、その他 5件、上位由来 2件）。

## 14. 要決定【要決定】

### Q1. 意味論の契約行と実際のテストの層が食い違う10件をどう揃えるか

**決めること**: D04 の3行と D07 の5行、D06 の2行（計10行）は、テスト自体は存在するが `tests/semantics/` に無い。これを「テストを移す」で揃えるか、「契約行の側の言い方を変える」で揃えるか。**結果への影響**: 前者はファイルの移動と import の整理が要る（約90件のテストの置き場所が動く）。後者は設計文書の言葉が変わるだけで、テストは1件も動かない。

| # | 選択肢 | 影響 |
|---|---|---|
| **1（推奨）** | **契約行の側を「層は問わず、名前付きで固定されていればよい」に読み替え、本書の対応表を正本にする**。テストは動かさず、第5〜7節の表で「どの契約行をどのテストが押さえているか」を追えるようにする | テストは1件も動かない。D04・D06・D07 の §11 の文言は変えず、本書が対応の正本になる。**契約行と層の対応が緩むため、新しいテストをどの層に書くかは第2.2節の判断に委ねる** |
| 2 | `tests/semantics/evaluation/` と `tests/semantics/strategy/` へ該当するテストを**移す** | 契約行と層が1対1で揃い、意味論層を見れば契約の充足が分かる。約90件の移動と `import` の書き換えが要り、`git blame` が切れる。移動だけの PR を1本立てる |
| 3 | **現状維持**（対応表も作らない） | 何も動かないが、契約行の充足を人が毎回探すことになる。全体計画 §8.4 が求める「意味論テスト一覧」が無いままになる |

**推奨理由**: 契約行を押さえるテストは**実在して緑になっている**ので、検証の中身は足りている。足りないのは「どれがどれを押さえているか」の索引であり、それは本書の表で埋まる。90件を移す作業は履歴を切る割に、得られるのは同じ索引である。

### Q2. D01 §9 のテスト配置表に統合層と受入層の行を足すか

**決めること**: `tests/integration/` と `tests/acceptance/` は段階2 の実装で追加され、いま 4ファイル・80件がある。D01 §9 の表は6行のままで、この2層を知らない。**結果への影響**: D01 は承認済みの設計文書なので、行を足すなら改訂が要る。

| # | 選択肢 | 影響 |
|---|---|---|
| **1（推奨）** | **D01 §9 に2行を足す改訂を、本書の承認と同じ PR に含める** | テストの配置の正本が D01 §9 に一本化され、本書は責務と命名だけを足す形になる。D01 が v1.1 になる |
| 2 | **本書にだけ書き、D01 §9 は次の改訂でまとめて直す** | 本 PR が小さく収まるが、**テストの配置の正本が2か所に分かれる**期間ができる（D01 §9 に6行、本書 §2.1 に7行） |
| 3 | **統合層を廃し、単体と意味論に吸収する** | D01 §9 を変えずに済むが、T01 の8経路（21件）と CLI の経路（26件）の置き場所が無くなる。どちらも1つの型では閉じないので意味論に寄せることになり、意味論層が「契約行と1対1」でなくなる |

**推奨理由**: 配置の正本が2か所にあると、次にテストを足す人がどちらを見ればよいか分からない。行を2つ足すだけの改訂なので、本書の承認と一緒に閉じるのが安い。

### Q3. 人工データの日付（2015年）をアクセス分類の表にどう収めるか

**決めること**: ADR-0014 のアクセス分類は 2016〜2023 / 2024〜2025 / 2026年分 の3行で、**2016年より前について何も言っていない**。T01 再現生成器は 2015年1月のデータを作り、受入テストはその partition を研究履歴（`RESEARCH_HISTORY`）として snapshot に載せて通している。**結果への影響**: 分類の表に無い期間を研究履歴として扱ってよいかが決まっていないと、実データで同じことをしたときに歯止めが無い。

| # | 選択肢 | 影響 |
|---|---|---|
| **1（推奨）** | **人工データはアクセス分類の対象外と本書で明記し、ADR-0014 は変えない**。「人工データの snapshot は常に研究履歴として作る。実データの分類は ADR-0014 の表だけが決める」と書く | ADR は変わらない。人工データと実データで分類の決め方が違うことが明文になる。**実データに 2015年以前が現れたときは別途決める**必要が残る（そのときに Q3 をもう一度開く） |
| 2 | **ADR-0014 の表を「2016年より前 = 研究履歴」まで広げる** | 人工データも実データも同じ規則で決まる。ただし**手元に無い期間について先に決める**ことになり、その期間のデータを将来入手したときに「未観測だったか」を確かめないまま研究履歴になる |
| 3 | **人工データの日付を 2016年以降へ動かす** | 表の中に収まる。T01 の日付をもう一度動かすことになり、**曜日の並びと冬時間の条件が同じ年**を選び直す必要がある（2015年を選んだ理由と同じ作業）。T01・受入テスト・本書の記述が連動して変わる |

**推奨理由**: 人工データは観測されたデータではないので、「未観測の期間を誤って研究に使わない」という分類の目的にそもそも当たらない。表を広げる（選択肢2）と、その目的が薄まる。日付を動かす（選択肢3）と、2026-09-22 に決めたばかりの T01 の日付をまた動かすことになる。

## 15. 承認後にすること

1. 第14節の Q1〜Q3 の決定を本文へ反映し、本書を v1.0 とする。Q2 で選択肢1 が選ばれた場合は、**同じ PR で D01 §9 を改訂する**。
2. **第9.3節の2つの入り口（gap・SL/TP 同時到達）を実装する**。本書の承認後の別 PR で、`tests/fixtures/synthetic/market.py` に足し、上位 §7.2 #5 の手計算検証を1件ずつ書く。
3. **上位 §7.2 #1 の先読み不変テストを書く**（未来の足を足した入力で同じ run を回し、trace が一致すること）。第13.3節 #1。
4. 第13節の残りの未整備を、埋める順に並べて全体計画 §10 の「次のアクション」へ渡す。
5. 段階3 の着手時に、第9.6節の3点を D08 v0.2 として確定させる（遅延シナリオ4ケースの生成器の入り口・比較の置き場所）。
