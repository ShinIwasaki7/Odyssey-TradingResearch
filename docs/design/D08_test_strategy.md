...
v1.5（2026-09-25、段階3 実装 PR 5/5）: 第9.6節の方針3（遅延4ケースの trace を1つの意味論テストで突き合わせる）を使ったことと、第13.2節の #6（有効性の再検査の4区分）が実装済みになったことを記録した（未整備 5件 → 4件）。規約と本文の方針は変えていない。
...
この4点は段階3 の着手時（2026-09-24、段階3 実装の PR 分割の承認）に確定した。1・2・4 は段階3 実装 PR 1/5 で実装し、3 は遅延4ケースの突き合わせを書く PR 5/5 で使う。**3 は段階3 実装 PR 5/5 で使った**（v1.5。`tests/semantics/backtest/test_delay_scenarios.py`。4ケースの run は `tests/fixtures/acceptance/t02_run.py` が同じ素の足から組む）。
...
**未整備は合計 11件**（意味論の対応 3件、その他 6件、上位由来 2件）。起草時は20件だったが、**Q1 の決定で10件が「充足」に変わった**ため 10件になり、**2026-09-23 の Q24 の決定で1件（その他 #6）が加わって 11件**になった。テストが増えたわけではない。**v1.3 で3件（その他 #3、上位由来 #1・#2）を実装したので、残る未整備は 8件**（意味論の対応 3件、その他 5件）。**v1.4 で意味論の対応3件を実装したので、残る未整備は 5件**（その他 5件）。**v1.5 でその他 #6 を実装済みと記録したので、残る未整備は 4件**（その他 4件）。実装済みの行は経緯を追えるように表に残している。
