...
v1.8（2026-09-25、PR #R1PR）: 設計文書の必須表（全体計画書 §8.5。R1（PR #26 承認））に合わせ、第1.2節に「状態×出来事表・値の伝播表: 該当なし（理由）」を 1 段落足した。本文の規則は変えていない。
...
**未整備は合計 11件**（意味論の対応 3件、その他 6件、上位由来 2件）。起草時は20件だったが、**Q1 の決定で10件が「充足」に変わった**ため 10件になり、**2026-09-23 の Q24 の決定で1件（その他 #6）が加わって 11件**になった。テストが増えたわけではない。**v1.3 で3件（その他 #3、上位由来 #1・#2）を実装したので、残る未整備は 8件**（意味論の対応 3件、その他 5件）。**v1.4 で意味論の対応3件を実装したので、残る未整備は 5件**（その他 5件）。**v1.6 でその他 #6 を実装済みと記録したので、残る未整備は 4件**（その他 4件）。実装済みの行は経緯を追えるように表に残している。
