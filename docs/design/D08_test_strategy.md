...
| 3 | 人工データ | 2つの生成器の役割分担、**現に実装されている入力と不変条件の記述**（第9.1〜9.2節）、**未実装の2条件（gap・SL/TP 同時到達）の入り口と不変条件の仕様**（第9.3節。実装は後続 PR）、日付の扱い（第9.5節）、段階3 の遅延シナリオ4ケースへの**拡張方針**（第9.6節） | **遅延シナリオ4ケースの具体的な中身**（**段階3・D05 v0.2 と D03 §3.6**）、実データを使うテストのマーカー設計（**段階4**。v1.8 で第12節に決めた）、第9.3節の仕様の**実装そのもの**（本書の承認後の別 PR） |
...
| 6 | CI | テストの実行の並びと前提（第12節）。既存の `.github/workflows/ci.yml` の記述 | CI の並列化・キャッシュ・実行時間の目標（**本書 v0.2**）、実データを使うジョブの分離（**段階4**。v1.8: 段階4 では実データのジョブを CI に置かない。第12節） |
...
### 2.3 段階4 の受入テストの規約【提案】（v1.8、D07 v2.0 §17.1・§21.4）
...
**v1.8（D07 v2.0）で足した5行**。D07 v2.0 §17.1 が段階4 の意味論の行を5行足した。いずれも段階4 の実装 PR で書くので、本書の承認時点では**未整備**である（第13.1節）。既存の5行の意味は変わらない（#2 の致命検査に C9・C10 が、#5 の通貨の検査に費用の列が加わるだけで、既存のテストは PR 1 で追随する）。
...
### 10.5 評価の golden を指標集合 v2 へ上げる方針【提案】（v1.8、D07 v2.0 §5.5・§17.2）
...
2. **更新は独立したコミット**にし（第10.3節）、メッセージに「D07 v2.0 の指標集合 v2 への更新（第10.2節の2）」と、変わった値の要約を書く。予定される差分は次のとおりで、**これ以外の差分が出たら実装のバグ**として扱う（第10.2節の1）。
   ...
   - `consistency_checks.tsv`: 列 `passed` が `outcome` になり、C9・C10 の2行が足される
...
**実データを使うテストは段階4以降に別マーカーで分ける**【合意済み】D01 §9。段階2 の時点でマーカーが1つも無いのは、**実データを使うテストがまだ1件も無いから**であり、欠落ではない。マーカーの名前と既定の除外設定は、実データを使う最初のテストと同じ PR で決める【提案】。→ **v1.8 で決めた**【提案】: マーカーは **`realdata`**、`pyproject.toml` の pytest 設定の既定（`addopts = "-m 'not realdata'"`）で除外し、CI では実行しない。実行するときは `uv run pytest -m realdata` を明示する。導入は段階4 の実装 PR 4（D07 §17.2）で、マーカーを使う最初のテストと同じ PR である。実データのテストは承認済みの snapshot（`data/snapshots/<id>/` の実体）が無い環境では**失敗ではなくスキップ**にする（実体は git 管理外であり、無いのは環境の事実であって検査の不合格ではない）。
...
### 13.1 意味論テストの対応（0件。v1.8 で段階4 の5件を追加）
...
| **未整備（段階4 で追加、v1.8）** | 5 | D07 v2.0 §17.1 の #6〜#10（第7.2節の後半の表）。段階4 の実装 PR 1・3 で埋める。設計が先に決まり、実装が後に続く順序（CLAUDE.md）による未整備であり、既存の対応の欠落ではない |
...
**未整備は合計 11件**（意味論の対応 3件、その他 6件、上位由来 2件）。起草時は20件だったが、**Q1 の決定で10件が「充足」に変わった**ため 10件になり、**2026-09-23 の Q24 の決定で1件（その他 #6）が加わって 11件**になった。テストが増えたわけではない。**v1.3 で3件（その他 #3、上位由来 #1・#2）を実装したので、残る未整備は 8件**（意味論の対応 3件、その他 5件）。**v1.4 で意味論の対応3件を実装したので、残る未整備は 5件**（その他 5件）。**v1.6 でその他 #6 を実装済みと記録したので、残る未整備は 4件**（その他 4件）。**v1.8 で段階4 の意味論の行5件（第13.1節）を足したので 9件**（意味論の対応 5件、その他 4件）。実装済みの行は経緯を追えるように表に残している。
