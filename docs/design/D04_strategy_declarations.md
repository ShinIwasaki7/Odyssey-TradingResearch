...
| 10 | **待機・遡りと読み方・接続元の組合せが許可されたものであること**（v1.9）: `WaitForInput` は `LatestAvailable` と `HistoryWindow` に、`UsePrevious` は **`LatestAvailable` かつ接続元が市場データ参照のときにだけ**書ける【合意済み】上位設計書 §4.3.13・D05 §6.9（最新1件の読み方が保持するのは最新1件だけであり、履歴を持つのは履歴窓で読む相手がいる出力参照だけなので、出力参照の遡りは許さない） | `InputSpec.read_spec`、`MissingInputPolicy`、`InputBinding.sources` |
