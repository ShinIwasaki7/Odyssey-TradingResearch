...
**必須表（全体計画書 §8.5、R1）**: 境界表は上の表である。**状態×出来事表・値の伝播表: 該当なし**。理由: 本書は宣言の形だけを決め、取引機会の状態機械（行4）、部品状態の更新規則（行6）、評価要求と実行時の識別子（`opportunity_id` の採番など。行4・行5・行10）は D05・D06 が決める。再武装（第10.4節）が宣言する「直前の評価で成立していたか」も、宣言するのは状態の型と初期値だけで、更新規則は D05 が持つ（行6）。第13.2節のダイジェストは宣言の内容から計算する値であり、実行時に部品をまたいで運ぶ規則は D05・D06 の値の伝播表が扱う。
...
本書が追加するのは保存形式の版（スキーマ版）だけである（Q1 決定、選択肢1）【合意済み】: 3クラスそれぞれのトップレベルに `schema_version: int`（`>= 1`）を置く。**版は保存形式が変わったときに上げる。段階3 では 1 のまま（2026-09-24 決定）**。v1.9（2026-09-22）は第6.3節に欠損方針の2区分（入力を待つ・過去値へ遡る）を足したときに 2 へ上げると書いたが、上げると検証戦略 A を含むすべての契約・戦略の指紋と実験設定ファイル（`schema_version: 1`）が変わる。区分の追加は既存の設定ファイルの解釈を変えない（2区分を書いた設定ファイルは以前は読めなかった）ため、人間の決定（2026-09-24）により上げないことにした。版を上げるのは、同じ設定ファイルでも解釈規則が変わった事実を内容ハッシュへ載せるためである（第13.2節）。設定ファイル先頭の `schema_version`（ADR-0018）と一致しなければ `app.config` が拒否する。トップレベルに置くことで版が内容ハッシュ（第13.2節）に入り、保存形式の解釈規則を変えた事実が再現性の差として現れる。**不採用**: 設定ファイルだけに持つ案、コンパイル結果にだけ記録する案（いずれもハッシュから保存形式が読めない）。
...
| `UsePrevious.allowed_reasons`（v1.12） | なし（遡りを許す欠損理由の集合） | 列挙値の順（2026-09-24 決定） |
...
動作区分は `SKIP_EVALUATION` / `ERROR` / `WAIT_FOR_INPUT` / `USE_PREVIOUS` の4つ【合意済み】。**v1.9（2026-09-22）で4区分すべてが型として定義された**。段階2 は前2者だけを定義しており、後2者は D05 が意味論と一体でフィールドを確定してから足す約束だった（Q8 決定）。D05 §6.8・§6.9 の起草と、2026-09-22 の人間の決定（Q15・Q16、いずれも選択肢1）でフィールドが確定したため、本節に区分を足した。**保存形式の版（第3節の `schema_version`）は 1 のままである**（v1.9 は 1 から 2 へ上げると書いたが、2026-09-24 の人間の決定で上げないことにした。理由は第3節）。
...
**遡りを許す欠損理由（`UsePrevious.allowed_reasons`）は集合である**（v1.12、2026-09-24 の人間の決定）。並びに意味は無いので、第3節の正規化の表のとおり構築時に列挙値の順へ整列し、重複を拒否する（`KernelValueError`）。書いた順の違いだけで内容ハッシュ（第13.2節）が分かれないようにするためである。
...
**この2区分を段階2 で置かなかった経緯**（Q8 決定、選択肢1。第1.2節の境界表の行1 に対する明示の例外）。宣言形だけを先に決めても、待機期限・遡り上限・記録といったフィールドは待機の意味論と一体でしか定まらず、フィールドの無い空の区分が残るだけになる。そこで D05 がフィールドごと確定してから区分を足し、その追加を保存形式の版の引き上げとして扱うと決めていた。**v1.9 がそのとおりに実施したものである**（ただし版の引き上げは、2026-09-24 の人間の決定により行わないことにした。第3節）。**不採用**: 4区分をフィールドなしで先に置く案（空の区分が残る）、段階2 で4区分を確定する案（D05 の検討を前倒しし段階2の範囲を超える）。
...
- **解除時期**: 段階3（**v1.9 で解除済み**）。再検査の結果そのものを残す書き先が D05 v2.0 でできた（**取引機会の有効性の再検査の記録 `ValidityRecheck`**。D05 §7.3）ため、第12節の拒否一覧から外した。この記録は「成立した / 成立しなかった / 読めず見送った / 読めず失敗した」の4つの結末をすべて持つので、どの経路でも1件が判断履歴に残る。確認試行の記録（`ConfirmationAttempt`）や遷移記録では、読めなかった場合を表せないため書き先にならない。
...
期限型のフィールド【提案】: `BarsDeadline(bars: int)`（`>= 1`、**確認足の系列の確定足で数える**。v1.10、2026-09-23 の人間の決定。D05 §15.2 の Q19、選択肢1。意味論の正本は D05 §7.7）、`DurationDeadline(duration: timedelta)`（正）。**確認足の系列で数える理由**（v1.10）: 起草時は「Trigger の系列の確定足で数える」と書いていたが、期限は「あと何回の確認の機会があるか」を表すものであり、Trigger の系列で数えると同じ `bars=2` が時間足の比で別の回数を意味する（1時間足の Trigger と15分足の確認なら4倍）。この宣言は段階2 では能力検査で拒否していて一度も動いていないため、書き換えても実行結果は変わらない。`on_deadline` は `DeadlineAction` 列挙で、段階2の値は `EXPIRE` の1つだけであり、期限切れの取引機会は終端理由 `EXPIRED`（上位設計書 §4.5）で終わる。段階3で有効化するまで能力検査が `AwaitConfirmation` を拒否するため、段階2の実行には現れない。
...
| 10 | **待機・遡りと読み方・接続元の組合せが許可されたものであること**（v1.9）: `WaitForInput` は `LatestAvailable` と `HistoryWindow` に、`UsePrevious` は **`LatestAvailable` かつ接続元が市場データ参照のときにだけ**書ける【合意済み】上位設計書 §4.3.13・D05 §6.9（最新1件の読み方が保持するのは最新1件だけであり、履歴を持つのは履歴窓で読む相手がいる出力参照だけなので、出力参照の遡りは許さない）。**取引機会の有効性の束縛（`ValidityBinding`）は必ず出力参照を読むので、その `on_missing` に `UsePrevious` は書けない**（v1.12。同じ理由の適用範囲の明記） | `InputSpec.read_spec`、`MissingInputPolicy`、`InputBinding.sources`、`ValidityBinding.on_missing` |
| 11 | **市場状態から取引機会への因果辺を含めて循環を検出すること**（v1.9）: `market_state` 役割がある戦略では、下表の因果辺3本目を引いたうえで #6 の循環検出を行う | 役割フィールド、`OutputSpec.data_type` |
| 12 | **パラメータどうしの関係が成り立つこと**（v1.9。**v1.10 で第3列を確定**）: 契約は各パラメータの範囲（`ParameterSpec.bounds`）しか持てないため、指標部品の `window_bars >= 2 * period` のような関係は解決済みの値でしか確かめられない。関係は**部品の登録が持つ検証の純粋関数**（`ComponentRegistration.parameter_constraint`。D05 §4.1）に書き、コンパイラが**パラメータ解決の後**（D05 §5.1 の段3）に1回呼ぶ。`False` が返った場合も関数が例外で終わった場合も拒否する（2026-09-23 の人間の決定。D05 §15.2 の Q22、選択肢1） | 解決済みの `ComponentInstance.parameters`（各パラメータが `ParameterSpec` を満たしたあとに関係を確かめる）と、`strategy.catalog` 側の `ComponentRegistration.parameter_constraint`。**本書に関係の宣言形は足さない**（関係は戦略を書く人が選ぶ設定ではなく、部品の計算規則から出てくるものだから） |
| 13 | **出力参照を履歴窓で読む接続で、保持する本数と保持の主キーが定まること**（v1.9。**v1.10 で条件2件を追加**）: `OutputRef` を `HistoryWindow` で読む接続では、(a) 窓が `BarsWindow` であり解決済みの本数が 1 以上であること、(b) **上流の使用箇所が足の確定（`OnBarClose`）だけで起動し、その系列がただ1つ**であること、(c) 上流が**実行時イベント（`OnRuntimeEvent`）で起動しない**こと、(d) **読む側の使用箇所も足の確定だけで起動する**こと。(a) は経過時間の窓（`DurationWindow`）では上流の出力が出る間隔が宣言から分からず保持本数を導けないため、(b)(c) は保持の主キーである「観測した足」が定まらないため、(d) は窓の末尾を決める基準（評価要求の対象区間）が無いと待機の有無で読む範囲が変わるためである（D05 §6.12 の保持と読み取りの規則、§5.6 の検査 f） | `InputSpec.read_spec`（`HistoryWindow.window`）、`InputBinding.sources`、**上流の使用箇所の `EvaluationSchedule.triggers`**（`OnBarClose.series` と起動条件の区分）、`ParameterRef` の解決結果 |
| 14 | **`exit` 役割の使用箇所が足の確定で起動するなら、その系列がただ1つであること**（v1.9）: 区間の違う起動条件を2件宣言すると、同じ判断時点に同じ建玉への損切り水準の更新が2件出て、最終的な水準が処理順に依存する（D05 §5.6 の検査 g） | `EvaluationSchedule.triggers`、`OnBarClose.series`、役割フィールド |
| 15 | **出力参照だけが欠けうる待機で、本数の期限を数える系列がただ1つに定まること**（v1.11、2026-09-23）: **接続元が出力参照（`OutputRef`）だけの入力**に、**本数で数える待機期限**（`WaitForInput(deadline=BarsDeadline(...))`）を書いた使用箇所は、**足の確定（`OnBarClose`）の起動条件の系列がただ1つ**でなければならない。出力参照の入力には系列が無いため、期限はその使用箇所の起動系列で数える（D05 §6.8。2026-09-23 の人間の決定 Q28、選択肢1）。区間の違う足の確定は区間ごとに別の評価要求になる（D05 §6.2 の手順3）ので区間が違えば要求ごとに定まるが、**区間が同じで系列だけが違う起動条件は1件の要求へ集約される**ため定まらない。系列が定まらないと**期限に到達する判断時点が処理順や系列名の並びで変わる**（2026-09-23 の人間の決定 Q30、選択肢1。D05 §5.6 の検査 h・§15.4） | `InputSpec.read_spec`（`WaitForInput.deadline` の区分）、`InputBinding.sources`（接続元が `OutputRef` だけか）、`EvaluationSchedule.triggers`（`OnBarClose.series`） |
...
| **取引機会の参照**（v1.13） | `trigger` 役割に接続された出力（`opportunity@v1`）を持つ使用箇所 | `RuntimeInputRef(OPPORTUNITY)` を読む使用箇所（第4.3節） | 確認は**同じ判断時点の開始足**で行う（D05 §7.7）ので、確認部品はその判断時点で生まれた取引機会より後に評価されなければならない。取引機会はランタイムが供給し明示の入力接続が無いため、辺が無いと確認部品が取引機会を出す部品より前に並ぶ評価順が通ってしまう（2026-09-24 の人間の再決定。PR #27） |

循環検出は**明示辺と因果辺の和**の上で行う。たとえば注文意図を出す使用箇所が `OnRuntimeEvent(POSITION_OPENED)` でも起動する宣言は自己ループになり、拒否される。これがないと、注文が約定して次の注文を起動する帰還路をコンパイラが通してしまう。検証戦略 A（第14節）では Trigger→注文（明示辺）と注文→利確 Exit（因果辺）の2本が加わるだけで、循環はない。`RuntimeInputRef(ACCOUNT)` は特定の注文に由来しないため辺を引かない。段階3で実行時イベントを増やすときは、この表に辺を追加する。段階3 の市場状態の辺（3本目）は、`market_state` が `None` の戦略（検証戦略 A）では引かれないため、段階2 の循環検出の結果は変わらない。取引機会の参照の辺（4本目、v1.13）も、`RuntimeInputRef(OPPORTUNITY)` を読む使用箇所を持たない検証戦略 A では引かれない。
...
| 拒否していた構成 | 解除できる理由 |
|---|---|
| `AwaitConfirmation`（確認待ちの発注方針）と、`execution_filter` が `None` でない戦略 | 確認評価の起動・開始足の決め方・`CONFIRMED` から先の経路を D05 §7.7 が確定した。**期限は確認足の系列の確定足で数える**（v1.10。第10.1節。D05 §15.2 の Q19、選択肢1） |
...
| **期間値（`timedelta`）を含む宣言**（v1.12）。`InputReadSpec.max_age`・`DurationWindow.duration`・`DurationDeadline.duration` を持つ契約と戦略定義 | 正規化エンコードが期間を符号化しないため、契約・戦略定義・解決済み設定の指紋を計算できない（第13.2節）。指紋の無い構成は固定した実験として動かせないので、**能力検査（#7）で `UNSUPPORTED_CONFIGURATION` として拒否する**（2026-09-24 の人間の決定）。例外のまま外へ出さない（D05 §5.1 の「失敗は戻り値で返す」）。符号化規則が D02 に足されたら解除する。段階3 の検証戦略 B は本数で数える宣言だけで書けるので、この拒否にかからない |
...
**段階3 の宣言を書くキー**（入場方針 `entry_policy`・有効性束縛 `opportunity_validity`・市場状態と確認の役割）の具体的な一覧は、本節の一般規則の適用として**単一実行評価の設計書 D07 v2.0 §18.4** に置く（v1.14）。本節の規則は変えない。
...
- 後続確認（`AwaitConfirmation`）と確認期限の宣言 → 段階3・D05（**宣言は第10.1節で定義済み。能力検査の拒否は v1.9 で解除**。期限をどの系列で数えるかは v1.10 で確定した＝確認足の系列。D05 §15.2 の Q19）。
