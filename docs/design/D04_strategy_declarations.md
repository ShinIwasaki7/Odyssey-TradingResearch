...
| 10 | **待機・遡りと読み方・接続元の組合せが許可されたものであること**（v1.9）: `WaitForInput` は `LatestAvailable` と `HistoryWindow` に、`UsePrevious` は **`LatestAvailable` かつ接続元が市場データ参照のときにだけ**書ける【合意済み】上位設計書 §4.3.13・D05 §6.9（上流の出力は最新1件しか保持しないため遡れない） | `InputSpec.read_spec`、`MissingInputPolicy`、`InputBinding.sources` |
