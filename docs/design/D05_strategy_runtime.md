...
| 【提案】 | 本書が推奨する設計。承認で確定 |
| 【要決定】 | 承認時にユーザーが選択する事項。段階2 の Q1〜Q9 は **2026-09-21 に決定済み**（第13節）、段階3 の Q10〜Q18 は **2026-09-22 に**、Q19〜Q22 と、紙上トレース [T02](../traces/T02_paper_trace_strategy_b.md) が立てた Q23〜Q29 は **2026-09-23 に決定済み**（第15節）。**未決は、Q28 の反映で新しく生じた Q30 の1件だけである**（第15.4節）。段階3 の宣言では実行結果が変わらないので、Q30 は実装を止めない |
...
**足りない入力が出力参照だけのときは、その使用箇所の起動条件の系列で数える**【確定】（Q28 決定、2026-09-23、選択肢1。紙上トレース T02 §14 #8）。出力参照（`OutputRef`）の入力には対応する系列が無いため、上の「その入力の系列」が引けない。そこで、その使用箇所の起動条件 `OnBarClose(name, series)` が持つ系列を使い、`WaitUntilBars(series=その系列, remaining=n)` とする。その使用箇所が「どの足のリズムで動くか」をいちばん素直に表すからである。検証戦略 B の `market_state` と `daily_above_ema` はどちらも日足で起動するので、上流（`daily_ema`）の期限と一致し、日足の連鎖3段の期限がそろう（T02 §7.1）。**不採用**: 上流をたどって最初に見つかった市場データ参照の系列で数える案（結果が同じになる場面が多いが、上流が複数あるときにどれを選ぶかの規則を新しく作ることになる）、出力参照だけの待機では本数の期限を使えないことにしてコンパイル時に拒否する案（期間で数える期限しか書けなくなるが、期間値を含む宣言はダイジェストを計算できない（第5.5節）ため実験として固定できない）。
  ...
  **その使用箇所の足の確定の起動条件が複数の系列にまたがる場合の扱いは未決である**（第15.4節の Q30）。区間の違う足の確定は区間ごとに別の評価要求になる（第6.2節 手順3）ので、区間が違えば数える系列は要求ごとに定まる。定まらないのは**区間が同じで系列が違う起動条件が1件の要求へ集約された**場合だけであり、起動系列がただ1つであることをコンパイル時に検査しているのは `execution_filter` 役割と `exit` 役割に限られる（第5.6節の検査 b・g）。**段階3 の宣言（第9.2節）では該当する使用箇所がどれも1系列で起動するため、実行結果は Q30 の結論によらない**。
...
| **v2.2**（2026-09-23、PR #24） | 紙上トレース T02 が立てた**要決定 Q23〜Q29 に対する人間の決定**（7件とも選択肢1）の反映（第3節の型表・第6.2節 手順10・第6.3節・第6.8節・第7.7節・第9.3節・第9.4節・第15.3節）。Q25 は本書ではなく D06 v1.6 が受け取り、Q24 の意味論テストは D08 v1.1 が受け取る | 承認待ち（T02 と同じ PR）。**Q28 の反映で新しく生じた Q30 だけが未決**（第15.4節）。段階3 の宣言では実行結果が変わらないため、実装は止めない |
| v2.3 以降 | 第10.2節が段階4 以降へ送った項目と、第15.4節の要決定（Q30）の反映 | 未起草 |
...
## 15. 段階3 の決定（Q10〜Q29 は決定済み。**Q30 だけが未決**）

段階2 の Q1〜Q9 は 2026-09-21 に決定済みで（第13節）、段階3 の Q10〜Q18 は **2026-09-22 に**、Q19〜Q22 と、紙上トレース T02 が立てた **Q23〜Q29 は 2026-09-23 に決定済み**である（第15.3節）。**Q28 の決定を本文へ書くときに新しく生じた Q30 だけが未決**である（第15.4節）。番号は再利用しない。
...
**Q22 の時点で残る要決定は無かった**。第10.2節が段階4 以降へ送った項目は本改訂の範囲外であり、要決定ではない。その後、紙上トレース T02 が Q23〜Q29 を立て（第15.3節で決定済み）、その反映から Q30 が生じた（第15.4節）。
...
### 15.4 Q30（Q28 の決定から新しく生じた要決定。**未決**）

Q28 の決定を本文へ書くときに、**決定そのものでは決まらない場合が1つ残った**。確定させず要決定として残す（CLAUDE.md「前提を silent に決めない」）。

| # | 決めること | 本書のどこに反映するか |
|---|---|---|
| Q30 | 待機の本数期限を「その使用箇所の起動条件の系列」で数えるとき（Q28 決定）、その使用箇所が**系列の違う足の確定を2件以上宣言している**場合にどの系列で数えるか。区間が違えば要求が分かれるので数える系列は要求ごとに定まるが（第6.2節 手順3）、**区間が同じで系列だけが違う起動条件は1件の要求へ集約される**ため定まらない。起動系列がただ1つであることをコンパイル時に検査しているのは `execution_filter` 役割（検査 b）と `exit` 役割（検査 g）だけである | 第6.8節の「期限を絶対の形へ解決する」。選択肢1 を採るなら第5.6節の検査に1件足す（D04 §12 への改訂依頼になる） |

**段階3 の実行結果は Q30 の結論によらない**。第9.2節の宣言では、出力参照だけの待機が起きる使用箇所（`market_state`・`daily_above_ema`）がどちらも日足1系列でしか起動しないからである。したがって Q30 は段階3 の実装を止めない。選択肢と推奨の理由は T02 §15.2 が正本である。
