...
| `catalog` | `registry.py`、`inputs.py`（部品が受け取るものの形。第4.4節）、`initial.py`（状態の初期値の組み立て）、`features/extreme.py`、`triggers/breakout.py`、`orders/market.py`、`protection/level_stop.py`、`exits/fixed_rr.py` | `features/`（EMA・ATR）、`conditions/`、`permissions/`、`filters/` |
...
- 追加の時刻制約（`TemporalConstraints` のウォームアップ本数と観測区間の一致）を**守らせる仕組み** → 本書 v0.2（段階2は能力検査で拒否。第11節の2）。
...
2. **`WarmupSpec` を段階2の部品で使わない**。`WarmupSpec.series` は具体的な `SeriesId` を持つ（D04 §9.2）ため、系列を使用箇所が選ぶ再利用可能な部品の契約には書けない。段階2はウォームアップ不足を履歴読み取りの `WARMUP_INSUFFICIENT` で判定でき（第4.3節）、完了条件「warmup 中の注文ゼロ」を満たす。系列を使用箇所から解決する規則が要るかは、EMA を足す v0.2 で扱う。**空でない `TemporalConstraints` は能力検査で拒否する**（v1.3。D04 §12 の拒否一覧）。本書 §6 はウォームアップ本数も観測区間の一致も守らせる仕組みを持たないため、通すと宣言が黙って無視され、ウォームアップ中に出力を出す部品や観測区間の揃わない入力を混ぜる部品が宣言どおりに動かないまま結果を変える。守らせる仕組みは v0.2 で足す（第10節）。
