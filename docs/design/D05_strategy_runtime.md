...
作成日: 2026-09-21
状態: **v2.9（2026-09-25、PR #40。設計文書の必須表を追加）。段階3 の範囲（v2.0〜v2.7）は承認済み（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35）**（第14節）。段階3 範囲の Q10〜Q30 はすべて決定済み（第15節）。段階3 実装 PR 2/5（コンパイラ）・PR 3/5（ランタイム前半）・PR 4/5（ランタイム後半）・PR 5/5（エンジン＋受入）が仮置きした点への人間の決定（2026-09-24・2026-09-25）を反映した。段階2 で承認済みの範囲（**承認（2026-09-21、PR #15）**、v1.0〜v1.4）は本改訂で変えていない。
...
表2・表3 は本文の節の言い換えではなく網羅の確認であり、本文と食い違えば本文が正本である。本文から埋められないマスは「要決定（R1-D05-n）」と書き、各表の直後に一覧した。
...
| E15 run の失敗（D06 §4.2 の停止判定） | 到達しない | 要決定（R1-D05-4） | 要決定（R1-D05-4） | 変化なし。失敗した run では末尾の `step`（E14）を呼ばない（D06 §10.2）ので、待機中のまま決着しない | 変化なし |
...
**要決定のマス**（本文から埋められなかったもの。人間の決定を得て本文と本表の両方を直す）。

| 番号 | 何が決まっていないか |
|---|---|
| R1-D05-4 | 同じ `step` で `Failed` が出て「以降の評価を行わずに返す」（第6.2節）とき、まだ番が来ていない評価要求の記録をどうするか。第6.4節の「評価記録は起動した使用箇所ごとに必ず1件残す」との関係が書かれていない |
...
| O7 生成時に `SNAPSHOT_AT_OPPORTUNITY` の束縛が読めない | 要決定（R1-D05-3） | 変化なし（固定は生成時に1回だけで、以後更新しない。第7.3節） | 変化なし（同左） | 変化なし（同左） | 変化なし |
...
| O17 自身の注文が受け付けられた（`AdmissionNotice.accepted=True`） | 到達しない | 到達しない（受付通知は `EntryProposal` を渡した機会、すなわち `ORDER_PENDING` の機会にだけ届く。遷移4・5、D06 §6.6） | 到達しない（同左） | 遷移5 → `TERMINATED`（`FULFILLED_BY_ORDER_ACCEPTANCE`）。記録は通知が配送された処理点（第7.2節） | 要決定（R1-D05-1） |
| O18 自身の注文が受付前の審査で拒否された（`accepted=False`） | 到達しない | 到達しない（O17 と同じ理由） | 到達しない（同左） | 遷移6 → `TERMINATED`（`ORDER_ATTEMPT_REJECTED`） | 要決定（R1-D05-1） |
| O19 他の機会の注文が受け付けられ、`on_order_accepted=CLOSE_OTHERS` | 到達しない | 遷移7 → `TERMINATED`（`CLOSED_BY_ORDER_ACCEPTANCE`） | 遷移7（同左） | 遷移7（同左。遷移7 の出発点は非終端すべて） | 変化なし（終端は復活しない） |
...
| 番号 | 何が決まっていないか |
|---|---|
| R1-D05-1 | 同じ `step` に受付通知が2件以上届いたとき（`max_active >= 2` で同じ判断時点に2つの機会が発注試行へ進んだ場合）の**適用の順序**。他の機会の受付による遷移7 が先に適用されると、自身の受付・拒否の通知が既に `TERMINATED` の機会へ届く。そのとき遷移要求として拒否する（`KernelValueError`）のか、通知を無視するのかが書かれていない |
| R1-D05-3 | 生成時に `SNAPSHOT_AT_OPPORTUNITY` の束縛が読めないときの扱い。第7.3節は `on_missing=Error` で失敗する場合に「固定は置換より前」とだけ書き、`SkipEvaluation` のとき機会を生成するのか・どの結末で記録するのか、`Error` のとき失敗をどの記録（評価記録か `ValidityRecheck` か）に残すのかが書かれていない |
...
全体計画書 §8.5 の表3 である。本書が生成するか、本書を通って部品・層・成果物をまたぐ値を1行ずつ挙げる。記録先の表番号と主キーの正本は D06 §9.2（判断履歴の19表）であり、本表は参照するだけである。規則の正本は各行の根拠の節である。
...
| 確認の開始足 `confirmation_start_bar` と確認期限 `deadline_at` | ランタイムが機会の生成時に決める（第7.7節） | `OpportunityLifecycle` に保持し、`include_start_bar` と遷移11 の判定に使う | 要決定（R1-D05-5） | 要決定（R1-D05-5） |
...
| 番号 | 何が決まっていないか |
|---|---|
| R1-D05-5 | 確認の開始足と確認期限（`OpportunityLifecycle.confirmation_start_bar` / `deadline_at`）を判断履歴のどこに残すか。表3（遷移記録）にも表18（確認試行）にも列が無く、本文は「開始足の識別子は保存する」（第7.7節）と書くが、それが実行時の状態の中のことか判断履歴のことかが書かれていない |
| R1-D05-6 | 待機記録が固定した足（`WaitingRequest.pinned_bars`）と待機期限を判断履歴に残すか。表16 の列に無く、「記録しない」とも書かれていない。再開した評価がどの足を読んだかを判断履歴から確かめる経路に関わる |
...
| v2.10 以降 | 第10.2節が段階4 以降へ送った項目 | 未起草 |
