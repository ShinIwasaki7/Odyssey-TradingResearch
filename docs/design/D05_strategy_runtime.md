...
| 【提案】 | 本書が推奨する設計。承認で確定 |
| 【要決定】 | 承認時にユーザーが選択する事項。段階2 の Q1〜Q9 は **2026-09-21 に決定済み**（第13節）。**段階3 の Q10 以降は未決**で、第15節に選択肢と推奨を置く |
| 【確定】 | 承認後に人間の決定で本文へ確定した事項（v1.3・v1.4 で使った印） |
...
**v2.0-draft のレビューの対象範囲は下表の第3列**である（段階3 で本改訂が決めること）。第2列は段階2 で承認済みであり、**本改訂では変えない**。第4列に属する指摘は「対象外（担当へ）」として記録し、本書では直さない。ただし D04 §1.2 と同じ例外を置く。**本書の文が第4列の挙動を暗示していて誤解を招く場合は、その暗示を消す修正だけ行う**。第4列の内容を本書に書き足すことはしない。

第2列の内容を段階3 の都合で書き換える必要が出た場合は、黙って直さず「段階2 の確定の改訂」として第11節に差異として挙げ、要決定（第15節）に上げる。
...
| 3 | コンパイラ | 手順・検査の実行順・失敗時の扱い・`CompiledStrategy` の中身・依存グラフ構築・評価順の導出・ハッシュ計算の手順（第5節） | 段階3 で解除する能力検査の一覧（第5.6節）、段階3 で必要になる検査5件の**D04 §12 への改訂提案**とその実行順・失敗時の扱い（第5.6節・第12.1節）、`CompiledRoles` に確認の計画を載せること（第5.3節の改訂） | 検査項目そのものと各検査が読む宣言（**D04 §12 が正本**。本書は提案するだけで、確定は D04 の改訂で行う） |
| 4 | ランタイムの評価 | 起動判定、入力解決、欠損の扱い、評価要求のライフサイクル、部品状態の保持、出力の付番と決定論（第6節） | 複数の時間足をまたぐ入力と出力の鮮度・観測区間（第6.7節）、待機（`WAIT_FOR_INPUT`、第6.8節）、遡り（`USE_PREVIOUS`、第6.9節）、追い越し（`REQUEST_SUPERSEDED`、第6.10節）の意味論と宣言形 | 待機記録・追い越し記録の判断履歴への保存形式（**D06 v0.3**）、遅延シナリオ4ケース別の処理順の検証（**D08**） |
...
| `runtime` | `ports.py`、`evaluator.py`、`requests.py`、`opportunities.py` | `waiting.py`（待機記録と再開。第6.8節・第6.9節）、`supersession.py`（追い越しの検査。第6.10節）、`confirmation.py`（確認の制御。第7.7節） |

D01 §7.2 の `runtime/` の一覧にある `waiting.py` / `supersession.py` を段階2 で作らなかったのは、待機と追い越しの意味論が本改訂の範囲だからである（第10節）。`confirmation.py` は D01 §7.2 の一覧に無いモジュールの追加であり、D01 §7.2 の但し書き（「モジュール名は初期構成であり、後続文書が本書の層規則内で追加・分割できる」）の範囲内である。`catalog` の部品は D01 §7.2 が挙げるサブディレクトリ（`features/` `conditions/` `permissions/` `filters/` `orders/` `protection/` `exits/`）にそのまま収まる。
...
**段階3（v2.0）でこの表を改訂する行**【提案】＋【要決定】Q13。段階2 で「使わない」としていた4件が段階3 で使われ、そのうち2件は部品が返す型と配送される型が分かれる。第4.2節の表そのものは置き換えず、次の差分を重ねる。

| データ型 | 部品が返す型 | 配送される型（`OutputRecord.payload`） | 段階3 |
|---|---|---|---|
| `condition_state@v1` | `ConditionState` | `Observation[ConditionState]`（Q13 の推奨案） | 使う |
...
| `price@v1` | `Price` | `Observation[Price]`（Q13 の推奨案） | 使う |
| `price_offset@v1` | `PriceOffset` | `Observation[PriceOffset]`（Q13 の推奨案） | `atr` を作る場合に使う |
...
### 4.5 段階3 の指標部品（EMA・ATR）【提案】＋【要決定】Q11・Q12

検証戦略 B（上位設計書 §7.1）は「日足終値と日足 EMA から市場状態を作る」「15分足終値と15分足 EMA で確認する」ため、**指数移動平均（EMA）が必須**である。真の値幅の平均（ATR）は戦略 B に現れないが、全体計画 §5.3.3 の初版カタログに挙がっている。作る範囲は第15節 Q12 で選ぶ。
...
### 4.6 段階3 の条件部品（価格比較・論理合成・遷移検出）【提案】＋【要決定】Q12
...
**N 本継続（`sustained_condition`）と「A の後 N 本以内の B」（`within_bars`）は段階3 では作らない**【提案】（Q12 の推奨案）。どちらも経過本数を数える状態が要り、`condition_state@v1` では表せないため、**新しいデータ型（経過本数を持つ状態）を D04 §5 のレジストリへ足すことになる**。検証戦略 B はどちらも使わず、段階3 の完了条件にも現れない。上位設計書 §4.6 が「段階的に用意する」と書いているとおり、必要になった段階で足す。
...
### 4.10 段階3 のトレーリング Exit【提案】＋【要決定】Q12・Q14
...
この部品は**建玉が存在する足の確定でだけ評価される**。その起動の作り方は第6.11節（Q14）で決める。`ManagementAction` に `UpdateStop` を足す改訂は第4.2節に書き、D04 §11.2 への改訂依頼として第12.1節に挙げる。適用の意味論（どの執行足から有効か、同じ判断時点の決済要求との競合）は D06 の責務である（D06 §8.3。段階3 は D06 v0.3）。
...
|---|---|---|
| `AwaitConfirmation`（確認待ちの発注方針） | **解除** | 確認評価の起動と期限の数え方を第7.7節が確定する |
...
| 出力参照（`OutputRef`）を履歴窓（`HistoryWindow`）で読む接続 | **拒否を続ける**（Q18） | 段階3 の部品はどれも上流の出力の履歴を読まない（第6.5節・第15節 Q18） |
...
段階3 で**新しく必要になる検査**は次の5件である【提案】。**検査項目そのものと「その検査が読む宣言」の正本は D04 §12 であり（第1.1節・第1.2節の行3）、本節はそれを確定するものではない**。ここに挙げるのは D04 §12 への**改訂提案**であり、5件は第12.1節の依頼7 として D04 の改訂で確定する。**本書が決めるのは、確定後の5件をコンパイラのどの段で実行し、失敗をどの拒否の区分で返すか**（第5.1節・第5.2節）だけである。この分担を崩して本書側で検査を確定させると、正本の検査一覧に載っていない検査が生まれ、D04 §12 だけを読んで作ったコンパイラがそれらを実施しない。**D04 の改訂が入るまでは、この5件はコンパイラに実装しない**。
...
### 6.7 複数の時間足をまたぐ入力、鮮度と観測区間【提案】＋【要決定】Q13
...
**`VALUE` の出力をランタイムが `Observation` で包む**【提案】（Q13 の推奨案）。上位設計書 §4.3.15 が確定した `Observation[T]` の4フィールドをそのまま使い、**包むのはランタイム、部品は内容だけを返す**（第4.2節の取引機会と同じ手法）。
...
### 6.8 待機（`WAIT_FOR_INPUT`）【提案】＋【要決定】Q15
...
### 6.9 遡り（`USE_PREVIOUS`）【提案】＋【要決定】Q16
...
### 6.10 評価要求の追い越し（`REQUEST_SUPERSEDED`）【提案】＋【要決定】Q17
...
**現在コンテキストの対象を読む使用箇所は、足の確定で起動するとき対象1件につき1要求を作る**【提案】（Q14 の推奨案）。第6.2節 手順2 の表に1行足す。
...
| 11 | `OPEN` / `CONFIRMED` → `TERMINATED` | `AwaitConfirmation` の期限に到達した（**v2.0 で発火条件を確定**。`BarsDeadline(n)` は確認足の系列で機会の生成した足の次を1本目と数えて n 本目の確定（`ScheduledBoundary`）を受けた時点、`DurationDeadline(d)` は `decision_time - created_decision_time >= d` になった時点。第7.7節） | ライフサイクル検査 | `EXPIRED` | 3 |
...
**取引許可はランタイムが役割フィールド経由で読んで適用する**【提案】（Q10 の推奨案）。適用の時点は P3（取引機会を出す出力が出た直後、第7.4節の手順1と2のあいだ）である。
...
**期限の数え方**【提案】（遷移11 の発火条件）。`AwaitConfirmation.deadline` を機会の生成時点で絶対の形へ解決し、`OpportunityLifecycle.deadline_at` に持つ。
...
D04 §10.1 は `BarsDeadline` を「Trigger の系列の確定足で数える」と書いている【合意済み】。**本改訂は「確認足の系列で数える」に改める**ことを提案する（第11節の差異6）。期限は「あと何本の確認の機会があるか」を表すものであり、Trigger の系列（検証戦略 B なら1時間足）で数えると、確認足（15分足）の本数が時間足の比で変わってしまう。D04 §10.1 への改訂依頼として第12.1節に挙げる。**不採用**: Trigger の系列のまま数える案（同じ `bars=2` が確認2回にも確認8回にもなり、宣言から確認の回数が読めない）、期限型を増やして系列を宣言できるようにする案（`ConfirmationPlan.series` と二重に系列を書くことになる）。
...
**`breakout_trigger` も版を上げる**（第4.9節）。日足の高値を待つために `level` 入力の `on_missing` が `WaitForInput(deadline=DurationDeadline("30m"), on_deadline=SKIP_EVALUATION, on_superseded=EXPIRE_REQUEST)` になる。`on_superseded=EXPIRE_REQUEST` は上位設計書 §4.3.14 の「Trigger の標準方針: 古い対象足が追い越されたら、その待機評価を失効させる」の具体値である。

**期間値を含む宣言はダイジェストを計算できない**【合意済み】D02 §9.3・D04 §13.2・本書 §5.5。`DurationDeadline("30m")` はこの制限にかかる。検証戦略 B は**期間値を使わない形（`BarsDeadline` と `BarsWindow` だけ）でも書ける**ので、段階3 の受入れは本数で数える宣言で行い、期間値の符号化規則は D02 の改訂に委ねたままにする【提案】。上の `breakout_trigger` v2 の待機期限も `BarsDeadline(bars=1)`（日足1本ぶん）に置き換えられる。**要決定ではない**が、D02 が期間の符号化規則を足すまでは期間値を使う戦略が実験として固定できない、という制約が残る（第12.1節）。
...
- 合成部品のうち **N 本継続**と「**A の後 N 本以内の B**」 → 段階4 以降（第4.6節、Q12）。経過本数を数える新しいデータ型が要り、検証戦略 B が使わない。
- **上流の出力を履歴窓で読む仕組み**（出力の履歴の保持と打ち切り規則） → 段階4 以降（第6.3節、Q18）。段階3 も能力検査で拒否を続ける。
...
6. **確認期限を数える系列を「Trigger の系列」から「確認足の系列」へ改める**（段階3、第7.7節）。D04 §10.1 は `BarsDeadline(bars)` を「Trigger の系列の確定足で数える」と書いている。期限は「あと何回の確認の機会があるか」を表すものであり、Trigger の系列で数えると、同じ `bars=2` が確認の回数として2回にも8回にもなる（1時間足 Trigger と15分足の確認なら4倍）。宣言から確認の回数が読めないため、確認足の系列で数える形へ改める。D04 §10.1 への改訂依頼として第12.1節に挙げる。**これは段階2 の確定（第1.2節の第2列）ではなく、段階2 では能力検査で拒否していて一度も動いていない宣言の意味を、動かす前に決め直すものである**。
...
### 12.1 段階3（v2.0）のための他文書への改訂依頼【提案】

本改訂が承認された場合に、**同じ PR か直後の PR で改訂が要る**文書を1か所にまとめる。件数は D04 が9件、D06 が5件、D03 が2件、上位設計書が1件、D02 が1件である。いずれも本改訂の要決定（第15節）の結果で内容が変わるため、**決定が出るまで他文書には手を入れない**。

**D04（戦略宣言モデル）への改訂依頼: 9件**

| # | 改訂 | 本書のどこ | どの要決定に依存するか |
|---|---|---|---|
| 1 | §6.3 の `MissingInputPolicy` に `WaitForInput` / `UsePrevious` の2区分と、`WaitDeadlineAction` / `OnSuperseded` の2つの列挙を足す。**保存形式の版（`schema_version`）を 2 へ上げる**【合意済み】D04 §6.3・§19 Q8 | §6.8・§6.9 | Q15・Q16 |
| 2 | §4.3 の `RuntimeTarget` に `OPPORTUNITY` を足す（対応するデータ型は登録済みの `opportunity@v1`） | §6.11 | Q14 |
| 3 | §11.2 の `ManagementAction` の要求種別に `UPDATE_STOP`（`UpdateStop(stop_loss: Price)`）を足す | §4.2・§4.10 | Q12 |
| 4 | §10.1 の `BarsDeadline` の説明を「Trigger の系列の確定足で数える」から「**確認足の系列の確定足で数える**」へ改める | §7.7・第11節の6 | — |
| 5 | §12 の段階2 拒否一覧を「段階3 で解除するもの」と「段階3 でも拒否を続けるもの」に分ける（第5.6節の表がその内容） | §5.6 | Q12・Q18 |
| 6 | §12 の拒否一覧の「空でない `TemporalConstraints`」を、**ウォームアップ本数（拒否を続ける）と観測区間の一致（解除）に分ける** | §6.7・第11節の2 | — |
| 7 | §12 の検査一覧に段階3 の検査5件（確認部品の `include_start_bar`、確認足の系列の一意性、待機・遡りと読み方の組合せ、市場状態の因果辺、パラメータどうしの関係）を足す | §5.6 | Q10・Q15・Q16 |
| 8 | §12 の「エンジン上の因果辺」の表に、市場状態の使用箇所から取引機会を出す使用箇所への辺を足す | §7.6 | Q10（選択肢1 を採る場合のみ） |
| 9 | §5 の `position_context@v1` の例示に有効な損切り水準を足す（**段階2 から持ち越している改訂依頼**。D06 §8.4 が `effective_stop_loss` として確定済み） | 第11節の1 | — |

**D06（バックテスト縦断）への改訂依頼: 5件**

| # | 改訂 | 本書のどこ |
|---|---|---|
| 1 | §4.1 の rank 4 `OPPORTUNITY_LIFECYCLE` の内容に、**待機中の評価要求の期限・追い越しの検査**を足す（新しいフェーズは足さない。上位 §4.3.14 が「期限・追い越し・機会失効の検査は実評価とは別のライフサイクル処理として起動できる」と1つにまとめている） | §6.8・§6.10 |
| 2 | §4.4 の「戦略ランタイムが返した記録の処理点はエンジンの時計で番号を振り直す」の対象に、**待機の出来事（`WaitEvent.at`）**を足す。段階2 は取引機会の遷移だけが処理点を持つランタイム側の記録だった | §6.8 |
| 3 | §8.3 に `UpdateStop`（トレーリング）の適用意味論を足す（どの執行足から有効か、同じ判断時点の決済要求との競合、価格刻みへの丸め方向）。D06 §12 が既に「段階3・本書 v0.2」としている | §4.10 |
| 4 | §9.2 の trace 表に**待機記録・追い越し記録・遡り記録・確認試行の表**を足す。D06 §12 が既に「段階3（D05 v0.2 が意味論を決めた後）」としている | §6.8〜6.10・§7.7 |
| 5 | §10.1 の末尾処理に、**run 末尾に残った待機要求の終端**を足す。`is_run_end=True` の `step` は起動判定も評価も行わない（本書 §6.1）ので、残った待機要求も同じ `step` で `Skipped(Reason(RUN_END, ...))` として決着させる | §6.1・§6.8 |

**D03（市場データと時刻）への改訂依頼: 2件**

| # | 改訂 | 本書のどこ |
|---|---|---|
| 1 | §6.2 に、**末尾の足を指定して履歴窓を読む操作**を足す。待機からの再開では「最初の対象足から解決した窓」を読み直す必要があるが（上位 §4.3.14「`HistoryWindow` も同じ基準から窓を固定する」）、現在の `history(series, window, at, end_offset_bars)` は判断時刻 `at` を基準にしか読めず、再開時には窓が後ろへずれる。`LatestAvailable` 側は既存の `bar(series, bar_start, at)` で固定した足を読めるので、足りないのは履歴窓の1操作だけである | §6.8 |
| 2 | §7.2 の「`OnBarClose` の結び付け（案）」を**確定**にする。待機の再開契機を `Publication` の到着とする規則（同節）が段階3 で実装されるため、案のままにしておく理由が無くなる | §6.8 |

**上位設計書への改訂依頼: 1件**

| # | 改訂 | 本書のどこ |
|---|---|---|
| 1 | §4.5 の終端理由の表で、`MARKET_STATE_INVALIDATED` の説明を「`REQUIRE_UNTIL_ORDER_REQUEST` の条件が成立しなくなって終端」から「**市場状態によって無効になって終端**（継続成立を要求した条件が崩れた場合と、生成時点で市場状態が取引を許していなかった場合）」へ広げる。新しい語は作らない | §7.6 |

**D02（共通カーネル）への改訂依頼: 1件（条件つき）**

| # | 改訂 | 本書のどこ |
|---|---|---|
| 1 | §9.3 の正規化エンコードに**期間（`timedelta`）の符号化規則**を足すかどうか。足さない限り、期間で期限や窓を宣言した戦略は内容ハッシュを計算できず、実験として固定できない【合意済み】D02 §9.3・D04 §13.2・本書 §5.5。**検証戦略 B は本数で数える宣言だけで書けるため段階3 の受入れは止まらない**（第9.2節）が、待機期限と確認期限を時間で書きたい戦略は段階3 でも書けないままである | §5.5・§9.2 |
...
| **v2.0**（本改訂） | 段階3 の範囲（第1.2節の表の第3列）。複数の時間足・市場状態・後続確認・待機・遡り・追い越し・段階3 のカタログ・検証戦略 B | **承認待ち**。第15節の Q10〜Q18 を決めたうえで承認する |
| v2.1 以降 | 第10.2節が段階4 以降へ送った項目 | 未起草 |

承認の範囲は第1.2節の表の**第3列**である。第2列（段階2 で確定）は本改訂では変えていない。第4列は担当文書が決める。
...
## 15. 段階3 の要決定（Q10〜Q18。**未決**）

段階2 の Q1〜Q9 は 2026-09-21 に決定済みで（第13節）、番号は再利用しない。ここから先が本改訂で人間に決めてもらう9件である。各項目は「何を決めるか / 影響 / 選択肢（3つ以内、**先頭が推奨**）/ 推奨の理由」の形で書く。**選択肢のラベルに書いていない解釈を足さない**。

### Q10 市場状態の取引許可をどこで適用するか

**何を決めるか**: 「上昇トレンドのときだけ買う」のような取引許可を、ランタイムが取引機会の生成時に適用するのか、Trigger 部品が自分の入力として読んで適用するのか。

**影響**: 日足が遅れて届いたときに取引機会が作られるかどうかが変わる。ランタイムが適用する案では、市場状態が待機しているあいだ取引機会の生成も待つ規則をランタイムが1つ持つ。部品が適用する案では、待機が入力の欠損方針だけで説明でき例外規則は要らないが、許可の判定が Trigger 部品ごとに写され、既存の突破 Trigger に許可の入力を足した契約の版が要る。

| # | 選択肢 | 影響（1行） |
|---|---|---|
| 1（推奨） | ランタイムが役割フィールド経由で読み、取引機会の生成時（P3）に適用する | 上位設計書 §4.3.11 の確定した因果順序をそのまま実施でき、許可の判定が1か所に収まる |
| 2 | Trigger 部品の明示入力として接続し、部品が適用する | 待機が入力の欠損方針だけで説明でき、ランタイムの例外規則が1つ減る |
| 3 | 1 と 2 の両方を許し、戦略の宣言で選ぶ | 同じ挙動を決める場所が2か所になる |

**推奨の理由**: 上位設計書 §4.3.11 は確定事項として「Trigger を評価し、更新後の MarketState で取引可否を判断する」「許可された取引機会について ExecutionFilter を評価する」と書いており、判断の主体をランタイム側に置いている。判定を部品へ移すと、同じ許可の読み方が Trigger 部品の数だけ写され、許可を読み忘れた部品が黙って発注する経路ができる。

### Q11 指数移動平均（EMA）の初期化と更新をどう決めるか

**何を決めるか**: EMA を「毎回の評価で履歴窓から計算し直す」か、「状態に前回値を持って再帰的に更新する」か。

**影響**: 同じ日の同じ足に対する EMA の値が、run をいつから始めたかで変わるかどうかが決まる。待機からの再開で対象区間を固定して読み直すとき（第6.8節）に、状態を巻き戻す必要が出るかどうかも決まる。計算量は窓の本数に比例する（毎回計算する案）か一定（状態を持つ案）かで変わる。

| # | 選択肢 | 影響（1行） |
|---|---|---|
| 1（推奨） | 履歴窓から毎回計算する（窓の古い側の `period` 本の単純平均を種にして再帰） | run の開始位置に依存せず、待機からの再開でも同じ窓を読むだけで同じ値が出る |
| 2 | 状態に前回値を持って再帰更新する | 計算量が一定だが、run の開始位置で値が変わり、再開時に状態の巻き戻しが要る |
| 3 | 両方を別の契約として登録し、戦略が選ぶ | 同じ指標に2つの計算規則が並び、実験の同一性の議論が二重になる |

**推奨の理由**: 上位設計書 §4.3.14 が「EMA 等の状態付き Feature は対象区間に対応する履歴出力を参照または再現し、現在の最新キャッシュを過去区間の答えとして使わない」と定めている。窓だけで決めればこの要求が構造的に満たされ、ウォームアップも既存の履歴読み取りの規則（`WARMUP_INSUFFICIENT`）で成立する。

### Q12 段階3 で足す部品カタログの範囲

**何を決めるか**: 段階3 で作る部品を、検証戦略 B に必要な分に限るか、全体計画 §5.3.3 の初版カタログ12部品まで広げるか。

**影響**: 段階3 の作業量と、新しいデータ型を D04 のレジストリへ足すかどうかが決まる。N 本継続と「A の後 N 本以内の B」は経過本数を数える状態が要り、`condition_state@v1` では表せないため新しいデータ型が要る。

| # | 選択肢 | 影響（1行） |
|---|---|---|
| 1（推奨） | 検証戦略 B に必要な部品 ＋ 基本合成（AND / OR / 遷移検出）＋ ATR の計11件 | 新しいデータ型が要らず、上位設計書 §7 の段階3 が挙げる「基本合成」に収まる |
| 2 | 全体計画 §5.3.3 の12部品に合成部品5件すべてを加える | 初版カタログを一度に埋められるが、経過本数の状態型を D04 へ足すことになる |
| 3 | 検証戦略 B に必要な部品だけ（合成部品と ATR を作らない） | 最小だが、上位設計書 §7 の段階3 の「基本合成」が段階4 へずれる |

**推奨の理由**: 段階3 の完了条件は「戦略 B を同じ基盤で記述できる」ことであり、合成部品は完了条件に現れない。一方で上位設計書 §7 の段階3 は「基本合成」を作るものに挙げており、AND / OR / 遷移検出はどれも状態型を増やさずに作れる。ATR は検証戦略 B に不要だが、観測区間の一致（第6.7節）を実際に使う唯一の部品であり、その仕組みを検証する材料になる。

### Q13 出力の鮮度と観測区間をどう運ぶか

**何を決めるか**: 「この出力はどの足を見た結果で、いつの時点の鮮度か」を、どの型で運ぶか。

**影響**: 複数の時間足をまたぐ戦略で、鮮度の上限（`max_age`）が正しく効くかどうかが決まる。段階2 の規則（出力の鮮度はそれを計算した判断時刻）のままだと、前日の日足から作った市場状態が今日の鮮度を持ち、鮮度の上限がまったく効かない。

| # | 選択肢 | 影響（1行） |
|---|---|---|
| 1（推奨） | `VALUE` の出力をランタイムが `Observation` で包む（部品は内容だけを返す） | 上位設計書 §4.3.15 が確定した型をそのまま使い、正本の改訂が要らない |
| 2 | ランタイムが出力ごとの鮮度表を実行時の状態に別に持つ | 判断履歴から出力を読み直しても鮮度が復元できず、遅延シナリオの差分を追跡できない |
| 3 | 出力記録（`OutputRecord`）に鮮度の項目を足す | 上位設計書 §4.3.15 が確定した7フィールドの改訂になり、`Observation` と役割が重なる |

**推奨の理由**: 上位設計書 §4.3.15 の例が、まさにこの場面（22:00 に終了した日足から EMA を 22:00:02 に計算した場合の鮮度は 22:00）を `Observation` で説明している。正本が用意している型をそのまま使えば、語彙も型も増えない。

### Q14 取引機会・建玉を対象とする評価を、足の確定でどう起動するか

**何を決めるか**: 後続確認（確認足の確定ごとに、確認待ちの機会ごとに評価）とトレーリング（1時間足の確定ごとに、建玉ごとに評価）の起動を、どう表すか。

**影響**: 宣言から「対象ごとに1回評価する」ことが読めるかどうかと、宣言の型（起動条件の区分）が増えるかどうかが決まる。

| # | 選択肢 | 影響（1行） |
|---|---|---|
| 1（推奨） | 現在コンテキストの対象（取引機会・建玉）を読む使用箇所が足の確定で起動するとき、ランタイムが対象1件につき1要求を作る | 既存の「イベント1件につき1要求」と同じ数え方で、起動条件の区分が増えない |
| 2 | 確認足ごと・建玉ごとの起動条件の区分を D04 に足す | 宣言から対象ごとの評価が読めるが、起動条件の区分が2つ増える |
| 3 | 取引機会を配送イベントとして確認足ごとに再配送する | 同じ機会のイベントが何度も判断履歴に並び、ADR-0032 の冪等性の規則と衝突する |

**推奨の理由**: 段階2 が既に「建玉を読む入力は現在コンテキスト（`CurrentContext`）で、対象は評価要求が持つ」という形を採っている（約定後の利確、第8節）。取引機会も同じ形に乗せれば、確認とトレーリングが1つの規則で説明でき、足すのは宣言側の対象区分（`RuntimeTarget.OPPORTUNITY`）1件だけで済む。

### Q15 入力を待つ設定（`WAIT_FOR_INPUT`）に、どのフィールドを置くか

**何を決めるか**: 「必要な入力が届くまで評価を保留する」という宣言に何を書かせるか。

**影響**: 待っても解消しない欠損（データ開始前、鮮度切れ）で待機に入れてしまうかどうかと、宣言の項目数が決まる。

| # | 選択肢 | 影響（1行） |
|---|---|---|
| 1（推奨） | 待機期限・期限切れの動作・追い越し時の動作の3つ。待機できる欠損理由はランタイムの固定規則 | 上位設計書 §4.3.13・§4.3.14 が要求する追加条件をちょうど満たし、無意味な待機を宣言できない |
| 2 | 上の3つに「待機を許す欠損理由」を加えた4つ | どの欠損で待つかを戦略が選べるが、待っても解消しない理由を書けてしまう |
| 3 | 待機期限だけを置き、期限切れと追い越しの動作はランタイムの固定規則 | 宣言が短いが、暗黙の既定値が2つ生まれ ADR-0031 の方針と揃わない |

**推奨の理由**: 待機が解消しうるのは「期待される最新足が未到着」と「窓内の期待足が欠けている」の2つだけで、どちらもデータの到着で解消する。データ開始前（ウォームアップ不足）と鮮度切れは待っても直らないので、選ばせる意味がない。

### Q16 過去値へ遡る設定（`USE_PREVIOUS`）に、どのフィールドを置くか

**何を決めるか**: 「入力が欠けたとき、指定した上限内の過去の有効値を明示的に使う」という宣言に何を書かせるか。

**影響**: 破損データや計算の失敗まで古い値で隠してしまうかどうかが決まる。

| # | 選択肢 | 影響（1行） |
|---|---|---|
| 1（推奨） | 遡り上限（本数か経過時間）と、遡りを許す欠損理由の2つ | 上位設計書 §4.3.13 の「破損データや計算例外まで一律に過去値で隠さない」を宣言で保証できる |
| 2 | 遡り上限だけ | 宣言が短いが、どんな欠損でも古い値で埋まる |
| 3 | 上限・許す欠損理由・遡った事実の記録先の3つ | 記録先は判断履歴の1か所しかなく、宣言で選ぶ意味がない |

**推奨の理由**: 待機（Q15）と扱いが逆なのは、遡りでは「どこまでを隠してよい欠損とみなすか」が戦略ごとの判断だからである。上位設計書 §4.3.13 はこの点を明記している。実際に使った観測の記録はランタイムの責務なので、宣言には置かない。

### Q17 評価要求の追い越しの既定を、どこに置くか

**何を決めるか**: 「同じ系列の新しい足が来たとき、待っていた古い評価要求を閉じるか待ち続けるか」の既定値を、宣言に必須で書かせるか、ランタイムが役割ごとに持つか。

**影響**: 判断履歴から「なぜこの要求が閉じたのか」を追うときに、戦略の宣言だけを見れば足りるか、実験の固定設定まで見る必要があるかが決まる。

| # | 選択肢 | 影響（1行） |
|---|---|---|
| 1（推奨） | 宣言（待機設定の項目）を必須にし、ランタイムに既定を持たせない | 挙動を決める場所が1か所で、暗黙の既定値が生まれない |
| 2 | 役割ごとの既定をランタイムが持ち、宣言は上書きだけ | 宣言を書かなくても動くが、既定の出どころを追うのに実験の固定設定を見る必要が出る |
| 3 | 1 と 2 の両方を持つ | 同じ規則が2か所になる |

**推奨の理由**: ADR-0031 が「暗黙の既定値を設けない」という方針を取引機会の有効性について確定しており、待機と追い越しでも同じ方針を採るのが一貫する。上位設計書 §4.3.14 が挙げる標準方針（Trigger は失効、確認は対象足を進める）は、検証戦略 B の宣言例（第9.2節）が具体値として示す。

### Q18 上流の出力を履歴窓で読む仕組みを、いつ作るか

**何を決めるか**: 「他の部品の出力の過去 N 本」を読む接続を、段階3 で作るか、拒否を続けるか。

**影響**: ランタイムが出力の履歴を何本ぶん保持し、いつ捨てるかという仕組みを段階3 で作るかどうかが決まる。段階2 は最新1件しか保持していない。

| # | 選択肢 | 影響（1行） |
|---|---|---|
| 1（推奨） | 段階4 以降へ送り、段階3 もコンパイル時に拒否を続ける | 段階3 で使わない記憶と打ち切りの規則を作らずに済む |
| 2 | 段階3 で作る | 「N 本継続」の合成部品を状態なしで書けるようになるが、保持する本数と捨てる時点の規則が要る |
| 3 | 段階3 で作るが、窓の本数に上限を置く | 上限という段階3 だけの設定が1つ増える |

**推奨の理由**: 検証戦略 B の入力はすべて市場データの履歴で足り、上流の出力を遡って読む接続は1つも現れない。N 本継続の合成部品も段階3 では作らない（Q12）ので、この仕組みを必要とする部品が段階3 に存在しない。
