...
|---|---|---|---|
| 1 | 実行時の内容型 | 上位設計書 §4.3.15 に無い内容型（`OrderIntent` / `ProtectionLevels` / `ManagementAction` / `TradeDirection` / `OrderType` / `OpportunityContent`）のフィールド、`ConditionState` の項目、データ型識別子13件と内容型の対応表、部品が返す型と配送される型の切り分け（第4.2節。D04 §1.2 の行3が本書へ委ねた範囲） | `position_context@v1` / `account_context@v1` の項目（**D06**）、注文有効期限の既定値（**D06**） |
