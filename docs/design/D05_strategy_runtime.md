...
作成日: 2026-09-21
状態: **v2.9（2026-09-25、PR #R1PR。設計文書の必須表を追加）。段階3 の範囲（v2.0〜v2.7）は承認済み（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35）**（第14節）。段階3 範囲の Q10〜Q30 はすべて決定済み（第15節）。段階3 実装 PR 2/5（コンパイラ）・PR 3/5（ランタイム前半）・PR 4/5（ランタイム後半）・PR 5/5（エンジン＋受入）が仮置きした点への人間の決定（2026-09-24・2026-09-25）を反映した。段階2 で承認済みの範囲（**承認（2026-09-21、PR #15）**、v1.0〜v1.4）は本改訂で変えていない。
v2.9（2026-09-25、PR #R1PR）: 設計文書の必須表（R1（PR #26 承認）。全体計画書 §8.5）を加えた。評価要求の状態×出来事表（第6.10.1節）、取引機会の状態×出来事表（第7.2.1節）、値の伝播表（第7.8節）を置き、第1.2節に置き場所の一覧を足した。表は既存の節の規則を並べ替えたものであり、**本文の規則は変えていない**。本文から埋められなかったマス5件を「要決定（R1-D05-1・3〜6）」として各表の直後に一覧した（人間の決定を待つ）。
...
**必須表（全体計画書 §8.5、R1）**。本書は状態機械を2つと、部品・層をまたいで運ばれる値を定めるので、全体計画書 §8.5 の3つの表をすべて持つ。表1 は本節の上の表（境界表）である。
...
| **v2.9**（2026-09-25、PR #R1PR） | 設計文書の必須表（R1。全体計画書 §8.5）の追加（第1.2節・第6.10.1節・第7.2.1節・第7.8節）。既存の規則を表に並べ替えただけで、本文の規則は変えていない。埋められなかったマス5件は要決定 R1-D05-1・3〜6 として一覧した | R1（PR #26 で人間が承認）の実施 |
