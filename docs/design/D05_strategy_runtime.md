...
v2.9（2026-09-25、PR #40）: 設計文書の必須表（R1（PR #26 承認）。全体計画書 §8.5）を加えた。評価要求の状態×出来事表（第6.10.1節）、取引機会の状態×出来事表（第7.2.1節）、値の伝播表（第7.8節）を置き、第1.2節に置き場所の一覧を足した。表は既存の節の規則を並べ替えたものであり、**本文の規則は変えていない**。本文から埋められなかったマス5件を「要決定（R1-D05-1・3〜6）」として各表の直後に一覧した（人間の決定を待つ）。
...
| **v2.9**（2026-09-25、PR #40） | 設計文書の必須表（R1。全体計画書 §8.5）の追加（第1.2節・第6.10.1節・第7.2.1節・第7.8節）。既存の規則を表に並べ替えただけで、本文の規則は変えていない。埋められなかったマス5件は要決定 R1-D05-1・3〜6 として一覧した | R1（PR #26 で人間が承認）の実施 |
