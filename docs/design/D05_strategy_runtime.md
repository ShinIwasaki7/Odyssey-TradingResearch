...
作成日: 2026-09-21
状態: **v2.10（2026-09-25、必須表の空欄を埋める文書 PR。人間の決定5件を反映）。段階3 の範囲（v2.0〜v2.7）は承認済み（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35）**（第14節）。段階3 範囲の Q10〜Q30 はすべて決定済み（第15節）。段階3 実装 PR 2/5（コンパイラ）・PR 3/5（ランタイム前半）・PR 4/5（ランタイム後半）・PR 5/5（エンジン＋受入）が仮置きした点への人間の決定（2026-09-24・2026-09-25）を反映した。段階2 で承認済みの範囲（**承認（2026-09-21、PR #15）**、v1.0〜v1.4）は本改訂で変えていない。
v2.10（2026-09-25、必須表の空欄を埋める文書 PR）: v2.9 が「要決定」とした必須表の空欄5件に**人間が決定した（2026-09-25、5件とも推奨案）**ので、表と本文へ【確定】として書いた。(1) 同じ `step` に受付通知が2件以上届いたとき（R1-D05-1）は、**自分の注文の通知（受付・拒否）を先に全件当て、「他の機会を閉じる」（遷移7）はその後で当てる**（第7.2節・第7.2.1節）。(2) 取引機会を作る時点で値を固定する束縛が読めないとき（R1-D05-3）は、見送りの方針なら**機会を作り、有効性の再検査の記録（表19）に「読めず見送り」を生成時点で1件残す**（第7.3節・第7.2.1節）。(3) 評価の失敗で打ち切ったとき、まだ評価していない要求（R1-D05-4）は**記録しない**。要求 ID に欠番が出ることを明記した（第6.2節・第6.4節・第6.10.1節。いまの実装どおり）。(4) 確認の開始足と確認期限（R1-D05-5）は**取引機会の遷移記録（表3）の「生成」の行に列を足して残す**（第3節・第7.7節・第7.8節）。(5) 待機で固定した足（R1-D05-6）は**待機の出来事（表16）の「待機に入った」行に列を足して残す**。待機期限は評価記録（表2）に既にある（第3節・第6.8節・第7.8節）。**(1)(2)(4)(5) は実装変更を伴い、後続の実装 PR で行う**（本版の時点の実装は、(1) では構造エラーで止まり、(2) では何も記録せず、(4)(5) では列を書き出さない。(1)(2) はいまの検証戦略 A・B では起きない）。
v2.9（2026-09-25、PR #40）: 設計文書の必須表（R1（PR #26 承認）。全体計画書 §8.5）を加えた。部品状態の状態×出来事表（第6.5.1節）、評価要求の状態×出来事表（第6.10.1節）、出力の保持の状態×出来事表（第6.12.1節）、取引機会の状態×出来事表（第7.2.1節）、値の伝播表（第7.8節）を置き、第1.2節に置き場所の一覧を足した。表は既存の節の規則を並べ替えたものであり、**本文の規則は変えていない**。本文から埋められなかったマス5件を「要決定（R1-D05-1・3〜6）」として各表の直後に一覧した（人間の決定を待つ）。
v2.8（2026-09-25、段階3 の文書整合 PR）: 段階3 完了の点検で、段階3 の範囲の状態が「承認待ち」のまま残っていることが見つかった。段階3 の実装は PR #35 の merge（2026-09-25）で完了しているので、人間の決定（2026-09-25）により、**状態欄を「承認済み（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35）」に改めた**（冒頭の状態・第1.2節・第14節の表）。本文の規則は1つも変えていない。
v2.7（2026-09-25、段階3 完了の文書 PR）: 段階3 実装 PR 5/5（エンジン＋受入、PR #35）が置いた**仮置き8件を人間が承認した（2026-09-25）**。8件の正本はバックテスト基盤の設計書（D06 v1.8 §4.4・§8.3・§9.2）とテスト戦略（D08 v1.7 §2.2）であり、本書には**本書の規則とつながる2点だけ**を【確定】として書き足した（第4.10節: トレーリングが返した損切り水準の更新をエンジンがどう適用するか＝仮置き1〜5 の要約、第6.6節: エンジンが処理点の番号を振り直すときも本節の共有された順序を保つこと＝仮置き6）。新しい規則は置いていない（8件は実装が既に採っていた解釈であり、段階2 の実行結果も検証戦略 A の固定出力も変わらない）。**段階3 の実装は PR #35 の merge（2026-09-25）で完了した**（全体計画書 §8.2）。
v2.6（2026-09-25、段階3 実装 PR 5/5）: 段階3 実装 PR 4/5（ランタイム後半、PR #33）が置いた**仮置き14件を人間が承認した（2026-09-25）**ので、該当する節へ【確定】として書き足した（第6.3節: 終端した機会を読む入力＝13、第6.8節の手順2: 失効の範囲と閉じた理由の欄＝2・3、第6.10節: 対象ごとの追い越し＝10、第6.11節: 建玉の知り方＝1・通し番号の振り方＝14、第7.3節: 再検査の「読めない」の細目＝5・6・7、第7.6節: 連鎖による待機の表し方＝9、第7.7節: 確認の経路の細目＝4・8・11・12）。あわせて、**確認の通し番号（`attempt_index`）を評価記録に写さない**人間の決定（2026-09-24）を受け、第6.11節に「判断履歴での並びは評価記録の `request_id` の昇順で読める」ことを1文足した。新しい規則は置いていない（14件は実装が既に採っていた解釈であり、段階2 の実行結果も検証戦略 A の固定出力も変わらない）。損切り水準の更新（`UpdateStop`）を管理要求の区分に加えたこと（第3節の `ManagementAction`、2026-09-24 の人間の決定）は、D04 §11.2 と本書第3節が既に3区分として書いていたものの実装であり、本書の記述は変えていない。
v2.5（2026-09-24、段階3 実装 PR 4/5）: 段階3 実装 PR 3/5（ランタイム前半、PR #32）が置いた**仮置き8件を人間が承認した（2026-09-24）**ので、該当する節へ【確定】として書き足した（第6.6節・第6.7節・第6.8節・第6.9節・第6.10節）。あわせて、PR #32 が要決定として上げた「**待機中に、受け取った取引機会が終わった要求をどう決着させるか**」への**人間の決定（2026-09-24）**を反映した。**見送り（`Skipped`）で閉じ、待機の出来事の種類を1つ足して理由を残す**（第3節の `WaitEventKind` に `OPPORTUNITY_ENDED` を足し、第6.8節の手順2 に結末を書いた。判断履歴の表16 の語彙として D06 v1.7 §9.2 も追随した）。新しい規則は置いていない（仮置き8件は実装が既に採っていた解釈であり、段階2 の実行結果も検証戦略 A の固定出力も変わらない）。
v2.4（2026-09-24、PR #27）: 検証戦略 B の評価順についての**人間の再決定（2026-09-24）**を反映した。(1) **取引機会を出す使用箇所から、取引機会を読む使用箇所（`RuntimeInputRef(OPPORTUNITY)` を入力に持つ使用箇所）への因果辺を4本目として足した**（D04 v1.13 §12）。種類名は `OPPORTUNITY_CONTEXT` とした（第3節の型表・第5.4節）。確認は同じ判断時点の開始足で行う（第7.7節）ので、確認部品は取引機会を出す部品の後に評価されなければならない。(2) この辺を含めて第5.4節の段の規則を検証戦略 B に当てた出力を、**第9.2節末尾の評価順として書き直した**（先の決定「設計書と T02 を規則の出力に合わせる」の適用）。変わったのは P1 の6件の並びだけで、市場状態 → 取引機会 → 確認 → 注文 の並びは v2.3 と同じである。検証戦略 A は取引機会を読む使用箇所を持たないので、評価順も指紋も変わらない。
...
| 【提案】 | 本書が推奨する設計。承認で確定 |
| 【要決定】 | 承認時にユーザーが選択する事項。段階2 の Q1〜Q9 は **2026-09-21 に決定済み**（第13節）、段階3 の Q10〜Q18 は **2026-09-22 に**、Q19〜Q22 と、紙上トレース [T02](../traces/T02_paper_trace_strategy_b.md) が立てた Q23〜Q29、およびその反映から生じた Q30 は **2026-09-23 に決定済み**（第15節）。**本書に未決の項目は残っていない** |
...
**v2.0 のレビューの対象範囲は、下表の「段階3 で本改訂（v2.0）が決めること」の列**である。**この列は承認済みである**（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35。v2.8 で記録。第14節）。列は名前で呼ぶ（番号で数えると、段階2 の4列から段階3 の5列へ増えたときに指す先がずれる）。
...
**必須表（全体計画書 §8.5、R1）**。本書は状態機械を4つ（評価要求・取引機会・部品状態・出力の保持）と、部品・層をまたいで運ばれる値を定めるので、全体計画書 §8.5 の3つの表をすべて持つ。表1 は本節の上の表（境界表）である。

| 表 | 置き場所 |
|---|---|
| 表2 状態×出来事表（部品状態） | 第6.5.1節 |
| 表2 状態×出来事表（評価要求） | 第6.10.1節 |
| 表2 状態×出来事表（出力の保持） | 第6.12.1節 |
| 表2 状態×出来事表（取引機会） | 第7.2.1節 |
| 表3 値の伝播表 | 第7.8節 |

表2・表3 は本文の節の言い換えではなく網羅の確認であり、本文と食い違えば本文が正本である。本文から埋められないマスは「要決定（R1-D05-n）」と書き、各表の直後に一覧した。**v2.10 で5件とも人間が決定し（2026-09-25）、マスを【確定】に置き換えた**。各表の直後の一覧は決定の記録として残す。
...
| `EdgeKind` | `compiler` | enum | `EXPLICIT_INPUT` / `FILL_TRIGGER` / `POSITION_CONTEXT` / `MARKET_STATE` / `OPPORTUNITY_CONTEXT`（後4者は D04 §12 の因果辺。`MARKET_STATE` は市場状態から取引機会を出す使用箇所への辺で、v2.3、2026-09-24 の人間の決定。第7.6節。`OPPORTUNITY_CONTEXT` は取引機会を出す使用箇所から `RuntimeInputRef(OPPORTUNITY)` を読む使用箇所への辺で、v2.4、2026-09-24 の人間の再決定。第7.7節） | §5.4 |
...
| `WaitEventKind` | `runtime.waiting` | enum | `WAIT_STARTED` / `INPUT_ARRIVED` / `RESUMED` / `DEADLINE_REACHED` / `SUPERSEDED` / `RUN_END_CLOSED`（run 末尾で閉じた。§6.1） / `OPPORTUNITY_ENDED`（受け取った取引機会が待機のあいだに終わったので閉じた。§6.8 の手順2。v2.5、2026-09-24 の人間の決定） | §6.8 |
...
| `OpportunityTransition` | `runtime` | レコード（**改訂**、v2.10。2026-09-25 の人間の決定、R1-D05-5） | 段階2 の8項目に `confirmation_start_bar: BarKey \| None` / `confirmation_deadline_at: WaitDeadline \| None` を足す。**生成の遷移（`from_state=None`）の行だけが値を持ち**、機会の `OpportunityLifecycle` が生成時に決めた値（期限は生成時点の値）を写す。他の遷移の行と、確認待ちでない戦略の機会では `None`。**実装は後続の実装 PR** | §7.7・§7.8 |
| `WaitEvent` | `runtime.waiting` | レコード（**改訂**、v2.10。2026-09-25 の人間の決定、R1-D05-6） | 段階3 の5項目に `pinned_bars: Mapping[str, BarKey]`（入力名 → 固定した対象足。`WaitingRequest.pinned_bars` の写し）を足す。**「待機に入った」（`WAIT_STARTED`）の出来事だけが値を持ち**、他の種類では空。待機期限は足さない（評価記録の結末 `Waiting.deadline_at` が既に持つ）。**実装は後続の実装 PR** | §6.8・§7.8 |
...
| `RuntimeStepResult` | `runtime` | レコード（**改訂**） | 段階2 の5項目に `wait_events: tuple[WaitEvent, ...]`・`validity_rechecks: tuple[ValidityRecheck, ...]`・`confirmation_attempts: tuple[ConfirmationAttempt, ...]`（いずれも既定は空）を足す。確認試行の列は **Q26 決定（2026-09-23、選択肢1）**で足したもので、その `step` で作った・書き換えた試行だけを毎回返す | §6.8・§7.3・**§7.7** |
...
**エンジンが本部品の更新をどう適用するか**【確定】（v2.7、2026-09-25 の人間の承認。PR #35 の仮置き1〜5。**正本は D06 v1.8 §8.3 の損切り水準の更新の表**で、ここには本部品とのつながりだけを書く）。(1) 本部品は現在の水準より有利な値だけを返すが、価格刻みへの丸めで現在の水準と同じ値に戻ることがある。**丸めた結果が現在の水準と同じなら、エンジンは「不利ではない」として適用する**（版は1つ進む）。(2) 更新が有効になるのは**判断時刻以降に始まる最初の予定上の執行足**で、約定後の評価起動点（第8節）が返した更新も同じ足を使う（段階3 の宣言では、約定後の評価起動点から更新は出ない）。その足が予定に無ければ適用せず `DATA_ERROR` で残す。(3) 丸めた後の水準は判断履歴の表12 の適用結果に、検査に使った参照価格の足は根拠記録に残る。本節の計算規則（不利な向きへ動かさない判定を部品側に置くこと）は変えない。
...
- 節点は使用箇所（`instance_id`）。辺は D04 §12 が確定した5種類（明示入力＋因果辺4本。3本目の市場状態の辺は段階3 で足した `EdgeKind.MARKET_STATE`（v2.3）、4本目の取引機会の参照の辺は `EdgeKind.OPPORTUNITY_CONTEXT`（v2.4。取引機会を出す使用箇所 → 取引機会を読む使用箇所。確認部品が同じ判断時点の取引機会を開始足で確認できるようにするため。第7.7節））で、`EdgeKind` で区別して保持する。因果辺も**種類を残したまま**保持するのは、循環が明示接続によるものか帰還路によるものかを `CompileError.message` で示すためである。
- 循環検出は明示辺と因果辺すべての和の上で行う。閉路があれば `DEPENDENCY_CYCLE`。
...
段階3 で**新しく必要になる検査**は次の8件である【提案】。**検査項目そのものと「その検査が読む宣言」の正本は D04 §12 であり（第1.1節・第1.2節の行3）、本節はそれを確定するものではない**。ここに挙げるのは D04 §12 への**改訂提案**であり、8件はいずれも D04 の改訂で確定済みである（起草時の7件は第12.1節の依頼7 として v1.9・v1.10 へ、検査 h は v1.11 へ）。**本書が決めるのは、確定後の8件をコンパイラのどの段で実行し、失敗をどの拒否の区分で返すか**（第5.1節・第5.2節）だけである。この分担を崩して本書側で検査を確定させると、正本の検査一覧に載っていない検査が生まれ、D04 §12 だけを読んで作ったコンパイラがそれらを実施しない。**D04 の改訂が入るまでは、コンパイラに実装しない**（PR #22 で D04 v1.9 へ反映し、Q19〜Q22 の決定に伴う検査 e・f の確定を v1.10 へ、Q30 の決定に伴う検査 h を本 PR で v1.11 へ反映した。**未反映の検査は無い**）。起草時は5件だったが、Q18 の決定（選択肢2）により検査 f が、独立レビューの指摘により検査 g が、Q30 の決定（2026-09-23、選択肢1）により検査 h が加わった（検査 h は D04 v1.11 の検査 #15 として確定済みである）。
...
| h | **接続元が出力参照（`OutputRef`）だけの入力に、本数で数える待機期限**（`WaitForInput(deadline=BarsDeadline(...))`）**を書いた使用箇所は、足の確定（`OnBarClose`）の起動条件の系列がただ1つ**であること（下段落。Q30 決定、2026-09-23、選択肢1） | `InputSpec.read_spec`（`WaitForInput.deadline` の区分）、`InputBinding.sources`（接続元がすべて `OutputRef` か）、`EvaluationSchedule.triggers`（`OnBarClose.series`） | `SCHEDULE_NOT_ALLOWED` |
...
**出力参照だけが欠けうる待機の起動系列も1つに限る**【確定】（検査 h。Q30 決定、2026-09-23、選択肢1）。出力参照の入力には系列が無いため、本数で数える待機期限はその使用箇所の起動条件の系列で数える（第6.8節。Q28 決定）。区間の違う足の確定は区間ごとに別の評価要求になる（第6.2節 手順3）ので、区間が違えば数える系列は要求ごとに定まる。定まらないのは**区間が同じで系列だけが違う起動条件が1件の要求へ集約された**場合で、そのとき期限に到達する判断時点が処理順や系列名の並びで変わり、**同じ入力から2通りの判断履歴が出る**（期限切れで見送った評価の件数が変わるので、D07 §6.1 の集計も変わる）。検査 b（確認部品）・検査 g（決済部品）が同じ理由で置いた制限を3か所目に当てるだけであり、**新しい考え方を持ち込まない**。**この制限は段階3 の宣言を1つも狭めない**。第9.2節で出力参照だけの待機が起きる使用箇所（`market_state`・`daily_above_ema`）は、どちらも日足1系列でしか起動しないためである。検査の正本は D04 §12 の #15（v1.11）であり、本節が決めるのは拒否の区分だけである（本節冒頭の分担）。**不採用**: 集約した要求では系列 ID の昇順で先頭を使う案（検査は増えないが、待機の期限の長さが戦略の意図と無関係な系列名の並び順で決まり、判断履歴を読む人に説明できない）、実行時に失敗させる案（宣言から書けてしまう構成を run の途中まで見つけられず、検査 f・g が採った「宣言時に止める」方針と逆になる）。
...
10. `RuntimeStepResult` を返す。**確認試行は、この `step` で作った・書き換えた `ConfirmationAttempt` だけを `confirmation_attempts` に載せる**【確定】（Q26 決定、2026-09-23、選択肢1。紙上トレース T02 §14 #4）。`OpportunityLifecycle.attempts`（実行時の状態）の全件ではなく差分だけを返す。エンジンはこれを主キー `(opportunity_id, bar_key)` で判断履歴の表18 へ**置き換えとして**書く（D06 §4.2・§9.2）。待機をはさんだ試行の結末を同じ1件の上で置き換える規則（第7.7節）が、そのまま表への置き換えとして写るので、エンジン側に新しい規則が要らない。この列が無いと、確認試行は `RuntimeState` の中にしか無く、**エンジンが表18 を書く経路が存在しない**（T01 §13 #19 で `EntryProposal` に根拠の出力 ID が無かったのと同じ形の阻害要因である）。**不採用**: run 末尾に `OpportunityLifecycle.attempts` をまとめて渡す案（run が途中で失敗すると確認の経過が1行も残らず、`is_run_end=True` の `step` に記録だけを返す例外がもう1つ増える）、表18 を評価記録と遷移記録からの派生表と位置づけてエンジンが書くのをやめる案（D06 §9.2 が表18 を独立させた理由と噛み合わない）。
...
**打ち切ったとき、まだ評価していない要求は記録しない**【確定】（v2.10、2026-09-25 の人間の決定。R1-D05-4。いまの実装どおり）。「以降の評価を行わずに返す」とき、同じ部品の残りの要求も、評価順で後の部品の要求も、評価記録を出さない。**評価の失敗で打ち切った `step` では、失敗の評価記録がその `step` の最後の評価記録である**（第6.4節の「必ず1件」の例外）。**有効性の再検査の失敗（`MISSING_FAILED`、第7.3節）で打ち切った `step` も同じ例外に入る**。この場合は失敗の評価記録（`Failed`）を出さず、失敗は `ValidityRecheck` に残る。その `step` でまだ番が来ていない要求の評価記録も出さない。同じ部品で既に作った要求は要求 ID（`RequestId`）を採番済みなので、**判断履歴の要求 ID に欠番が出うる**。失敗した run は評価（D07）が採用評価から外すので、指標には影響しない。**不採用**: 未評価の要求を全件「見送り」で記録する案（run の失敗で打ち切ったことを表す診断の語を新しく足す必要がある）、採番済みの要求だけ記録する案（どこまで作ったかが実装の順序に依存する）。
...
- **1回の評価で複数の入力が欠け、欠損方針が食い違うときは、強い方針が勝つ**【確定】（Q27 決定、2026-09-23、選択肢1。紙上トレース T02 §14 #19）。優先順位は **`Error` > `SkipEvaluation` > `WaitForInput` > `UsePrevious`** とし、欠けた入力すべての `on_missing` のうちこの順でいちばん強いもの1つを、その評価全体に適用する。`breakout_trigger` v2 は `level` が待機（`WaitForInput`）、`price` が見送り（`SkipEvaluation`）なので、日足と1時間足が同時に遅れるとこの規則が効き、**待たずに見送る**。第6.8節が「待っても解消しない欠損理由では待機に入らない」と決めた考え方を、入力の組にそのまま広げたものである。待っても解消しない入力が1つでもあるなら、待機しても評価は成立しない。**不採用**: 入力ごとに方針を適用して待機できる入力があれば待機に入る案（解消しない入力を抱えたまま期限まで run が止まる）、方針が食い違う構成をコンパイル時に拒否する案（安全だが `breakout_trigger` v2 を作り直すことになり、第9.2節の宣言と契約の版が増える）。
...
- **評価要求が指す取引機会が既に終端していたら、その機会を読む入力は欠損として扱う**【確定】（v2.6、2026-09-25 の人間の承認。PR #33 の仮置き13）。`RuntimeInputRef(OPPORTUNITY)` の入力（第6.11節）は、ランタイムが保持する `OpportunityLifecycle` から `Opportunity` を渡すが、機会が `TERMINATED` なら渡す値が無いので `INPUT_MISSING_OR_INVALID` の欠損とし、その入力の `on_missing` に従う。終端した機会の内容を読ませると、第6.2節の「終端した取引機会のイベントは下流へ配送しない」を入力の側から迂回できてしまう。
...
- 評価記録は起動した使用箇所ごとに必ず1件残す。入力不足で評価しなかったことも記録に残す【合意済み】上位 §4.3.15。**この規則は正常に進んだ `step` のものである**。評価の失敗、または有効性の再検査の失敗（`MISSING_FAILED`、第7.3節）で打ち切った `step` では、まだ番が来ていない要求の記録を出さない（第6.2節の「失敗は戻り値で返す」の後段。v2.10【確定】、R1-D05-4）。
...
#### 6.5.1 部品状態の状態×出来事表（必須表。R1）

全体計画書 §8.5 の表2 である。状態を宣言した使用箇所（`state_spec` が `None` でないもの。D04 §9.1）1つの `RuntimeState.component_states[instance_id]` について、本節の規則を状態×出来事に並べ替えた。状態を持たない使用箇所は `new_state=None` を返すだけで、本表の対象にならない（第3節の `StatelessImplementation`）。規則の正本は本節と第6.2節であり、本表は網羅の確認である。

状態は3つで数える。「run 開始前」（値が無い）、「初期値」（`StateSpec.initial` から構築した値のまま）、「更新済み」（直近の `Evaluated` で部品が返した `new_state`）。`EDGE` の Trigger の再武装（D04 §10.4）はこの状態の値（`condition_state@v1` の `satisfied`）で表され、再武装の判定そのものは部品の計算規則が持つ（第4節）。本表は値の置き換えの規則だけを扱う。

| 出来事 | run 開始前 | 初期値 | 更新済み |
|---|---|---|---|
| S1 run 開始（`reset_on=(RUN_START,)`） | → 初期値。`LiteralInitialState` から構築し、実装側の既定値に委ねない（本節、D04 §9.1） | 到達しない。run 開始は1 run に1回で、状態は run をまたいで復元しない（本節） | 到達しない（同左） |
| S2 評価が `Evaluated` で決着した（部品を呼び、戻り値の検査4点に合格） | 到達しない（評価は run 開始の後にだけ起きる） | → 更新済み。`new_state` で置き換える。差分・部分更新はしない（本節） | → 更新済み（同左。値は新しい `new_state`） |
| S3 部品を呼んだが戻り値の検査に違反した（`new_state` の型違いを含む）、または部品が例外で終わった | 到達しない | 変化なし。`OutputRecord` を作らず状態も更新せず、`Failed(DATA_ERROR)` で決着する（第6.2節の検査4点と「失敗は例外ではなく戻り値で返す」） | 変化なし（同左） |
| S4 評価が `Skipped` で決着した（欠損の見送り、遡りの不成立、待機期限の見送り、機会の終端による見送り、末尾の見送りを含む） | 到達しない | 変化なし（本節。欠損で飛ばした評価を再武装の判定に効かせない） | 変化なし（同左） |
| S5 評価が欠損で `Failed` になった（`on_missing=Error`、待機期限の失敗） | 到達しない | 変化なし（本節） | 変化なし（同左） |
| S6 評価が `Waiting` を記録した | 到達しない | 変化なし。部品を呼んでおらず `new_state` が存在しない（本節、v2.1） | 変化なし（同左） |
| S7 待機中の要求が `Superseded` で決着した | 到達しない | 変化なし（同左） | 変化なし（同左） |
| S8 待機から再開した評価が決着した | 到達しない | 決着の結末に応じて S2〜S5 と同じ（本節「その1回ぶんだけ上の規則が当てはまる」） | 同左 |
| S9 同じ `step` の前の評価が `Failed` になり、この使用箇所の評価が行われなかった | 到達しない | 変化なし（部品を呼ばない。第6.2節「以降の評価を行わずに返す」） | 変化なし（同左） |
| S10 run 末尾（`is_run_end=True` の `step`） | 到達しない | 変化なし。この `step` は起動判定を行わず、残った待機要求は `Skipped` で決着する（第6.1節、S4） | 変化なし（同左） |
| S11 run の失敗（D06 §4.2 の停止判定） | 到達しない | 変化なし。状態は run 内のメモリにだけあり、判断履歴にも出さない（本節） | 変化なし（同左） |

状態の値そのものは判断履歴に出さない。`EvaluationId` から評価履歴を辿れば再現できる（本節）。本表に要決定のマスは無い。
...
**エンジンが番号を振り直しても、この共有された順序は保たれる**【確定】（v2.7、2026-09-25 の人間の承認。PR #35 の仮置き6。**正本は D06 v1.8 §4.4**）。エンジンは1回の `step` が返した取引機会の遷移・待機の出来事・有効性の再検査の3列を、本節の番号で刻まれた `(フェーズ順位, 通し番号)` の順に1本に並べてから、自分の採番列へ載せ替える。したがって判断履歴の処理点の順は、`step` の中での発生順と一致する。

**使用箇所の評価が属するフェーズは役割で決める**【確定】（v2.5、2026-09-24 の人間の承認。PR #32 の仮置き1）。待機の出来事（`WaitEvent.at`）や評価の中で刻む処理点のフェーズは、その使用箇所の役割から引く。市場状態（`market_state`）→ `P2_MARKET_STATE`、取引機会（`trigger`）→ `P3_TRIGGER`、後続確認（`execution_filter`）→ `P4_CONFIRMATION`、注文意図・保護水準・決済（`order` / `protection` / `exit`）→ `P5_ORDER_INTENT`、役割の無い使用箇所 → `P1_FEATURE` とする。受付結果や実行時イベントを運ぶ第2回の `step` の評価は、約定後の評価起動点（`POST_FILL_EVALUATION`、第8節）で刻む。上位設計書 §4.3.12 の P1〜P5 が役割の名前だからであり、紙上トレース T02 §1.5・§7.2 の記録と同じである。ライフサイクル検査の出来事は `OPPORTUNITY_LIFECYCLE` で刻む（第6.8節）。
...
**`subject` と `freshness_time` の細目**【確定】（v2.5、2026-09-24 の人間の承認。PR #32 の仮置き4・6・8）。

- **区間が同じで系列の違う足の確定を1件の要求へ集約したとき**（第6.2節 手順3、Q6 決定）、`subject` は**系列の正規表記がいちばん小さい足**とする（仮置き4）。対象区間は1つなので、どの足を選んでも `observation_interval` は同じであり、選び方を決めておくことで同じ入力から同じ記録が出る。
- **代表の鮮度に数えるのは値の入力だけ**である（仮置き6）。最新1件の読み取り（`ValueSample`）と履歴窓（`ValueWindow`）を数え、配送イベント（`EventDelivery`）と現在状態（`ContextSnapshot`）は数えない。後2者は鮮度基準時刻を持たないからである。値の入力が1つも無い評価は `decision_time` になる（上の表の最終行と同じ）。
- **対象区間を持たない評価**（実行時イベントで起動し、その連鎖で起動した評価。第6.2節 手順3）の `VALUE` の出力は、`subject` と `observation_interval` を**両方とも空**にした `Observation` で包み、最新1件（`RuntimeState.latest_outputs`）だけを更新する（仮置き8）。観測した足が定まらない出力は履歴に積まない（第6.12節）。上位設計書 §4.3.15 の4項目のうち、足と区間を空にできる形である。
...
**系列の違う市場データ参照を2つ以上接続した入力は待機に入れない**【確定】（v2.5、2026-09-24 の人間の承認。PR #32 の仮置き5）。`pinned_bars` は入力名ごとに1本の足しか固定できない（第3節の `WaitingRequest`）ので、系列の違う接続元を持つ入力が待機を宣言したまま欠けた場合は**構造エラー**（`KernelValueError`）で止める。同じ系列の複数の項目（高値と安値など）は1本の足の鍵で固定できるので待機に入れる。段階3 のカタログには該当する契約が無い。
...
**足りない入力が出力参照だけのときは、その使用箇所の起動条件の系列で数える**【確定】（Q28 決定、2026-09-23、選択肢1。紙上トレース T02 §14 #8）。出力参照（`OutputRef`）の入力には対応する系列が無いため、上の「その入力の系列」が引けない。そこで、その使用箇所の起動条件 `OnBarClose(name, series)` が持つ系列を使い、`WaitUntilBars(series=その系列, remaining=n)` とする。その使用箇所が「どの足のリズムで動くか」をいちばん素直に表すからである。検証戦略 B の `market_state` と `daily_above_ema` はどちらも日足で起動するので、上流（`daily_ema`）の期限と一致し、日足の連鎖3段の期限がそろう（T02 §7.1）。**不採用**: 上流をたどって最初に見つかった市場データ参照の系列で数える案（結果が同じになる場面が多いが、上流が複数あるときにどれを選ぶかの規則を新しく作ることになる）、出力参照だけの待機では本数の期限を使えないことにしてコンパイル時に拒否する案（期間で数える期限しか書けなくなるが、期間値を含む宣言はダイジェストを計算できない（第5.5節）ため実験として固定できない）。

  **この規則が使える形になっていることは、コンパイル時に検査する**【確定】（Q30 決定、2026-09-23、選択肢1。第5.6節の検査 h、D04 §12 の #15 v1.11）。区間の違う足の確定は区間ごとに別の評価要求になる（第6.2節 手順3）ので、区間が違えば数える系列は要求ごとに定まる。定まらないのは**区間が同じで系列が違う起動条件が1件の要求へ集約された**場合だけである。そこで、**出力参照だけが接続元の入力に本数で数える待機期限を書いた使用箇所は、足の確定の起動条件の系列がただ1つでなければならない**とし、そうでない宣言は `SCHEDULE_NOT_ALLOWED` で拒否する。**段階3 の宣言（第9.2節）は1つも狭まらない**（該当する使用箇所がどれも1系列で起動するため）。
...
2. 期限（`deadline_at`）と追い越し（第6.10節）と、受信済み機会の失効（第7.3節の `REQUIRE_UNTIL_ORDER_REQUEST` の再検査）を検査する。**期限・追い越し・失効は再開より先に判定する**。
   ...
   **受け取った取引機会が待機のあいだに終わった要求は、見送りで閉じる**【確定】（v2.5、2026-09-24 の人間の決定）。失効が成立した要求は `Skipped(まだ足りていなかった入力の診断)` で決着させ、待機の出来事 `WaitEvent(OPPORTUNITY_ENDED)` を1件残す。「なぜ閉じたか」は待機の出来事の種類が表し、評価の結末の区分は増やさない（run 末尾で閉じた要求を `RUN_END_CLOSED` と見送りで表したのと同じ形。第6.1節）。失敗（`Failed`）にしないのは、機会の終端は戦略の状態の正常な推移であり、データ誤りとして集計させないためである。構造エラーで止める扱い（PR #32 の実装）は、確認待ちのあいだに期限で終わった機会を待つ要求が1件あるだけで run が止まるので採らない。要求が確認評価のものなら、その確認試行の結末も `SKIPPED` へ置き換わる（第7.7節）。

   **失効を判定する「受け取った取引機会」の範囲と、閉じた理由の欄**【確定】（v2.6、2026-09-25 の人間の承認。PR #33 の仮置き2・3）。失効の判定に含める機会は、配送で受け取った機会（`WaitingRequest.opportunity`）に加えて、**要求が対象として指す機会**（確認評価の要求の `opportunity_id`。第6.11節）である。確認評価の要求は機会を配送では受け取らないが、その機会の確認を問うている以上、機会が終わればその問いも意味を失うからである。`WaitEvent(OPPORTUNITY_ENDED)` の `reason` には、**終わった機会の終端理由**（例: 確認期限による `EXPIRED`）を写す。待機の出来事の種類は「機会が終わったから閉じた」ことだけを表し、なぜ機会が終わったかは終端理由で読む（取引機会の終端理由の語彙は増やさない。上位設計書 §4.5）。

   **この並び（期限 → 追い越し → 失効）は判定順である**【確定】（Q29 決定、2026-09-23、選択肢1。紙上トレース T02 §14 #12）。同じ判断時点で2つ以上が同時に成立したら、**先に成立したものだけで決着させ、後ろの判定は行わない**。期限と追い越しが同時なら**期限が勝つ**ので、記録は `WaitEvent(DEADLINE_REACHED)` と `on_deadline` に従う評価記録（`Skipped` または `Failed`）になり、`WaitEvent(SUPERSEDED)` と `Superseded` は残らない。期限は宣言（`WaitForInput.deadline`）が決めた上限で、追い越しは市場データの到着順が決める事象なので、**宣言が決めたほうを優先する**ほうが判断履歴から読みやすい。確認期限が確認の評価に勝つ規則（第7.7節の「判定の順序」）とも向きがそろう。この順序は評価の結果区分別の集計（D07 §6.1）の値を変えるので、決めないと同じ入力から2通りの集計が出る。**不採用**: 追い越しを先に判定する案（新しい足の要求が既に立っている実態には近いが、期限という宣言上の上限を到着順が上書きすることになる）、同時成立が起きない遅延設定だけで段階3 を進める案（実データの公開遅延は選べないので段階4 以降で必ず戻ってくる）。
...
**固定した足を判断履歴に残す**【確定】（v2.10、2026-09-25 の人間の決定。R1-D05-6）。`WaitEvent(WAIT_STARTED)` に、待機に入ったときに固定した足（`pinned_bars`）を写す（第3節の `WaitEvent` の改訂）。これで、再開した評価がどの足を読んだかを判断履歴（表16）だけで確かめられる。**待機期限は写さない**。待機に入った評価記録の結末 `Waiting.deadline_at` が既に持ち、表2 に書き出されているからである。**実装は後続の実装 PR で行う**（それまでの実装は表16 に固定した足を書き出さない）。**不採用**: 記録せず再計算に任せる案（確かめるたびに公開予定からの再計算が要る）、評価記録の結末 `Waiting` に足す案（戦略側が正本の評価記録の型が待機専用の項目で太る）。
...
**遡りの細目**【確定】（v2.5、2026-09-24 の人間の承認。PR #32 の仮置き3・7）。

- **1回の評価で遡った接続元が2つ以上あり、読んだ足が違うとき**は、出力の `Observation` の `subject` と `observation_interval` を**いちばん古い足**のものにする（仮置き3）。古さを隠さない向きに倒すためである。
- **経過時間で書いた遡りの上限**（`DurationWindow(d)`）は、`history` の経過時間の窓と同じく「**足の終了時刻が `(at - d, at]` に入る足**」まで遡れる、と数える（仮置き7）。上限の数え方を読み方ごとに変えないためであり、数えるのはビューの側である（上の表の「上限の渡し方」）。
...
5. **追い越しは「押しのける側の要求」があるときにだけ成立する**【確定】（v2.5、2026-09-24 の人間の承認。PR #32 の仮置き2）。対象系列に新しい足が公開されただけでは閉じず、**その使用箇所が固定した足より新しい足の要求を持っている**（前の `step` までに作った、または同じ判断時点に足の終了予定があって評価の段で作る）ときに閉じる。後着優先（上の2・3）の実施であり、`by_request_id`（押しのけた側）を必ず埋めるためである。同じ `step` の中で失敗して新しい要求が作られなかった場合は、追い越しを取り消して待機に戻す（押しのけた側を持たない `Superseded` を残さない）。
6. **取引機会・建玉を対象とする要求では、押しのける側を「同じ対象」の要求に限る**【確定】（v2.6、2026-09-25 の人間の承認。PR #33 の仮置き10）。第6.11節の要求（確認評価・トレーリング）は同じ使用箇所・同じ系列の要求が対象ごとに並ぶ。上の1〜5 の「その使用箇所・その系列」を**同じ機会（または同じ建玉）の要求**に読み替え、別の機会の新しい要求は押しのける側にしない。第7.7節の「同じ機会に新しい対象足の要求を作る」の実施であり、別の機会の確認足が進んだだけでこの機会の問いが閉じることは無い。
...
### 6.10.1 評価要求の状態×出来事表（必須表。R1）

全体計画書 §8.5 の表2 である。第6.4節・第6.8〜6.11節が決めた評価要求（`EvaluationRequest`）の扱いを、状態×出来事に並べ替えた。規則の正本は各節であり、本表は網羅の確認である。

状態は次の5つで数える。「評価待ち」は名前の付いた結末ではなく、要求を作ってから同じ `step` の評価順（第5.4節）で番が来るまでの間、または待機から再開してから評価するまでの間を指す。

- **生成前**: 要求がまだ無い。
- **評価待ち（新規）**: 起動判定（第6.2節 手順1〜3、第6.11節）で作られ、まだ評価していない。
- **評価待ち（再開後）**: 待機中の要求が再開した（第6.8節 手順4）。元の `request_id` のまま評価する。
- **待機中**: 結末 `Waiting` を記録した。終端ではない（第6.4節）。
- **決着済み**: `Evaluated` / `Skipped` / `Failed` / `Superseded` のいずれかを記録した。終端（第6.4節）。

| 出来事 | 生成前 | 評価待ち（新規） | 評価待ち（再開後） | 待機中 | 決着済み |
|---|---|---|---|---|---|
| E1 起動条件の成立（足の確定・入力イベントの配送・実行時イベントの通知） | 要求を作り「評価待ち（新規）」へ。数は第6.2節 手順2 の表と第6.11節の表のとおり。確認評価は対象足が開始足で `include_start_bar=False` なら作らない（第7.7節）、評価の前の再検査が不成立・読めず失敗なら作らない（第7.7節の細目、仮置き8）。対象が0件なら作らない（第6.11節） | 到達しない。同じ対象区間の起動は1件に集約する（第6.2節 手順3、Q6 決定）。別の対象・別の区間なら別の要求（生成前の欄） | 変化なし。同じ使用箇所の新しい要求は別の要求である（生成前の欄） | 変化なし。新しい要求は別の要求であり、この要求が閉じるかどうかは E12 で決まる（第6.10節） | 変化なし。決着した要求は復活させない（第6.4節） |
| E2 評価: 必須入力がすべて読めた（遡りで代用した入力を含む） | 到達しない（要求が無い） | `Evaluated` で決着（第6.4節）。遡りは `substitutions` に残す（第6.9節） | `Evaluated` で決着。元の `request_id` を保つ（第6.8節） | 到達しない。待機中は評価しない（第6.8節 手順3） | 到達しない（評価しない） |
| E3 評価: 欠損があり、欠けた入力の最も強い方針が `SkipEvaluation`（遡りが成立しなかった `UsePrevious` を含む） | 到達しない | `Skipped` で決着（第6.3節、方針の強さは Q27 決定。遡り不成立は第6.9節） | `Skipped` で決着（第6.8節） | 到達しない | 到達しない |
| E4 評価: 欠損があり、最も強い方針が `Error` | 到達しない | `Failed(DATA_ERROR)` で決着し、以降の評価を行わずに返す（第6.2節・第6.3節） | 同左（第6.8節） | 到達しない | 到達しない |
| E5 評価: 欠損があり、最も強い方針が `WaitForInput`、欠損理由が待てば解消するもの | 到達しない | `Waiting` を記録して「待機中」へ。`WaitEvent(WAIT_STARTED)` を残す（第6.8節） | 到達しない。再開した評価は `Evaluated` / `Skipped` / `Failed` のいずれかで決着する（第6.8節） | 到達しない | 到達しない |
| E6 評価: 最も強い方針が `WaitForInput` だが、欠損理由が `WARMUP_INSUFFICIENT` / `MAX_AGE_EXCEEDED` | 到達しない | `Skipped` で決着（第6.8節の表） | `Skipped` で決着（同表） | 到達しない | 到達しない |
| E7 評価: 部品の例外・戻り値の検査違反・観測区間の不一致・確認結果が無い確認評価 | 到達しない | `Failed(DATA_ERROR)` で決着し、以降の評価を行わずに返す（第6.2節・第6.7節・第7.7節の細目、仮置き11） | 同左 | 到達しない | 到達しない |
| E8 評価: 待機を宣言した入力が系列の違う市場データ参照を2つ以上持ち、欠けた | 到達しない | 拒否: `KernelValueError`（第6.8節、仮置き5） | 到達しない（待機に入れないので再開も無い） | 到達しない（同上） | 到達しない |
| E9 足りない入力の一部が読めるようになった | 到達しない | 到達しない（待機していない） | 到達しない | 変化なし。`missing` を更新して待機を続け、`WaitEvent(INPUT_ARRIVED)` を処理点1つにつき1件残す（第6.8節 手順3） | 変化なし |
| E10 足りない入力がすべて読めるようになった | 到達しない | 到達しない | 到達しない | 「評価待ち（再開後）」へ。`WaitEvent(RESUMED)`。市場データ参照はライフサイクル検査、出力参照は評価の段で判定する（第6.8節 手順1・4） | 変化なし |
| E11 待機の期限に到達した | 到達しない | 到達しない（期限は待機記録のもの） | 到達しない | `on_deadline` に従い `Skipped` か `Failed(DATA_ERROR)` で決着。`WaitEvent(DEADLINE_REACHED)`（第6.8節）。E12・E13 と同時なら期限が勝つ（Q29 決定） | 変化なし |
| E12 追い越し: 対象系列に新しい足が公開され、同じ使用箇所（対象つきの要求は同じ対象）に押しのける側の要求がある | 到達しない | 到達しない。追い越しは待機中の要求だけを調べる（第6.10節） | 到達しない（同上） | `EXPIRE_REQUEST` なら `Superseded(by_request_id)` で決着し `WaitEvent(SUPERSEDED)`、`KEEP_WAITING` なら変化なし（出来事も残さない）。押しのける側が同じ `step` で作られなかったら取り消して待機に戻す（第6.10節 5・6）。期限と同時なら期限が勝つ（Q29 決定） | 変化なし |
| E12' 対象系列に新しい足が公開されたが、押しのける側の要求が無い | 到達しない | 到達しない | 到達しない | 変化なし（第6.10節 5） | 変化なし |
| E13 受け取った取引機会、または要求が対象として指す取引機会が終端した | 到達しない | 変化なし。評価のときに機会を読む入力が欠損になり、E3〜E5 の扱いになる（第6.3節、v2.6） | 同左 | `Skipped` で決着し `WaitEvent(OPPORTUNITY_ENDED)` を残す。`reason` に機会の終端理由を写す（第6.8節 手順2、v2.5・v2.6）。期限・追い越しより後に判定する（Q29 決定） | 変化なし |
| E14 run 末尾（`is_run_end=True` の `step`） | 到達しない。この `step` は要求を作らない（第6.1節） | 到達しない（同上） | 到達しない。この `step` は再開の判定を行わない（第6.1節） | `Skipped` で決着し `WaitEvent(RUN_END_CLOSED)`。取引機会の終端の後、`request_id` の昇順（第6.1節） | 変化なし |
| E15 run の失敗（D06 §4.2 の停止判定） | 到達しない | 記録しない。評価記録を出さないまま run が止まる。既に採番した `request_id` は判断履歴に現れず、欠番になる（第6.2節、v2.10【確定】、R1-D05-4） | 決着の評価記録を出さない（同左）。待機に入ったときの評価記録（`Waiting`）は表2 に残っているので、この要求 ID は欠番にならない | 変化なし。失敗した run では末尾の `step`（E14）を呼ばない（D06 §10.2）ので、待機中のまま決着しない | 変化なし |

- 評価記録（`EvaluationRecord`）は「待機中」へ入るときと決着するときに1件ずつ出る。`EvaluationId` は毎回新しく採番し、`request_id` は保つ（第6.8節）。部品の状態は `Evaluated` / `Skipped` / `Failed` のうち部品を呼んだ評価でだけ更新する（第6.5節）。
- 確認評価の要求の結末は、確認試行（`ConfirmationAttempt`）の結末に写る（第7.7節の表）。確認試行は独自の出来事を持たず、本表の決着を写すだけなので別の表を置かない。写さないのは `Failed` だけである（第7.7節の細目、仮置き11）。

**v2.9 で要決定としたマスと、その決定**（v2.10、2026-09-25 の人間の決定）。

| 番号 | 何が決まっていないか |
|---|---|
| R1-D05-4 | 同じ `step` で `Failed` が出て「以降の評価を行わずに返す」（第6.2節）とき、まだ番が来ていない評価要求の記録をどうするか。第6.4節の「評価記録は起動した使用箇所ごとに必ず1件残す」との関係が書かれていない。→ **決定（2026-09-25、v2.10）: 記録しない**。「必ず1件」は正常に進んだ `step` の規則とし、要求 ID の欠番を明記した（第6.2節・第6.4節）。実装変更なし |
...
**通し番号の振り方**【確定】（v2.6、2026-09-25 の人間の承認。PR #33 の仮置き14）。`attempt_index` は、同じ `step` の中でその使用箇所が作った要求に、**作った順で 0 から**振る。作る順は、起動条件の種類で **足の確定 → 入力イベント → 実行時イベント**、足の確定の中は**対象区間の順**、同じ対象区間の中は**対象の識別子の昇順**である（上の段落の「対象の識別子の昇順」を、1つの `step` で複数の起動が重なった場合まで広げたもの）。

**通し番号は評価記録に写さない**【確定】（2026-09-24 の人間の決定。第3節の `EvaluationRecord` は変えない）。判断履歴での並びは**評価記録の `request_id` の昇順で読める**。上の段落が「`request_id` だけでは並びが読めない」としたのは採番の順を決める前の理由であり、`RequestId` の採番を通し番号と同じ順で行うと決めたので、同じ使用箇所・同じ対象区間・同じ判断時刻の評価記録を `request_id` の昇順に並べれば通し番号の順と一致する。

**開いている建玉の知り方**【確定】（v2.6、2026-09-25 の人間の承認。PR #33 の仮置き1）。現在コンテキストのポート（`RuntimeContextView`、第6.1節）は建玉の一覧を返さない。そこでランタイムは、約定通知（`RuntimeEventNotice(POSITION_OPENED)`、第8節）で届いた建玉の識別子を自分の状態に覚えておき（`RuntimeState.open_positions`）、建玉1件につき1要求を作るときに、**現在コンテキストで読めなくなった建玉（決済済み）を外す**。ポートに一覧の操作を足さないのは、ポートを1つ増やすと呼び出し順の規則がもう1本要るためである（D06 §4.2 の「入口が2つになると」と同じ理由）。
...
#### 6.12.1 出力の保持の状態×出来事表（必須表。R1）

全体計画書 §8.5 の表2 である。保持本数の計画に載る出力参照 `r`（`OutputRetentionPlan.by_output[r] = m`）1つの `RuntimeState.output_history[r]` について、本節の規則を状態×出来事に並べ替えた。最新1件の保持（`latest_outputs[r]`）は同じ処理で同時に更新する（本節「最新1件の保持はそのまま残す」）ので、別の表を置かず各マスに併記する。

| 出来事 | 空（0件） | 上限未満（1〜m−1件） | 上限ちょうど（m件） |
|---|---|---|---|
| H1 run 開始 | 変化なし（空から始まる。保持した出力は run 開始以外で初期化しない。本節「いつ捨てるか」） | 到達しない（run 開始は1 run に1回） | 到達しない（同左） |
| H2 `r` の `VALUE` 出力が出た。`subject` があり、同じ足の行がまだ無い | → 1件（m=1 なら上限ちょうど）。`latest_outputs[r]` も同じ出力へ更新する | 観測した足の順の位置へ積む（到着順ではない）。m件に達すれば上限ちょうどへ | 足の順の位置へ積み、m を超えた1件を先頭（最も古い足）から落とす。上限ちょうどのまま（本節「いつ捨てるか」） |
| H3 `r` の `VALUE` 出力が出た。同じ `subject` の行が既にある | 到達しない（行が無い） | 置き換える。件数も並びの位置も変えない（Q20 決定）。`latest_outputs[r]` は到着順で更新する | 同左（打ち切りは起きない） |
| H4 `r` の `VALUE` 出力が出た。`subject` が `None`（対象区間を持たない評価） | 変化なし。積まず `latest_outputs[r]` だけ更新する（本節「観測した足が定まらない出力は積まない」） | 同左 | 同左 |
| H5 `r` が `EVENT` / `COMMAND` の出力 | 到達しない（保持本数の計画に載るのは `VALUE` の出力参照だけ。本節「何を保持するか」、第6.5節） | 到達しない（同左） | 到達しない（同左） |
| H6 下流の入力が `HistoryWindow` で `r` を読む | 変化なし。`n + k` 件に満たないので `INPUT_MISSING_OR_INVALID`（本節「どう読むか」） | 変化なし。足りなければ同左 | 変化なし。対象区間の終了時刻で末尾を決めて切る（本節「どう読むか」） |
| H7 評価が `Skipped` / `Failed` / `Waiting` / `Superseded` で出力を出さなかった | 変化なし（出力記録が無い） | 変化なし | 変化なし |
| H8 run 末尾・run の失敗 | 変化なし（新しい出力は出ない。第6.1節・D06 §10.2） | 変化なし | 変化なし |

保持した履歴そのものは判断履歴に書き出さない（本節「判断履歴に何を残すか」）。本表に要決定のマスは無い。
...
- **同じ `step` に受付通知が2件以上届いたときは、自分の注文の通知（遷移5・6）を先に全件当て、他の機会を閉じる遷移7 はその後で当てる**【確定】（v2.10、2026-09-25 の人間の決定。R1-D05-1）。受け付けられた注文の機会は必ず `FULFILLED_BY_ORDER_ACCEPTANCE` で終わり、注文側の記録（`AttemptDecision`、D06）と食い違わない。遷移7 を当てる時点で、同じ `step` に通知が届いた `ORDER_PENDING` の機会は既に終端しているので、「受付待ちの機会を他の受付で閉じる」組み合わせは起きない。**実装は後続の実装 PR で行う**。それまでの実装は通知を届いた順に1件ずつ当てるので、`max_active >= 2` と `CLOSE_OTHERS` の組み合わせで2件が同時に受け付けられると、2件目の通知が終端済みの機会に届いて構造エラー（`KernelValueError`）で止まる。検証戦略 A・B はどちらも `max_active=1` なので起きない。**不採用**: 届いた順に当て、終端済みの機会への通知を無視する案（注文は受け付けられたのに機会は「他の受付で閉じた」と記録され、両者が食い違う）、構造エラーを正本にして組み合わせをコンパイル時に拒否する案（その組み合わせの戦略が書けなくなる）。
...
### 7.2.1 取引機会の状態×出来事表（必須表。R1）

全体計画書 §8.5 の表2 である。上の遷移の一覧（遷移1〜12）が正本であり、本表はそれを状態×出来事に並べ替え、**遷移の一覧に無い組み合わせ**を第6節・第7.3〜7.7節の根拠つきで埋めたものである。「生成前」の列は新しく生まれる機会、他の列は既にある機会への影響を表す。

| 出来事 | 生成前 | `OPEN` | `CONFIRMED` | `ORDER_PENDING` | `TERMINATED` |
|---|---|---|---|---|---|
| O1 Trigger が発火し、有効な機会数が `max_active` 未満（市場状態が方向を許す、または `market_state` が無い） | 遷移1 → `OPEN` | 変化なし（他の機会の生成は既存を変えない。第7.4節） | 変化なし（同左） | 変化なし（同左） | 変化なし（有効な機会に数えない。第7.1節） |
| O2 Trigger が発火し、上限に達していて `KEEP_EXISTING` | 遷移2 → `TERMINATED`（`CONCURRENCY_LIMIT_REACHED`） | 変化なし（第7.4節 手順3） | 変化なし（同左） | 変化なし（同左） | 変化なし |
| O3 Trigger が発火し、上限に達していて `SUPERSEDE_EXISTING`、置換できる機会がある | 既存の遷移3 の後に遷移1 → `OPEN`（第7.4節 手順3） | 置換できる機会のうち最も古い1件なら遷移3 → `TERMINATED`（`SUPERSEDED`）。それ以外は変化なし（D04 §10.3 の鍵） | 同左 | 変化なし（置換の対象にしない。第7.4節 3a） | 変化なし |
| O4 Trigger が発火し、上限に達していて `SUPERSEDE_EXISTING`、置換できる機会が無い | 遷移2 → `TERMINATED`（`CONCURRENCY_LIMIT_REACHED`。第7.4節 3a） | 到達しない（`OPEN` の機会があれば置換できる） | 到達しない（同左） | 変化なし（第7.4節 3a） | 変化なし |
| O5 Trigger が発火したが、市場状態が発火の方向を許さない（許可が一度も無い場合を含む） | 遷移12 → `TERMINATED`（`MARKET_STATE_INVALIDATED`。第7.6節） | 変化なし（許可されない発火は同時保持の枠を消費しない。第7.4節 2a） | 変化なし（同左） | 変化なし（同左） | 変化なし |
| O6 取引機会を出す評価が、市場状態の連鎖で待機に入る | 変化なし（機会を生成しない。評価要求が待機する。第7.6節） | 変化なし | 変化なし | 変化なし | 変化なし |
| O7 生成時に `SNAPSHOT_AT_OPPORTUNITY` の束縛が読めない | 束縛の欠損方針が見送り（`SkipEvaluation`）なら、固定なしで O1〜O5 のとおり機会を生成し、有効性の再検査の記録（表19）に `MISSING_SKIPPED` を読めなかった束縛1つにつき1件、生成の遷移の直後に新しい処理点を振って残す。読めない束縛のうち1つでも失敗（`Error`）なら機会を生成せず、取引機会を出す評価の記録が `Failed(DATA_ERROR)` になり run が止まる（第7.3節、v2.10【確定】、R1-D05-3。見送りの記録は後続の実装 PR） | 変化なし（固定は生成時に1回だけで、以後更新しない。第7.3節） | 変化なし（同左） | 変化なし（同左） | 変化なし |
| O8 確認評価が `confirmed=True` を出した（P4） | 到達しない（機会が無い） | 遷移10 → `CONFIRMED`。確認結果の出力記録の後に刻む（第7.7節） | 変化なし（遷移10 の出発点は `OPEN` だけ。第7.7節の細目、仮置き12） | 変化なし（同左） | 到達しない（終わった機会を待つ確認要求は再開より先に失効で閉じ、評価中の要求には機会を読む入力が欠損として渡る。第6.3節・第6.8節 手順2） |
| O9 確認評価が `confirmed=False`・見送り・待機・追い越しで終わった | 到達しない | 変化なし（第7.7節の確認試行の表） | 変化なし（`CONFIRMED` から先は遷移4・7・8・9・11 だけ。第7.7節） | 変化なし（`ORDER_PENDING` から先は遷移5・6・7・9 だけ。第7.2節） | 到達しない（O8 と同じ理由） |
| O10 確認期限に到達した（ライフサイクル検査、rank 4。`AwaitConfirmation` の機会だけ） | 到達しない | 遷移11 → `TERMINATED`（`EXPIRED`）。同じ判断時点の確認成立より先に判定し、期限が勝つ（第7.7節） | 遷移11（同左） | 到達しない（`ORDER_PENDING` は P5 で入り、同じ判断時点の約定後の評価起動点で受付通知により終端する。遷移5・6、D06 §4.2 手順7・9） | 変化なし（終端は復活しない。第7.2節） |
| O11 有効性の再検査で束縛が成立していた（`SATISFIED`） | 到達しない | 変化なし（第7.3節） | 変化なし（同左） | 到達しない（再検査は確認要求を作る前の P4 と `EntryProposal` を作る直前の P5 で、`OPEN` / `CONFIRMED` の機会についてだけ走る。第7.3節・第7.7節） | 到達しない（同左） |
| O12 有効性の再検査で束縛が成立していなかった（`NOT_SATISFIED`） | 到達しない | 遷移8 → `TERMINATED`（`MARKET_STATE_INVALIDATED`。P4 なら確認要求を作らない。第7.7節の細目、仮置き8） | 遷移8（P5 直前。第7.7節の `CONFIRMED` から先） | 到達しない（O11 と同じ理由） | 到達しない（同左） |
| O13 再検査で読めず、`on_missing=SkipEvaluation`（`MISSING_SKIPPED`） | 到達しない | 変化なし（今回の再検査を行わず機会を残す。第7.3節） | 変化なし（同左） | 到達しない（O11 と同じ理由） | 到達しない（同左） |
| O14 再検査で読めず、`on_missing=Error`（`MISSING_FAILED`） | 到達しない | 変化なし。`ValidityRecheck` に `DATA_ERROR` を残し、run が止まる（第7.3節、D06 §4.2 の停止判定 (b)）。その後は O21 | 変化なし（同左） | 到達しない（O11 と同じ理由） | 到達しない（同左） |
| O15 再検査で読めず、束縛の欠損方針が `WaitForInput` | 到達しない | 拒否: `KernelValueError`（第7.3節の細目、仮置き7） | 拒否: 同左 | 到達しない（O11 と同じ理由） | 到達しない（同左） |
| O16 注文意図と保護水準が同じ機会の要求から揃い、`EntryProposal` を渡した（P5） | 到達しない | 遷移4 → `ORDER_PENDING`（`ImmediateEntry` の戦略）。確認待ちの戦略では、注文意図の部品は `confirmed=True` の確認結果の配送でだけ起動するので到達しない（第7.7節） | 遷移4 → `ORDER_PENDING` | 到達しない（`EntryProposal` は同じ機会の組が揃った時点で1件作り（第6.2節 手順9）、`ORDER_PENDING` は同じ判断時点のうちに受付通知で終端する。D06 §4.2 手順7・9） | 到達しない（終端した機会のイベントは配送せず、既に配送済みの機会も終端によって発注試行へ進む経路が閉じる。第6.2節・第7.4節 手順5） |
| O17 自身の注文が受け付けられた（`AdmissionNotice.accepted=True`） | 到達しない | 到達しない（受付通知は `EntryProposal` を渡した機会、すなわち `ORDER_PENDING` の機会にだけ届く。遷移4・5、D06 §6.6） | 到達しない（同左） | 遷移5 → `TERMINATED`（`FULFILLED_BY_ORDER_ACCEPTANCE`）。記録は通知が配送された処理点（第7.2節） | 到達しない。自分の注文の通知を遷移7 より先に全件当てる（第7.2節、v2.10【確定】、R1-D05-1。実装は後続の実装 PR） |
| O18 自身の注文が受付前の審査で拒否された（`accepted=False`） | 到達しない | 到達しない（O17 と同じ理由） | 到達しない（同左） | 遷移6 → `TERMINATED`（`ORDER_ATTEMPT_REJECTED`） | 到達しない（O17 と同じ理由） |
| O19 他の機会の注文が受け付けられ、`on_order_accepted=CLOSE_OTHERS` | 到達しない | 遷移7 → `TERMINATED`（`CLOSED_BY_ORDER_ACCEPTANCE`） | 遷移7（同左） | 到達しない。`ORDER_PENDING` の機会には同じ `step` に自身の通知が届き、遷移7 より先に遷移5・6 で終端している（第7.2節、v2.10【確定】、R1-D05-1。遷移7 の出発点は非終端すべてのまま） | 変化なし（終端は復活しない） |
| O20 他の機会の注文が受け付けられ、`on_order_accepted=KEEP_OTHERS` | 到達しない | 変化なし（規則を書かない限り暗黙に終端させない。D04 §10.3・ADR-0032） | 変化なし（同左） | 変化なし（同左） | 変化なし |
| O21 run の失敗（D06 §4.2 の停止判定） | 到達しない | 変化なし。失敗した run では末尾の `step`（O22）を呼ばず、機会は最後に観測された状態のまま最終 snapshot に残り、架空の終端理由を付けない（D06 §10.2） | 変化なし（同左） | 変化なし（同左） | 変化なし |
| O22 run 末尾（`is_run_end=True` の `step`） | 到達しない（この `step` は起動判定を行わない。第6.1節） | 遷移9 → `TERMINATED`（`RUN_END`）。`opportunity_id.seq` の昇順（第6.1節） | 遷移9（同左） | 遷移9（同左） | 変化なし |

- 遷移の記録（`OpportunityTransition`）は遷移1件ごとに残る。生成（遷移1・2・12）は `from_state=None` である（第7.2節）。「変化なし」「到達しない」のマスは記録を残さない。
- 終端済みの機会への遷移要求は `KernelValueError` で拒否する（第7.2節）。本表の `TERMINATED` の列で「変化なし」としたマスは、遷移要求そのものが作られない組み合わせである。

**v2.9 で要決定としたマスと、その決定**（v2.10、2026-09-25 の人間の決定）。

| 番号 | 何が決まっていないか |
|---|---|
| R1-D05-1 | 同じ `step` に受付通知が2件以上届いたとき（`max_active >= 2` で同じ判断時点に2つの機会が発注試行へ進んだ場合）の**適用の順序**。他の機会の受付による遷移7 が先に適用されると、自身の受付・拒否の通知が既に `TERMINATED` の機会へ届く。そのとき遷移要求として拒否する（`KernelValueError`）のか、通知を無視するのかが書かれていない。→ **決定（2026-09-25、v2.10）: 自分の注文の通知を先に全件当て、遷移7 は後**（第7.2節）。実装は後続の実装 PR |
| R1-D05-3 | 生成時に `SNAPSHOT_AT_OPPORTUNITY` の束縛が読めないときの扱い。第7.3節は `on_missing=Error` で失敗する場合に「固定は置換より前」とだけ書き、`SkipEvaluation` のとき機会を生成するのか・どの結末で記録するのか、`Error` のとき失敗をどの記録（評価記録か `ValidityRecheck` か）に残すのかが書かれていない。→ **決定（2026-09-25、v2.10）: 見送りなら機会を作り表19 に `MISSING_SKIPPED` を1件、失敗なら評価記録に `Failed`**（第7.3節）。見送りの記録は後続の実装 PR |
...
- **生成時に束縛が読めないとき**【確定】（v2.10、2026-09-25 の人間の決定。R1-D05-3）。束縛の欠損方針が**見送り（`SkipEvaluation`）なら、固定なしで機会を作り、有効性の再検査の記録 `ValidityRecheck` に `MISSING_SKIPPED` を1件残す**（`reason` は空。下の「読めない」の細目の仮置き6 と同じ）。**「1件」は読めなかった束縛1つにつき1件である**。`ValidityRecheck` は `source` を1つしか持たず、P4・P5 の再検査も「1回の再検査につき1件」（下の段落）だからである。束縛が2つ以上あって複数が読めないときは、束縛の宣言順に1件ずつ残し、**それぞれに `step` の通し番号（第6.6節）から新しい処理点を振る**（P4・P5 の再検査と同じ振り方）。振る位置は、生成の遷移を刻んだ直後で、その機会を載せた出力記録より前である。これで主キー `(opportunity_id, at)` は重複しない。読めた束縛は固定し、記録は残さない（固定は再検査ではない）。**読めない束縛のうち1つでも欠損方針が失敗（`Error`）なら、下の失敗の扱いになる**（強い方針が勝つ。第6.3節と同じ考え方）。記録型は束縛の種類を列に持つので、型は変えない。判断履歴の表19 には、P4・P5 の再検査の行に加えて生成時点の行も並ぶ（D06 §9.2）。**失敗（`Error`）なら**、機会を作らず、取引機会を出す評価の記録を `Failed(DATA_ERROR)` にして run を止める（いまの実装どおり。評価の中で起きた失敗なので評価記録が書き先になる）。**見送りの記録は後続の実装 PR で足す**（それまでの実装は、見送りのとき何も記録せずに機会を作る）。検証戦略 A・B はこの束縛を使っていない。**不採用**: いまの実装を正本にする案（固定できなかったことが判断履歴から読めない）、機会を作らず評価を見送りにする案（発火は必ず取引機会として記録するという決定記録 ADR-0032 とぶつかる）。
...
**「読めない」の判定と、読めなかったときの細目**【確定】（v2.6、2026-09-25 の人間の承認。PR #33 の仮置き5・6・7）。

| 事項 | 規則 |
|---|---|
| 読めない（仮置き5） | 束縛が指す出力が**まだ1件も出ていない**か、その使用箇所が**より新しい足について待機中**であること。後者は入力の「待機の伝播」（第6.8節）と同じ判定であり、上流が待機している間に古い出力を最新として読まない |
| 読めずに見送った記録の理由（仮置き6） | `ValidityRecheck.reason` は**空**（`None`）。欠損を表す理由コードが語彙（上位設計書 §4.7.14）に無く、見送ったことは結末の区分（`MISSING_SKIPPED`）そのものが表す。読めずに失敗した場合は上の表のとおり `DATA_ERROR` |
| 束縛の欠損方針が待機（`WaitForInput`）のとき（仮置き7） | 読めなければ**構造エラー**（`KernelValueError`）で止める。上の表が結末を決めているのは見送りと失敗の2つだけで、再検査は評価の外側で走るので待機の記録（第6.8節）を持てないためである |
...
| 鮮度がどこに残るか | **取引許可の出力記録そのもの**（D06 §9.2 の表1）である【提案】（v2.1。紙上トレース T02 §14 #2）。取引機会の遷移記録（`OpportunityTransition`）にも有効性の再検査の記録（`ValidityRecheck`）にも鮮度の欄は置かない。適用した許可は「**その発火の遷移記録の処理点より前に出た、その出力参照の最後の出力**」として一意に決まるので、判断履歴から後で復元できる。**判断時刻だけで絞らない**のは、日足と Trigger の足が同じ判断時点で確定する場合に、同じ `step` の P2 で出たばかりの許可を P3 が適用するからである（上位設計書 §4.3.11 の因果順序）。判断時刻より前で切ると、その更新を除いて1つ前の許可を指してしまう。**「前」は出力の識別子の大小で比べる**（v2.2。独立レビューの指摘を受けた明確化）。出力記録は `ProcessingPoint` を持たず（`decision_time` と `sequence` だけである。上位設計書 §4.3.15）、`sequence` は `step` ごとに 0 へ戻る（第6.6節）ので、出力記録どうしを `(decision_time, phase.rank, sequence)` で並べることはできない。**代わりに `OutputId` を使う**。連番 ID の採番順は run 全体で処理順に一致すると定められている（D02 §7.3）ので、`OutputId` は**判断時点も `step` もまたいで処理順に単調増加する**。したがって復元は次の1文で閉じる。

  > **適用した許可は、その発火が作った取引機会を載せた出力記録の `OutputId` より小さい `OutputId` を持つ、その出力参照（`CompiledRoles.market_state`）の最後の出力である。**

  取引機会の出力記録は、許可されなかった発火も含めてどの発火についても表1 に残り（第7.4節・第6.2節 手順7）、遷移記録からは `opportunity_id` で結べる。許可を読むのは取引機会の組み立て（手順6）で、その出力記録の付番（手順7）は**その直後**なので、間に同じ出力参照の新しい出力が入ることはない。**`step` がどれかを判別する必要も、フェーズを見る必要も無い**。これは新しい規則ではなく、ID の採番順（D02 §7.3）と手順6・7 の順序（第6.6節）からの帰結である。**不採用**: 遷移記録に鮮度の欄を足す案（同じ値が表1 と表3 に二重に並ぶ）、判断時刻だけで絞る案（同じ判断時点の更新を取りこぼす）、出力記録に処理点や `step` の識別子を足す案（`OutputRecord` の正本は上位設計書 §4.3.15 であり本書の改訂で足せない。`OutputId` の大小だけで一意に決まるので足す必要も無い）、評価記録の起動条件の種類から `step` を当てる案（`OnInputEvent` は第1回でも第2回でも成立しうるので当てられない） |
...
**市場状態の連鎖による待機の表し方**【確定】（v2.6、2026-09-25 の人間の承認。PR #33 の仮置き9）。取引機会を出す評価が市場状態の連鎖で待機に入るとき、足りないものを**入力名 `market_state`・接続元「市場状態の役割が指す出力」**の診断（`MissingInputDiagnosis`）として `Waiting` に載せ、その出力が同じ `step` で出たら再開する（第6.8節の「出力参照の再開は評価の段で判定する」と同じ判定）。**自分の入力の待機が先に成立したらそちらを使い、連鎖の待機を重ねない**（上の「自身の入力の待機と連鎖の待機が同時に成立する場合」は、検証戦略 B では両者が同じ結果になるとだけ書いている）。連鎖の起点が複数で**期限を比べられない**（系列の違う本数の期限どうし、本数の期限と時刻の期限）ときは構造エラーで止める。
...
| 判断履歴への記録 | 開始足と確認期限は、取引機会の遷移記録（表3）の**生成の行（`from_state=None`）に1回だけ**載せる（`OpportunityTransition.confirmation_start_bar` / `confirmation_deadline_at`。第3節）。どちらも生成時に1回決まり、その後の行には載せない。これで期限切れ（`EXPIRED`）と開始足の除外（`include_start_bar`）を判断履歴だけで検算できる【確定】（v2.10、2026-09-25 の人間の決定。R1-D05-5。**実装は後続の実装 PR**）。上の「保存する」は実行時の状態（`OpportunityLifecycle`）の中のことであり、判断履歴への記録は本行が定める |

**確認部品は取引機会を出す部品の後に評価する**【確定】（v2.4、2026-09-24 の人間の再決定。PR #27）。同じ判断時点で生まれた取引機会を開始足で確認するには、評価順の上で確認部品が取引機会を出す部品より後になければならない。取引機会を読む入力（`RuntimeInputRef(OPPORTUNITY)`）には明示の接続が無いので、D04 §12 の因果辺の表に4本目（取引機会の参照、`EdgeKind.OPPORTUNITY_CONTEXT`）を足し、評価順（第5.4節）がこの前後を保証する。
...
**確認試行の記録**【提案】。`OpportunityLifecycle.attempts` に `ConfirmationAttempt(opportunity_id, bar_key, request_id, outcome)` を持つ。同じ機会が期限まで何本の確認足で何を返したかが、機会の記録だけから読める。**エンジンへは `RuntimeStepResult.confirmation_attempts` で、その `step` で作った・書き換えた試行だけを返す**【確定】（Q26 決定、2026-09-23、選択肢1。第3節の型表・第6.2節 手順10）。
...
**確認の経路の細目**【確定】（v2.6、2026-09-25 の人間の承認。PR #33 の仮置き4・8・11・12）。

| 事項 | 規則 |
|---|---|
| 期限切れと待機の検査の順（仮置き4） | 同じライフサイクル検査（D06 の rank 4）の中で、**取引機会の確認期限（遷移11）を先に**判定し、その後に待機中の評価要求を検査する（第6.8節の手順2）。期限で終わった機会を待つ要求は、同じ判断時点で失効（`OPPORTUNITY_ENDED`）として閉じる |
| 確認評価の前の再検査の置き場所（仮置き8） | 確認評価のたびの再検査（上の段落）は、**確認要求を作る前**に行う。不成立・読めず失敗なら要求を作らない（評価記録も確認試行も残らない。上の段落の「確認を始める前に機会が終わった」と同じ扱い） |
| 確認結果が無い確認評価（仮置き11） | 確認部品の評価が確認結果（`ConfirmationOutcome`）を出さなかったら**評価の失敗**（`Failed(DATA_ERROR)`）にする。失敗で終わった確認要求は確認試行に写さない（試行の結末の区分に失敗が無く、run はそこで止まる。D06 §4.2 の停止判定） |
| 遷移10 の対象（仮置き12） | 確認が成立したときに機会が `OPEN` でなければ（既に確認済みなど）**状態を変えない**。遷移10 の出発点は `OPEN` だけである（第7.2節） |

### 7.8 値の伝播表（必須表。R1）

全体計画書 §8.5 の表3 である。本書が生成するか、本書を通って部品・層・成果物をまたぐ値を1行ずつ挙げる。記録先の表番号と主キーの正本は D06 §9.2（判断履歴の19表。v2.10 の決定で D06 v1.11 が表20「公開記録」を足した）であり、本表は参照するだけである。規則の正本は各行の根拠の節である。

| 値 | 生成元 | 渡り方 | 記録先 | 主キー |
|---|---|---|---|---|
| 取引機会の識別子 `opportunity_id: OpportunityId` | ランタイムが `IdAllocator`（D02 §7.3）で採番する。Trigger の出力が出た評価の手順6（P3）、許可されない発火・上限で有効にしない発火も含む（第4.2節・第6.2節・第7.4節・第7.6節） | `Opportunity.opportunity_id`（出力記録の payload）→ 入力イベントで起動した要求の `EvaluationRequest.opportunity_id`（第6.2節 手順3）、確認評価の要求（第6.11節）→ `EntryProposal.opportunity_id` → エンジンの注文要求（D06）→ `AdmissionNotice.opportunity_id` と `RuntimeEventNotice.opportunity_id` でランタイムへ戻る | 表3 `OPPORTUNITY_TRANSITIONS`、表2 `EVALUATIONS` の列、表4 `ORDER_REQUESTS` の列、表18 `CONFIRMATION_ATTEMPTS`、表19 `VALIDITY_RECHECKS`、表1 `OUTPUTS` の payload | 表3 `(opportunity_id, at)`、表18 `(opportunity_id, bar_key)`、表19 `(opportunity_id, at)`。表2・表4 では列（主キーはそれぞれ `evaluation_id`・`attempt_id`） |
| 機会の生成時点 `created_at: ProcessingPoint` / `created_decision_time: UtcTime` | ランタイムが生成の処理点で決める（第7.1節） | `OpportunityLifecycle` に保持する。`created_decision_time` は置換の鍵（D04 §10.3）と `DurationDeadline` の解決（第7.7節）に使う | 表3 の生成の遷移（`from_state=None`）の `at` が `created_at` と同じ処理点である（第7.2節） | 表3 `(opportunity_id, at)` |
| 評価要求の識別子 `request_id: RequestId` | ランタイムが `IdAllocator` で採番する。採番順は評価順、対象つきの要求は対象の識別子の昇順（第6.4節・第6.11節） | `EvaluationRequest` → `EvaluationRecord.request_id`、`WaitingRequest.request`、`WaitEvent.request_id`、`Superseded.by_request_id`（押しのけた側）、`ConfirmationAttempt.request_id`。待機から再開しても変えない（第6.8節） | 表2 の列、表16 `WAIT_EVENTS`、表18 の列 | 表16 `(request_id, at)`。表2 では列（主キーは `evaluation_id`） |
| 評価の識別子 `evaluation_id: EvaluationId` | ランタイムが評価記録1件ごとに採番する。待機に入る記録と決着の記録で別の値（第6.4節・第6.8節） | `OutputRecord.evaluation_id`（第6.6節）、遡りの記録の親（D06 §9.2 の表17 の `SubstitutionOwner`） | 表2 `EVALUATIONS`、表1 の列、表17 `INPUT_SUBSTITUTIONS` | 表2 `evaluation_id`、表17 `(evaluation_id, input_name, source_index)` |
| 出力の識別子 `output_id: OutputId` | ランタイムが手順7 で採番する。run 全体で処理順に単調増加（第6.6節・第7.6節、D02 §7.3） | `Evaluated.output_ids`、`EntryProposal.intent_output_id` / `protection_output_id`、`ManagementRequest.source_output_id`（第6.2節 手順9）、`ValidityRecheck.output_id`（第7.3節）、`SubstitutedInput.used_output_id`（第6.9節） | 表1 `OUTPUTS`、表4 の `intent_output_id`、表12 `MANAGEMENT_APPLICATIONS` の元の `output_id`、表15 `EVIDENCE` の `output_ids`、表17、表19 | 表1 `output_id` |
| 処理点 `ProcessingPoint`（`at`） | ランタイムが `step` 内の通し番号と `PublicationBatch.phases` から組み立てる（第6.1節・第6.6節）。エンジンが自分の採番列へ載せ替える（第6.6節、D06 §4.4） | `OpportunityTransition.at`、`WaitEvent.at`、`ValidityRecheck.at`、`WaitingRequest.started_at` | 表3・表16・表19 の `at` | 表3 `(opportunity_id, at)`、表16 `(request_id, at)`、表19 `(opportunity_id, at)` |
| 出力記録の通し番号 `sequence` | ランタイムが `step` ごとに 0 から振る。遷移・待機の出来事・再検査と同じ1本の番号を共有する（第6.6節） | `OutputRecord.sequence` | 表1 の列 | 表1 `output_id` |
| 判断時刻 `decision_time: UtcTime` | エンジンが `PublicationBatch.decision_time` で渡す（第6.1節） | `OutputRecord.decision_time`、`EvaluationRecord.decision_time`（再開後は再開した判断時刻。第6.8節）、`EntryProposal.decision_time`、`ManagementRequest.decision_time` | 表1・表2 の列、表4・表12 の元の要求 | 各表の主キー（表1 `output_id`、表2 `evaluation_id`） |
| 対象区間 `target_interval` / `signal_interval` / `observation_interval` | 足の確定ではエンジンが渡す `BarClosure.interval`（第6.1節）。入力イベントでは上流の要求から引き継ぎ、実行時イベントでは `None`（第6.2節 手順3） | `EvaluationRequest.target_interval` → `Opportunity.signal_interval`（第4.2節）、`Observation.observation_interval`（第6.7節）、`EvaluationRecord.target_interval` | 表2 の列、表1 の payload | 表2 `evaluation_id`、表1 `output_id` |
| 観測の対象と鮮度 `subject` / `freshness_time` | ランタイムが評価ごとに決める（第4.2節・第6.7節） | `Observation` で包んで配送し、下流の入力の `ValueSample` へ写す（第6.7節） | 表1 の payload | 表1 `output_id` |
| 建玉の識別子 `position_id: PositionId` | エンジンが採番する（D06）。本書は受け取るだけ | `RuntimeEventNotice.position_id` → `EvaluationRequest.position_id`（第6.2節・第8節）、`RuntimeState.open_positions`（第6.11節）→ `RuntimeContextView.position_context` → `ManagementRequest.position_id` | 表2 の列、表12 | 表12 `(position_id, at)`。表2 では列 |
| 発注試行の識別子 `attempt_id: AttemptId` | エンジンが採番する（D06）。本書は受け取るだけ | `AdmissionNotice.attempt_id` → `OpportunityLifecycle.attempt_id` と `OpportunityTransition.attempt_id`（第7.2節） | 表3 の `attempt_id`、表4 | 表4 `attempt_id`。表3 では列 |
| 置換の相手 `counterpart: OpportunityId` | ランタイムが遷移3 のときに入れる（第7.2節） | `OpportunityTransition.counterpart` | 表3 の列 | 表3 `(opportunity_id, at)` |
| 確認足 `bar_key: BarKey`（確認試行） | ランタイムが確認評価の対象足から決める（第7.7節） | `ConfirmationAttempt.bar_key` → `RuntimeStepResult.confirmation_attempts` → エンジンが置き換えとして書く（第6.2節 手順10） | 表18 | 表18 `(opportunity_id, bar_key)` |
| 確認の開始足 `confirmation_start_bar` と確認期限 `deadline_at` | ランタイムが機会の生成時に決める（第7.7節） | `OpportunityLifecycle` に保持し、`include_start_bar` と遷移11 の判定に使う。生成の遷移記録へ写す（`OpportunityTransition.confirmation_start_bar` / `confirmation_deadline_at`。第3節） | 表3 の生成の行（`from_state=None`）の列。他の行では空（v2.10【確定】、R1-D05-5。実装は後続の実装 PR） | 表3 `(opportunity_id, at)` |
| 待機記録の固定した足 `pinned_bars` と待機期限 `deadline_at` | ランタイムが待機に入る評価で決める（第6.8節） | `WaitingRequest` に保持し、再開の読み直しと追い越し・期限の判定に使う。固定した足は `WaitEvent(WAIT_STARTED).pinned_bars` へ写す（第3節）。期限は `Waiting.deadline_at`（評価記録の結末） | 固定した足は表16 の「待機に入った」行の列（v2.10【確定】、R1-D05-6。実装は後続の実装 PR）。期限は表2 の結末の列 `outcome_deadline_at_*`（既存） | 表16 `(request_id, at)`、表2 `evaluation_id` |
...
| 公開バッチの識別子 `batch_id: EventId` | エンジンが作る（D06 §4.3） | `PublicationBatch.batch_id` → `RuntimeState.last_batch_id`（二重適用の検査。第6.1節） | 記録しない（D06 v1.11 §4.3【確定】、R1-D06-1）。本書の記録型にも項目を持たない | — |

**v2.9 で要決定としたマスと、その決定**（v2.10、2026-09-25 の人間の決定）。

| 番号 | 何が決まっていないか |
|---|---|
| R1-D05-5 | 確認の開始足と確認期限（`OpportunityLifecycle.confirmation_start_bar` / `deadline_at`）を判断履歴のどこに残すか。表3（遷移記録）にも表18（確認試行）にも列が無く、本文は「開始足の識別子は保存する」（第7.7節）と書くが、それが実行時の状態の中のことか判断履歴のことかが書かれていない。→ **決定（2026-09-25、v2.10）: 表3 の生成の行に列を足す**（第7.7節）。実装は後続の実装 PR |
| R1-D05-6 | 待機記録が固定した足（`WaitingRequest.pinned_bars`）と待機期限を判断履歴に残すか。表16 の列に無く、「記録しない」とも書かれていない。再開した評価がどの足を読んだかを判断履歴から確かめる経路に関わる。→ **決定（2026-09-25、v2.10）: 固定した足は表16 の「待機に入った」行に列を足す。期限は表2 に既にあるので足さない**（第6.8節）。実装は後続の実装 PR |
...
評価順は第5.4節の規則で一意に定まり、`daily_ema` → `m15_ema` → `no_short` → `stop_level` → `daily_above_ema` → `m15_above_ema` → `market_state` → `entry_trigger` → `entry_filter` → `entry_order` → `initial_stop` → `trailing` の順になる（v2.4 で規則の出力に合わせて書き直した。同じ段の中は `instance_id` 順。段は、上流を持たない `daily_ema`・`m15_ema`・`no_short`・`stop_level`、その直下の `daily_above_ema`・`m15_above_ema`、`market_state`、`entry_trigger`、`entry_filter`、`entry_order`・`initial_stop`、`trailing` の7段である。`market_state` → `entry_trigger` は第7.6節の因果辺、`entry_trigger` → `entry_filter`・`entry_order` は取引機会の参照の因果辺（第7.7節、v2.4）、`entry_order` → `trailing` は D04 §12 の因果辺）。
...
**T では取引機会は生まれない**【確定】（Q23 決定、2026-09-23、選択肢1。紙上トレース T02 §3.1・§7.3）。日足は1時間足から集約する（D03 §5.1）ので、T で確定した日足の高値は、同じ T で確定した1時間足の高値を必ず含む。したがって `1時間足の終値 > 日足高値` は**日足境界では構造的に成立しない**。本節の4ケースが示すのは「**待機・再開・期限切れ・追い越しが判断履歴にどう現れるか**」であり、**取引機会が生まれるのは日足境界ではない1時間足の確定**（本書の人工データでは翌日 09:00Z。T02 §3.2）である。検証戦略 B の宣言そのものは意図どおり動いている（日中は前日の高値と比べる）ので、**宣言は変えない**。採らなかった案（当該足を除いて日足高値を読む部品を足す案、例文を残して到達可否だけを注記する案）は T02 §15.1 の「採らなかった案」に残してある。

| # | シナリオ | 日足の `available_at` | 判断履歴に出るもの |
|---|---|---|---|
| 1 | 遅延なし | T | `daily_ema` → `market_state` → `entry_trigger` がすべて T で評価される。**T では突破が成立しないので取引機会は生まれない**（上の確定。`EvaluationRecord(outcome=Evaluated(()))` が残る）。取引機会が生まれるのは**日足境界ではない1時間足の確定** T′ であり、そこで遷移1、同じ T′ の15分足で確認が成立すれば遷移10、注文意図と保護水準が出て遷移4 となる（T02 §3.2〜§3.5） |
| 2 | 日足のみ2秒遅延 | T+2秒 | **T**: `daily_ema`（`ema` v2）は `history` が窓の末尾の足を読めず `INPUT_MISSING_OR_INVALID` を返すため、`Waiting` の評価記録と `WaitEvent(WAIT_STARTED)`。`daily_above_ema` と `market_state` は上流が待機中で出力が「まだ出ていない」ため、同じく待機（第6.8節の「待機の伝播」）。`entry_trigger` は第7.6節の規則で市場状態の連鎖に連なって待機する。`m15_ema` は独立なので通常どおり評価される。**T+2秒**: `WaitEvent(INPUT_ARRIVED)` と `WaitEvent(RESUMED)`、固定した日足を読み直して `daily_ema` が評価され、評価順に沿って `daily_above_ema` → `market_state` → `entry_trigger` が同じ判断時点で続く（第6.8節 手順1）。**本ケースが追うのはここまで**で、T が日足境界である以上、再開した `entry_trigger` からも取引機会は生まれない（上の確定。T02 §7）。日足境界でない1時間足の確定で待機して再開した場合は、その機会の `created_decision_time` が**再開した判断時刻**、`signal_interval` は**待機を始めた1時間足の区間**のままであり、確認の開始足は「再開した時点で利用可能な最新の15分足」になる |
...
|---|---|---|---|
| 1 | 遅延なしで、同じ判断時点に市場状態 → 取引機会 → 確認成立 → 注文意図まで進む。**その判断時点は日足境界ではない1時間足の確定である**（Q23 決定。§9.3 の確定） | §7.6・§7.7・§9.3 の1 | 日足 EMA の評価、市場状態の配送、後続確認 |
| 2 | 開始足で未確認、次の15分足で確認が成立する | §7.7（`include_start_bar` と確認試行の記録） | 後続確認 |
| 3 | 期限（15分足4本）に到達して `EXPIRED` で終端する | §7.7 の期限の数え方、遷移11 | 後続確認 |
| 4 | 確認待ちのあいだに日足の条件が崩れ `MARKET_STATE_INVALIDATED` で終端する → **段階3 の宣言では到達しない**（Q24 決定。下の到達可否） | §7.3・§7.7 の末尾（確認評価のたびの再検査） | 後続確認 |
| 5 | 日足が2秒遅れ、市場状態が待機して**同じ判断時点で再開する**（取引機会の生成は日足境界ではない1時間足の確定で追う。Q23 決定） | §6.8・§7.6・§9.3 の2 | 待機、遅延シナリオ |
...
**上の9経路は [T02](../traces/T02_paper_trace_strategy_b.md) が追った**（v1.0、2026-09-23、PR #24）。文書名と置き場所は新しい T02（`docs/traces/`）とし、その理由は T02 §18 にある。追跡の結果、**経路1 と経路5 のうち「日足・1時間足・15分足がそろって終了する判断時点で取引機会が生まれる」部分と、経路4 の全体は到達しない**ことが分かった（T02 §3.1・§6・§7.3）。扱いは Q23・Q24 の決定（2026-09-23、いずれも選択肢1）で確定し、上の表に反映済みである（第15.3節）。

**経路4 は段階3 の宣言では到達しない**【確定】（Q24 決定、2026-09-23、選択肢1）。確認期限が15分足4本＝ちょうど1時間で、日足境界も1時間境界の上にある（D03 §3.2）ため、日足の条件が変わりうる唯一の確認足が**期限の足と重なる**。期限の判定（D06 の rank 4）は確認評価（rank 8）より先なので、必ず期限切れ（`EXPIRED`）で終わる（T02 §6）。**検証戦略 B の宣言も部品も変えない**。ADR-0031 が求める「確認待ち中の条件再検査」そのものは、**宣言を変えた小さな戦略を使う D08 の意味論テストで、再検査の記録（`ValidityRecheck`、第7.3節）の4区分をすべて通して検証する**（D08 §13.2 の未整備6）。**「読めず失敗した」の区分だけは有効性束縛の欠損方針が `Error` の宣言でしか生じない**（第7.3節）ので、その意味論テストは欠損方針が `SkipEvaluation` の版と `Error` の版の2つを宣言する。採らなかった案（確認期限を8本に延ばして到達させる案、到達しない経路を一覧に残したままにする案）は T02 §15.1 の「採らなかった案」に残してある。
...
| **v2.0**（本改訂） | 段階3 の範囲（第1.2節の「段階3 で本改訂（v2.0）が決めること」の列）。複数の時間足・市場状態・後続確認・待機・遡り・追い越し・上流の出力の履歴窓・段階3 のカタログ・検証戦略 B | **Q10〜Q18 は 2026-09-22 に、Q19〜Q22 は 2026-09-23 に決定済み**（第15節）。第12.1節の他文書への反映も同じ2日で済ませた（反映17件・見送り1件）。**承認済み**（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35） |
| **v2.1**（2026-09-23） | 紙上トレース [T02](../traces/T02_paper_trace_strategy_b.md) が見つけた未記述のうち、**設計の選択を含まないもの**の明記（第6.5節・第6.6節・第6.7節・第6.8節・第6.12節・第7.6節・第7.7節・第8節・第9.4節）。**新しい規則は置かず、既に確定している規則の帰結と適用範囲を書いただけである** | **承認済み**（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35）。v2.0 と同じ PR ではなく T02 の PR で提案した |
| **v2.2**（2026-09-23、PR #24） | 紙上トレース T02 が立てた**要決定 Q23〜Q29 に対する人間の決定**（7件とも選択肢1）の反映（第3節の型表・第6.2節 手順10・第6.3節・第6.8節・第7.7節・第9.3節・第9.4節・第15.3節）。Q25 は本書ではなく D06 v1.6 が受け取り、Q24 の意味論テストは D08 v1.1 が受け取る | **承認済み**（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35）。T02 と同じ PR で提案した。**Q30 も同じ日に決定済み**（第15.4節。コンパイル時の検査を1件足す。D04 v1.11）。**本書に未決の項目は残っていない** |
| **v2.3**（2026-09-24、PR #27） | 段階3 実装 PR 2/5（コンパイラ）が仮置きした点に対する**人間の決定**の反映（第3節の型表・第5.3節・第5.4節・第5.5節・第6.9節・第12.1節）。遡りの上限の解決値を入力の計画に載せること、市場状態の因果辺の種類名 `MARKET_STATE`、保存形式の版を 1 のままとすることの3件 | **承認済み**（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35）。PR #27 で提案した。検証戦略 B の評価順（第9.2節末尾）は本版では未反映で、v2.4 が反映した |
| **v2.4**（2026-09-24、PR #27） | 検証戦略 B の評価順についての**人間の再決定**の反映（第3節の型表・第5.4節・第7.7節・第9.2節）。取引機会の参照の因果辺 `OPPORTUNITY_CONTEXT` を足し、その辺を含めた規則の出力を第9.2節末尾の評価順とした | **承認済み**（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35）。PR #27 で提案した |
| **v2.5**（2026-09-24、段階3 実装 PR 4/5） | 段階3 実装 PR 3/5（PR #32）の**仮置き8件の承認**と、待機中に受け取った取引機会が終わった要求の決着についての**人間の決定**の反映（第3節の型表・第6.6節・第6.7節・第6.8節・第6.9節・第6.10節） | 人間の決定（2026-09-24）の反映。PR 4/5 と同じ PR |
| **v2.6**（2026-09-25、段階3 実装 PR 5/5） | 段階3 実装 PR 4/5（PR #33）の**仮置き14件の承認**と、確認の通し番号を評価記録に写さない**人間の決定**の反映（第6.3節・第6.8節・第6.10節・第6.11節・第7.3節・第7.6節・第7.7節） | 人間の決定（2026-09-24・2026-09-25）の反映。PR 5/5 と同じ PR |
| **v2.7**（2026-09-25、段階3 完了の文書 PR） | 段階3 実装 PR 5/5（PR #35）の**仮置き8件の承認**のうち、本書の規則とつながる2点の反映（第4.10節・第6.6節）。8件の正本は D06 v1.8 と D08 v1.7 | 人間の決定（2026-09-25）の反映 |
| **v2.8**（2026-09-25、段階3 の文書整合 PR） | 段階3 の範囲（v2.0〜v2.7）の状態欄を「承認待ち」から承認済みへ改めた（冒頭の状態・第1.2節・本表）。本文の規則は変えていない | 人間の決定（2026-09-25）の反映 |
| **v2.9**（2026-09-25、PR #40） | 設計文書の必須表（R1。全体計画書 §8.5）の追加（第1.2節・第6.5.1節・第6.10.1節・第6.12.1節・第7.2.1節・第7.8節）。既存の規則を表に並べ替えただけで、本文の規則は変えていない。埋められなかったマス5件は要決定 R1-D05-1・3〜6 として一覧した | R1（PR #26 で人間が承認）の実施 |
| **v2.10**（2026-09-25、必須表の空欄を埋める文書 PR） | v2.9 が要決定とした必須表の空欄5件（R1-D05-1・3〜6）への**人間の決定**（5件とも推奨案）の反映（第1.2節・第3節の型表・第6.2節・第6.4節・第6.8節・第6.10.1節・第7.2節・第7.2.1節・第7.3節・第7.7節・第7.8節）。4件（R1-D05-1・3・5・6）は実装変更を伴い、後続の実装 PR で行う | 人間の決定（2026-09-25）の反映 |
| v2.11 以降 | 第10.2節が段階4 以降へ送った項目 | 未起草 |
...
## 15. 段階3 の決定（Q10〜Q30 はすべて決定済み）

段階2 の Q1〜Q9 は 2026-09-21 に決定済みで（第13節）、段階3 の Q10〜Q18 は **2026-09-22 に**、Q19〜Q22 と、紙上トレース T02 が立てた **Q23〜Q29、およびその反映から生じた Q30 は 2026-09-23 に決定済み**である（第15.3節・第15.4節）。**未決の項目は残っていない**。番号は再利用しない。
...
**Q22 の時点で残る要決定は無かった**。第10.2節が段階4 以降へ送った項目は本改訂の範囲外であり、要決定ではない。その後、紙上トレース T02 が Q23〜Q29 を立て（第15.3節で決定済み）、その反映から生じた Q30 も同じ日に決定した（第15.4節）。

### 15.3 Q23〜Q29 の決定（2026-09-23、PR #24。**7件すべて決定済み**）

段階3 の設計（v2.0）を検証戦略 B で1判断時点ずつ追った結果（[T02](../traces/T02_paper_trace_strategy_b.md)）、**設計の選択を含む未記述が7件**見つかった。**7件とも 2026-09-23 に人間が決定し、いずれも提示時の推奨案（選択肢1）である**。選択肢と推奨の理由は T02 §15 が正本であり、本節は決定と反映先だけを持つ。

| # | 決めたこと | 決定（すべて選択肢1＝推奨） | 反映先 |
|---|---|---|---|
| Q23 | 突破水準（日足高値）が同じ判断時点で確定した日足を含むため、日足境界では突破が構造的に成立しない。宣言を変えるか、例文を改めるか | **宣言は変えず、例文のほうを改める**。取引機会が生まれるのは日足境界ではない1時間足の確定である | 第9.3節（本文の確定・ケース1・ケース2）・第9.4節の経路1・5 |
| Q24 | 確認待ちのあいだに市場状態が失効する経路（第9.4節の経路4）が、確認期限と日足境界の位置から到達しない。どう扱うか | **到達しない経路として記録し**、再検査の経路そのものは宣言を変えた小さな戦略で意味論テストする | 第9.4節の経路4と到達可否の段落、D08 §13.2 の未整備6（v1.1） |
| Q25 | 足の確定で生まれた保護水準の更新（トレーリング）を、どのフェーズで建玉へ適用するか | **受付（rank 10）の直前に適用する**。新しいフェーズは足さない | 本書ではなく D06 §4.2 の手順・§8.3（v1.6） |
| Q26 | 確認試行（`ConfirmationAttempt`）をエンジンがどの経路で受け取り、判断履歴の表18 をいつ書き出すか | **戻り値に `confirmation_attempts` を足し**、その `step` の差分だけを返して主キーで置き換える | 第3節の型表（`RuntimeStepResult`）・第6.2節の手順10・第7.7節、D06 §4.2・§9.2（v1.6） |
| Q27 | 1回の評価で欠損した入力の欠損方針（`on_missing`）が食い違うときの優先順位 | **強い方針が勝つ**（`Error` > `SkipEvaluation` > `WaitForInput` > `UsePrevious`） | 第6.3節（欠損の扱い） |
| Q28 | 足りない入力が出力参照だけの待機で、本数で数える期限をどの系列の確定足で数えるか | **その使用箇所の起動条件（`OnBarClose`）の系列**で数える | 第6.8節の「期限を絶対の形へ解決する」 |
| Q29 | 同じ判断時点で待機要求の期限到達と追い越しが同時に成立したときの優先順位 | **既に書かれている並び（期限 → 追い越し → 失効）を判定順とする**。期限が勝つ | 第6.8節の手順2 |

各項目で採らなかった案は、本文の該当節に「不採用」として1行ずつ残してある。

### 15.4 Q30 の決定（2026-09-23、PR #24。**決定済み**）

Q28 の決定を本文へ書くときに、**決定そのものでは決まらない場合が1つ残った**ため要決定として上げ、**同じ日に人間が決定した**（提示時の推奨案である選択肢1）。

| # | 決めたこと | 決定 | 反映先 |
|---|---|---|---|
| Q30 | 待機の本数期限を「その使用箇所の起動条件の系列」で数えるとき（Q28 決定）、その使用箇所が**系列の違う足の確定を2件以上宣言している**場合にどの系列で数えるか。区間が違えば要求が分かれるので数える系列は要求ごとに定まるが（第6.2節 手順3）、**区間が同じで系列だけが違う起動条件は1件の要求へ集約される**ため定まらない | **コンパイル時の検査を1件足す**（選択肢1）。接続元が出力参照だけの入力に本数で数える待機期限を書いた使用箇所は、**足の確定の起動条件の系列がただ1つ**でなければならない | 第5.6節の検査 h と同節の段落、第6.8節の「期限を絶対の形へ解決する」、**D04 §12 の検査 #15（v1.11）** |

**段階3 の宣言は1つも狭まらない**。第9.2節では、出力参照だけの待機が起きる使用箇所（`market_state`・`daily_above_ema`）がどちらも日足1系列でしか起動しないからである。採らなかった案（系列 ID の昇順で先頭を使う案、実行時に失敗させる案）は第5.6節に「不採用」として残した。
