...
作成日: 2026-09-21
状態: **v2.0（2026-09-22）。段階3 範囲は Q10〜Q18 決定済み、Q19〜Q22 は未決**。段階2 で承認済みの範囲（**承認（2026-09-21、PR #15）**、v1.0〜v1.4）は本改訂で変えていない。
...
| 【提案】 | 本書が推奨する設計。承認で確定 |
| 【要決定】 | 承認時にユーザーが選択する事項。段階2 の Q1〜Q9 は **2026-09-21 に決定済み**（第13節）、段階3 の Q10〜Q18 は **2026-09-22 に決定済み**（第15節）。**未決は Q19〜Q22**で、第15節に選択肢と推奨を置く |
...
**パラメータどうしの関係はコンパイル時に検査する**【提案】＋【要決定】Q22。`window_bars >= 2 * period` は契約の構築時には確かめられない。どちらも使用箇所が値を選ぶパラメータであり（D04 §7）、契約は各パラメータの**範囲（`ParameterSpec.bounds`）しか持たない**からである。種を作る `period` 本のあとに平滑化する足が1本も残らない設定を通すと、EMA が単純移動平均に化けたまま動く。そこで第5.6節の検査 e として、コンパイラのパラメータ解決の段（第5.1節 段3）で確かめる。同じ検査を `atr` にも使う。

**ただし、この関係を書いておく場所がまだ無い**【要決定】Q22。`ComponentContract` の確定済みフィールドにも `ParameterSpec` にも、**2つのパラメータの関係を表す項目が1つも無い**（D04 §3・§7）。したがって現状の宣言だけを読んでも、コンパイラは `window_bars >= 2 * period` を導き出せない。関係をどこに書くかは設計の選択であり、**本改訂では確定しない**（第15節 Q22）。決まるまで検査 e は実装できないので、**`ema` と `atr` を登録するのは Q22 の決定の後**とする。
...
**最新1件の保持（`latest_outputs`）はそのまま残す**【提案】。段階2 が決めた `RuntimeState.latest_outputs` は、保持本数の計画に載らない出力参照（履歴で読む相手が1つも無い出力）にとって唯一の保持先であり、消すと最新値の読み取りが成り立たない。履歴も持つ出力参照については、**`output_history[r]` の末尾の1件が `latest_outputs[r]` と一致する**ことを不変条件とし、両方を同じ処理（出力の付番と送出。第6.2節 手順7）で同時に更新する。片方だけを更新する経路を作らない。**不採用**: `latest_outputs` を廃して履歴の末尾から引く案（保持本数の計画に載らない出力の最新値が読めなくなり、段階2 の確定を必要なく壊す）、2つを別の手順で更新する案（同じ出力について2つの「最新」が食い違いうる）。
...
| `strategy`（**段階3 の実装への改訂依頼**） | 段階2 の実装（PR #18）と本改訂の設計が食い違う箇所はないが、段階3 では次を足す必要がある。いずれも本改訂で設計が確定した範囲である。(1) `declarations/missing.py` に「入力を待つ」（`WaitForInput`）と「過去値へ遡る」（`UsePrevious`）の2区分と2つの列挙を足し、`declarations/contract.py` の保存形式の版（`SCHEMA_VERSION`）を 1 から 2 へ上げる（D04 §6.3 v1.9）。(2) `declarations/refs.py` の `RuntimeTarget` に取引機会（`OPPORTUNITY`）を足す（D04 §4.3 v1.9）。(3) `records/payloads.py` に `ConfirmationOutcome` と `UpdateStop` を足す（第4.2節）。(4) `compiler/capability.py` の拒否一覧を D04 §12 の新しい2表に合わせて分ける。(5) `runtime` に `waiting.py` / `supersession.py` / `confirmation.py` / `output_history.py` を足す（第2節）。**Q19〜Q22 が決まるまで実装しない部分**: 確認期限の本数の数え方（Q19）、保持した出力の1件の数え方（Q20）、待機から再開したときの窓の基準（Q21）、2つのパラメータの関係の検査と、それに依存する `ema` / `atr` の登録（Q22） |
...
| **v2.0**（本改訂） | 段階3 の範囲（第1.2節の「段階3 で本改訂（v2.0）が決めること」の列）。複数の時間足・市場状態・後続確認・待機・遡り・追い越し・上流の出力の履歴窓・段階3 のカタログ・検証戦略 B | **Q10〜Q18 は 2026-09-22 に決定済み**（第15節）。第12.1節の他文書への反映も同じ日に済ませた。**残る未決は Q19〜Q22** で、いずれも段階3 の実装の一部（確認期限の本数の数え方、出力の履歴の1件の数え方と待機からの再開、2つのパラメータの関係の置き場所）を止めるだけであり、本改訂の他の範囲は決定済みである |
...
## 15. 段階3 の決定（Q10〜Q18。**決定済み**）と、残る要決定（Q19〜Q22。**未決**）
...
### 15.2 残る要決定（Q19〜Q22。**未決**）

Q10〜Q18 の決定と、その反映の作業・独立レビューから新しく生じた4件である。各項目は「何を決めるか / 影響 / 選択肢（3つ以内、**先頭が推奨**）/ 推奨の理由」の形で書く。**選択肢のラベルに書いていない解釈を足さない**。
...
#### Q22 「2つのパラメータの関係」をどこに書くか

**何を決めるか**: 指数移動平均（EMA）の「履歴の窓は平滑化の期間の2倍以上でなければならない」（`window_bars >= 2 * period`）のような、**2つのパラメータの関係**を、どこに書いて誰が確かめるか。

**影響**: いまの宣言には、パラメータ1つずつの範囲（最小・最大・許可値）しか書く場所が無い。関係の置き場所が決まらないと、窓が足りない設定が素通りして、EMA が単純移動平均に化けたまま run が最後まで走る。決め方によって、宣言（設定ファイル）を読むだけで関係が分かるかどうかも変わる。

| # | 選択肢 | 影響（1行） |
|---|---|---|
| 1（推奨） | 契約の登録（`ComponentRegistration`）が検証の純粋関数を1つ持ち、コンパイラがパラメータ解決の後に呼ぶ | 宣言の型を1つも増やさずに済み、契約と実装を1組で登録する既存の形（第4.1節）にそのまま載る |
| 2 | 戦略宣言モデル（D04）に関係の宣言形を足す（左辺のパラメータ名・比較・右辺のパラメータ名と係数） | 設定ファイルを読むだけで関係が分かるが、式をどこまで書けるようにするか（係数だけか、四則演算まで許すか）を決める必要がある |
| 3 | 検査を置かず、窓が足りない設定は部品が実行時に失敗を返す | 新しい仕組みが要らないが、宣言の誤りが run の途中まで分からず、判断履歴にデータ起因の失敗として残る |

**推奨の理由**: 部品は「契約＋実装＋実装参照」を1組で登録する形が既に確定しており（第4.1節）、登録時の検査も4件ある。関係の検証はその5件目として自然に収まり、宣言の型も設定ファイルの書き方も変えずに済む。関係は部品の計算規則から出てくるもの（種を作ったあとに平滑化する足が残るか）であり、戦略を書く人が選ぶ設定ではないので、宣言に書けなくても困らない。
