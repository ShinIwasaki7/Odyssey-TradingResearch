...
**v2.0-draft のレビューの対象範囲は、下表の「段階3 で本改訂（v2.0）が決めること」の列**である。列は名前で呼ぶ（番号で数えると、段階2 の4列から段階3 の5列へ増えたときに指す先がずれる）。

| 列 | 呼び方 | v2.0-draft での扱い |
|---|---|---|
| 3列目 | **段階2 で確定** | 承認済み。**本改訂では変えない**。ここへの指摘は「段階2 の確定への異議」として記録し、本 PR では直さない |
...
| **v2.0**（本改訂） | 段階3 の範囲（第1.2節の「段階3 で本改訂（v2.0）が決めること」の列）。複数の時間足・市場状態・後続確認・待機・遡り・追い越し・段階3 のカタログ・検証戦略 B | **承認待ち**。第15節の Q10〜Q18 を決めたうえで承認する |
| v2.1 以降 | 第10.2節が段階4 以降へ送った項目 | 未起草 |

承認の範囲は第1.2節の表の**「段階3 で本改訂（v2.0）が決めること」の列**である。「段階2 で確定」の列は本改訂では変えていない。「後続が決めること」の列は担当文書が決める。
