...
**打ち切ったとき、まだ評価していない要求は記録しない**【確定】（v2.10、2026-09-25 の人間の決定。R1-D05-4。いまの実装どおり）。「以降の評価を行わずに返す」とき、同じ部品の残りの要求も、評価順で後の部品の要求も、評価記録を出さない。**失敗した `step` では、失敗の評価記録がその `step` の最後の評価記録である**（第6.4節の「必ず1件」の例外）。同じ部品で既に作った要求は要求 ID（`RequestId`）を採番済みなので、**判断履歴の要求 ID に欠番が出うる**。失敗した run は評価（D07）が採用評価から外すので、指標には影響しない。**不採用**: 未評価の要求を全件「見送り」で記録する案（run の失敗で打ち切ったことを表す診断の語を新しく足す必要がある）、採番済みの要求だけ記録する案（どこまで作ったかが実装の順序に依存する）。
...
- 評価記録は起動した使用箇所ごとに必ず1件残す。入力不足で評価しなかったことも記録に残す【合意済み】上位 §4.3.15。**この規則は正常に進んだ `step` のものである**。評価の失敗で打ち切った `step` では、まだ番が来ていない要求の記録を出さない（第6.2節の「失敗は戻り値で返す」の後段。v2.10【確定】、R1-D05-4）。
...
| O7 生成時に `SNAPSHOT_AT_OPPORTUNITY` の束縛が読めない | 束縛の欠損方針が見送り（`SkipEvaluation`）なら、固定なしで O1〜O5 のとおり機会を生成し、有効性の再検査の記録（表19）に `MISSING_SKIPPED` を生成の処理点で1件残す。失敗（`Error`）なら機会を生成せず、取引機会を出す評価の記録が `Failed(DATA_ERROR)` になり run が止まる（第7.3節、v2.10【確定】、R1-D05-3。見送りの記録は後続の実装 PR） | 変化なし（固定は生成時に1回だけで、以後更新しない。第7.3節） | 変化なし（同左） | 変化なし（同左） | 変化なし |
...
- **生成時に束縛が読めないとき**【確定】（v2.10、2026-09-25 の人間の決定。R1-D05-3）。束縛の欠損方針が**見送り（`SkipEvaluation`）なら、固定なしで機会を作り、有効性の再検査の記録 `ValidityRecheck` に `MISSING_SKIPPED` を生成の処理点で1件残す**（`reason` は空。下の「読めない」の細目の仮置き6 と同じ）。記録型は束縛の種類を列に持つので、型は変えない。判断履歴の表19 には、P4・P5 の再検査の行に加えて生成時点の行も並ぶ（D06 §9.2）。**失敗（`Error`）なら**、機会を作らず、取引機会を出す評価の記録を `Failed(DATA_ERROR)` にして run を止める（いまの実装どおり。評価の中で起きた失敗なので評価記録が書き先になる）。**見送りの記録は後続の実装 PR で足す**（それまでの実装は、見送りのとき何も記録せずに機会を作る）。検証戦略 A・B はこの束縛を使っていない。**不採用**: いまの実装を正本にする案（固定できなかったことが判断履歴から読めない）、機会を作らず評価を見送りにする案（発火は必ず取引機会として記録するという決定記録 ADR-0032 とぶつかる）。
