...
| 鮮度がどこに残るか | **取引許可の出力記録そのもの**（D06 §9.2 の表1）である【提案】（v2.1。紙上トレース T02 §14 #2）。取引機会の遷移記録（`OpportunityTransition`）にも有効性の再検査の記録（`ValidityRecheck`）にも鮮度の欄は置かない。適用した許可は「**その発火の遷移記録の処理点より前に出た、その出力参照の最後の出力**」として一意に決まるので、判断履歴から後で復元できる。**判断時刻だけで絞らない**のは、日足と Trigger の足が同じ判断時点で確定する場合に、同じ `step` の P2 で出たばかりの許可を P3 が適用するからである（上位設計書 §4.3.11 の因果順序）。判断時刻より前で切ると、その更新を除いて1つ前の許可を指してしまう。**「前」の比べ方は次の2段で決まる**（v2.2。独立レビューの指摘を受けた明確化）。出力記録は `ProcessingPoint` を持たず（`decision_time` と `sequence` だけである。上位設計書 §4.3.15）、`sequence` は `step` ごとに 0 へ戻る（第6.6節）ので、`(decision_time, phase.rank, sequence)` の全順序を出力記録の側で組み立てることはできない。代わりに、**(1) どの `step` の記録かを先に決め、(2) 同じ `step` の中では `sequence` の昇順で比べる**。(1) が決まるのは、エンジンが同じ判断時点で行う3回の呼び出しの入力が重ならないからである（D06 §4.2 の表。第1回＝足の確定と公開だけ、第2回＝受付結果と約定の通知だけ、第3回＝run 末尾の合図だけ）。したがって、出力記録から `evaluation_id` で評価記録（表2）を引き、その `trigger_names` が指す起動条件の種類を見れば、第1回（足の確定・入力イベント）か第2回（実行時イベント）かが判断履歴だけで決まる。遷移記録は `ProcessingPoint` を持つので同じ判定がフェーズ順位からできる。これは新しい規則ではなく、既に確定している3回の呼び出しの入力の切り分けからの帰結である。**不採用**: 遷移記録に鮮度の欄を足す案（同じ値が表1 と表3 に二重に並ぶ）、判断時刻だけで絞る案（同じ判断時点の更新を取りこぼす）、出力記録に処理点を足す案（`OutputRecord` の正本は上位設計書 §4.3.15 であり、本書の改訂で足せない。上の2段で一意に決まるので足す必要も無い） |
