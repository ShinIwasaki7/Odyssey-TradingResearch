...
6. **レビュー**: push → PR → `@codex review` → 4分間隔ポーリング → 分類 → 修正 → 再依頼を、承認停止なしで自走する。**終了条件は巡数ではなく指摘の残数**で、レビュー対象範囲の中の必ず直す指摘（設計違反などの P0、バグ・契約違反などの P1）がどちらも0件になった巡で終了する。設計文書 PR はその文書の §1.2 の境界表で対象範囲を示し、依頼本文にも添える。2巡目以降は毎巡、指摘の傾向を1行で残してから直し、同種の指摘が繰り返すときは根本対処を先に入れる。承認済みの設計と衝突する指摘は理由を書いて「不採用」とし、要決定へ上げる（設計を捻じ曲げない）。安全弁は6巡（[PR レビュー方針](../pr_review_policy.md) §3.1・§4）。
   6b. **設計適合レビュー**: 実装 PR は Codex の収束後、別セッションが read-only で設計文書との適合を見る（同 §5）。設計文書に無い振る舞いを実装が決めた「仮置き」は PR 本文に番号付きで列挙し、merge 前に人間が1件ずつ決め、決定は設計文書の改訂として同じ PR に含める（同 §6）。
