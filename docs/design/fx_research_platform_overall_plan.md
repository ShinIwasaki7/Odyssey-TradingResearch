...
- 4分間隔、初回 P0/P1 なしなら終了、最大2巡という運用は維持する。**この巡数の規定は 2026-09-20 の人間の決定で置き換えられた**（[PR レビュー方針](../pr_review_policy.md) §4.1: 終了条件は巡数ではなく、レビュー対象範囲の中の P0・P1 が0件であること。安全弁は6巡）。4分間隔は変わらない。
- merge 判断は人間に残す。
...
| D02 | [共通カーネル型設計](D02_common_kernel.md)（承認 2026-09-20。v1.6 は PR #18） | 第5.1節・第7.1節 | D01、B-4 | 段階0 |
| D03 | [市場データ・時刻基盤設計](D03_marketdata_and_time.md)（承認 2026-09-20。v1.3・v1.4 は PR #19、v1.5 は PR #20） | 第5.2節・第7.2節。受入れ手順と検査仕様、封印分離 | D02、C-1〜C-4 | 段階0〜1 |
| D04 | [戦略宣言モデル詳細設計](D04_strategy_declarations.md)（承認 2026-09-20。v1.8 は PR #18） | 第5.3.1節・第7.3節前半。補助型全フィールド、宣言から導かれるコンパイル時検査の一覧、設定ファイル表現 | D02、D03（系列定義） | 段階0 |
| D05 | [戦略ランタイム・カタログ・コンパイラ設計](D05_strategy_runtime.md)（v1.2 承認 2026-09-21、段階2 の最小ランタイム範囲。v1.3 は PR #18、v1.4 は PR #20） | 第5.3.2〜5.3.5節・第7.3節後半。部品カタログの構成と初版の部品、コンパイラの実装（依存グラフ構築・ハッシュ計算）、取引機会の状態機械。待機・追い越し・後続確認は v0.2（段階3前） | D04 | 段階0〜3 |
| D06 | [バックテストエンジン設計](D06_backtest_vertical_slice.md)（承認 2026-09-21、PR #16。v1.2 は PR #17、段階2 の最小縦断範囲） | 第5.4節・第7.4節。フェーズ順序、4型全フィールド、状態機械、理由コード、trace/result 形式 | D02〜D05 | 段階0〜2 |
| D07 | [単一実行評価設計](D07_single_run_evaluation.md)（v1.1 承認 2026-09-21、PR #17、段階2 の単一実行評価境界。v1.2 は PR #19、v1.3 は PR #20） | 第5.5.1節・第7.5節前半。指標・集計・診断・評価の状態・結果の保存と再現性。年率化とリスク調整指標、実験 manifest、研究ポリシーは v0.2（段階4） | D06 | 段階2〜4 |
| D08 | テスト戦略 | 第8.4節を具体化。意味論テスト一覧、人工データ生成仕様、golden trace | D06 | 段階1〜 |
| T01 | [紙上トレース](../traces/T01_paper_trace.md)（承認 2026-09-21、PR #16。v1.1 は PR #17、v1.2 は PR #18、v1.3 は PR #20） | 検証戦略 A・B の時刻表を D03〜D06 の型で追跡し、同時刻・数量・末尾処理に暗黙の前提がないことを確認。検証戦略 A は8経路を通し、検証戦略 B は D05 v0.2 待ちの範囲を明示した | D03〜D06 | 段階0 完了条件 |
...
6. **レビュー**: push → PR → `@codex review` → 4分間隔ポーリング → 分類 → 修正 → 再依頼を、承認停止なしで自走する。**終了条件は巡数ではなく指摘の残数**で、レビュー対象範囲の中の必ず直す指摘（設計違反などの P0、バグ・契約違反などの P1）がどちらも0件になった巡で終了する。対象範囲の正本は [PR レビュー方針](../pr_review_policy.md) §3.1 の4行（A: §1.2 を持つ設計文書はその §1.2 / B: §1.2 を持たない文書は PR 本文の範囲表 / C: 実装は PR 本文に挙げた設計文書の節 / D: 骨格・ツールおよび他に当たらないものは PR 本文の範囲表）で決め、**該当する行をすべて併合する**。決めた範囲は依頼本文にも添える。2巡目以降は毎巡、指摘の傾向を1行で残してから直し、同種の指摘が繰り返すときは根本対処を先に入れる。承認済みの設計と衝突する指摘は理由を書いて「不採用」とし、要決定へ上げる（設計を捻じ曲げない）。安全弁は6巡（[PR レビュー方針](../pr_review_policy.md) §3.1・§4）。
   6b. **設計適合レビュー**: 実装 PR は Codex の収束後、別セッションが read-only で設計文書との適合を見る（同 §5）。設計文書に無い振る舞いを実装が決めた「仮置き」は PR 本文に番号付きで列挙し、merge 前に人間が1件ずつ決め、決定は設計文書の改訂として同じ PR に含める（同 §6）。**この2つで PR を変えたら、最終 head に対して 6 をもう一巡し、そこで新たな修正が出たら 6b へ戻る。2つのレーンが同じ head で新たな修正を生まなくなってから merge を問う**（同 §4.1。0件の判定はその巡が見たコミットに対するものなので、後から push したものは検証されていない）。
