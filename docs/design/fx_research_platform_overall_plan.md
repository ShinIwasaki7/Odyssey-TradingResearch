...
作成日: 2026-09-18
状態: **ドラフト。第6節の決定（2026-09-18、ADR-0001〜0017／2026-09-20、ADR-0018〜0033）を反映済み。残る要決定は第6節・第10節を参照**。**2026-09-25（PR #40）: 第8.5節「設計文書の必須表」を追加した**（R1。PR #26 で人間が承認した根本対処）。本書は上位文書 [fx_research_platform_greenfield_design.md](fx_research_platform_greenfield_design.md) を出発点として、全体アーキテクチャ・ディレクトリ構成・各基盤の設計方針・構築計画を提案するもの。本書の承認をもって段階0（契約設計）を開始する。実装は本書および後続の各設計文書の承認後に行う。
...
| `domain.research_policy` | 初版は共通1種類の `ResearchPolicy`（ID＋版）。事前固定・記録・複雑性計測の規則。複雑性の計測方法（合成部品内部を含む条件数等）を定義するが、初期に上限は設けない。**→ 2026-09-25 の人間の決定で、段階4 から複雑性にも上限を置いて検査する**（D07 v2.0 §20。上位文書との差異は D07 §13 の差異4） | §6 |
| `application.evaluate_run` | `BacktestResult` → 指標・診断・状態。診断には遅延シナリオ別の参照価格経過時間・不利約定幅・緊急決済件数を含む | §4.7.12 |
| `application.manifest` | 実験 manifest（仮説、戦略定義ハッシュ、探索計画、分割、評価規則、データ/モデル/ポリシー参照、seed、環境）。別プロセスで再現できること。**段階4 の置き場所は D07 v2.0 §19（型は `domain.experiment`、保存は `adapters.fs_store`、実行は `application.run_experiment`）** | §5.3、§7 |
...
| D03 | [市場データ・時刻基盤設計](D03_marketdata_and_time.md)（承認 2026-09-20。v1.3・v1.4 は PR #19、v1.5 は PR #20、**v1.6 は PR #22、v1.7 は PR #12**、v1.10 は PR #40＝必須表（第8.5節）、v1.11 は必須表の空欄を埋める文書 PR（PR #43）、v1.12 は D07 v2.0 の PR） | 第5.2節・第7.2節。受入れ手順と検査仕様、封印分離 | D02、C-1〜C-4 | 段階0〜1 |
| D04 | [戦略宣言モデル詳細設計](D04_strategy_declarations.md)（承認 2026-09-20。v1.8 は PR #18、**v1.9・v1.10 は PR #22、v1.11 は PR #24**、v1.14 は PR #40＝必須表の該当なしの明記（第8.5節）、v1.15 は D07 v2.0 の PR。保存形式の版は 1（v1.12 の決定。段階3 でも上げない）） | 第5.3.1節・第7.3節前半。補助型全フィールド、宣言から導かれるコンパイル時検査の一覧、設定ファイル表現 | D02、D03（系列定義） | 段階0 |
| D05 | [戦略ランタイム・カタログ・コンパイラ設計](D05_strategy_runtime.md)（v1.2 承認 2026-09-21、段階2 の最小ランタイム範囲。v1.3 は PR #18、v1.4 は PR #20、**v2.0 は PR #22**＝段階3 の範囲、**v2.1・v2.2 は PR #24**、v2.3・v2.4 は PR #27、v2.5 は PR #33、v2.6 は PR #35、**v2.7 は段階3 完了の文書 PR**、v2.8 は段階3 の文書整合 PR、v2.9 は PR #40＝必須表（第8.5節）、v2.10 は必須表の空欄を埋める文書 PR。段階3 の範囲は承認済み（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35）） | 第5.3.2〜5.3.5節・第7.3節後半。部品カタログの構成と初版の部品、コンパイラの実装（依存グラフ構築・ハッシュ計算）、取引機会の状態機械。段階3 の複数時間足・市場状態・後続確認・待機・遡り・追い越し・上流出力の履歴窓は **v2.0**（他文書が「v0.2」と呼んでいた呼び方は 2026-09-23 に v2.0 へ統一した。要決定 Q10〜Q30 はすべて決定済み（2026-09-23）で、未決の項目は残っていない。段階3 の宣言では実行結果が変わらないため実装は止まらない） | D04 | 段階0〜3 |
| D06 | [バックテストエンジン設計](D06_backtest_vertical_slice.md)（承認 2026-09-21、PR #16。v1.2 は PR #17、v1.3・v1.4 は PR #19、**v1.5 は PR #22、v1.6 は PR #24**、v1.7 は PR #33、**v1.8 は段階3 完了の文書 PR**、v1.9 は段階3 の文書整合 PR、v1.10 は PR #40＝必須表（第8.5節）、v1.11 は必須表の空欄を埋める文書 PR。段階2 の最小縦断範囲＋段階3 の記録と適用意味論） | 第5.4節・第7.4節。フェーズ順序、4型全フィールド、状態機械、理由コード、trace/result 形式 | D02〜D05 | 段階0〜2 |
| D07 | [単一実行評価設計](D07_single_run_evaluation.md)（v1.1 承認 2026-09-21、PR #17、段階2 の単一実行評価境界。v1.2 は PR #19、v1.3 は PR #20、**v1.4 は PR #24**、v1.5 は PR #40＝必須表（第8.5節）、**v2.0 は段階4 の設計 PR**＝段階4 の範囲。他文書が「v0.2」と呼んでいた版） | 第5.5.1節・第7.5節前半。指標・集計・診断・評価の状態・結果の保存と再現性。段階4（v2.0）で指標集合 v2（年率化・リスク調整）、取引単位の費用、実験設定の書式 v2、実験の記録票、研究ポリシー v1、別プロセスでの再現、レポート、実データでの実行。要決定 Q7〜Q13 は 2026-09-25 に決定済み（複雑性の上限は検証戦略 B の約3倍） | D06 | 段階2〜4 |
| D08 | [テスト戦略](D08_test_strategy.md)（**承認 2026-09-23、PR #23**。v1.0。**v1.1 は PR #24**、v1.2 は PR #25、v1.3 は PR #30、v1.4 は PR #31、v1.5 は PR #34、v1.6 は PR #35、**v1.7 は段階3 完了の文書 PR**、v1.8 は PR #40＝必須表の該当なしの明記（第8.5節）、**v1.9 は D07 v2.0 の PR**＝段階4 の受入テストの規約と実データのマーカー） | 第8.4節を具体化。段階1〜2 で実際に書かれたテスト資産を正本に、テストの7層・意味論テスト一覧（契約行との対応表）・人工データ生成仕様・固定出力の運用・決定論の検証・CI の並びを定める。Q1〜Q3 の決定により、契約行の充足は層を問わないこと・人工データはアクセス分類の対象外であることを確定し、D01 §9 を v2.5 に改訂した | D06 | 段階1〜 |
| T01 | [紙上トレース](../traces/T01_paper_trace.md)（承認 2026-09-21、PR #16。v1.1 は PR #17、v1.2 は PR #18、v1.3 は PR #20、**v1.4 は PR #22**、v1.5 は PR #40＝必須表との対応（第8.5節）） | 検証戦略 A・B の時刻表を D03〜D06 の型で追跡し、同時刻・数量・末尾処理に暗黙の前提がないことを確認。検証戦略 A は8経路を通し、検証戦略 B は D05 v2.0 待ちの範囲を明示した | D03〜D06 | 段階0 完了条件 |
| T02 | [紙上トレース（検証戦略 B・段階3）](../traces/T02_paper_trace_strategy_b.md)（**承認 2026-09-23、PR #24**。v1.0。v1.1 は PR #27、**v1.2 は段階3 実装 PR 5/5**＝受入れテストの実測値との突き合わせ、v1.3 は PR #40＝必須表との対応（第8.5節）） | 段階3 の設計（D05 v2.0）が挙げた9経路を、4つの遅延シナリオの人工データで1判断時点ずつ追う。未記述20件を洗い出し、うち13件を D05 v2.1・D07 v1.4 へ反映、7件を要決定 Q23〜Q29 として上げて同日に決定・反映した（D05 v2.2・D06 v1.6・D08 v1.1）。**経路2つが宣言と規則から到達しないことを示した**のが最大の結果である | D03〜D08 | 段階3 の着手条件 |
...
| 3. MTF と後続確認 | MarketState、複数足入力、取引機会、ExecutionFilter、WAIT_FOR_INPUT、追い越し、期限/失効、合成部品、遅延シナリオ4ケース。検証戦略 B | D05 | 戦略 B を同じ基盤で記述でき、時刻境界と取消理由を trace できる。遅延シナリオ別の差分を追跡できる。**2026-09-25 に段階3 実装 PR 5/5 で3条件を受入れテスト（`tests/acceptance/test_stage3_completion.py`）と遅延4ケースの意味論テスト（`tests/semantics/backtest/test_delay_scenarios.py`）で確かめた**（T02 §20）。**完了 2026-09-25（PR #25・#27・#32・#33・#35）**。PR #35 の merge をもって段階3 を完了した。PR #35 の仮置き8件への人間の決定（2026-09-25）は D05 v2.7・D06 v1.8・D08 v1.7 に反映した |
| 4. 単一実行評価の整備 | 実験 manifest、指標定義の確定、診断、研究ポリシー検査、結果保存。許可された実データでの実行。実験設定の書式に検証戦略 B と遅延シナリオを書けるようにする拡張 | **D07 v2.0**（段階4 の範囲。D08 v1.9 が受入テストの規約） | 結果から設定と入力を特定でき、別プロセスで再現可能。失敗/0取引も説明できる。**PR の分け方（2026-09-25 の人間の決定、案A）**: 設計1本（D07 v2.0）→ 実装4本（1 評価の中身 / 2 実験設定の書式 v2 / 3 記録票・研究ポリシー・再現 / 4 レポート・実データ・段階4 受入。D07 §17.2）。封印期間の許可判定は段階5 へ送った（D07 §24） |
...
### 8.5 設計文書の必須表【合意済み】（R1。2026-09-24 に人間が承認、PR #26 の決定 Q4）

設計文書 PR に対する過去の指摘の過半（338 件中 192 件）は「ある状態である出来事が起きたときの振る舞いが書かれていない」という空欄であり、境界表（§1.2）や他文書への反映漏れも巡をまたいで再発した（[AGENTS.md の設計](agents_md_for_codex_review.md) §2.1・§2.3 の RC7、§5 の R1）。そこで、状態機械や実行時の値の伝播を定める設計文書に、次の3つの表を必須とする。表は本文の言い換えではなく**網羅の確認**のためにある。本文と表が食い違えば本文の節が正本であり、表を本文に合わせて直す。

**対象の決め方**。文書番号ではなく、**その文書が状態機械や実行時の値の伝播を定める節を持つかどうか**で決める。

| 文書の種類 | 求めるもの |
|---|---|
| §1.2 を持ち、状態機械または実行時の値の伝播を定める節を持つ文書（D03・D05・D06・D07） | 表1〜3（持たない側の表は §1.2 に「該当なし（理由）」の 1 行）。表2・表3 は該当する節の中か直後に置き、§1.2 の直後に置き場所の一覧を書く |
| §1.2 を持つが、状態機械も実行時の値の伝播も定めない文書（D04・D08。D04 は宣言の形だけを決め、取引機会の状態機械・部品状態の更新・実行時の識別子を D05・D06 へ委ねている。D04 §1.2 の行4〜6） | 表1 と、§1.2 に「状態×出来事表・値の伝播表: 該当なし（理由）」の 1 行 |
| §1.2 を持たない文書（D01・D02、決定記録、本書、方針文書など） | 何も求めない。レビュー対象範囲の正本は PR 本文の範囲表のまま（[PR レビュー方針](../pr_review_policy.md) §3.1 の行 B） |
| 紙上トレース（T01・T02） | 自ら状態機械を定めず、設計文書の表を1判断時点ずつ追う文書である。表2 の代わりに**経路ごとに通るマスの対応表**（どの経路が設計文書の表のどのマスを通るか）を置き、表3 の代わりに既存の「各表に残る行の一覧」を実例として指す。レビュー対象範囲は追跡する設計文書の §1.2 に従う |

**表1: 境界表（§1.2。既存）**。領域ごとに1行で、「本書が決めること（対象内）」と「後続・他文書に委ねること（対象外・担当）」を分ける。前提として他文書から受け取るものがあれば、別表に出どころの文書と節を書く。レビュー対象範囲の正本である（方針 §3.1 の行 A）。D04〜D08 の既存の §1.2 はこの形を満たす。

**表2: 状態×出来事表**。状態機械 1 つにつき 1 表。

| 項目 | 規則 |
|---|---|
| 行と列 | 一方に状態（生成が出来事なら「生成前」を含める）、他方に出来事（その状態機械に届きうる契機のすべて）。どちらを行にするかは読みやすい方でよい |
| マスの書き方 | 次の4つのどれか。(a) **遷移**: 遷移先・記録する理由・根拠の節（遷移の一覧に番号があればその番号）。(b) **変化なし**: 状態を変えない理由と根拠の節。(c) **到達しない**: その組み合わせが起きない理由と根拠の節。(d) **拒否**: 構造エラーとして拒否する（例外名と根拠の節） |
| 空欄 | 置かない（[PR レビュー方針](../pr_review_policy.md) §4.2 の「状態×出来事の表で空欄をゼロにする」）。本文から埋められないマスは「未定」ではなく**「要決定」**と書いて表の直後に一覧し、人間の決定を得て本文と表の両方を直す |
| 遷移の一覧との関係 | 文書が番号付きの遷移の一覧を持つなら、それを正本とする。本表はそれを状態×出来事に並べ替え、**遷移の一覧に無い組み合わせ**を本文の根拠つきで埋めたものである |

**表3: 値の伝播表**。複数の部品・層・成果物をまたいで運ばれる値（識別子・区間・処理点・run の識別子など）1 つにつき 1 行。

| 列 | 書くこと |
|---|---|
| 値 | 値の名前と型 |
| 生成元 | 採番・計算する部品と、その時点（フェーズ・手順）。根拠の節 |
| 渡り方 | 受け取る部品と、運ぶ型のフィールド |
| 記録先 | 保存される表・ファイルと列（記録しないなら「記録しない（理由）」） |
| 主キー | 記録先でその行を一意にする鍵（その値が主キーの一部でなければ、記録先の表の主キー） |

**確かめる時点**。設計文書を追加・改訂する PR で、設計適合レビュー（方針 §5）と Codex が [`AGENTS.md`](../../AGENTS.md) の「Design-document PRs」に従って、3つの表（または該当なしの明記）の有無と空欄の無さを確かめる。表の内容に関わる改訂では、表と本文を同じ PR で直す。
...
**現在地（2026-09-25）: 段階3 が完了し、段階4「単一実行評価の整備」に入った**（第8.2節）。段階4 の着手順は次のとおり。

1. **実データの分類（段階4 の前提）**: FX の祝日を取引日単位の休場（前日 17:00〜当日 17:00 NY）として書ける形（D03 v1.9 §3.4.1、PR #34）まで merge 済み。分類の第1弾（分類の決定5件（2026-09-24 の人間の決定）を分類ファイルとカレンダーへ反映する PR）が進行中である。
2. **段階4 の設計（D07 v2.0、PR #41）**: 人間の決定6件と要決定 Q7〜Q13 の決定（いずれも 2026-09-25。複雑性の上限は部品数 30・使用箇所数 36・パラメータ数 33・判断を出す使用箇所数 18、実データは 2016〜2023 の1本に 2018 年の1本を足す）を反映した。設計 PR の承認後、実装 PR 1（評価の中身）と PR 2（実験設定の書式 v2）から着手する。PR 3 は 1・2 の後、PR 4 は 3 と snapshot 承認の後（D07 §17.2）。
3. **必須表の決定の実装（6 件）**: 必須表の空欄 10 件への人間の決定（2026-09-25、すべて推奨案。D03 v1.11・D05 v2.10・D06 v1.11 に反映済み）のうち、実装変更を伴う 6 件。暫定 snapshot の再受入れを失敗させる 1 件は成果物の書き込み統一（R4）の PR に含める。判断履歴に表20「公開記録」・表3 の確認の開始足と期限の列・表16 の固定した足の列を足す 3 件は 1 本の実装 PR、受付通知の適用順と生成時の固定が読めないときの記録の 2 件は戦略ランタイムの小さな実装 PR 1 本にまとめる（後の 2 件はいまの検証戦略では起きないので急がない）。

以下は段階−1 の着手時点（2026-09-18）に書いた次のアクションであり、記録として残す。1〜4 は完了した。
