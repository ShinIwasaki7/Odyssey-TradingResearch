...
| D03 | [市場データ・時刻基盤設計](D03_marketdata_and_time.md)（承認 2026-09-20。v1.3・v1.4 は PR #19、v1.5 は PR #20、**v1.6 は PR #22、v1.7 は PR #12**、v1.10 は D07 v2.0 の PR） | 第5.2節・第7.2節。受入れ手順と検査仕様、封印分離 | D02、C-1〜C-4 | 段階0〜1 |
| D04 | [戦略宣言モデル詳細設計](D04_strategy_declarations.md)（承認 2026-09-20。v1.8 は PR #18、**v1.9・v1.10 は PR #22、v1.11 は PR #24**、v1.14 は D07 v2.0 の PR。保存形式の版は 2） | 第5.3.1節・第7.3節前半。補助型全フィールド、宣言から導かれるコンパイル時検査の一覧、設定ファイル表現 | D02、D03（系列定義） | 段階0 |
| D05 | [戦略ランタイム・カタログ・コンパイラ設計](D05_strategy_runtime.md)（v1.2 承認 2026-09-21、段階2 の最小ランタイム範囲。v1.3 は PR #18、v1.4 は PR #20、**v2.0 は PR #22**＝段階3 の範囲、**v2.1・v2.2 は PR #24**、v2.3・v2.4 は PR #27、v2.5 は PR #33、v2.6 は PR #35、**v2.7 は段階3 完了の文書 PR**、v2.8 は段階3 の文書整合 PR。段階3 の範囲は承認済み（段階3 完了 2026-09-25、PR #25・#27・#32・#33・#35）） | 第5.3.2〜5.3.5節・第7.3節後半。部品カタログの構成と初版の部品、コンパイラの実装（依存グラフ構築・ハッシュ計算）、取引機会の状態機械。段階3 の複数時間足・市場状態・後続確認・待機・遡り・追い越し・上流出力の履歴窓は **v2.0**（他文書が「v0.2」と呼んでいた呼び方は 2026-09-23 に v2.0 へ統一した。要決定 Q10〜Q30 はすべて決定済み（2026-09-23）で、未決の項目は残っていない。段階3 の宣言では実行結果が変わらないため実装は止まらない） | D04 | 段階0〜3 |
| D06 | [バックテストエンジン設計](D06_backtest_vertical_slice.md)（承認 2026-09-21、PR #16。v1.2 は PR #17、v1.3・v1.4 は PR #19、**v1.5 は PR #22、v1.6 は PR #24**、v1.7 は PR #33、**v1.8 は段階3 完了の文書 PR**、v1.9 は段階3 の文書整合 PR。段階2 の最小縦断範囲＋段階3 の記録と適用意味論） | 第5.4節・第7.4節。フェーズ順序、4型全フィールド、状態機械、理由コード、trace/result 形式 | D02〜D05 | 段階0〜2 |
| D07 | [単一実行評価設計](D07_single_run_evaluation.md)（v1.1 承認 2026-09-21、PR #17、段階2 の単一実行評価境界。v1.2 は PR #19、v1.3 は PR #20、**v1.4 は PR #24**、**v2.0 は段階4 の設計 PR**＝段階4 の範囲。他文書が「v0.2」と呼んでいた版） | 第5.5.1節・第7.5節前半。指標・集計・診断・評価の状態・結果の保存と再現性。段階4（v2.0）で指標集合 v2（年率化・リスク調整）、取引単位の費用、実験設定の書式 v2、実験の記録票、研究ポリシー v1、別プロセスでの再現、レポート、実データでの実行。**要決定 Q7〜Q13 は未決** | D06 | 段階2〜4 |
| D08 | [テスト戦略](D08_test_strategy.md)（**承認 2026-09-23、PR #23**。v1.0。**v1.1 は PR #24**、v1.2 は PR #25、v1.3 は PR #30、v1.4 は PR #31、v1.5 は PR #34、v1.6 は PR #35、**v1.7 は段階3 完了の文書 PR**、**v1.8 は D07 v2.0 の PR**＝段階4 の受入テストの規約と実データのマーカー） | 第8.4節を具体化。段階1〜2 で実際に書かれたテスト資産を正本に、テストの7層・意味論テスト一覧（契約行との対応表）・人工データ生成仕様・固定出力の運用・決定論の検証・CI の並びを定める。Q1〜Q3 の決定により、契約行の充足は層を問わないこと・人工データはアクセス分類の対象外であることを確定し、D01 §9 を v2.5 に改訂した | D06 | 段階1〜 |
...
| T02 | [紙上トレース（検証戦略 B・段階3）](../traces/T02_paper_trace_strategy_b.md)（**承認 2026-09-23、PR #24**。v1.0。v1.1 は PR #27、**v1.2 は段階3 実装 PR 5/5**＝受入れテストの実測値との突き合わせ） | 段階3 の設計（D05 v2.0）が挙げた9経路を、4つの遅延シナリオの人工データで1判断時点ずつ追う。未記述20件を洗い出し、うち13件を D05 v2.1・D07 v1.4 へ反映、7件を要決定 Q23〜Q29 として上げて同日に決定・反映した（D05 v2.2・D06 v1.6・D08 v1.1）。**経路2つが宣言と規則から到達しないことを示した**のが最大の結果である | D03〜D08 | 段階3 の着手条件 |
...
| 4. 単一実行評価の整備 | 実験 manifest、指標定義の確定、診断、研究ポリシー検査、結果保存。許可された実データでの実行。実験設定の書式に検証戦略 B と遅延シナリオを書けるようにする拡張 | **D07 v2.0**（段階4 の範囲。D08 v1.8 が受入テストの規約） | 結果から設定と入力を特定でき、別プロセスで再現可能。失敗/0取引も説明できる。**PR の分け方（2026-09-25 の人間の決定、案A）**: 設計1本（D07 v2.0）→ 実装4本（1 評価の中身 / 2 実験設定の書式 v2 / 3 記録票・研究ポリシー・再現 / 4 レポート・実データ・段階4 受入。D07 §17.2）。封印期間の許可判定は段階5 へ送った（D07 §24） |
...
2. **段階4 の設計（D07 v2.0）**: 人間の決定6件（2026-09-25。PR の分け方は案A）を写した設計 PR を出した。**要決定 Q7〜Q13**（複雑性の上限の値、年率化の日の区切り、実データ実行と能力検査の衝突、勝敗の基準、戦略ファイルの置き場所、書式 v1 と再現の環境、待機の数え方。D07 §26）を人間が決め、設計 PR を承認したら、実装 PR 1（評価の中身）と PR 2（実験設定の書式 v2）から着手する。PR 3 は 1・2 の後、PR 4 は 3 と snapshot 承認 PR の後（D07 §17.2）。
