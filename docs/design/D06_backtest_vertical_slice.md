...
| 状態×出来事表: run（実行前・実行中・`RunStatus` の3値） | 第10.6節（要決定のマス1件: R1-D06-2） |
...
| 保存先 `runs/<run_id>/` が既にある | **要決定（R1-D06-2）** | **要決定（R1-D06-2）** | **要決定（R1-D06-2）** | **要決定（R1-D06-2）** |

失敗した run では第3回の `step` を呼ばない（第10.2節）。取引機会は最後に観測された状態のまま残り、架空の終端理由を付けない。

**要決定のマス**:

- **R1-D06-2**: 保存先 `runs/<run_id>/` が既にあるときの扱い。ADR-0006 は「既定は失敗し、置換は明示の指示でのみ行い、旧成果物の manifest を残す」と定め、衝突検査と置換の指示を「D06/D07 の CLI 設計に含める」と委ねたが、本書にはどの時点で検査するか・置換の指示の名前・旧 manifest の残し方が書かれていない。承認済み・未実施の根本対処 R4（成果物の書き込みを「存在すれば失敗」に統一する。改訂先は第9.1節・第9.3節）の対象である。
