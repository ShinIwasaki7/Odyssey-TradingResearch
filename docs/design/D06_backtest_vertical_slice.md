...
- **表16 の「待機に入った」行に1列を足す**: 固定した足 `pinned_bars`（D05 v2.10 §3 の `WaitEvent` の改訂。入力名と足の組）。要素数が行ごとに変わるので、第9.1節の可変長の入れ子の規則で正規化エンコード文字列の `list` 列にする。`WAIT_STARTED` 以外の行では空。**待機期限は足さない**（表2 の `outcome_deadline_at_*` に既にある）。
- **表19 には生成時点の行も並ぶ**: 取引機会を作る時点で値を固定する束縛が読めず、見送りの方針で機会を作った場合、`MISSING_SKIPPED` の行が生成の処理点で1件残る（D05 v2.10 §7.3）。主キー `(opportunity_id, at)` は変わらない（生成時点の再検査は1回だけ）。
