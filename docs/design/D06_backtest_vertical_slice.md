...
| 末尾3集計 `FinalSummaries` | run_end の手順の後に計算（§10.3）。`status` が `COMPLETED` のときだけ作る | `BacktestResult.summaries` → D07 の整合検査 C5・指標（D07 §10.2） | 記録しない（`BacktestResult` の項目として渡すだけ。§9.4） | —（run に1件） |
| 件数 `trade_count` / `opportunity_count`・`unresolved_intrabar_count`・`swap_modeled` | エンジンが run の終わりに数える（§9.4）。`swap_modeled` は常に `False`（ADR-0029） | `BacktestResult` → D07 の整合検査 C3・C6 と評価 manifest（D07 §8.3・§10.2） | `UNRESOLVED_SL_PRIORITY` の件数と割合は run manifest の足内競合の群（§9.3）。他は `BacktestResult` の項目として渡すだけ（§9.4） | run manifest は run に1件 |
