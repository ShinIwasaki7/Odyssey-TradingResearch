...
- **R1-D06-2**: 保存先 `runs/<run_id>/` が既にあるときの扱い。ADR-0006 は「既定は失敗し、置換は明示の指示でのみ行い、旧成果物の manifest を残す」と定め、衝突検査と置換の指示を「D06/D07 の CLI 設計に含める」と委ねたが、本書にはどの時点で検査するか・置換の指示の名前・旧 manifest の残し方が書かれていない。承認済み・未実施の根本対処 R4（成果物の書き込みを「存在すれば失敗」に統一する。改訂先は第9.1節・第9.3節）の対象である。→ **決定（v1.12、R4）【確定】: 存在すれば、何も書かずに失敗する**（暫定 snapshot の R1-D03-1、評価の成果物の R1-D07-3 と同じ規則。失敗は `ArtifactAlreadyExists`）。検査の時点は実行前と書き出しの直前の2回（第9.1節）、置換の指示の名前は `--replace`、旧 manifest は置換の世代ごとに `manifest.replaced.<NNN>.json` として残し、既存の世代は上書きしない（第9.3節。2026-09-25 の人間の決定）。置換の指示の名前は段階2 の実装が ADR-0006 に従って既に持っていたもので、本版はそれを正本として書いた。
