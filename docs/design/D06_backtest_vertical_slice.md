...
作成日: 2026-09-21
状態: **承認（2026-09-21、PR #16）**。**v1.12（2026-09-25、成果物の書き込みを「存在すれば失敗」に統一する根本対処 R4 の PR）: 必須表に残っていた最後の空欄（保存先 `runs/<run_id>/` が既にあるときの扱い、R1-D06-2）を、R4 の規則で埋めた**。(1) 既定は**存在すれば、run を始めずに何も書かずに失敗する**（空のディレクトリも、前の実行が途中で残したディレクトリも同じ。失敗は `ArtifactAlreadyExists`）。(2) 検査は run の識別子が決まった時点（実行前）と、書き出しの直前（書き出し先を新しく作ること自体で確かめる）の2回行う。(3) 置換は明示の指示 `odyssey-fx run --replace` でだけ行い、評価の成果物（`eval/`）を畳む（ADR-0006。段階2 からの実装どおり）。**旧 manifest は置換の世代ごとに別名 `manifest.replaced.<NNN>.json`（001 から欠番なく）で残し、既存の世代は上書きしない**（2026-09-25 の人間の決定。段階2 の実装は `manifest.replaced.json` 1つを置換のたびに上書きしており、R4 と ADR-0006 の「旧 manifest を記録に残す」に反していたため改めた）。(4) 書き込み前検査の置き場（層の規則により `evaluation.adapters.fs_store` と `marketdata.adapters.parquet_store` の2か所に同じ規則で置く）、失敗の型、run の検査を2回行うこと、書き出し先の名前と識別子の一致を snapshot だけで検査することは、PR #46 の仮置きを人間が承認した（2026-09-25）。第9.1節・第9.3節・第10.6節を改めた。**実行の意味論・判断履歴・識別子は変えていない**。v1.11（2026-09-25、必須表の空欄を埋める文書 PR）: 必須表の空欄のうち本書の2件と、戦略ランタイム（D05 v2.10）・市場データ（D03 v1.11）の決定が本書の判断履歴に及ぶ3件に、**人間が決定した（2026-09-25、すべて推奨案）**ので、【確定】として書いた。(1) 公開バッチの識別子 `batch_id` は**判断履歴に記録しない**。注文イベントの識別子 `event_id` に欠番が出ることを明記した（R1-D06-1。第4.3節・第9.5節・第10.2節。いまの実装どおり）。(2) `BacktestResult` は **`runs/<run_id>/result.json`（JSON）** に保存する（R1-D06-3。第9.1節・第9.4節・第9.5節。いまの実装どおり）。(3) 判断履歴に**表20「公開記録」**を足し、**表3 の生成の行に確認の開始足と期限の列**、**表16 の「待機に入った」行に固定した足の列**を足す。表19 に生成時点の行が並ぶことも書いた（第9.2節・第9.4節・第9.5節。D03 の R1-D03-3、D05 の R1-D05-3・5・6。**実装は後続の実装 PR**）。保存先 `runs/<run_id>/` が既にあるときの扱い（R1-D06-2）は、成果物の書き込み統一（R4）の PR で決めるので、本版でも要決定のまま残した。v1.10（2026-09-25、PR #40）: 設計文書の必須表（R1（PR #26 承認）。全体計画書 §8.5）を加えた。状態×出来事表を4つ（第5.1.1節 注文、第8.1.1節 予約、第8.3.1節 建玉と保護水準、第10.6節 run）、値の伝播表を1つ（第9.5節）足し、第1.2節に置き場所の一覧を置いた。表は既存の本文の規則を並べ替えたもので、本文から埋められなかったマス3件（公開バッチの識別子の記録先 R1-D06-1、保存先 `runs/<run_id>/` が既にあるときの扱い R1-D06-2、`BacktestResult` を保存するファイル R1-D06-3）は「要決定」と書いた。**本文の規則は変えていない**。v1.9（2026-09-25、段階3 の文書整合 PR）: 第6.4節の手順3 の表と第17節の #13 が「段階3 で再判断する」としていた**参照価格の鮮度の上限**について、段階3 完了の点検を受けた人間の決定（2026-09-25）を記録した。**段階3 では上限を変えない**（検証戦略 B で必要にならなかった。2026-09-25 の人間の決定、v1.9）。次の再判断は、段階4 以降で必要が出たときに行う。上限を置かない規則そのものは変えていない。v1.8（2026-09-25、段階3 完了の文書 PR）: 段階3 実装 PR 5/5（エンジン＋受入、PR #35）が置いた**仮置き8件を人間が承認した（2026-09-25）**ので、該当する節へ【確定】として書き足した（第3節の型表・第8.3節・第9.2節の表12: 丸めた後の損切り水準の記録先＝1、第8.3節の損切り水準の更新の表: 「不利」の境界＝2・次の執行足の求め方＝3・次の執行足が予定に無いとき＝4・根拠記録＝5、第4.4節: 処理点の振り直しの順＝6、第3節の型表・第9.2節の表17: 行の形＝7。8 の受入テストの経路はテスト戦略 D08 v1.7 §2.2 が受け取る）。あわせて、記述の追随を2件行った（新しい規則ではない）: 第1.2節の「受け取るもの」の表の `ManagementAction` を、D04 §11.2（v1.9）が段階3 で足した `UPDATE_STOP` を含む3種別に揃えた。第1.2節の行6 の段階3 の表の件数を、第9.2節と同じ4件に正した。**新しい規則は置いていない**（8件は実装が既に採っていた解釈であり、段階2 の実行結果も検証戦略 A の固定出力も変わらない）。v1.7（2026-09-24、段階3 実装 PR 4/5）: 戦略ランタイムの待機の出来事の種類に `OPPORTUNITY_ENDED`（受け取った取引機会が待機のあいだに終わったので閉じた）が足されたこと（2026-09-24 の人間の決定。D05 v2.5 §3・§6.8 の手順2）を、判断履歴の表16（`WAIT_EVENTS`）の語彙として第9.2節に追記した。表の列と主キーは変えない（種類の列の値が1つ増えるだけである）。**段階2 の挙動は変えていない**（段階2 の run は待機の出来事を1件も出さない）。v1.6（2026-09-23、PR #24）: 検証戦略 B の紙上トレース [T02](../traces/T02_paper_trace_strategy_b.md) が立てた要決定に人間の決定が出たので、段階3 の範囲で反映した（**段階2 の挙動は変えていない**）。あわせて独立レビューの指摘を反映した: run 末尾の呼び出しでも段階3 の3列を判断履歴へ渡すこと（第4.2節の手順11・第10.1節・第10.2節。空であることを検査する列も正した）、第1回の `step` が返した全数量決済を受付（rank 10）へ渡すこと（第4.2節の手順6・7）である。(1) **足の確定で生まれた損切り水準の更新（トレーリング）を、受付（rank 10）の直前に建玉へ適用する**（Q25 決定、選択肢1。第4.2節の手順6・第8.3節）。同じ手順で、**第1回の `step` が返した全数量決済を受付（rank 10）へ渡す**ことも明記した（第6.2節の表が既に定めていた経路であり、手順の側に書き落としていた）。従来の「適用フェーズは約定後の評価（`POST_FILL_EVALUATION`）」では、受付通知も約定通知も無い判断時点で第2回の `step` を呼ばないため、足の確定で出た更新を適用する場所が無かった。**新しいフェーズは足さず**、処理点のフェーズを受付（`ADMISSION`）にする。更新を受付より前に効かせるので、同じ判断時点の新しい注文のリスク審査が更新後の水準を見る。(2) **取引機会の有効性の再検査が失敗した（`MISSING_FAILED`）場合も、評価の失敗と同じ経路で run を止める**（独立レビューの指摘。第4.2節）。再検査は評価の外側で走るため評価記録が作られず、従来の停止条件（評価記録の `Failed`）では止まらないまま注文が受け付けられる経路が残っていた。(3) **確認試行（表18）を、戦略ランタイムの戻り値に足した列（`RuntimeStepResult.confirmation_attempts`）から主キー `(opportunity_id, bar_key)` で置き換えて書く**（Q26 決定、選択肢1。第4.2節の手順5・9・第9.2節）。従来は確認試行がランタイムの内部状態にしか無く、エンジンが表18 を書く経路が存在しなかった。v1.5（2026-09-22、PR #22）: 戦略ランタイム設計（D05 v2.0）の要決定 Q10〜Q18 に対する人間の決定を受け、**D05 §12.1 が挙げた本書への改訂依頼5件をすべて反映した**（段階3 の範囲。段階2 の確定は変えていない）。(1) 第4.1節の rank 4「取引機会のライフサイクル検査」の内容に、**待機中の評価要求の期限と追い越しの検査**を足した（新しいフェーズは足さない）。(2) 第4.4節の「戦略ランタイムが返した記録の処理点はエンジンの時計で番号を振り直す」の対象に、**待機の出来事**（`WaitEvent.at`）と**有効性の再検査**（`ValidityRecheck.at`）を足した。(3) 第8.3節に**損切り水準の更新（トレーリング、`UpdateStop`）の適用意味論**を足した。(4) 第9.2節の判断履歴の表に**段階3 の4表**（待機の出来事・遡った入力・確認試行・取引機会の有効性の再検査）を足した。上流の出力の履歴読み取りについては、D05 §6.12 の決定により**新しい表を足さない**。(5) 第10.1節の末尾処理に、**run 末尾に残った待機要求の終端**を足した。v1.4（2026-09-22、PR #19）: 独立レビュー（Codex）7巡目の保留1件を人間が決定した。(1) **週末持ち越し禁止の判定は週末だけに効かせる**。カレンダーに「休場を適用する前の週の開場区間を返す操作」を足し、受付側はそれで週末境界だけを判定する（第5.3節。同じ PR で D03 §3.4 を改訂）。祝日・短縮セッションをまたぐ候補は有効時間の問題（`NO_CANDIDATE`）として扱う。(2) 判断履歴の数値表記は末尾ゼロを落とす現在の規則のままとし、紙上トレース T01 の `149.500` は値としての等価を示す書き方であって保存文字列は `149.5` であることを第9.1節に明記した。v1.3（2026-09-22、PR #19）: 段階2 のバックテスト基盤の実装で残った11件の仮置きを人間がすべて決定し、本文へ反映した（**未決の項目は残っていない**）。主なものは、(1) 保護水準の更新を決済要求との衝突で捨てた理由に新しい理由コード `SUPERSEDED_BY_EXIT` を置く（第8.3節。同じ PR で上位設計書 §4.7.14 と D02 §8.1 を改訂）、(2) 判断履歴の数値列を人が読める固定小数表記にし、同じ値が常に同じ文字列になる4つの規則を定めた（第9.1節）、(3) 候補の始値を実在する足ではなく**足のスケジュール**から決めるため、執行系列のポートに4つ目の操作を足した（第5.3節。同じ PR で D03 §6.3 を改訂）、(4) 公開が1件も無い判断時点では戦略ランタイムを呼ばない（第4.2節）、(5) 予約と建玉の表は run の終わりに最終状態を1行ずつ書く（第9.2節）、(6) 「診断として残す」の残す先は run manifest の警告群とする（第9.3節）、(7) 層規則に合わせた型の置き場所5件を第3節に確定した。v1.2（2026-09-21、PR #17）: D07 §15 の改訂依頼1〜3 に対する人間の決定（Q12〜Q14、いずれも選択肢1）を反映した。(1) 約定1件ごとの費用を区分別の金額列として読めるようにした（第9.2節の表9、Q12）。(2) run 中の台帳 snapshot の含み損益を、直前に完了した執行足の終値で評価すると定めた（第8.1節、Q13。受付時の参照価格 Q10 と同じ出どころ）。(3) 平坦化の規則の3つの穴（複合表の接頭辞・入れ子レコード・区分タグ付き union）を埋めた（第9.1節、Q14）。あわせて `Reason` の列名の例を一般規則に合わせて `*_code` / `*_detail` に直した（D07 §4.2 が既に使っている列名と一致させるため）。v1.1（2026-09-21、PR #17）: D07 §15 の改訂依頼4 を受け、`BacktestResult` から資産推移の2項目（`balance_series` / `equity_series`）を落とした（第9.4節）。要素の型が定義されておらず、同じ推移が台帳 snapshot の表（表14）と結果 DTO の2経路で読める状態だったためである。**第9.4節が既に定めた「集計前のレコードは `trace_tables` 経由で渡し、`BacktestResult` の中で集計しない」の適用であり、設計の選択は伴わない**（D07 は表14 を直接読むため、段階2の指標・集計・trace はどれも変わらない）。v1.0（2026-09-21）: 第16節の要決定 Q1〜Q11 を人間がすべて決定し（Q7 のみ選択肢2、他の10件は提示時の推奨案である選択肢1）、本文へ反映した。**未決の項目は残っていない**。決定に伴い、**同じ PR で正本を改訂した**: 処理段階の名前に数字を許す規則（D02 §3.3、v1.5）、公開バッチに run 末尾の合図を足す項目（D05 §6.1、v1.1）、保護水準の置き方が不正なときの受付前拒否の理由コード `PROTECTION_INVALID`（上位設計書 §4.7.14 と D02 §8.1、v1.5）、発注の根拠になった出力の識別子を戦略ランタイムの戻り値へ足す改訂（D05 §3・§6.2、v1.2。第6.1節の阻害要因の解消）。あわせて紙上トレース [T01](../traces/T01_paper_trace.md) を書き（置き場所は D01 §7.1 に従い `docs/traces/`）、そこで洗い出した未記述を本書へ反映した（第17節に一覧）。段階2（最小縦断＝戦略定義→注文→約定→単一評価）と紙上トレース T01 に必要な範囲だけを扱う。検証戦略 A（1時間足の高値突破→後続確認なしの成行→初期損切り＋固定リスクリワード比の利確、上位設計書 §7.1）が動くことを必要十分条件とする。ADR-0016 条件2 のうち「D06 の最小縦断範囲」を本書で充足する。全体計画 §6 D-2 が「骨子だけで実装へ進めない」と列挙した5点（注文状態、フェーズ順、原子性、run_end、trace 型）は、最小縦断の範囲でもすべて本書で確定させる。第16節に決定の一覧を置く。
...
| 6 | 記録 | trace の行の種類（段階2 の15件と、**v1.5 で足した段階3 の4件**）とフィールド、run manifest の項目、`BacktestResult` の項目、保存形式（第9節） | 指標の算出、終端理由別・診断理由別の集計、実験 manifest への固定（**D07**）、swap 未計上をユーザーへ伝える出力の形（**D07**、ADR-0029） |
...
| 出どころ | 受け取るもの |
|---|---|
| D04 §5・§11 | データ型識別子13件の登録、役割出力の型要求、`ManagementAction` の種別（段階2 の `SET_TAKE_PROFIT` / `CLOSE_POSITION` に、D04 §11.2（v1.9）が段階3 で足した `UPDATE_STOP` を加えた3種別。v1.8 で表記を追随） |
...
**必須表（[全体計画書](fx_research_platform_overall_plan.md) §8.5、R1）の置き場所**。本書は状態機械と実行時の値の伝播を定めるので、境界表（上の表）に加えて次の2種類の表を持つ。いずれも本文の規則を並べ替えたもので、規則を足さない。

| 表 | 置き場所 |
|---|---|
| 状態×出来事表: 注文（`PENDING` / `FILLED` / `CANCELED` / `EXPIRED`。受付前の発注試行は「生成前」の行） | 第5.1.1節 |
| 状態×出来事表: 予約（`HELD` / `TRANSFERRED` / `RELEASED`） | 第8.1.1節 |
| 状態×出来事表: 建玉（`OPEN` / `CLOSED`）と保護水準 | 第8.3.1節 |
| 状態×出来事表: run（実行前・実行中・`RunStatus` の3値） | 第10.6節（要決定のマスは v1.12 で無くなった。R1-D06-2 は根本対処 R4 で決定） |
| 値の伝播表 | 第9.5節（v1.10 の要決定のマス2件 R1-D06-1・R1-D06-3 は v1.11 で決定済み） |
...
| `SubstitutionOwner` | `trace.recorder` | レコード | `evaluation_id: EvaluationId`。表17（`INPUT_SUBSTITUTIONS`）の複合行の主（v1.8。第9.2節） | §9.2 |
| `ManagementApplication` | `trace.recorder` | レコード | `at: ProcessingPoint` / `applied: bool` / `reason: Reason \| None` / `protection_version: int \| None` / `rounded_take_profit: Price \| None` / `realized_reward_risk: Decimal \| None` / `rounded_stop_loss: Price \| None`（v1.8。損切り水準の更新を適用したときの丸めた後の水準。第8.3節） | §8.3・§9.2 |
...
6. **rank 10 の直前**: **5（第1回の `step`）が返した** `management_requests` のうち**保護水準の更新（`UpdateStop`）を建玉へ適用する**（第8.3節）【確定】（Q25 決定、2026-09-23、選択肢1。紙上トレース T02 §14 #5）。**第2回の `step` が返した更新はここでは扱わない**（9 で `POST_FILL_EVALUATION` に適用する）。適用の内容と `effective_from` は第8.3節のままで、**新しいフェーズは足さない**。処理点は `ADMISSION`（rank 10）であり、表12（`MANAGEMENT_APPLICATIONS`）の主キー `(position_id, at)` はそのまま使える。**同じ判断時点の決済要求との競合は、この適用より前に解決する**（決済が勝ち、更新は `SUPERSEDED_BY_EXIT` で適用しない。第8.3節）。**5 が返した全数量決済（`ClosePosition()`）は、競合を解決したあと 7（rank 10）の受付へ渡す**（第6.2節の表の「それを返した `step` の直後の受付フェーズ。第1回なら rank 10」）。第2回が返した決済だけが rank 13 へ回る（10）。段階2 では足の確定で起動する `exit` が無いため第1回から決済要求は出ないが、段階3 のトレーリングと同じ起動条件で決済を返す部品を書けるので、経路を空けておく。
7. rank 10: 要求組立から受付までを行い、`AttemptDecision` と `AdmissionNotice` を作る（第6節）。受け付けるのは **5 の `proposals`（新規エントリー）と、6 が渡した第1回の全数量決済**である（第6.2節の表）。リスク審査（第6.4節）は、6 で適用し終えた保護水準を読む。
...
10. rank 13: **9（第2回の `step`）が返した** `management_requests` のうち全数量決済を要求へ組み立て、第6節と同じ手順で受け付ける（第1回が返した決済は 6 で 7 へ渡してある）。要求が1件も無ければ何もしない。
11. run_end の判断時点だけ rank 14 を行う（第10節）。**第3回の `step`** を `is_run_end=True` の公開バッチで呼び、残存する取引機会の終端（`RUN_END`）を受け取って trace へ渡す（Q2 決定、D05 §6.1 v1.1）。**第1回・第2回と同じく、`evaluations` と段階3 の3つの列（`wait_events` / `validity_rechecks` / `confirmation_attempts`）も trace へ渡す**（v1.6。独立レビューの指摘）。この `step` は残った待機中の評価要求を見送り（`Skipped`）で決着させる（第10.1節）ので、**その要求が確認評価のものなら確認試行の結末も `WAITING` から `SKIPPED` へ置き換わる**。差分を受け取らないと、正常終了した判断履歴の表18 に**決着していない試行が残る**。停止判定はこの回には当たらない（`is_run_end=True` の `step` は起動判定も評価も行わないため `Failed` も `MISSING_FAILED` も生じない。D05 §6.1）。
...
2. その `step` が返した `proposals` と `management_requests` は**一切使わない**。同じ `step` の中で失敗より前に生まれたものも使わない（どこまでが有効な判断だったかが宣言から読めないため）。**第1回で失敗したときは、手順6 が行うこと（保護水準の更新の適用と、全数量決済を受付へ渡すこと）も行わない**（管理要求を一切使わないので、適用する対象も渡す対象も無い）。
...
|---|---|---|
| 第1回（rank 4〜9） | rank 0〜2 の内部約定・台帳更新・期限切れ | **手順6（保護水準の更新の適用と決済要求の引き渡し）と rank 10 以降をすべて実行しない**（更新の適用・受付・始値処理・第2回の `step`・rank 13） |
...
**`batch_id` は判断履歴に記録しない**【確定】（v1.11、2026-09-25 の人間の決定。R1-D06-1。いまの実装どおり）。役目は、戦略ランタイムが同じ値での2度目の呼び出しを実行時に拒むこと（D05 §6.1）だけであり、二重呼び出しはその検査で止まる。バッチそのものは処理点（判断時刻・フェーズ・通し番号）で既に特定できる。`batch_id` は注文イベントと同じ採番列（`EventId`）から取るので、**表8 の `event_id` には欠番が出る**。欠番は記録の欠落ではなく、公開バッチに使った番号である。**不採用**: 表20（公開記録）に列を足す案、公開バッチ専用の表を足す案（どちらも読む側がいない）。
...
- **戦略ランタイムが返した記録の処理点は、エンジンの時計で番号を振り直す**【確定】（v1.3。D02 §3.3）。ランタイムは自分の `step` の中で 0 から数えるため、そのまま残すと、エンジンが同じフェーズで刻んだ記録（受付の判断・保護水準の適用・台帳 snapshot・末尾の取消）と `(時刻, フェーズ, 通し番号)` が重なり、判断履歴を処理点の順に読み直したときにどちらが先だったのかが決まらない。フェーズはランタイムが選んだものをそのまま使い、**番号だけ**をエンジンの採番列へ載せ替える。対象は取引機会の遷移（`OpportunityTransition.at`）で、これが段階2で処理点を持つ唯一のランタイム側の記録である。**段階3 では待機の出来事（`WaitEvent.at`）と取引機会の有効性の再検査（`ValidityRecheck.at`）も対象に入る**（v1.5。D05 §6.8・§7.3 が `RuntimeStepResult` に2つの列を足したため）。待機の開始・入力の到着・再開・期限到達・追い越しと、再検査の結果は、いずれもランタイムが `step` の中で刻む処理点であり、取引機会の遷移と同じ理由で番号を振り直さないとエンジン側の記録と順序が決まらない。フェーズはランタイムが選んだものをそのまま使う規則も同じである。
  **振り直す順は、ランタイムが刻んだ `(フェーズ順位, 通し番号)` の順である**【確定】（v1.8、2026-09-25 の人間の承認。PR #35 の仮置き6）。エンジンは1回の `step` が返した遷移・待機の出来事・再検査の3列を1本に合わせ、ランタイムが刻んだ処理点の `(phase.rank, sequence)` で並べてから、その順にエンジンの採番列へ載せ替える。D05 §6.6 はこの3つの記録に `step` 内の1本の通し番号を共有させており、列ごとに順に載せ替えると、その前後関係（例: 同じ `step` の待機の出来事と遷移のどちらが先か）が判断履歴から失われるためである。
...
#### 5.1.1 状態×出来事表（必須表。R1）

上の遷移表（遷移1〜6）を正本とし、それを状態×出来事に並べ替えて、**遷移表に無い組み合わせ**を本文の根拠つきで埋めたものである（[全体計画書](fx_research_platform_overall_plan.md) §8.5 の表2）。本表は規則を足さない。本文と食い違えば本文の節を正本として本表を直す。受付前の発注試行（注文がまだ無い状態）は「生成前」の行で扱う。`PENDING` はエントリー注文と決済注文で届く出来事が違うため2行に分ける。3つの終端状態は、どの出来事についても同じ扱いなので1行にまとめる。

| 状態 ＼ 出来事 | 1. 受付の審査を通過（rank 10・13） | 2. 受付前拒否 | 3. エンジン生成の即時決済（保護水準の到達 rank 0、始値の gap・緊急決済 rank 11）で対象建玉が閉じる | 4. 固定した候補の始値が来る（rank 11） | 5. 期限到達 `expires_at <= T`（rank 2） | 6. 予定の始値を過ぎても実データに候補足が無い（判断時点の先頭の検査） | 7. その他の実行中のデータ不整合で run が失敗 | 8. run_end の末尾処理（rank 14 の手順5） | 9. 同じ `event_id` の再配送 |
|---|---|---|---|---|---|---|---|---|---|
| 生成前（発注試行） | → `PENDING`（遷移1。`AttemptAccepted`。§5.1・§6.5）。エンジン生成の決済は出来事3 の列 | 変化なし: 注文も予約も作らず、`OrderRequest` と `AttemptRejected` だけを残す（§4.4・§5.2） | → `PENDING` → `FILLED` を1つの確定単位で行う（遷移1・2。§4.4 の「エンジン生成の即時決済」・§6.2） | 到達しない: 候補の始値は受付時に注文へ固定されるもので、注文の無い試行には届かない（§7.1） | 到達しない: 期限は受付時刻から計算され、注文が無ければ存在しない（§5.3） | 到達しない: 検査の対象は `PENDING` の注文だけ（§10.4） | 到達しない: 取り消す対象は未約定注文だけ（§10.4） | 到達しない: 手順5 の対象は残った受付済み `PENDING` だけ。末尾の新しい要求は手順4 で受付前拒否（`RUN_END`）になる（§10.1） | 変化なし: `processed_event_ids` に含まれていれば確定単位を実行しない（§4.4） |
| `PENDING`（エントリー注文） | 到達しない: 受付は常に新しい注文を生成前から作り、同じ `attempt_id` で再試行しない（§5.2・§6.1） | 到達しない: 同左（拒否は発注試行の終端で、既存の注文に届かない。§5.2） | 到達しない: 1建玉の制限は未約定のエントリー注文による枠の占有を含み、エントリー注文と開いた建玉は同時に存在しない（§8.2） | → `FILLED`（遷移2。予約は `TRANSFERRED`、建玉を作る。§4.4・§7.1） | → `EXPIRED`（遷移3。理由 `EXPIRED`、予約を解放。§5.1・§5.3）。段階2 の実行ポリシーでは発生しない（§5.1） | → `CANCELED`（遷移5。理由 `DATA_ERROR`。期限切れより前に検査して run を失敗させる。§10.4） | → `CANCELED`（遷移5。理由 `DATA_ERROR`、予約を解放。§10.4） | → `CANCELED`（遷移4。理由 `RUN_END`、予約を解放。§10.1 の手順5）。段階2 の既定値（`entry_delay_bars=0`）ではエントリー注文は同じ判断時点で約定するため起きない（§5.3） | 変化なし（§4.4） |
| `PENDING`（戦略の決済注文。rank 10・13 で受付） | 到達しない: 生成前の行と同じ理由（§5.2） | 到達しない: 同左（§5.2） | → `CANCELED`（遷移6。理由 `POSITION_CLOSED`。対象建玉が先に閉じた決済注文。§5.1） | → `FILLED`（遷移2。建玉を終える。§4.4・§7.1） | → `EXPIRED`（遷移3。§5.1・§5.3）。段階2 では発生しない（§5.1） | → `CANCELED`（遷移5。`DATA_ERROR`。§10.4） | → `CANCELED`（遷移5。`DATA_ERROR`。§10.4） | → `CANCELED`（遷移4。`RUN_END`。rank 13 で受け付けた決済注文の候補が run_end と一致した場合。§5.1・§5.3・§10.1） | 変化なし（§4.4） |
| `FILLED` / `CANCELED` / `EXPIRED`（終端） | 到達しない: 生成前の行と同じ理由（§5.2） | 到達しない: 同左（§5.2） | 変化なし: 閉じる建玉の決済は新しい注文として作る（§4.4）。終端した注文には作用しない | 拒否: 終端状態からの遷移は無く、遷移要求は `KernelValueError`（§5.1・§4.4） | 変化なし: 対象は `PENDING` の注文だけ（§4.1 の rank 2・§4.2 の手順3） | 変化なし: 対象は `PENDING` の注文だけ（§10.4） | 変化なし: 対象は未約定注文だけ（§10.4） | 変化なし: 対象は残った受付済み `PENDING` だけ（§10.1 の手順5） | 変化なし（§4.4）。異なる `event_id` で同じ終端効果を求められた場合は拒否（`KernelValueError`。§4.4） |
...
| 鮮度 | 直前に完了した執行足に限られるため、鮮度は**構造的に執行足1本分以内**に収まる。別の鮮度上限を置かない【提案】。段階2 の仮置きとして「段階3 で再判断する」としていた（第17節の #13）が、**段階3 では上限を変えない**（検証戦略 B で必要にならなかった。2026-09-25 の人間の決定、v1.9）。次の再判断は、段階4 以降で必要が出たときに行う【確定】 |
...
#### 8.1.1 予約の状態×出来事表（必須表。R1）

予約（`RiskReservation`）の状態 `HELD` / `TRANSFERRED` / `RELEASED`（第3節の `ReservationStatus`）について、第4.4節の確定単位の表と本節の規則を状態×出来事に並べたもの（[全体計画書](fx_research_platform_overall_plan.md) §8.5 の表2）。本表は規則を足さない。

| 状態 ＼ 出来事 | 1. エントリーの受付 | 2. 決済の受付 | 3. エントリー約定 | 4. エントリー注文の終端（期限切れ・取消。`RUN_END` / `DATA_ERROR`） | 5. 決済約定（建玉の終了） | 6. run_end の最終 snapshot（§10.1 の手順7） | 7. 同じ `event_id` の再配送 |
|---|---|---|---|---|---|---|---|
| 生成前 | → `HELD`（`RiskReservation` を作る。§4.4 の「エントリーの受付」） | 変化なし: 決済は予約を作らない（§4.4 の「決済の受付」） | 到達しない: 約定するのは受付で予約を確保した注文だけ（§4.4・§5.1 の遷移1） | 到達しない: 同左 | 変化なし: 予約を作らない（§4.4） | 変化なし（§10.1） | 変化なし（§4.4） |
| `HELD` | 到達しない: 受付は注文ごとに新しい予約を作る（§4.4） | 到達しない: 1建玉の制限により、`HELD` の予約（未約定のエントリー注文）と決済の対象になる開いた建玉は同時に存在しない（§8.2） | → `TRANSFERRED`（`PositionRiskAllocation` を作る。§4.4 の「エントリー約定」・§8.1） | → `RELEASED`（§4.4 の「終端」・§10.1 の手順5・§10.4） | 到達しない: 出来事2 と同じ理由（§8.2） | 到達しない: 手順5 で残った `PENDING` を取り消し、予約は解放済み（§10.1） | 変化なし（§4.4） |
| `TRANSFERRED` | 到達しない（§4.4） | 変化なし: 既存建玉の割当は決済の約定まで保持する（§4.4 の「決済の受付」） | 到達しない: 注文は1回だけ全数量で約定する（§5.1・§8.2。部分約定は対象外、§12） | 到達しない: 約定した注文は `FILLED` で終端済み（§5.1） | 変化なし: 解放するのは建玉割当（`PositionRiskAllocation`）で、予約の状態は変えない（§4.4 の「決済約定」） | 変化なし: 建玉割当も解放しない（§10.1 の手順7） | 変化なし（§4.4） |
| `RELEASED` | 到達しない（§4.4） | 変化なし（§4.4） | 到達しない: 対応する注文は `EXPIRED` / `CANCELED` で終端済み（§5.1） | 到達しない: 同左 | 到達しない: 建玉は作られていない（§4.4） | 変化なし（§10.4 の「取消で予約が解放された状態を最後の snapshot に残す」） | 変化なし（§4.4） |
...
| 検査 | 価格刻みへ丸め（第6.5節）、買いなら `stop_loss < 参照価格の bid`、売りなら `参照価格の ask < stop_loss`。**丸めた結果が現在の水準より不利になる更新は適用しない**（部品が有利な向きだけを返すのに、丸めで不利側へ動くことがあるため）。適用しなかった要求は表12 に理由 `PROTECTION_INVALID` で残し、run は止めない（上の保護水準の検査と同じ扱い）。**丸めた結果が現在の水準と同じ更新は「不利ではない」として適用する**（`version` は1つ進む）【確定】（v1.8、2026-09-25 の人間の承認。PR #35 の仮置き2）。部品は現在の水準より有利な値だけを返す（D05 §4.10）が、丸めで現在の水準と同じ値に戻ることがあり、そのときも適用しない理由にしない |
...
| `effective_from` | **次の執行足**。初期の保護水準の例外（上表の「初期の保護水準」）は当たらない。終値で計算した更新をその足の過去の高値・安値へ適用しない【合意済み】上位 §4.7.7。**次の執行足は、判断時刻以降に始まる最初の予定上の執行足**とする（受付の候補の始値と同じ求め方。第5.3節）【確定】（v1.8、2026-09-25 の人間の承認。PR #35 の仮置き3）。第2回の `step`（約定後の評価起動点）が返した更新も同じ足を使う。その足の高値・安値はまだ観測していないので、終値で計算した更新を過去へ当てることにはならない。1本後ろにすると、その足のあいだ建玉がどの損切りでも判定されなくなる。段階3 の宣言では第2回の `step` から更新は出ない。**次の執行足が予定に無いとき**（予定表の探索範囲を越えた場合だけ到達しうる）は、参照価格を作れないときと同じく**適用せず、理由 `DATA_ERROR` で表12 に残す**【確定】（v1.8、PR #35 の仮置き4） |
...
| 適用結果の記録 | 表12 の適用結果（`ManagementApplication`）に**丸めた後の水準 `rounded_stop_loss`** を残す（列名は `application_rounded_stop_loss`）【確定】（v1.8、2026-09-25 の人間の承認。PR #35 の仮置き1）。初期の利確の `rounded_take_profit` と同じ扱いである。要求の値（列 `action_stop_loss`）と実際に建玉へ入った水準は丸めで違いうるので、両方を判断履歴から読めるようにする。適用しなかった要求では `None` |
| 根拠記録 | 適用した更新ごとに、利確と同じ種別 `PROTECTION_UPDATE` の根拠記録（表15）を1件残す。載せるのは、要求の根拠になった出力（`output_ids`）に加えて、**検査に使った参照価格の足**（`market_refs`）である【確定】（v1.8、2026-09-25 の人間の承認。PR #35 の仮置き5）。初期の利確の検査は約定価格だけを読むが、損切り水準の更新の検査は執行足の bid / ask を読むためである |
...
#### 8.3.1 建玉の状態×出来事表（必須表。R1）

建玉（`Position`）の状態 `OPEN` / `CLOSED`（第3節の `PositionStatus`）と、それに付く保護水準（`ProtectionState`）について、第4.4節・第7節・本節の規則を状態×出来事に並べたもの（[全体計画書](fx_research_platform_overall_plan.md) §8.5 の表2）。保護水準の更新は建玉の状態を変えず、`ProtectionState.version` を進めるだけなので「変化なし」の欄に書く。本表は規則を足さない。

| 状態 ＼ 出来事 | 1. エントリー約定（rank 11） | 2. 保護水準の到達（rank 0。片側・足内競合） | 3. 既存建玉の始値 gap（rank 11 の手順1） | 4. 約定直後の gap・約定ずれ超過（rank 11 の手順4・5） | 5. 戦略の決済注文の約定（rank 11） | 6. 利確の設定 `SetTakeProfit` | 7. 損切りの更新 `UpdateStop` | 8. 全数量決済の要求 `ClosePosition()` | 9. 同じ判断時点の更新と決済要求の競合 | 10. 同じ判断時点の2件目の保護水準の更新 | 11. run_end の判断時点 | 12. 実行中の失敗（`DATA_ERROR`。停止判定を含む） |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 生成前 | → `OPEN`（初期の損切りを有効化、`effective_from` は約定した執行足。§4.4・§7.3・§7.5 の手順3） | 到達しない: 対象の建玉が無い（§7.3） | 到達しない: 同左（§7.5） | 到達しない: 手順4・5 は手順3 で初期化した新規建玉に対して行う（§7.5） | 到達しない: 決済注文は開いた建玉に対してだけ受け付ける（§8.3 の `ClosePosition()` の検査） | 到達しない: 管理要求は建玉を対象とする評価から生まれ、開いた建玉が無ければ `position_context` は `None` でランタイムの `on_missing` に従う（§8.4） | 到達しない: 同左（§8.4） | 到達しない: 同左（§8.4） | 到達しない: 同左 | 到達しない: `exit` は1件で足の確定の系列も1つに限る（§8.3、D04 §12 #14） | 変化なし（§10.1） | 変化なし（§10.4） |
| `OPEN` | 到達しない: 1建玉の制限により、開いた建玉があるあいだはエントリー注文が受け付けられない（§8.2） | → `CLOSED`（`CloseCause` は `STOP_LOSS` / `TAKE_PROFIT`、`IntrabarResolution` を残す。両側なら第7.4節の手順で決め、降りきれなければ損切り。§4.4・§7.3・§7.4） | → `CLOSED`（通常の損切り決済。§7.5 の手順1） | → `CLOSED`（gap なら通常の損切り、gap が無く約定ずれ超過なら `EMERGENCY`。両方なら損切りだけを実行し、約定ずれ超過は run manifest の警告に残す。`POSITION_OPENED` は配送しない。§7.5・§9.3） | → `CLOSED`（`STRATEGY_EXIT`。§4.4 の「決済約定」・§7.1） | 変化なし（`OPEN` のまま）: `take_profit` を設定し `version` を1増やす。`effective_from` は初期なら約定した足、それ以外は次の執行足。検査に失敗すれば適用せず `PROTECTION_INVALID` を表12 に残し、run は止めない（§8.3） | 変化なし（`OPEN` のまま）: `stop_loss` を置き換え `version` を1増やす。`effective_from` は次の執行足。不利になる丸めは `PROTECTION_INVALID`、参照価格か次の執行足が無ければ `DATA_ERROR` で適用しない（§8.3） | 変化なし（`OPEN` のまま）: `CloseRequest(STRATEGY_EXIT)` を直後の受付フェーズへ回す。建玉は決済約定まで開いている（§6.2・§8.3） | 変化なし: 決済を優先し、更新は `SUPERSEDED_BY_EXIT` で適用しない（§8.3） | 到達しない: 生成前の行と同じ理由（§8.3） | 変化なし（`OPEN` のまま）: 更新は `RUN_END` で適用せず、決済要求は `RUN_END` で受付前拒否する。未決済のまま含み損益を評価し最終 snapshot に残す。建玉割当も解放しない（§8.3・§10.1 の手順4・7） | 変化なし（`OPEN` のまま）: 架空の価格で閉じない。第2回の `step` で失敗した建玉は初期の利確を持たないまま残る（§4.2・§10.4） |
| `CLOSED` | 到達しない: 約定は新しい建玉を作る（§4.4） | 到達しない: 閉鎖状態と `PositionId` で二重決済を防ぐ（§7.5） | 到達しない: 同左（§7.5） | 到達しない: 同左（§7.5） | 到達しない: 対象建玉が先に閉じた決済注文は `POSITION_CLOSED` で取り消される（§5.1 の遷移6） | 拒否（run は止めない）: `POSITION_CLOSED` で拒否する（§8.3）。同じ始値で閉じた建玉には `POSITION_OPENED` を配送しないので、正常な経路では起きない（§7.5） | 拒否（run は止めない）: `POSITION_CLOSED`（§8.3） | 拒否（run は止めない）: `POSITION_CLOSED`（§8.3） | 到達しない: 開いた建玉への更新と決済の競合を扱う規則であり、閉じた建玉には出来事6〜8 の拒否が当たる（§8.3） | 到達しない: 生成前の行と同じ理由（§8.3） | 変化なし（§10.1） | 変化なし（§10.4） |
...
- **`BacktestResult`（第9.4節）は `runs/<run_id>/result.json`（JSON）に保存する**【確定】（v1.11、2026-09-25 の人間の決定。R1-D06-3。いまの実装どおり）。`ResultWriter` が run manifest（`manifest.json`、第9.3節）と同じディレクトリに書き、別プロセスで評価するとき D07 はここを読む。読むときは同じディレクトリの `manifest.json` の時間足の定義も使う。JSON にするのは ADR-0027 の「manifest は JSON」に合わせるためである。**不採用**: run manifest に統合する案（実行条件の記録と実行結果が1つのファイルに混ざり、manifest の役割がぼやける）。
...
- **保存先 `runs/<run_id>/` は「存在すれば、何も書かずに失敗する」**【確定】（v1.12。成果物の書き込みを統一する根本対処 R4、R1-D06-2）。空のディレクトリも、前の実行が途中で落ちて残した書きかけのディレクトリも「ある」と数える（中身を見て続きを書くと新旧の表が混ざる）。検査は2回行う。(1) **実行前**: 構成ルート（`app.composition.execute_run`）が run の識別子を計算した直後、run を始める前に保存先が無いことを確かめる（`fs_store.require_run_directory_absent`）。(2) **書き出しの直前**: 判断履歴の書き出し口（`FileSystemTraceSink`）が最初の表を書く前に、保存先を**新しく作ること自体で**確かめる（`runs/` から保存先の親までにシンボリックリンクや別の種類のものがあれば、リンク先に書かずに失敗する）（`fs_store.reserve_run_directory` → `create_artifact_directory`。確かめた後に別の実行が作る隙間を残さない）。どちらの失敗も**型付きの構造エラー `ArtifactAlreadyExists`**【確定】（2026-09-25 の人間の決定。検査の置き場・2回の検査も同じ）（`evaluation.domain.errors`。基底は `EvaluationError` → `KernelValueError`）で、CLI は1行の失敗として表示し終了コード 1 で終える。案内には置換の指示（下）を書く。置換の指示（`--replace`）があるときは検査せず、第9.3節の置換の手順に従う。評価の保存先（D07 §8.2）も同じ書き込み前検査を通る。
...
`costs` 列は**そのまま残す**。原通貨額と換算根拠（第7.6節）は区分別の列に入らず、正規化エンコード列が引き続きその正本である。該当する区分の `CostEntry` が無い約定では、その区分の2列を**どちらも `None`** にする（金額 0 の `CostEntry` が記録された場合と区別するため）。読む側は D07 v2.0（段階4。他文書が「D07 v0.2」と呼んでいた版）で、取引単位の費用と入場費用を含む取引損益に使う（D07 §7.3）。段階2 の D07 は run 単位の集計（第10.3節の `cost_breakdown`）だけを使うため、この列が増えても段階2 の指標は変わらない。**不採用**: 正規化エンコード列の復号規則を D02 §9.3 に足す案（符号化の正本に復号の責務が増え、区分別に読むたびに文字列の解析が要る）、費用を子表に分ける案（表が15を超える。第9.1節と同じ理由）。
...
- **待機の出来事（表16）の種類の語彙は D05 §3 の `WaitEventKind` が正本である**（v1.7）。待機に入った・入力が届いた・再開した・期限に到達した・追い越された・run 末尾で閉じた、に加えて、**受け取った取引機会が待機のあいだに終わったので閉じた（`OPPORTUNITY_ENDED`）**の7種類である（2026-09-24 の人間の決定。D05 v2.5 §6.8 の手順2）。閉じた要求の評価記録（表2）は見送り（`Skipped`）であり、「なぜ閉じたか」は表16 の種類で読む。
...
- **遡った入力（表17）の主キーに接続元の位置を含める**【提案】。1つの入力名に複数の接続元を書けるため（D04 §4.1 の `arity`）、同じ評価で同じ入力名の2つの系列を遡ると行が2件出る。入力名までを主キーにすると片方が上書きされ、実際に代用した観測が判断履歴から消える。`SubstitutedInput` が持つ `source_index`（D05 §3）を鍵に含めて一意にする。**行の形は、`SubstitutionOwner(evaluation_id)` を主、遡った入力（`SubstitutedInput`）を接頭辞なしの従とする複合行（`CompositeRow`、第9.1節）とする**【確定】（v1.8、2026-09-25 の人間の承認。PR #35 の仮置き7）。`SubstitutedInput` 自体は `evaluation_id` を持たない（評価記録の `substitutions` の要素である）ので、表を独立させると親の評価が行から読めなくなる。エンジンが評価記録の識別子を主として組み合わせ、列は `evaluation_id` と遡った入力の項目になる。
...
- **損切り水準の更新は表12 をそのまま使う**【提案】。`UpdateStop` は `ManagementAction` の区分であり、表12 は既に区分タグ付き union を全変種のフィールドの和集合として開く規則を持つ（第9.1節の規則2）。新しい表を足すと、同じ建玉への保護水準の要求が2つの表に分かれる。列は、区分タグ付き union の規則で要求の側に `action_stop_loss` が、適用結果の側に丸めた後の水準 `application_rounded_stop_loss` が増える（後者は v1.8、PR #35 の仮置き1。第8.3節）。
...
**v1.11 で足す表と列**【確定】（2026-09-25 の人間の決定。D03 v1.11 の R1-D03-3、D05 v2.10 の R1-D05-3・5・6）。**実装は後続の実装 PR（判断履歴に表と列を足す PR）で行う**。本版の時点の実装は、表20 を書かず、下の列も書き出さない。

| # | `TraceTable` | 正本の型 | 主キー | 辿れる先 |
|---|---|---|---|---|
| 20 | `PUBLICATIONS`（公開記録） | `PublicationRecord`（D03 §3.6。`bar_key` / `bar_end` / `scheduled_at` / `available_at`） | `bar_key` | `bar_key` → 足の自然キーを持つ列（表18 の `bar_key` など） |

- **表20（公開記録）**: 遅延シナリオを当てた結果、各足が実際にいつ見えるようになったかを足1本につき1行残す（上位 §4.3.13、D03 §3.6）。予定の公開時刻と実際の公開時刻の差が遅延の効果であり、判断履歴だけで確かめられる。確認の開始足（D05 §7.7）の検算にも使う。`BarKey` は第9.1節の規則1 で開く。追加後、判断履歴の表は**20件**になる。
- **表3 の生成の行に2列を足す**: 確認の開始足 `confirmation_start_bar` と確認期限 `confirmation_deadline_at`（D05 v2.10 §3 の `OpportunityTransition` の改訂）。生成の行（`from_state=None`）だけが値を持ち、他の行は `None`。期限は区分タグ付き union（`WaitDeadline`）なので第9.1節の規則2 で開く。これで期限切れ（`EXPIRED`）と開始足の除外を判断履歴だけで検算できる。
- **表16 の「待機に入った」行に1列を足す**: 固定した足 `pinned_bars`（D05 v2.10 §3 の `WaitEvent` の改訂。入力名と足の組）。要素数が行ごとに変わるので、第9.1節の可変長の入れ子の規則で正規化エンコード文字列の `list` 列にする。`Mapping` を要素に分ける単位と並び順は次のとおり決める（第9.1節の規則はレコードの `tuple` だけを対象にしているため）。**要素は入力名と足の組1つ**で、2要素の配列 `[入力名, BarKey]` を D02 §9.3 の正規化エンコードで文字列にする。**並び順は入力名の Unicode コードポイント昇順**（D02 §9.3 がオブジェクトのキーを整列する順と同じ）。実装の辞書の挿入順に依存させないためである。`WAIT_STARTED` 以外の行では空。**待機期限は足さない**（表2 の `outcome_deadline_at_*` に既にある）。
- **表19 には生成時点の行も並ぶ**: 取引機会を作る時点で値を固定する束縛が読めず、見送りの方針で機会を作った場合、`MISSING_SKIPPED` の行が生成の処理点で1件残る（D05 v2.10 §7.3）。行は読めなかった束縛1つにつき1件で、束縛ごとに新しい処理点を振る（D05 v2.10 §7.3）ので、主キー `(opportunity_id, at)` は変わらない。
...
**置換（明示の指示があるときだけ）**【確定】（v1.12、R1-D06-2。ADR-0006 の具体化。段階2 の実装どおり）: 置換の指示は **`odyssey-fx run --replace`**（実行ユースケースへは書き出し口の `replace=True` として渡る）で、既定は失敗である（第9.1節）。置換するときは書き出しの直前に、(1) 旧 run manifest（`manifest.json`）を**次の世代の名前 `manifest.replaced.<NNN>.json`**（3桁以上の世代番号。001 から欠番なく増え、次の番号は「既に残っている世代の数 + 1」）として同じディレクトリへ写し、(2) 旧ディレクトリ直下のファイル（残した世代と、v1.12 より前の置換が残した番号の無い `manifest.replaced.json` を除く）を消し、(3) **評価の成果物（`eval/`）を畳む**。判断履歴だけを書き直すと、同じ実行の識別子の下に前の判断履歴から作った指標が残るためである。**旧 manifest はすべての世代を残し、既存の世代ファイルは決して上書きしない**【確定】（v1.12、2026-09-25 の人間の決定）。**置換の手順の先頭で、現在の `manifest.json` の有無に依らず必ず1回**、残っている世代が 001 から欠番なく並んでいる（`manifest.replaced` で始まる名前が、番号の無い旧形式を除いてちょうど `001`〜`N` の正規の名前である。`01`・`abc` など表記の崩れた名前も候補に数える）ことを確かめ（manifest の無い書きかけの run も同じ。2026-09-25 の人間の決定）、`runs/<run_id>` 自体がシンボリックリンク、またはディレクトリでないものなら、置換はそれに触れずに失敗し、旧 manifest（`manifest.json`）と残した世代がリンクでない通常のファイルでない、または評価の成果物（`eval`）がリンクでないディレクトリでないときも、何も作らず何も消さずに失敗し、欠番・表記の崩れがあれば**何も作らず何も消さずに `ArtifactAlreadyExists` で失敗する**（壊れた履歴に次の世代を足さない）。(1) の作成と存在の確認は1つの操作（排他的な作成）で行い、次の世代の名前のファイルが既にあれば（確かめた後に別の置換が作った場合）、**何も消さず何も上書きせずに `ArtifactAlreadyExists` で失敗する**（R4 の「存在すれば失敗」）。**同じ run を2つのプロセスが同時に置換すること（`--replace` の並行実行）は設計の対象外である**【確定】（2026-09-25 の人間の決定。PR #46）。置換の開始から書き終わりまでの排他は行わず、並行した置換の結果（成果物の混在や、同じ旧 manifest を記録した2つの世代）は保証しない。排他的な作成が守るのは、既存の世代ファイルを上書きしないことだけである。(1) を (2)・(3) より先に行うので、旧 manifest を残せなければ旧成果物は消えない。置換の指示を持つのは run の成果物だけで、評価の成果物（D07 §8.2）と snapshot（D03 §3.7.2）は持たない（R4）。
...
| 項目 | 内容 |
|---|---|
| `run_id` / `manifest_ref` | run manifest への参照。`BacktestResult` 自体は `runs/<run_id>/result.json` に保存する（第9.1節、v1.11） |
| `status: RunStatus` | 正常完走か失敗か |
| `trace_tables: Mapping[TraceTable, str]` | **第9.2節の表すべての Parquet パス**（段階2 は15表、段階3 は19表。v1.5。v1.11 の表20 を実装した後は20表）。D07 は必要な表をここから開く。段階3 で足した4表も同じ `TraceTable` の列挙値で公開し、読む側が段階で場合分けしなくて済むようにする |
...
複数の部品・層・成果物をまたいで運ばれる値について、生成元・渡り方・記録先・主キーを1行ずつ並べたもの（[全体計画書](fx_research_platform_overall_plan.md) §8.5 の表3）。本文の節（主に第4.4節・第6.1節・第9.2節）を正本とし、本表は規則を足さない。表番号は第9.2節の表1〜19、列名は第9.1節の平坦化の規則による。本書の外（D05）で生成される値は、本書が受け取って運ぶ範囲だけを書く。

| 値 | 生成元 | 渡り方 | 記録先 | 主キー |
|---|---|---|---|---|
| `run_id`（`RunId`） | `RunId = digest(ConfigDigest, CodeDigest, LockDigest, EnvDigest)`。実行の出どころの3ダイジェストは `app` が算出して実行ユースケースへ渡す（§9.3、ADR-0006） | `RunConfig` から解決し `OrderRequest.run_id` へ（§6.1）。全行が持つ（§9.1）。`BacktestResult.run_id`（§9.4） | 全表の `run_id` 列、run manifest の識別の群、保存先 `runs/<run_id>/`（§9.1・§9.3） | run manifest は run に1件。各表では主キーに含めない（全行が同じ値） |
| `config_digest` | run manifest の入力とポリシーの群から計算（§9.3） | `RunId` の入力（§9.3） | run manifest の識別の群（§9.3） | run manifest は run に1件 |
| 実行の出どころのダイジェスト `code_digest`（`CodeDigest`）・`lock_digest`（`LockDigest`）・`env_digest`（`EnvDigest`） | `app` が算出して実行ユースケースへ必須の入力として渡す（§9.3【確定】、D02 §9.4、ADR-0006） | `RunId` の入力（§9.3）。`code_digest` は D07 が評価 manifest の `run_code_digest` へ写す（D07 §8.3） | run manifest の識別の群（§9.3） | run manifest は run に1件 |
| git commit と dirty 状態 | `app` が実行の出どころとして渡す（§9.3【確定】） | 識別子には含めない（§9.3、ADR-0006） | run manifest の識別の群（§9.3） | run manifest は run に1件 |
| run の状態 `RunStatus` と失敗時の `Reason` | エンジン。能力検査の不合格で `FAILED_CAPABILITY`、停止判定・データ末尾・最終評価価格の欠如で `FAILED_DATA_ERROR`、それ以外は `COMPLETED`（§4.2・§10.3〜§10.5。第10.6節の表） | `BacktestResult.status`（§9.4）→ D07 が評価の状態 `REJECTED` を決める（D07 §10.1） | run manifest の状態の群（§9.3） | run manifest は run に1件 |
| 能力検査の結果 `DataCapabilityReport` | 実行前のデータ能力検査（§10.5） | `BacktestResult.capability_report`（§9.4） | run manifest の能力検査の群に**全体のまま**（§9.3・§10.5 の手順6） | run manifest は run に1件 |
| 末尾3集計 `FinalSummaries` | run_end の手順の後に計算（§10.3）。`status` が `COMPLETED` のときだけ作る | `BacktestResult.summaries` → D07 の整合検査 C5・指標（D07 §10.2） | `ResultWriter` が run manifest と一緒に保存する `BacktestResult` の `summaries`（D01 §4、§3 の `ResultWriter.write`、§9.4）。別プロセスの評価は保存したものを読む（D07 §10.2 の C5）。保存先は `runs/<run_id>/result.json`（JSON。第9.1節、v1.11【確定】、R1-D06-3） | `run_id`（run に1件） |
| 件数 `trade_count` / `opportunity_count`・`unresolved_intrabar_count`・`swap_modeled` | エンジンが run の終わりに数える（§9.4）。`swap_modeled` は常に `False`（ADR-0029） | `BacktestResult` → D07 の整合検査 C3・C6 と評価 manifest（D07 §8.3・§10.2） | `UNRESOLVED_SL_PRIORITY` の件数と割合は run manifest の足内競合の群（§9.3）。他は `ResultWriter` が保存する `BacktestResult` の項目（D01 §4、§9.4。ファイルは `runs/<run_id>/result.json`、§9.1） | `run_id`（run manifest・`BacktestResult` とも run に1件） |
| 判断履歴の表のパス `trace_tables` | `TraceSink` の実装が書き出す（§9.1） | `BacktestResult.trace_tables` → D07 の `ResultRepository.read_table`（§9.4、D07 §4.3） | `runs/<run_id>/` の Parquet（§9.1） | `TraceTable` の列挙値 |
| 診断 `warnings` | エンジンが理由コードに当てはまらない診断を足す（§9.3・§7.5 の手順6） | — | run manifest の診断の群（§9.3） | run manifest は run に1件 |
| 処理点 `ProcessingPoint`（時刻・フェーズ・通し番号） | エンジンの時計が刻む。戦略ランタイムが刻んだ取引機会の遷移・待機の出来事・有効性の再検査は、`(フェーズ順位, 通し番号)` の順にエンジンの採番列へ**番号だけ**載せ替える（§4.4） | `OrderRequest.created_at`（受け付けるフェーズで刻む。§6.1）、`OrderEvent.at`、`LedgerSnapshot.at`、`ManagementApplication.at`、`EvidenceRecord.at`、`Position.opened_at` → `PositionContext.opened_at`（§8.4）、`OpportunityTransition.at` / `WaitEvent.at` / `ValidityRecheck.at`（D05 の型。§4.4） | `*_time` / `*_phase` / `*_sequence` の3列（§9.1） | 表3 `(opportunity_id, at)`、表12 `(position_id, at)`、表14 `at`、表16 `(request_id, at)`、表19 `(opportunity_id, at)`（§9.2） |
| `batch_id`（`EventId`） | `IdAllocator.next(EventId)`。第2回・第3回の `step` は別の値（§4.2・§4.3・§10.2） | `PublicationBatch.batch_id` → `StrategyRuntime.step`（D05 §6.1 が同じ値での再呼び出しを拒否する） | 記録しない（§4.3、v1.11【確定】、R1-D06-1）。同じ採番列の表8 `event_id` に欠番が出る | — |
| `opportunity_id` | D05 のランタイムが採番（本書は受け取る。§1.2 の受け取るもの） | `EntryProposal.opportunity_id` → `EntryRequest.opportunity_id`（§6.1）→ `AdmissionKey.origin_seq`（§6.3）、`AdmissionNotice.opportunity_id`（§6.6）、`RuntimeEventNotice(POSITION_OPENED).opportunity_id`（表4 の要求から `attempt_id` で辿る。§4.2） | 表4 の `payload_opportunity_id`（§9.1 の規則2）。表3・18・19 は D05 の型の列（§9.2） | 表4 は `attempt_id` |
| `attempt_id`（`AttemptId`） | 要求を全順序で並べた**後に** `IdAllocator.next(AttemptId)`（§6.1・§6.3）。受け付けるフェーズ（rank 10・13、エンジン生成の決済は rank 0・11。§6.2） | `OrderRequest` → `AttemptDecision` → `RiskAssessment.attempt_id`（§6.4 の手順8）、`AcceptedOrder.attempt_id`、`AdmissionNotice.attempt_id`（§6.6）、`EvidenceRecord.attempt_id`、`OpportunityTransition.attempt_id`（D05 §7.2） | 表4・表5 の主キー、表6・表7・表15 の列、表3 の列（§9.2） | 表4・表5 `attempt_id` |
| `assessment_id`（`EvidenceId`） | 受付の手順3 まで到達した試行でだけ `IdAllocator.next(EvidenceId)`（§4.4・§6.4 の手順8） | `RiskAssessmentRef` として `AttemptDecision.assessment_ref` へ。予約は同じ値の識別子を持つ（§3） | 表6 の主キー、表5 の `assessment_ref`（§9.2） | 表6 `assessment_id` |
| `evidence_id`（`EvidenceId`） | `backtest.trace` が採番（§9.2、D02 §7.1）。`assessment_id` と同じ採番列 | `OrderRequest.evidence_ref`（§6.1）、`ConversionRate.evidence`（§8.5.1 の規則7） | 表15 の主キー、表4 の `evidence_ref`（§9.2） | 表15 `evidence_id` |
| `order_id`（`OrderId`） | 受付の確定単位で作る（エントリー・決済の受付、エンジン生成の即時決済。§4.4）。採番順は処理点の順（§4.4、D02 §7.3） | `AttemptAccepted.order_id` → `AcceptedOrder` → `OrderEvent.order_id` / `OrderState`、`FillRecord`（§3・§4.4） | 表7 の主キー、表5・表8・表9・表10 の列（§9.2） | 表7 `order_id` |
| 注文の `event_id`（`EventId`） | 注文を変える確定単位ごとに作る（§4.4） | `OrderEvent.event_id`、`OrderState.last_event_id`、`ReservationState.last_event_id`、`PositionRiskAllocation.created_event_id` / `released_event_id`、台帳の `processed_event_ids`（冪等性。§4.4） | 表8 の主キー、表10・表11 の列（§9.2）。`processed_event_ids` は台帳の中だけで記録しない（§4.4。判断履歴は表8 に全件ある）。公開バッチの `batch_id` と同じ採番列なので欠番が出る（§4.3） | 表8 `event_id` |
| `reservation_id`（`ReservationId`） | エントリーの受付の確定単位（§4.4） | `AcceptedEntryTerms.reservation_id`（§3）→ `ReservationState`、`PositionRiskAllocation.source_reservation_id`（§8.1） | 表10 の主キー、表7 の `terms_reservation_id`、表11 の `position_risk_allocation_source_reservation_id`（§9.1 の規則1・3） | 表10 `reservation_id`（run の終わりに最終状態を1行。§9.2） |
| `fill_id`（`FillId`） | 約定の確定単位（§4.4） | `FillRecord` → `OrderEvent.fill_id`、`Position.entry_fill_id` / `close_fill_id`、`IntrabarResolution.fill_id`（§7.4）、`ImmediateAfterFill.trigger_fill_id`（§7.5） | 表9 の主キー、表8・表11 の列、表13 の主キー（§9.2） | 表9 `fill_id`、表13 `fill_id` |
| `position_id`（`PositionId`） | エントリー約定の確定単位で建玉を作るとき（§4.4） | `Position` → `CloseRequest.position_id` / `AcceptedCloseTerms`（§6.1）、`AdmissionKey.origin_seq`（決済。§6.3）、`ProtectionHit.position_id`（§3）、`RuntimeEventNotice(POSITION_OPENED)`（§4.2）、`PositionContext.position_id`（§8.4）、`EvidenceRecord.position_id`、`IntrabarResolution.position_id` | 表11 の主キー、表4 の `payload_position_id`、表9・表13・表15 の列、表12 の主キーの一部（§9.2） | 表11 `position_id`（最終状態を1行。§9.2）、表12 `(position_id, at)` |
| `allocation_id`（`AllocationId`） | エントリー約定の確定単位で `PositionRiskAllocation` を作るとき（§4.4・§8.1） | `PositionRiskAllocation` → 台帳の `allocations`（§3） | 表11 の従の型の列 `position_risk_allocation_allocation_id`、表10 の辿れる先（§9.1 の規則3・§9.2） | 表11 `position_id` |
| 保護水準の版 `ProtectionState.version` | 建玉を作るときに初期の損切りで始まり、更新を適用するたびに1増える（§8.3） | `ProtectionHit.protection_version`（§3）、`ManagementApplication.protection_version`（§3・§8.3） | 表11 の建玉の列、表12 の `application_protection_version`、表7 の受付条件の列（§9.1） | 表11 `position_id`、表12 `(position_id, at)` |
| 出力 ID（`OutputId`） | D05 のランタイムが採番（本書は受け取る） | `EntryProposal.intent_output_id` / `protection_output_id` → `EntryRequest.intent_output_id` / `InitialProtectionPlan.source_output_id`、`ManagementRequest.source_output_id` → `CloseRequest.source_output_id`（§6.1）、`EvidenceRecord.output_ids` | 表1 の主キー（`OutputSink` で受けた時点で書く。§4.2）、表4・表12・表15 の列（§9.2） | 表1 `output_id` |
| `evaluation_id` / `request_id` | D05 のランタイムが採番（本書は受け取る） | `RuntimeStepResult` の各列、`EvidenceRecord.evaluation_ids`、`SubstitutionOwner.evaluation_id`（§9.2） | 表2 の主キー、表16 の主キーの一部（`request_id`）、表17 の主キーの一部（`evaluation_id`）、表18 の列（§9.2） | 表2 `evaluation_id`、表16 `(request_id, at)`、表17 `(evaluation_id, input_name, source_index)` |
| 候補の執行足 `BarKey` | 受付時に予定から選んで固定する（`next_scheduled_open_after`。§5.3・§7.1） | `ExecutionCommitment.eligibility`（`ScheduledOpen`）、`ReferenceQuote.source_bar`（§6.4）、`ProtectionState.effective_from`（§7.3・§8.3） | 表7・表6・表11 の列（§9.1 の規則1） | 各表の主キー（`order_id` / `assessment_id` / `position_id`） |
| 入力と設定の参照（`snapshot_ref` / `strategy_ref` / `compiled_ref` / `run_interval` / `execution_series` / `seed` / `account`、各ポリシーの参照。遅延シナリオの `delay_scenario_ref`・`symbol_spec_ref`・`calendar_ref`・`timeframe_def_refs` を含む） | `RunConfig`（§3）。実行前に `compiled.compiled_ref == config.compiled_ref` などを照合（§10.5） | `ExitPlanRef.compiled_ref`（§6.1）、`MarketObservationRef.snapshot_ref`（§3）、`EvidenceRecord.policy_refs` | run manifest の入力とポリシーの群（§9.3）、表7 の `terms_exit_plan_ref_*`、表15 の列 | run manifest は run に1件、表15 `evidence_id` |
| 実現した公開時刻 `PublicationRecord`（`bar_key` / `bar_end` / `scheduled_at` / `available_at`） | 公開フィードが遅延シナリオを当てて決める（D03 §3.6・§7.1） | `TraceSink` で表20 へ書く。公開記録をエンジンへ渡す経路は後続の実装 PR が決める | 表20 `PUBLICATIONS`（§9.2、v1.11【確定】。D03 の R1-D03-3） | 表20 `bar_key` |
| 採番器の状態 `IdAllocator.snapshot()` | run の終わりの採番器（D02 §7.3） | — | run manifest の実行の構造の群（§9.3） | run manifest は run に1件 |

**v1.10 で要決定としたマスと、その決定**（v1.11、2026-09-25 の人間の決定）:

- **R1-D06-1（`batch_id` の記録先）: 記録しない**【確定】（v1.11、2026-09-25 の人間の決定。推奨案）。理由と欠番の扱いは §4.3。実装変更なし。
- **R1-D06-3（`BacktestResult` を保存するファイル）: `runs/<run_id>/result.json`（JSON）**【確定】（v1.11、2026-09-25 の人間の決定。推奨案）。§9.1。実装変更なし。
...
4. **決着した要求が確認評価のものなら、その確認試行の結末も同じ `step` で置き換えること**（v1.6。独立レビューの指摘）。待機に入った試行は `WAITING` で積まれており（D05 §7.7）、決着に合わせて `SKIPPED` へ置き換わる。エンジンはその差分を `RuntimeStepResult.confirmation_attempts` で受け取り、主キー `(opportunity_id, bar_key)` で表18 の行を置き換える（第4.2節の手順11・第9.2節）。置き換えないと、**正常に終わった run の判断履歴に、決着していない試行が残る**。
...
| 項目 | 値 |
|---|---|
| `batch_id` | `IdAllocator.next(EventId)` で採番した新しい値（第1回・第2回とは別）。判断履歴には記録しない（第4.3節、v1.11） |
...
| 戻り値 | `transitions`（残存機会の終端）に加えて、**残っていた待機要求の決着**として `evaluations` と `wait_events` が非空になりうる（D05 §6.1 v2.0）。その要求が確認評価のものなら `confirmation_attempts` も非空になりうる（v1.6。第4.2節の手順11・第10.1節の要求4）。**`outputs` / `proposals` / `management_requests` は空のままである**（末尾では新しい判断が生まれないため） |
| `is_run_end` | `True` |

- エンジンは戻り値を表へ書き、**新しい判断が生まれていないこと**を検査する【確定】（検査する列を v1.6 で正した）。書くのは `transitions`（残存機会の `RUN_END` 終端）→ 表3、`evaluations` → 表2、`wait_events` → 表16、`confirmation_attempts` → 表18（置き換え）である。**検査するのは `outputs` / `proposals` / `management_requests` が空であること**で、空でなければ `KernelValueError`（末尾で新しい判断が生まれない規則、上位 §4.7.13 D の「終了だから未公開情報を解禁することはしない」の実施箇所）。**`evaluations` を検査の対象から外す**のは、待機要求の決着が評価記録として残ると v1.5 で決めたためである（第10.1節の要求1。v1.5 の改訂が本項の列挙に届いていなかった）。
...
### 10.6 run の状態×出来事表（必須表。R1）

1回の run の状態を、出来事ごとに並べたもの（[全体計画書](fx_research_platform_overall_plan.md) §8.5 の表2）。終わった状態は `RunStatus`（第3節）の3値である。規則の正本は第4.2節（停止判定）・第10.1節・第10.3〜10.5節であり、本表は規則を足さない。

| 出来事 ＼ 状態 | 実行前（能力検査） | 実行中 | `COMPLETED` | `FAILED_DATA_ERROR` / `FAILED_CAPABILITY` |
|---|---|---|---|---|
| **通常の開始**: 実行前のデータ能力検査がすべて合格した | **遷移**: 実行中へ。検査結果を全体のまま run manifest と `BacktestResult` に残す（第10.5節の手順6） | 到達しない（能力検査は開始前に1回だけ行う。第10.5節） | 到達しない（同左） | 到達しない（同左） |
| 能力検査の不合格（戦略の参照の不一致、執行系列の欠落、銘柄別の値の不足、執行系列が run 区間を覆わない、解像度階層の不適合、既知の欠損） | **遷移**: `FAILED_CAPABILITY`。開始前に失敗し、検査結果を全体のまま保存する（第10.5節の手順1〜6） | 到達しない（同左） | 到達しない（同左） | 到達しない（同左） |
| **通常の判断時点**: 1つの判断時点の処理が停止判定に当たらずに終わった | 到達しない（判断時点は開始後に来る。第4.2節） | **変化なし**: 次の判断時点へ進む（第4.2節） | 到達しない（終わった run に判断時点は来ない） | 到達しない（同左） |
| 停止判定に当たった（評価の失敗、または有効性の再検査の `MISSING_FAILED`） | 到達しない（`step` は開始後に呼ぶ） | **遷移**: `FAILED_DATA_ERROR`。その `step` の記録を保存し、要求を使わず、未約定注文を `CANCELED`（`DATA_ERROR`）にして予約を解放する（第4.2節の停止判定・第10.4節） | 到達しない | 到達しない |
| 必要データが run_end より前に尽きた、または予定した候補の足が無いまま過ぎた | 到達しない（開始前の既知の欠損は能力検査の不合格。第10.5節の手順5） | **遷移**: `FAILED_DATA_ERROR`（第10.4節）。保有建玉を架空価格で閉じない | 到達しない | 到達しない |
| **通常の終了**: run_end に到達し、最終評価価格がある | 到達しない | **遷移**: 第10.1節の手順1〜7 を行い、末尾3集計を作って `COMPLETED`（第10.1節・第10.3節）。第3回の `step` は1回だけ呼ぶ（第10.2節） | 到達しない | 到達しない |
| run_end に到達したが最終評価価格が無い | 到達しない | **遷移**: `FAILED_DATA_ERROR`。`FinalSummaries` を作らず `summaries=None`（第10.3節） | 到達しない | 到達しない |
| 保存先 `runs/<run_id>/` が既にある（空・書きかけを含む） | **拒否**: 置換の指示が無ければ、run を始めずに何も書かずに `ArtifactAlreadyExists` で失敗する。置換の指示（`--replace`）があれば実行中へ進み、書き出しの直前に第9.3節の置換の手順を行う【確定】（v1.12、R1-D06-2。第9.1節・第9.3節） | **拒否**: 実行前の検査の後、書き出しまでの間に別の実行が保存先を作った場合に限り起こる。書き出しの直前の確保（作ること自体で確かめる）で、1表も書かずに `ArtifactAlreadyExists` で失敗し、この run の結果は保存しない。置換の指示があれば置換する（第9.1節） | 到達しない（終わった run は保存先を検査しない。同じ識別子での再実行は新しい run の「実行前」として扱う） | 到達しない（同左） |

失敗した run では第3回の `step` を呼ばない（第10.2節）。取引機会は最後に観測された状態のまま残り、架空の終端理由を付けない。

**要決定だったマスの決定**（v1.12、根本対処 R4 の PR。下の起草時の問いの後ろに決定を書く）:

- **R1-D06-2**: 保存先 `runs/<run_id>/` が既にあるときの扱い。ADR-0006 は「既定は失敗し、置換は明示の指示でのみ行い、旧成果物の manifest を残す」と定め、衝突検査と置換の指示を「D06/D07 の CLI 設計に含める」と委ねたが、本書にはどの時点で検査するか・置換の指示の名前・旧 manifest の残し方が書かれていない。起草時には承認済み・未実施だった根本対処 R4（v1.12 で実施済み。成果物の書き込みを「存在すれば失敗」に統一する。改訂先は第9.1節・第9.3節）の対象である。→ **決定（v1.12、R4）【確定】: 存在すれば、何も書かずに失敗する**（暫定 snapshot の R1-D03-1、評価の成果物の R1-D07-3 と同じ規則。失敗は `ArtifactAlreadyExists`）。検査の時点は実行前と書き出しの直前の2回（第9.1節）、置換の指示の名前は `--replace`、旧 manifest は置換の世代ごとに `manifest.replaced.<NNN>.json` として残し、既存の世代は上書きしない（第9.3節。2026-09-25 の人間の決定）。置換の指示の名前は段階2 の実装が ADR-0006 に従って既に持っていたもので、本版はそれを正本として書いた。
...
| 13 | 参照価格に鮮度の上限を置くかどうか（仮置き） | 段階2は置かない。直前に完了した執行足に限るため鮮度が構造的に1本分以内に収まる。段階3で再判断する。→ **段階3 では上限を変えない**（検証戦略 B で必要にならなかった。2026-09-25 の人間の決定、v1.9）。次の再判断は、段階4 以降で必要が出たときに行う | §6.4 の手順3、T01 §5.3 |
