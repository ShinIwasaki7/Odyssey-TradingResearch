...
**置換（明示の指示があるときだけ）**【確定】（v1.12、R1-D06-2。ADR-0006 の具体化。段階2 の実装どおり）: 置換の指示は **`odyssey-fx run --replace`**（実行ユースケースへは書き出し口の `replace=True` として渡る）で、既定は失敗である（第9.1節）。置換するときは書き出しの直前に、(1) 旧 run manifest（`manifest.json`）を**次の世代の名前 `manifest.replaced.<NNN>.json`**（3桁以上の世代番号。001 から欠番なく増え、次の番号は「既に残っている世代の数 + 1」）として同じディレクトリへ写し、(2) 旧ディレクトリ直下のファイル（残した世代と、v1.12 より前の置換が残した番号の無い `manifest.replaced.json` を除く）を消し、(3) **評価の成果物（`eval/`）を畳む**。判断履歴だけを書き直すと、同じ実行の識別子の下に前の判断履歴から作った指標が残るためである。**旧 manifest はすべての世代を残し、既存の世代ファイルは決して上書きしない**【確定】（v1.12、2026-09-25 の人間の決定）。次の世代の名前のファイルが既にある（世代の並びが崩れている）ときは、(1) の作成と存在の確認を1つの操作（排他的な作成）で行い、**何も消さず何も上書きせずに `ArtifactAlreadyExists` で失敗する**（R4 の「存在すれば失敗」）。(1) を (2)・(3) より先に行うので、旧 manifest を残せなければ旧成果物は消えない。置換の指示を持つのは run の成果物だけで、評価の成果物（D07 §8.2）と snapshot（D03 §3.7.2）は持たない（R4）。
