...
**置換（明示の指示があるときだけ）**【確定】（v1.12、R1-D06-2。ADR-0006 の具体化。段階2 の実装どおり）: 置換の指示は **`odyssey-fx run --replace`**（実行ユースケースへは書き出し口の `replace=True` として渡る）で、既定は失敗である（第9.1節）。置換するときは書き出しの直前に、(1) 旧 run manifest（`manifest.json`）を **`manifest.replaced.json`** として同じディレクトリへ写し、(2) 旧ディレクトリ直下のファイル（`manifest.replaced.json` を除く）を消し、(3) **評価の成果物（`eval/`）を畳む**。判断履歴だけを書き直すと、同じ実行の識別子の下に前の判断履歴から作った指標が残るためである。旧 manifest は直前の1世代だけを残す（置換を重ねると `manifest.replaced.json` は直前の manifest に置き換わる）。置換の指示を持つのは run の成果物だけで、評価の成果物（D07 §8.2）と snapshot（D03 §3.7.2）は持たない（R4）。
