...
6. **rank 10 の直前**: 5 の `management_requests` のうち**保護水準の更新（`UpdateStop`）を建玉へ適用する**（第8.3節）【確定】（Q25 決定、2026-09-23、選択肢1。紙上トレース T02 §14 #5）。適用の内容と `effective_from` は第8.3節のままで、**新しいフェーズは足さない**。処理点は `ADMISSION`（rank 10）であり、表12（`MANAGEMENT_APPLICATIONS`）の主キー `(position_id, at)` はそのまま使える。**同じ判断時点の決済要求との競合は、この適用より前に解決する**（決済が勝ち、更新は `SUPERSEDED_BY_EXIT` で適用しない。第8.3節）。全数量決済は従来どおり 9（rank 13）で受け付ける。
...
| 適用の処理点 | **受付（rank 10）の直前**、フェーズは `ADMISSION`【確定】（Q25 決定、2026-09-23、選択肢1。第4.2節の手順6）。足の確定で起動するトレーリングは第1回の `step` が返すため、`POST_FILL_EVALUATION` では適用される機会が無い（その判断時点に受付通知も約定通知も無いときは第2回の `step` を呼ばないため） |
