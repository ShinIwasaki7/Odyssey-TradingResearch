...
| 値の伝播表 | 第9.5節（要決定のマス2件: R1-D06-1・R1-D06-3） |
...
| 末尾3集計 `FinalSummaries` | run_end の手順の後に計算（§10.3）。`status` が `COMPLETED` のときだけ作る | `BacktestResult.summaries` → D07 の整合検査 C5・指標（D07 §10.2） | `ResultWriter` が run manifest と一緒に保存する `BacktestResult` の `summaries`（D01 §4、§3 の `ResultWriter.write`、§9.4）。別プロセスの評価は保存したものを読む（D07 §10.2 の C5）。**保存するファイルの名前と形式は本書に無い（要決定（R1-D06-3））** | `run_id`（run に1件） |
| 件数 `trade_count` / `opportunity_count`・`unresolved_intrabar_count`・`swap_modeled` | エンジンが run の終わりに数える（§9.4）。`swap_modeled` は常に `False`（ADR-0029） | `BacktestResult` → D07 の整合検査 C3・C6 と評価 manifest（D07 §8.3・§10.2） | `UNRESOLVED_SL_PRIORITY` の件数と割合は run manifest の足内競合の群（§9.3）。他は `ResultWriter` が保存する `BacktestResult` の項目（D01 §4、§9.4。ファイルは R1-D06-3） | `run_id`（run manifest・`BacktestResult` とも run に1件） |
...
| `batch_id`（`EventId`） | `IdAllocator.next(EventId)`。第2回・第3回の `step` は別の値（§4.2・§4.3・§10.2） | `PublicationBatch.batch_id` → `StrategyRuntime.step`（D05 §6.1 が同じ値での再呼び出しを拒否する） | **要決定（R1-D06-1）** | **要決定（R1-D06-1）** |
...
| 注文の `event_id`（`EventId`） | 注文を変える確定単位ごとに作る（§4.4） | `OrderEvent.event_id`、`OrderState.last_event_id`、`ReservationState.last_event_id`、`PositionRiskAllocation.created_event_id` / `released_event_id`、台帳の `processed_event_ids`（冪等性。§4.4） | 表8 の主キー、表10・表11 の列（§9.2）。`processed_event_ids` は台帳の中だけで記録しない（§4.4。判断履歴は表8 に全件ある） | 表8 `event_id` |
...
- **R1-D06-1**: 公開バッチの識別子（`batch_id`）の記録先が本書に無い。第4.3節・第10.2節は採番だけを定め、第9.2節の19表のどれにも列が無く、記録しない理由も書かれていない。記録するならどの表のどの列か、記録しないならその理由を決める必要がある（同じ値での再呼び出しの拒否は D05 §6.1 の実行時の検査で、判断履歴からは確かめられない）。
- **R1-D06-3**: `BacktestResult` を保存する先（`runs/<run_id>/` の中のファイル名と形式）が本書に無い。`ResultWriter` が run manifest と一緒に保存する（D01 §4、第3節）ことと、`runs/<run_id>/` が保存先であること（第9.1節）は決まっている。実装は `result.json` を書いているので、本書に追記するかを決める必要がある（別プロセスで評価するとき D07 が読む先になる）。
