...
### 1.2 本書が決めること・他文書に委ねること（**レビュー対象範囲の正本**。v1.10、R1）

節番号は D04〜D08 の §1.2 と揃えた（第1.1節は置かない。語彙の出どころは冒頭の「上位文書」と下の「受け取るもの」の表が示す）。**本書のレビューの対象範囲は下表の第3列である**。第4列に属する指摘は「対象外（担当へ）」として記録し、本書では直さない。D04 §1.2 と同じ例外を置く: 本書の文が第4列の挙動を暗示していて誤解を招く場合は、その暗示を消す修正だけ行う。

| # | 領域 | 本書が決めること（対象内） | 他文書が決めること（対象外・担当） |
|---|---|---|---|
| 1 | 系列・時間足・足 | 価格基準と系列の識別、時間足定義と DST 切替日の起点規則、足の不変条件と自然キー（第3.1〜3.3節） | 時間足の参照型 `TimeframeRef` と時刻・価格の値型（**D02**） |
| 2 | カレンダー | 週の開閉、休場規則の3つの形、営業例外、4操作（第3.4節・第3.4.1節） | 週末持ち越し禁止の判定そのもの（**D06 §5.3**） |
| 3 | 公開予定と遅延 | 公開予定と遅延シナリオの型、非負の制約、`available_at` の求め方（第3.5節・第3.6節） | 遅延シナリオ4ケースの具体値（**D05 §9.3**）、遅延シナリオを実験設定に書く書式（**段階4、D07 の後続版**） |
| 4 | snapshot と受入れ | manifest の項目、`SnapshotId` の対象と正規順序、二段階フロー、完全性検査の種別と分類の形式、確定時の検査、除外規則（第2節・第3.7節・第3.9節・第4節） | 実データの分類そのもの（人間の決定。分類ファイル） |
| 5 | 期間のアクセス分類 | 3区分と期間境界、足の所属、partition 単位の物理分離、`access_log.jsonl` の追記と `HoldoutState` の導出、許可 partition 集合の受け取り（第3.8節） | 区分と状態の規則そのもの（**ADR-0014** が正本）、許可の発行と消費の手順を実装する `holdout_gate`（**D07・段階4**）、隔離期間の再分類（**別 ADR**） |
| 6 | 上位足の生成 | 集約の規則と不完全足の扱い（第5節） | なし |
| 7 | as-of ビューと執行系列ビュー | 構築時の検査、読み取り操作と欠損の返し方、執行系列の4操作（第6節） | ポートの定義（**利用側の `strategy`・`backtest`。D01 §4**）、`max_age` の判定（**D05 §6.3**） |
| 8 | 公開フィード | 4イベント、実行区間の終端の扱い、同時刻の順序、`OnBarClose` の結び付け（第7節） | フェーズの正式な列挙と順位（**D06 §4.1**）、待機・再開の意味論（**D05 §6.8**） |
| 9 | adapters・設定・CLI | 2つの adapters、設定ファイルの一覧と形式、3コマンド（第8〜10節） | 表形式ライブラリ（**ADR-0025**）、CLI ライブラリ（**ADR-0028**） |

**受け取るもの**（本書が再定義しないもの）:

| 出どころ | 受け取るもの |
|---|---|
| 上位設計書 §3・§4.3.9〜§4.3.10・§4.3.13・§4.7.13 C | データ前提、読み方と鮮度、公開予定と遅延、完全性検査とカレンダーの要求 |
| D01 §1・§3.2・§4・§10.2 | パッケージの責務、依存の向き、ポートの所在、データ配置 |
| D02 | 時刻・区間・価格・時間足参照の値型、正規化エンコードとダイジェスト |
| ADR-0013・ADR-0014・ADR-0015 | データ配置と git 管理、期間のアクセス分類と `HoldoutState`、初版の対象 |

**必須表（全体計画書 §8.5、R1）**: 状態×出来事表は、snapshot の段階（第3.7.2節）と封印期間の状態 `HoldoutState`（第3.8.1節）の2つ。値の伝播表は第7.4節。
...
- **保存先は判断履歴の表20「公開記録」である**【確定】（v1.11、2026-09-25 の人間の決定。R1-D03-3）。1 行が足 1 本（`PublicationRecord`）で、主キーは足の自然キー `bar_key`、列は `bar_key`・足の終了時刻 `bar_end`・予定の公開時刻 `scheduled_at`・実際の公開時刻 `available_at` である。表の番号・列名の平坦化・書き出しの正本は D06 v1.11 §9.1・§9.2。これにより、遅延シナリオの効果（予定と実際の差）を判断履歴から直接確かめられ、確認の開始足（D05 §7.7）を後から検算する材料にもなる。**実装は後続の実装 PR（判断履歴に表と列を足す PR）で行う**。それまでの実装は `PublicationLog` を組み立てる関数を持つが、run の書き出しからは呼んでおらず保存していない。
...
#### 3.7.2 snapshot の段階の状態×出来事表（必須表。v1.10、R1）

第3.7.1節の二段階フローと第10節の3コマンドを、段階×出来事に並べたもの。正本は両節であり、食い違えば両節に合わせて本表を直す。段階は「存在しない」「暫定」（`_pending/<provisional_id>/`）「確定・未承認」（`<snapshot_id>/`、`approval` なし）「承認済み」の4つ。

| 段階＼出来事 | 受入れ（`data accept`、第4節 1〜8） | 分類と確定が成功（`data classify`） | 分類と確定が失敗（未分類・競合・対応なし・内容不一致・再実行で新しい警告・除外で原系列が空・再実行で初めて現れた休場帯の足の除外） | 承認（`data approve`） | 読み取り（as-of ビュー・公開フィード・`SnapshotCatalog`） |
|---|---|---|---|---|---|
| 存在しない | → 暫定。`provisional_id` を計算して `_pending/` に書く（第3.7.1節の1、第10節）。構造検査に ERROR があれば受入れ失敗で何も作らない（第4節 4） | 拒否。暫定 snapshot の内容照合（第4節「再実行の入力」）が成り立たず、何も書かずに失敗する | 到達しない（分類の対象が無い） | 拒否。承認の入力は確定済み snapshot に限る（第10節） | 拒否。承認済みであることの構築時検査を満たさない（第6.1節） |
| 暫定 | 拒否。同じ `provisional_id` の暫定ディレクトリが既にあれば、何も書かずに失敗する。作り直すときは人間が先にそのディレクトリを消す【確定】（v1.11、R1-D03-1。第10節） | → 確定・未承認。最終 `snapshot_id` を計算しディレクトリを `<snapshot_id>/` へ移す（第3.7.1節の2、第4節 9） | 変化なし。暫定 snapshot は失敗時に変更されない（第4節「再実行の入力」）。人間は分類を直して同じ暫定 snapshot に対して再実行する | 拒否。暫定 snapshot は承認できない（第3.7.1節の3、第10節） | 拒否（`SnapshotNotApproved`。暫定は常に読めない。第3.7.1節の1・3） |
| 確定・未承認 | 変化なし。受入れは `_pending/` にだけ書き、確定ディレクトリに触れない（第3.7.1節の1） | 拒否。既に確定済みのディレクトリがあれば失敗する（第10節）。確定済み snapshot は上書きしない | 変化なし（失敗は確定ディレクトリに書かない。第10節） | → 承認済み。`approval` と `declaration_record` を記入する。`snapshot_id` は変わらない（第3.7節・第10節） | 拒否（`SnapshotNotApproved`。第3.7.1節の3、第6.1節） |
| 承認済み | 変化なし（同上） | 拒否（同上。第10節） | 変化なし（同上） | 拒否。既に承認されていれば何も書かずに失敗し、最初の承認者を示す。承認の記録は 1 件で固定し、上書きも追記もしない【確定】（v1.11、R1-D03-2。第10節） | 変化なし。読める。ただし読める partition は構築時の許可集合に限る（第3.8節・第6.1節）。読み取り関門は2つの検査報告と分類の対応を検証する（第4節「再実行の入力」） |

**v1.10 で要決定としたマスの決定**（v1.11、2026-09-25 の人間の決定。2 件とも推奨案）:

- **R1-D03-1（暫定×受入れ）: 存在すれば失敗する**【確定】。成果物の書き込みを「存在すれば失敗」に統一する根本対処（[AGENTS.md の設計](agents_md_for_codex_review.md) §5 の R4。v1.11 の時点では承認済み・未実施、v1.13 で実施済み）と同じ規則にし、暫定 snapshot だけの例外を作らない。人間の分類作業の途中で暫定 snapshot が黙って書き換わることもなくなる。v1.11 の時点では「実装は R4 の PR の書き込み前検査に含める」とした。v1.12 までの実装は存在を確かめずに上書きしていた（暫定 ID は内容から決まるので中身は同じで、変わるのは manifest の作成時刻 `created_at` だけだった）。**v1.13 で実装した**: 書き出しの最初に、書き出し先を**新しく作ること自体で**確かめる（`SnapshotStore.create_directory`。既にあれば作成が失敗する）。空のディレクトリも、書きかけのディレクトリも、書き出し先に置かれたシンボリックリンク（先が無いもの・循環するものを含む。辿らずに判定する）も「ある」と数える。snapshot の根から書き出し先の親まで（`_pending` など）がリンクや別の種類のものなら、リンク先に書かずに失敗する。失敗は**型付きの構造エラー `SnapshotAlreadyExists`**（`marketdata.domain.errors`。基底は `MarketDataError`）で、CLI は1行の失敗として表示し終了コード 1 で終える。同じ検査が書き出し先の名前と書く snapshot の識別子の一致も確かめる（食い違えば `MarketDataValueError`）。検査の置き場・失敗の型・名前と識別子の一致の検査は、2026-09-25 に人間が承認した（PR #46 の仮置き）。確定ディレクトリ（`<snapshot_id>/`）への `classify` の書き出しも同じ検査を通る（確定・未承認／承認済み×分類と確定が成功の「拒否」。暫定ディレクトリは畳まずに残す）。**承認（`approve`）の manifest の記入と閲覧記録の追記は、既存の snapshot への記録であって新しい成果物の書き出しではないので、この検査の対象外**である（承認の二重記入は R1-D03-2 が拒否する）。
- **R1-D03-2（承認済み×承認）: 拒否する**【確定】。いまの実装（既に承認されていれば承認者名つきで失敗する）を正本にした。`approval` は `snapshot_id` の対象外なので、上書きを許すと最初の承認者と日時が manifest から消える。**実装変更は要らない**。
...
- `SEALED` の読み取りは、許可発行 → 消費記録の追記 → origin への push 成功 → データ公開の順で fail-closed に行い、push 成功前にデータを返さない。push の non-fast-forward 拒否を compare-and-set として使い、複数クローン間の重複消費を防ぐ。origin に到達できなければ `SEALED` は読めない（ADR-0014 2026-09-20 追記。gate の実装は **D09（段階5）**。v1.12 で D07 から移した（D07 §24）。本パッケージは `access_log.jsonl` の追記と状態導出、許可 partition 集合の受け取りを担う）。
...
- **段階2 の実行と評価が読めるのは研究履歴（`RESEARCH_HISTORY`）だけである**（v1.5、2026-09-22 の人間の決定）。合成が as-of ビューへ渡す許可 partition 集合に、封印期間（`LEGACY_HOLDOUT` の `SEALED`）と未分類の隔離期間を1件も入れない。封印期間を読むための許可判定（`holdout_gate`）は**段階5（D09）で足す**（v1.12。当初は段階4 の予定だった。D07 §24）。それまで段階2〜4 の実行は封印されたデータへ触れる経路を1本も持たない。

#### 3.8.1 封印期間の状態 `HoldoutState` の状態×出来事表（必須表。v1.10、R1）

`LEGACY_HOLDOUT` の partition 1 つの状態。規則の正本は ADR-0014（「状態」「消費遷移の直列化」）であり、本表はそれと本節を並べ替えたもの。`holdout_gate` の実装は D07・段階4 であり、段階2・3 の実行はこの状態機械に触れない（本節の末尾）。
...
逆遷移（`CONSUMED` → `SEALED`）は存在しない（本節、ADR-0014）。
...
- `LEGACY_HOLDOUT` の partition は `evaluation.application.holdout_gate` を通過した場合だけ `allowed_partitions` に入る（gate の設計は D09・段階5。v1.12）。`QUARANTINED_UNASSIGNED` の partition は、別 ADR による再分類が行われるまで**いかなる経路でも** `allowed_partitions` に入らない（gate も許可を発行しない）。
...
### 7.4 値の伝播表（必須表。v1.10、R1）

本書が生成し、他の部品・成果物へ運ばれる値。正本は各行の根拠の節であり、本表は網羅の確認のためにある。

| 値 | 生成元 | 渡り方 | 記録先 | 主キー |
|---|---|---|---|---|
| 暫定 ID `provisional_id` | 受入れの手順8（第3.7.1節の1・第4節）。分類が空のまま `SnapshotId` と同じ対象から計算する | `data classify --pending` の引数（第10節） | ディレクトリ名 `data/snapshots/_pending/<provisional_id>/` と、その中の暫定 manifest | `provisional_id` |
| snapshot ID `snapshot_id` | 確定段階（第3.7.1節の2・第4節 9）。第3.7.1節の対象の `canonical.digest` | `data approve --snapshot`（第10節）、as-of ビュー・執行系列ビューの構築（第6節）、バックテストの実行設定と判断履歴の市場観測の参照（`snapshot_ref`。D06 §3） | `manifest.json` の `snapshot_id` とディレクトリ名 `data/snapshots/<snapshot_id>/`。run 側の記録先は D06 §9.3（run manifest） | `snapshot_id` |
| partition ID `PartitionId` | 受入れの手順7（第3.8節・第4節） | as-of ビューの許可集合 `allowed_partitions`（第6.1節）。封印期間は `holdout_gate` の許可を経て入る（D07） | manifest の `partitions`（`PartitionRecord`） | `partition_id`（整列の鍵は `(series_id, access_class, interval.start)`。第3.7.1節） |
| 足の自然キー `BarKey(series, bar_start)` | 足の構築（第3.3節）。上位足は集約で生成（第5.1節） | 公開フィードの4イベント（第7.1節）、as-of ビューと執行系列ビューの読み取り（第6節） | partition の Parquet 実体（系列と `bar_start` の列） | `(series, bar_start)`（連番 ID は持たない。第3.3節） |
| 公開時刻 `available_at` | 公開予定と遅延シナリオの適用（第3.5節・第3.6節）。上位足は構成足の最大値（第5.1節） | `Bar.available_at`、`Publication` イベント（第7.1節） | 実現した列を `PublicationLog` として run に保存する（第3.6節）。保存先は判断履歴の表20「公開記録」（列は `bar_key`・`bar_end`・`scheduled_at`・`available_at`。D06 §9.2）【確定】（v1.11、R1-D03-3。実装は後続の実装 PR） | 表20 `bar_key`（1本の足に1つ） |
...
- **R1-D03-3（実現した公開時刻の列の保存先）: 判断履歴に表20「公開記録」を新設する**【確定】（v1.11、2026-09-25 の人間の決定。推奨案）。上位設計書 §4.3.13 と第3.6節の「run に保存する」をそのまま満たす。保存しない案（遅延シナリオから再計算する）は、再計算に同じコード版が要り、上位設計書の改訂も要るので採らなかった。run manifest に埋め込む案は、足の本数だけ manifest が膨らむので採らなかった。**実装は後続の実装 PR で行う**（第3.6節）。
...
| コマンド | 入力 | 出力・効果 |
|---|---|---|
| `odyssey-fx data accept --datasource <yaml> --calendar <yaml> --timeframes <yaml> --symbols <dir> --out data/snapshots/` | 原ファイルと設定 | 第4節 1〜8 を実行し、`data/snapshots/_pending/<provisional_id>/` に暫定 manifest・検査結果・partition を書く。`provisional_id` を表示。**同じ `provisional_id` の暫定ディレクトリが既にあれば、何も書かずに失敗する**（v1.11【確定】、第3.7.2節。v1.13 で実装。失敗は `SnapshotAlreadyExists`） |
| `odyssey-fx data classify --pending <provisional_id> --decisions <yaml> --timeframes <yaml> --out data/snapshots/` | 分類対象の警告の分類（第4節 9 の形式）と、必要ならカレンダーの新版 | 暫定 snapshot の実体を内容照合してから `closure_decisions` を記入し、カレンダー変更またはセッション外データ異常の分類があれば暫定 snapshot の原系列の足から 5〜7 を再実行（原ファイルは読み直さない）。再実行が新たな分類対象の警告を生んだ場合は失敗して表示し、人間が分類を追記して再度実行する。最終 `snapshot_id` を計算して `data/snapshots/<snapshot_id>/` へ確定。分類対象の警告が未分類、競合する分類、対応する警告のない分類、内容不一致、既に確定済みのディレクトリがある場合は失敗（最後のものは受入れと同じ書き込み前検査。空・書きかけも含め、何も書かずに `SnapshotAlreadyExists` で失敗し、暫定ディレクトリは残す。v1.13） |
| `odyssey-fx data approve --snapshot <snapshot_id> --by <name> --comment <text> --out data/snapshots/` | 確定済み snapshot | `approval` と `declaration_record` を記入。暫定 snapshot は承認できない。**既に承認済みの snapshot は、何も書かずに失敗する**（v1.11【確定】、第3.7.2節）。`--out`（snapshot の基点）は必須である（v1.11 で実装に合わせて書き足した。PR #39） |
