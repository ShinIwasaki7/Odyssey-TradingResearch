...
- **R1-D03-1（暫定×受入れ）: 存在すれば失敗する**【確定】。成果物の書き込みを「存在すれば失敗」に統一する根本対処（[AGENTS.md の設計](agents_md_for_codex_review.md) §5 の R4、承認済み・未実施）と同じ規則にし、暫定 snapshot だけの例外を作らない。人間の分類作業の途中で暫定 snapshot が黙って書き換わることもなくなる。**実装は R4 の PR の書き込み前検査に含める**。それまでの実装は存在を確かめずに上書きする（暫定 ID は内容から決まるので中身は同じで、変わるのは manifest の作成時刻 `created_at` だけである）。
...
| コマンド | 入力 | 出力・効果 |
|---|---|---|
| `odyssey-fx data accept --datasource <yaml> --calendar <yaml> --timeframes <yaml> --symbols <dir> --out data/snapshots/` | 原ファイルと設定 | 第4節 1〜8 を実行し、`data/snapshots/_pending/<provisional_id>/` に暫定 manifest・検査結果・partition を書く。`provisional_id` を表示。**同じ `provisional_id` の暫定ディレクトリが既にあれば、何も書かずに失敗する**（v1.11【確定】、第3.7.2節。実装は R4 の PR） |
