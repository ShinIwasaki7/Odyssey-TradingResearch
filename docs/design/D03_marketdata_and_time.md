...
| 暫定 | **要決定（R1-D03-1）** | → 確定・未承認。最終 `snapshot_id` を計算しディレクトリを `<snapshot_id>/` へ移す（第3.7.1節の2、第4節 9） | 変化なし。暫定 snapshot は失敗時に変更されない（第4節「再実行の入力」）。人間は分類を直して同じ暫定 snapshot に対して再実行する | 拒否。暫定 snapshot は承認できない（第3.7.1節の3、第10節） | 拒否（`SnapshotNotApproved`。暫定は常に読めない。第3.7.1節の1・3） |
...
| 承認済み | 変化なし（同上） | 拒否（同上。第10節） | 変化なし（同上） | **要決定（R1-D03-2）** | 変化なし。読める。ただし読める partition は構築時の許可集合に限る（第3.8節・第6.1節）。読み取り関門は2つの検査報告と分類の対応を検証する（第4節「再実行の入力」） |

- **R1-D03-1**: 同じ原ファイル・設定で受入れを再実行し、同じ `provisional_id` の暫定ディレクトリが既にあるとき、上書きするか失敗するかが書かれていない。成果物の書き込みを「存在すれば失敗」に統一する根本対処（[AGENTS.md の設計](agents_md_for_codex_review.md) §5 の R4、承認済み・未実施）が `marketdata.adapters.parquet_store` を対象にしているので、その PR で決めるのが自然である。
- **R1-D03-2**: 承認済みの snapshot をもう一度承認したとき（`approval` の上書き・追記・拒否のどれか）が書かれていない。`approval` は `snapshot_id` の対象外なので識別子は変わらないが、manifest の内容は変わる。
...
- `SEALED` の読み取りは、許可発行 → 消費記録の追記 → origin への push 成功 → データ公開の順で fail-closed に行い、push 成功前にデータを返さない。push の non-fast-forward 拒否を compare-and-set として使い、複数クローン間の重複消費を防ぐ。origin に到達できなければ `SEALED` は読めない（ADR-0014 2026-09-20 追記。gate の実装は **D09（段階5）**。v1.11 で D07 から移した（D07 §24）。本パッケージは `access_log.jsonl` の追記と状態導出、許可 partition 集合の受け取りを担う）。
...
- **段階2 の実行と評価が読めるのは研究履歴（`RESEARCH_HISTORY`）だけである**（v1.5、2026-09-22 の人間の決定）。合成が as-of ビューへ渡す許可 partition 集合に、封印期間（`LEGACY_HOLDOUT` の `SEALED`）と未分類の隔離期間を1件も入れない。封印期間を読むための許可判定（`holdout_gate`）は**段階5（D09）で足す**（v1.11。当初は段階4 の予定だった。D07 §24）。それまで段階2〜4 の実行は封印されたデータへ触れる経路を1本も持たない。
...
- `LEGACY_HOLDOUT` の partition は `evaluation.application.holdout_gate` を通過した場合だけ `allowed_partitions` に入る（gate の設計は D09・段階5。v1.11）。`QUARANTINED_UNASSIGNED` の partition は、別 ADR による再分類が行われるまで**いかなる経路でも** `allowed_partitions` に入らない（gate も許可を発行しない）。
