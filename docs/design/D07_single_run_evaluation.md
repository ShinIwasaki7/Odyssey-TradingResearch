...
|---|---|---|---|
| 1 | 入力契約 | 入力の範囲（Q1。推奨案は `BacktestResult` / `RunManifest` / trace の9表の3つに限ること）、読む列と用途、読まない6表、読み出しのポートの操作（第4節） | 遅延シナリオ4ケースの比較（**段階3・D08**）。**Q1 で推奨案が採られた場合に限り**、snapshot 参照経由の市場データ読込と holdout の許可（**本書 v0.2・段階4**） |
| 2 | 指標 | 段階2の最小集合15件の式・入力列・単位・丸め・欠損時の扱いと、そのうち T01 で検算できる13件の検算値（第5節） | 年率化・リスク調整指標・分布指標（**本書 v0.2・段階4**）、複数 run の集約と選定（**段階5・D09**）、最大ドローダウン2件（#5・#6）の検算値（**D06 §8.1 の改訂依頼2 の後、本書 v0.1 の改訂**。第5.4節） |
...
- **評価は生の市場データを読み直さない**【提案】（Q1、推奨案）。段階2の指標15件はすべて trace と manifest から作れる（第5.2節の各行の「入力列」）。市場データを読む経路を作ると、as-of の規則（D03 §6）とアクセス分類の許可（D03 §6.1 の `allowed_partitions`）を評価側にも置くことになり、D03 が正本である規則が2か所に割れる。
...
2. **snapshot 参照を経由してだけ読む**: 読み出しの契約と holdout の許可が本書 v0.1 の対象に入り、段階2の開始前に第1.2節と第4節を書き足すことになる。
3. **実験設定で選べるようにする**: 選択肢2 の書き足しに加え、実験ごとに入力の範囲が変わる。

推奨理由: 段階2の指標15件はすべて trace と manifest から作れ、読まなければ as-of の規則とアクセス分類の規則が D03 の1か所に保たれる。

選択肢2・3 を選んだ場合、**その契約を書き足すまで ADR-0016 条件2（段階2の開始前に D07 の評価境界を確定する）を満たさない**。第1.2節の行1 は推奨案を前提に書かれており、決定に応じて同じ PR で書き換える。
