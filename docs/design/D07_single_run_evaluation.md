...
- **R1-D07-3**: 保存先に評価の成果物が既にあるときの扱い（失敗か置換か）。第8.2節は識別子ごとに保存先を分けることだけを定める。承認済み・未実施の根本対処 R4（成果物の書き込みを「存在すれば失敗」に統一する）の対象である。→ **決定（v2.3、R4）【確定】: 存在すれば、何も書かずに失敗する**（暫定 snapshot の R1-D03-1、run の成果物の R1-D06-2 と同じ規則）。評価は置換の指示を持たない（第8.2節）。第19.6節の再利用の規則をこれに合わせた（再利用の判断は書き込みの前に行い、書き込みの失敗を既存の成果物の検出に使わない）。
