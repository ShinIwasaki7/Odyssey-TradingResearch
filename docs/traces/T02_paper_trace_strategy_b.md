...
| **v1.3**（2026-09-25、PR #R1PR） | 設計文書の必須表（R1。全体計画書 §8.5）に合わせ、経路ごとに通る状態機械のマスの対応表を第21節に足した。経路・検算値・結論は変えない | R1 の承認（**済**。PR #26） |
...
遷移の番号は、取引機会が D05 §7.2 の遷移1〜12、注文が D06 §5.1 の遷移1〜6 である。評価要求の待機は D05 §6.8・§6.10 の待機の出来事（`WaitEventKind`、D05 §3）の並びで書く。

| 経路 | 取引機会（D05 §7.2） | 注文（D06 §5.1） | 評価要求の待機（`WaitEventKind`） | 本書の節 |
|---|---|---|---|---|
| `01-06 15:00Z` の発火（v1.2 で追記） | 12 | なし | なし | 第1.6節・第20.2節 |
| 経路1: 遅延なしで確認成立 → 発注 → 約定 | 1 → 10 → 4 → 5（表3 の4行） | エントリー注文 1 → 2 | なし | 第3節・第13節 |
| 経路2: 開始足で未確認、次の15分足で成立 | 1 → （開始足で未確認。遷移なし）→ 10 → 4 → 5 | 経路1 と同じ（本節は差分だけを追う） | なし | 第4節 |
| 経路3: 確認期限に到達 | 1 → 11（`EXPIRED`） | なし | なし | 第5節 |
| 経路4: 確認待ちのあいだに日足の条件が崩れる | `OPEN` / `CONFIRMED` からの 8 は**到達しない**（期限が先に勝つ。決定 Q24） | なし | なし | 第6節 |
| 経路5: 日足が2秒遅れ、待機して再開する（ケース2） | 取引機会は生まれない（第7.3節、決定 Q23） | なし | `WAIT_STARTED` → `INPUT_ARRIVED` → `RESUMED`。run 末尾の日足だけ `RUN_END_CLOSED`（第20.3節） | 第7節・第20.3節 |
| 経路6: 待機期限を超え `Skipped` で決着する（ケース3） | 取引機会は生まれない（第20.3節） | なし | `WAIT_STARTED` → `DEADLINE_REACHED`。同じ判断時点の新しい要求は `WAIT_STARTED` | 第8節 |
| 経路7: 1時間足の待機要求が追い越される（ケース4） | 追い越し以後、経路9 の発火は起きない（第20.3節） | なし | `WAIT_STARTED` → `SUPERSEDED`。新しい要求は `WAIT_STARTED` | 第9節 |
| 経路8: 建玉の保有中のトレーリング | 通らない | 保護水準の到達による決済注文 1 → 2。損切りの更新は管理要求の適用（D06 §8.3。表12） | なし | 第10節 |
| 経路9: 市場状態が許さない発火 | 12 | なし | なし | 第11節 |
...
- 待機の出来事 `OPPORTUNITY_ENDED`: 本書 v1.0 の後に D05 v2.5 が足した種類であり、本書の経路は通らない。
