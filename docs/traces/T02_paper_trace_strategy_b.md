...
- **設計の選択を含まない未記述は、本書と同じ PR で D05 v2.1（必要に応じて D07）の改訂として反映した**。設計の選択を含むものは起草時には確定させず、第15節に要決定（Q23〜Q29）として残した（CLAUDE.md「前提を silent に決めない」）。**7件とも 2026-09-23 に人間が決定し（すべて選択肢1）、本書と D05 v2.2・D06 v1.6・D08 v1.1 へ反映済みである**（第15.1節）。**その反映の途中で新しく生じた1件（Q30）だけは確定させず、第15.2節に残した**。
...
**追えなかった点**: D05 §7.6 は「`Observation.freshness_time` と `observation_interval` を判断履歴に残す」と書くが、**残す先のフィールドが無い**。`OpportunityTransition` にも `ValidityRecheck` にもその欄は無い。実際には許可の出力記録が表1 に残っており、「**その発火の遷移記録の処理点より前に出た最後の出力**」として一意に復元できるが、それは本書が導いたことで文書には書かれていなかった。**判断時刻だけで絞ると足りない**。日足と1時間足が同じ判断時点で確定する場合（第3.1節・第7.2節）は、同じ `step` の P2（rank 6）で出たばかりの許可を P3（rank 7）が適用するので、判断時刻より前で切るとその更新を取りこぼす。**「前」の比べ方も1段では足りない**。出力記録は処理点を持たず（`decision_time` と `sequence` だけ）、`sequence` は `step` ごとに 0 へ戻るので、出力記録どうしを `(時刻, フェーズ順位, 通し番号)` で並べることはできない。比べ方は2段で、**(1) どの `step` の記録かを評価記録（表2）の起動条件の種類で決め**（同じ判断時点の3回の呼び出しは入力が重ならない。D06 §4.2 の表）、**(2) 同じ `step` の中では `sequence` の昇順**で決まる。本書の経路では `market_state` が第1回の `step` でしか出ないので、実際には `sequence` の昇順だけで足りる。→ 第14節 #2（設計の選択を含まないので D05 §7.6 に明記した。2段の比べ方は独立レビューの指摘を受けて v2.2 で明確化した）。
...
- `market_state` が待機に入るとき、足りない入力は `long_allowed` **だけ**であり、その接続元は**出力参照**である。`BarsDeadline(1)` を `WaitUntilBars(series=その入力の系列, remaining=1)` へ解決する規則（D05 §6.8）は市場データ参照を前提にしており、**出力参照の入力には系列が無い**。本書は「その使用箇所の起動条件の系列（日足）」として追ったが、規則は書かれていない。→ 第14節 #8、**決定 Q28（2026-09-23、選択肢1）**: **その使用箇所の起動条件（`OnBarClose`）の系列で数える**（D05 v2.2 §6.8）。本書が追ったとおりであり、`market_state` の期限は日足で数え、上流の `daily_ema` の期限と一致する。なお、**その使用箇所が系列の違う足の確定を2件以上宣言している場合の扱いは未決**（第15.2節の Q30）であり、検証戦略 B の該当2件はどちらも1系列でしか起動しないので**本書の追跡結果は変わらない**。
...
| 5 | 規則の欠落 | **足の確定で生まれた保護水準の更新（`UpdateStop`）を適用するフェーズが無い**。D06 §8.3 は `POST_FILL_EVALUATION` と定めるが、そのフェーズは受付通知か約定通知がある判断時点でしか動かない。D06 §4.2 の手順5 は第1回の `step` の管理要求を rank 10 へ渡すが、保護水準の更新は注文ではない | 経路8 | **決定 Q25**（2026-09-23、選択肢1）: 受付（rank 10）の直前に適用する。新しいフェーズは足さず、処理点のフェーズは `ADMISSION`（D06 v1.6 §4.2 の手順6・§8.3） |
...
| 8 | 規則の欠落 | 足りない入力が**出力参照だけ**の待機で、`BarsDeadline(n)` をどの系列の確定足で数えるかが決まっていない（出力参照の入力に系列が無い） | 経路5（`market_state`） | **決定 Q28**（2026-09-23、選択肢1）: その使用箇所の起動条件（`OnBarClose`）の系列で数える（D05 v2.2 §6.8）。複数系列で起動する使用箇所の扱いだけが未決（第15.2節の Q30） |
...
分類の内訳: 規則の欠落8件（#5・#8・#9・#10・#13・#16・#19・#20）、規則の帰結の未記述4件（#3・#14・#17・#18）、到達可否3件（#1・#6・#15）、順序の欠落2件（#11・#12）、型の不足1件（#2）、阻害要因1件（#4）、他文書への引き渡し1件（#7）。合計20件。**うち13件は設計の選択を含まないので本 PR の D05 v2.1・D07 v1.4 の改訂として反映し、7件を要決定（Q23〜Q29）として残した**。**7件とも 2026-09-23 に人間が決定し（すべて選択肢1）、同じ PR で D05 v2.2・D06 v1.6・D08 v1.1 へ反映した**（上の表の対処欄）。したがって**本書が見つけた20件はすべて対処済み**であり、残る未決は Q28 の反映の途中で新しく生じた Q30 の1件だけである（第15.2節）。

## 15. 要決定の一覧（Q23〜Q29 は決定済み、Q30 が未決）

番号は D05 の連番を継ぐ（Q22 まで決定済み。D05 §15）。起草時（v0.1）は7件とも確定させずに選択肢と推奨を並べ、**2026-09-23 に人間が7件すべてを決定した**。決定はいずれも**選択肢1（起草時の推奨）**である。その反映の途中で新しく生じた1件（Q30）だけを、確定させずに第15.2節へ残した。
...
### 15.2 Q30: 待機の本数期限を数える系列が、複数系列で起動する使用箇所では決まらない（**未決**）

Q28 の決定（その使用箇所の起動条件の系列で数える）を D05 §6.8 へ書くときに、**決定そのものでは決まらない場合が1つ残った**ので、確定させずに残す。

**何を決める必要があるか**: ある使用箇所が、**区間の同じ足の確定を2つ以上の系列について宣言している**とき（例: 同じ1時間の区間を持つ2系列）、待機の本数期限をどちらの系列で数えるか。区間が違う起動条件は区間ごとに別の評価要求になる（D05 §6.2 の手順3）ので数える系列は要求ごとに定まるが、**区間が同じ起動条件は1件の要求へ集約される**（同節）ため定まらない。起動系列がただ1つであることをコンパイル時に検査しているのは、確認部品（`execution_filter` 役割、D05 §5.6 の検査 b）と決済部品（`exit` 役割、検査 g）だけで、**市場状態や条件の使用箇所には検査が無い**。

**結果にどう影響するか**: 数える系列が変われば待機の期限に到達する判断時点が変わり、`DEADLINE_REACHED` が刻まれる時刻と、期限切れで見送った評価の件数（D07 §6.1 の集計）が変わる。**段階3 の実行結果は変わらない**。検証戦略 B で出力参照だけの待機が起きる使用箇所（`market_state`・`daily_above_ema`）はどちらも日足1系列でしか起動しないからである（第1.3節）。したがって Q30 は段階3 の実装を止めない。

| # | 選択肢 | 影響 |
|---|---|---|
| **1（推奨）** | **コンパイル時の検査を1件足す**。出力参照の入力に本数で数える待機期限を宣言する使用箇所は、足の確定の起動条件の系列がただ1つでなければならない（確認部品・決済部品に置いた検査 b・g と同じ形） | 検査が1件増え、D04 §12 への改訂依頼になる。**段階3 の宣言は1つも狭まらない**（該当する2件はどちらも1系列）。宣言から読めない失敗が実行時に出ることがなくなる |
| 2 | **集約した要求では、成立した起動条件のうち系列 ID の昇順で先頭の系列**で数える | 検査を増やさずに一意に決まる。ただし待機の期限の長さが、戦略の意図と無関係な系列名の並び順で決まることになり、判断履歴を読む人が理由を説明できない |
| 3 | **承認済みの設計を維持し**、系列が複数ある集約要求で本数期限を使う待機は**実行時に失敗させる**（`Failed(DATA_ERROR)`） | 文書の改訂が最小。ただし宣言から書けてしまう構成を run の途中まで見つけられず、D05 §5.6 が検査 f・g で採った「宣言時に止める」方針と逆になる |

推奨理由: 選択肢1 は、同じ問題（区間の違う起動条件が1つの使用箇所に2件あると結果が処理順に依存する）に対して D05 §5.6 が既に2回使っている手当てを、3か所目に当てるだけである。新しい考え方を持ち込まない。
...
| 版 | 追記する内容 | 前提 |
|---|---|---|
| ~~v0.2~~ → **v1.0**（2026-09-23、PR #24） | 要決定 Q23〜Q29 の決定を反映した（到達しない2経路の扱い、トレーリングの適用フェーズ、確認試行の受け渡し、欠損方針と待機の3規則）。**起草時に予定していた v0.2 の内容がそのまま決定の反映で埋まったので、v0.2 を置かずに v1.0 とした** | Q23〜Q29 の決定（**済**） |
| v1.1 | 段階3 の実装が出たあと、受入れテストの実測値と本書の検算値（第16節）を突き合わせ、食い違いがあれば本書側の誤りを直す | 段階3 の実装 |
| v1.2 | Q30（第15.2節）の決定を反映する。段階3 の実行結果は変わらないので、**この版を待たずに実装へ進める** | Q30 の決定 |
