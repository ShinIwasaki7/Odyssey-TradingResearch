...
| 種別 | 本書の番号の意味 |
|---|---|
| `OpportunityId` / `AttemptId` / `OrderId` / `PositionId` / `ReservationId` / `FillId` / `EvidenceId` | **実際の値である**。検証戦略 B は追跡した判断時点より前に取引機会も注文も建玉も1つも作らない（人工データが `01-07 09:00Z` まで突破を成立させない。第2.1節・第3.1節）ので、そこで採番される値が run の 1 件目になる |
