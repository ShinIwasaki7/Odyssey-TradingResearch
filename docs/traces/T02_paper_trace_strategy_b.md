...
- **設計の選択を含まない未記述は、本書と同じ PR で D05 v2.1（必要に応じて D07）の改訂として反映した**。設計の選択を含むものは確定させず、第15節に**要決定（Q23〜Q29）**として残した（CLAUDE.md「前提を silent に決めない」）。
...
| 経路 | 状態 | 節 |
|---|---|---|
| 経路1・経路5 のうち「**日足・1時間足・15分足がそろって終了する判断時点**で取引機会が生まれる」部分 | **到達しない**。突破水準が同じ判断時点で確定した日足そのものを含むため、その判断時点では1時間足の終値が日足高値を超えられない（第3.1節） | 第3.1節・第7.3節・要決定 Q23 |
| 経路4（確認待ちのあいだに日足の条件が崩れて終端する） | **到達しない**。確認期限（15分足4本＝1時間）と日足境界が1時間境界の上にあることから、日足の条件が変わりうる唯一の確認足が期限の足と重なり、期限が先に勝つ（第6節） | 第6節・要決定 Q24 |

どちらも**経路を追うのをあきらめて範囲を狭めたのではなく、宣言と規則から到達しないことが導けた**ものである。取引機会の生成そのものは、日足境界ではない1時間足の確定（第3.2節）で問題なく追えた。
...
ケース3 を**系列全体の固定遅延**にしてあるのは意図的である。ケース4 のように1本だけを遅らせると、期限に到達する判断時点（次の日足の予定境界）で**新しい日足が公開されて追い越しも同時に成立する**。その優先順位が決まっていないため（第14節 #12、要決定 Q29）、期限だけを追いたいケース3 では系列全体を遅らせて追い越しが起きないようにした。
...
| 9 | `P5_ORDER_INTENT` | 注文意図と保護水準が**確認結果の配送**で起動する。トレーリングの評価（`exit` 役割が足の確定で起動する）もここに来る（第14節 #5、要決定 Q25） |
...
| 区間（UTC） | `open` | `high` | `low` | `close` |
|---|---|---|---|---|
| `[01-06 18:15, 01-07 08:45)`（59本） | 149.245 | 149.280 | 149.200 | **149.245** |
...
`daily_ema` の窓が 60 本あることは第1.1節の被覆（`2014-09-30T22:00Z` 以降）で満たす。`2014-10-01` から `2015-01-06` までの平日は 69 日あり、週末は日足が存在しない（カレンダーが区間を持たない）ので 60 本を超える。したがって `WARMUP_INSUFFICIENT` にはならない。`window_bars >= 2 * period`（60 >= 40）はコンパイル時の検査 e（D05 §5.6）を通る。
...
**追えなかった点**: 確認試行（`ConfirmationAttempt`）は `OpportunityLifecycle` の中にしか無く、`RuntimeStepResult` に列が無い（D05 §3 の改訂は `wait_events` と `validity_rechecks` の2つを足しただけである）。**エンジンが表18 を書くための受け渡し経路が存在しない**。T01 §13 #19（`EntryProposal` に根拠の出力 ID が無く正常経路でも `OrderRequest` を組み立てられなかった）と同じ形の**阻害要因**である。→ 第14節 #4、**要決定 Q26**。
...
| 手順 | 値 |
|---|---|
| 1. 有効性の再検査（`OrderRequest` 生成直前。遷移8 の判定点） | `ValidityRecheck(00000001, OutputRef("daily_above_ema","condition"), REQUIRE_UNTIL_ORDER_REQUEST, at=ProcessingPoint(09:00Z, P5_ORDER_INTENT, 8), outcome=SATISFIED, output_id=…)` → 表19 |
...
| 10・11 | 参照価格は直前に完了した執行足 `[09:00,09:15)` の終値 bid `149.450` → ask `149.470`。約定は `[09:15,09:30)` の始値 |
...
**この経路は D05 §9.4 が段階3 で追うと宣言した9経路の1つなので、到達しないことは設計側の判断を要する**。→ 第14節 #6、**要決定 Q24**。
...
- `market_state` が待機に入るとき、足りない入力は `long_allowed` **だけ**であり、その接続元は**出力参照**である。`BarsDeadline(1)` を `WaitUntilBars(series=その入力の系列, remaining=1)` へ解決する規則（D05 §6.8）は市場データ参照を前提にしており、**出力参照の入力には系列が無い**。本書は「その使用箇所の起動条件の系列（日足）」として追ったが、規則は書かれていない。→ 第14節 #8、**要決定 Q28**。
...
したがって §9.3 のケース2 の例は、**待機と再開の仕組みは正しく描いているが、その結果として取引機会が生まれるという部分だけが成り立たない**。→ 第14節 #1、**要決定 Q23**。
...
**なぜケース3 を系列全体の遅延にしたか**（第1.4節の再掲）。1本だけを遅らせる `InjectedBarDelay` を使うと、`01-08 22:00Z` に D(Jan8) が公開されるので、**同じ判断時点で期限の到達と追い越し（D05 §6.10）の両方が成立する**。D05 §6.8 の手順2 は「期限と追い越しと失効を検査する」と並べるだけで、**同時に成立したときにどちらの記録が残るか**（`DEADLINE_REACHED` ＋ `Skipped` か、`SUPERSEDED` ＋ `Superseded` か）を決めていない。→ 第14節 #12、**要決定 Q29**。
...
| 3 | `OPPORTUNITY_TRANSITIONS` | 5（`00000001` の遷移1・10・4・5 と、`00000002` の遷移12） | `(opportunity_id, at)` |
...
分類は T01 §13 と同じ区分を使う。**対処の列に「D05 v2.1」「D07」とあるものは本書と同じ PR で反映済み**、「要決定」とあるものは第15節へ送った。
...
| 6 | 到達可否 | 確認待ちのあいだに日足の条件が崩れる経路（D05 §9.4 の経路4）が、`BarsDeadline(bars=4)` と日足境界が1時間境界上にあることから**到達しない**（期限が先に勝つ） | 経路4 | **要決定 Q24** |
| 7 | 他文書への引き渡し | 段階3 で `EvaluationOutcome` に区分が2つ増え（`Waiting` / `Superseded`）、表16〜19 が増えるが、D07 §4.2 の「読まない6表」の列挙と §6.1 の「評価の結果区分別の集計」が段階3 を想定していない。待機して決着した要求は**同じ `request_id` で2件の評価記録**になるため、評価記録を数えると要求を2回数える | 経路5・経路6 | **D07 §4.2・§12**: 読まない表に16〜19 を足し、新しい結果区分と「要求と評価記録の件数が一致しなくなること」を段階3 の引き渡しとして明記（段階2 の指標15件は変わらない） |
| 8 | 規則の欠落 | 足りない入力が**出力参照だけ**の待機で、`BarsDeadline(n)` をどの系列の確定足で数えるかが決まっていない（出力参照の入力に系列が無い） | 経路5（`market_state`） | **要決定 Q28** |
...
| 12 | 順序の欠落 | 同じ判断時点で待機要求の**期限到達と追い越しが同時に成立**したときに、どちらの記録が残るかが決まっていない（`DEADLINE_REACHED` ＋ `Skipped` か、`SUPERSEDED` ＋ `Superseded` か） | 経路6（1本だけを遅らせた場合） | **要決定 Q29** |
...
| 19 | 規則の欠落 | 1回の評価で複数の入力が欠け、それぞれの欠損方針（`on_missing`）が食い違うときにどの方針に従うかが決まっていない。`breakout_trigger` v2 は `level` が待機、`price` が見送りなので、日足と1時間足が同時に遅れると成立する | 経路5（宣言から導いた。本書の人工データでは1時間足を遅らせていないので実演していない） | **要決定 Q27** |
...
分類の内訳: 規則の欠落8件（#5・#8・#9・#10・#13・#16・#19・#20）、規則の帰結の未記述4件（#3・#14・#17・#18）、到達可否3件（#1・#6・#15）、順序の欠落2件（#11・#12）、型の不足1件（#2）、阻害要因1件（#4）、他文書への引き渡し1件（#7）。合計20件。**うち13件は設計の選択を含まないので本 PR の D05 v2.1・D07 の改訂として反映し、7件を要決定（Q23〜Q29）として残した**。

## 15. 要決定（Q23〜Q29）

番号は D05 の連番を継ぐ（Q22 まで決定済み。D05 §15）。**いずれも本書では確定させていない**。選択肢は3つ以内で、**先頭が本書の推奨**である。承認済みの設計を維持する選択肢を必ず含めてある。

### Q23: 日足境界で突破が成立しないことをどう扱うか

決めること: 検証戦略 B の突破水準が「いま終わったばかりの日足の高値」になる判断時点では突破が構造的に成立しない。D05 §9.3 のケース2 と §9.4 の経路1・5 の例文は、その判断時点で取引機会が生まれるとしている。

| # | 選択肢 | 影響 |
|---|---|---|
| **1（推奨）** | **宣言は変えず、D05 §9.3 のケース2 と §9.4 の経路1・5 の例文を「取引機会が生まれるのは日足境界ではない1時間足の確定である」に改める** | 部品カタログは11件のまま。遅延シナリオ2 の追跡対象は「日足の連鎖が待機して再開すること」までになり、取引機会の生成は別の判断時点で追う（本書がそうした）。段階3 の受入れテストの期待値もその形になる |
| 2 | `entry_trigger.level` を「**1本前の日足の高値**」にする（`level` を `HistoryWindow(BarsWindow(1), exclude_latest_bars=1)` で読む `breakout_trigger` v3 を段階3 のカタログへ足す） | 部品が1件増える（Q12 が決めた11件が12件になる）。日足境界でも突破が成立しうるので §9.3 の例文はそのまま使える。上位設計書 §4.3.14 の「当該足を含むか除くかを明示する」に沿う |
| 3 | **承認済みの設計と例文を維持し**、日足境界で突破が成立しないことを D05 §9.3 に到達可否として明記して受入れテストで確かめる | 文書の改訂が最小。ただし §9.3 のケース2 の例（T+2秒 に O1 を生成）は成り立たないままになるので、例文と到達可否の記述が同じ節で食い違う |

推奨理由: 検証戦略 B の意図（上位 §7.1「利用可能な日足高値」）は日中に前日の高値を使うことであり、**宣言はその意図どおりに動いている**。直すべきなのは例文が選んだ判断時点だけで、部品を増やす必要が無い。

### Q24: 確認待ち中の市場状態の失効（経路4）が到達しないことをどう扱うか

決めること: D05 §9.4 の経路4 は、確認期限が15分足4本＝1時間で、日足境界が1時間境界の上にあるため到達しない。

| # | 選択肢 | 影響 |
|---|---|---|
| **1（推奨）** | **D05 §9.4 の経路4 を「段階3 の宣言では到達しない」として到達可否の記述に変え、経路そのものは D08 の意味論テストで宣言を変えた小さな戦略で検証する** | 検証戦略 B の宣言も部品も変えない。ADR-0031 が求める再検査の経路は、束縛の再検査そのもの（`ValidityRecheck` の4区分）を意味論テストで確かめることで担保する |
| 2 | 検証戦略 B の確認期限を `BarsDeadline(bars=8)`（2時間）に変え、日足境界をまたぐ確認待ちが起きるようにする | 経路4 が到達するようになる。D05 §9.2 の宣言と、段階3 の受入れテストの期待値・人工データがすべて変わる。上位 §7.1 の「期限内に成立すれば発注」は期限の長さを指定していないので、宣言の変更は上位文書に反しない |
| 3 | **承認済みの設計を維持し**、経路4 を9経路の一覧に残したまま「到達したら追う」とする | 段階3 の完了条件（D05 §9.4 の9経路を追える）が、到達しない経路を含んだまま残る |

推奨理由: 到達しない理由は**期限の長さと日足境界の位置の組み合わせ**という偶然であり、検証戦略 B の設計の欠陥ではない。宣言を変えると受入れテストの人工データまで作り直しになる一方、得られるのは経路1本の実演だけである。

### Q25: 足の確定で生まれた保護水準の更新をどのフェーズで適用するか

決めること: トレーリングが第1回の `step`（rank 4〜9）で返した `UpdateStop` を、どのフェーズで建玉へ適用するか。D06 §8.3 の「適用フェーズは `POST_FILL_EVALUATION`」は、受付通知も約定通知も無い判断時点では働かない。

| # | 選択肢 | 影響 |
|---|---|---|
| **1（推奨）** | **D06 §4.2 の手順に「rank 10 の直前に、第1回の `step` が返した保護水準の更新を建玉へ適用する」を足す**（適用の内容と `effective_from` は D06 §8.3 のまま） | フェーズは増えない。適用の処理点は `ADMISSION`（rank 10）になり、表12 の主キー `(position_id, at)` はそのまま使える。同じ判断時点の決済要求との競合の規則（D06 §8.3）も、受付の前に解決される |
| 2 | 保護水準の更新のための**新しいフェーズを D06 §4.1 に足す**（rank 9 と rank 10 のあいだ） | フェーズ集合が15件から16件に増え、run manifest に記録するフェーズ集合と、D07 が読む `at_phase` の値域が変わる。処理点がどのフェーズのものか判断履歴から直接読める |
| 3 | **承認済みの設計を維持し**、rank 12（`POST_FILL_EVALUATION`）を**通知の有無によらず毎判断時点で実行する**ことにする | D06 §4.2 の「通知が1件もなければ第2回の `step` を呼ばない」は変えずに、フェーズだけを通す形になる。段階2 の挙動は変わらない（段階2 の第1回の `step` は保護水準の更新を返さない）が、rank 11 の約定処理の後に適用されるので `effective_from`（次の執行足）の意味が1本ずれる |

推奨理由: 更新は**受付より前に効いているべき**である。同じ判断時点で新しい注文を受け付ける場合、その注文のリスク審査は更新後の保護水準を見るのが自然であり、選択肢3 のように約定の後へ回すと、同じ判断時点の約定が古い損切り水準で判定される。

### Q26: 確認試行（D06 表18）をエンジンがどう受け取り、いつ書き出すか

決めること: `ConfirmationAttempt` は `OpportunityLifecycle`（`RuntimeState` の中）にしか無く、`RuntimeStepResult` に列が無い。

| # | 選択肢 | 影響 |
|---|---|---|
| **1（推奨）** | **`RuntimeStepResult` に `confirmation_attempts: tuple[ConfirmationAttempt, ...]`（既定は空）を足し、その `step` で作った・書き換えた試行だけを毎回返す。エンジンは主キー `(opportunity_id, bar_key)` で表18 の行を置き換える** | D05 §3 の型表と §6.2 の手順10、D06 §4.2 の手順5・8 と §9.1 の書き出し規則に追記。待機をはさんだ試行の `outcome` の置き換え（D05 §7.7）が、表への置き換えとしてそのまま写る |
| 2 | **run 末尾に `OpportunityLifecycle.attempts` をまとめて渡し、表18 を1回で書く**（表10・表11 と同じ扱い。D06 §9.2） | 毎 `step` の受け渡しが増えない。run 末尾の `step`（`is_run_end=True`）の戻り値に載せることになり、その `step` は「起動判定・評価を行わない」と定められている（D05 §6.1）ので、記録だけを返す例外がもう1つ増える。run が途中で失敗すると確認の経過が1行も残らない |
| 3 | **承認済みの設計を維持し**、表18 を**評価記録（表2）と遷移記録（表3）から導出できる派生表**と位置づけて、エンジンが書くのをやめる | 新しい受け渡しが要らない。ただし `SKIPPED` / `WAITING` / `SUPERSEDED` の試行は評価記録から、`CONFIRMED` / `NOT_CONFIRMED` は出力記録から読む必要があり、D06 §9.2 が表18 を独立させた理由（「機会1件につき期限までの本数だけ並ぶため表3 に収まらない」）と噛み合わない |

推奨理由: 選択肢1 は、D05 §7.7 が既に決めている「同じ1件の `outcome` を置き換える」という規則を、そのまま表の置き換えとして実装できる。

### Q27: 1つの評価で欠損した入力の欠損方針が食い違うときにどうするか

決めること: 1回の評価で2つ以上の入力が欠け、それぞれの `on_missing` が違う（例: 一方が `WaitForInput`、他方が `SkipEvaluation`）ときに、どの方針に従うか。`breakout_trigger` v2 は `level` が `WaitForInput`、`price` が `SkipEvaluation` なので、1時間足と日足が同時に遅れると成立する。

| # | 選択肢 | 影響 |
|---|---|---|
| **1（推奨）** | **強い方針が勝つ順序を1つ決める（`Error` > `SkipEvaluation` > `WaitForInput` > `UsePrevious`）** | 「待っても解消しない入力があるなら待たない」という読み方になり、無駄な待機が判断履歴に並ばない。D05 §6.3 に1行足すだけで済む |
| 2 | **入力ごとに方針を適用し、待機できる入力があれば待機に入る**（待機中は他の入力の見送りを保留する） | 遅れて届く可能性のある入力を最大限待つ。ただし「待っても解消しない入力」があるまま期限まで待つことになり、D05 §6.8 が待機できる欠損理由を絞った意図（run が期限まで無駄に止まらないようにする）と逆になる |
| 3 | **承認済みの設計を維持し**、方針が食い違う構成を**コンパイル時に拒否する**（同じ使用箇所の入力は同じ欠損方針でなければならない） | 実行時の規則が増えない。`breakout_trigger` v2 は `price` も `WaitForInput` を持つ版に作り直すことになり、D05 §9.2 の宣言が変わる |

推奨理由: 選択肢1 は、D05 §6.8 が既に「待っても解消しない欠損理由では待機に入らない」と決めた考え方を、入力の組にそのまま広げたものである。選択肢3 は安全だが、契約の版が増える。

### Q28: 出力参照だけが欠けた待機で、本数の期限をどの系列で数えるか

決めること: `BarsDeadline(n)` は `WaitUntilBars(series=その入力の系列, remaining=n)` へ解決すると定められている（D05 §6.8）が、足りない入力の接続元が**出力参照**のときは系列が無い。検証戦略 B では `market_state` がこれに当たる。

| # | 選択肢 | 影響 |
|---|---|---|
| **1（推奨）** | **その使用箇所の起動条件（`OnBarClose.series`）の系列で数える** | 検証戦略 B では日足になり、上流（`daily_ema`）の期限と一致する。起動条件が1系列であることは `execution_filter` と `exit` の役割についてはコンパイル時に検査される（D05 §5.6 の検査 b・g）が、**それ以外の使用箇所については検査が無い**ので、複数系列で起動する使用箇所の扱いを別に決める必要がある |
| 2 | **上流をたどって、最初に見つかった市場データ参照の系列で数える** | 連鎖の起点と同じ系列になるので、日足の連鎖3段の期限が必ず揃う。たどる規則（複数の上流があるときにどれを選ぶか）を新しく決める必要がある |
| 3 | **承認済みの設計を維持し**、出力参照だけが欠けた待機では**本数の期限を使えない**ことにしてコンパイル時に拒否する | 実行時の規則が増えない。検証戦略 B の `price_compare` v2 と `permission_from_condition` v2 は期間で数える期限（`DurationDeadline`）を使うことになるが、**期間値を含む宣言はダイジェストを計算できない**（D05 §5.5）ので実験として固定できなくなる。事実上この選択肢は取れない |

推奨理由: 選択肢1 は、その使用箇所が「どの足のリズムで動くか」をいちばん素直に表す。選択肢2 は結果が同じになる場面が多いが、たどる規則を新しく作る必要がある。

### Q29: 同じ判断時点で待機要求の期限到達と追い越しが同時に成立したときにどちらが勝つか

決めること: D05 §6.8 の手順2 は「期限と追い越しと失効を検査する」と並べるだけで、同時に成立したときの優先順位を決めていない。記録が変わる（`DEADLINE_REACHED` ＋ `Skipped` か、`SUPERSEDED` ＋ `Superseded` か）ので、評価の結果区分別の集計（D07 §6.1）の値が変わる。

| # | 選択肢 | 影響 |
|---|---|---|
| **1（推奨）** | **D05 §6.8 の手順2 が挙げた順（期限 → 追い越し → 失効）を判定順として明記する** | 期限が勝つ。「待てる時間が尽きた」ほうが「新しい問いに置き換わった」より根本的な終わり方であり、確認期限が確認の評価に勝つ規則（D05 §7.7 の「判定の順序」）と向きが揃う |
| 2 | **追い越しを先に判定する** | 追い越しが勝つ。新しい足の要求が既に立っているのだから、古い要求は「期限切れ」ではなく「置き換えられた」と記録するほうが実態に近い、という読み方になる |
| 3 | **承認済みの設計を維持し**、同時成立が起きない範囲でだけ段階3 を進める（遅延シナリオの設定で避ける） | 本書のケース3 がこの扱いである（系列全体の遅延にして追い越しを避けた）。ただし実データの公開遅延は選べないので、段階4 以降で必ず戻ってくる |

推奨理由: 選択肢1 は、既に書かれている並びをそのまま規則にするだけで新しい規則を足さない。また、期限は宣言（`WaitForInput.deadline`）が決めた上限であり、追い越しは市場データの到着順が決める事象なので、**宣言が決めたほうを優先する**ほうが判断履歴から読みやすい。
...
| run 区間 | `Interval[2015-01-04T22:00Z, 2015-01-16T22:00Z)`（T01 と同じ。12日 = 1,036,800 秒） |
| 公開関数 | `bars_for(timeframe_id, definition, calendar, window=RUN_INTERVAL)`、`csv_text(bars)`、`series_of(timeframe_id)`、`apply_delay(bars, scenario)` |
...
| 取消理由を trace できる | 第5節（`EXPIRED`）、第9節（`REQUEST_SUPERSEDED`）、第11節（`MARKET_STATE_INVALIDATED`）。**ただし確認試行（表18）の受け渡し経路が無い** | **部分的**（要決定 Q26） |
| 遅延シナリオ別の差分を追跡できる | 第7節・第8節・第9節。4ケースの差が `WAIT_EVENTS`（表16）と評価記録の結果区分に現れることを追えた。**ただしケース2 で取引機会が生まれるという §9.3 の例は成り立たない** | **部分的**（要決定 Q23） |
| D05 §9.4 の9経路 | 経路1・2・3・5・6・7・8・9 は追えた。**経路4 は到達しない** | **部分的**（要決定 Q24） |
| トレーリングの適用 | 第10節。**適用フェーズが決まっていない** | **部分的**（要決定 Q25） |
...
| 版 | 追記する内容 | 前提 |
|---|---|---|
| v0.2 | 要決定 Q23〜Q29 の決定を反映する（到達しない2経路の扱い、トレーリングの適用フェーズ、確認試行の受け渡し） | Q23〜Q29 の決定 |
| v1.0 | 段階3 の実装が出たあと、受入れテストの実測値と本書の検算値を突き合わせて確定させる | 段階3 の実装 |
