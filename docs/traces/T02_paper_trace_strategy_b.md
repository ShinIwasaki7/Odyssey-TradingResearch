...
全体計画書 §8.5 は、状態機械や実行時の値の伝播を定める設計文書に3つの必須表を求め、紙上トレースには**表2（状態×出来事表）の代わりに経路ごとに通るマスの対応表**を、**表3（値の伝播表）の代わりに既存の「各表に残る行の一覧」**を置くとしている。本書は状態機械を定めず、D05・D06 の状態機械を1判断時点ずつ追う文書だからである。レビュー対象範囲は追跡する設計文書の §1.2（D05 §1.2・D06 §1.2）に従う。
...
上流の出力の保持（D05 §6.12.1）は、第12節（履歴窓をどこまで追えたか）が追った範囲を参照し、本表には列を置かない。

| 経路 | 評価要求（D05 §6.10.1） | 部品状態（D05 §6.5.1） | 取引機会（D05 §7.2） | 注文（D06 §5.1） | 予約（D06 §8.1.1） | 建玉（D06 §8.3.1） | 本書の節 |
|---|---|---|---|---|---|---|---|
| `01-06 15:00Z` の発火（v1.2 で追記） | E1→E2 | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 12 | 通らない | 通らない | 通らない | 第1.6節・第20.2節 |
| 経路1: 遅延なしで確認成立 → 発注 → 約定 | 足の確定ごとに E1→E2。確認評価の要求 E1→E2（`confirmed=True`。第3.3節）。注文意図と保護水準は確認結果の配送で E1→E2（第3.4節）。利確を出す部品が無いので管理要求は出ない（第3.5節） | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 1 → 10 → 4 → 5（表3 の4行） | エントリー注文: 生成前×出来事1 → 遷移1、`PENDING`×出来事4 → 遷移2 | 生成前×出来事1 → `HELD`、`HELD`×出来事3 → `TRANSFERRED`（第3.5節・第13節の表10） | 生成前×出来事1 → `OPEN`（初期の利確なし。第3.5節） | 第3節・第13節 |
| 経路2: 開始足で未確認、次の15分足で成立 | 開始足の確認評価は E1→E2（`confirmed=False`。`ConfirmationAttempt` は `NOT_CONFIRMED`、第4節）。次の15分足で E1→E2（`confirmed=True`） | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 1 → （開始足で未確認。遷移なし＝`OPEN`×O9）→ 10 → 4 → 5 | 経路1 と同じ（本節は差分だけを追う） | 経路1 と同じ | 経路1 と同じ | 第4節 |
| 経路3: 確認期限に到達 | 確認評価は4回とも E1→E2（`confirmed=False`。第5節） | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 1 → 11（`EXPIRED`） | 通らない | 通らない | 通らない | 第5節 |
| 経路4: 確認待ちのあいだに日足の条件が崩れる | 通らない（経路そのものが到達しない） | 通らない | `OPEN` / `CONFIRMED` からの 8 は**到達しない**（期限が先に勝つ。決定 Q24） | 通らない | 通らない | 通らない | 第6節 |
| 経路5: 日足が2秒遅れ、待機して再開する（ケース2） | 日足の連鎖の要求は E1→E5（`WAIT_STARTED`）→ E9（`INPUT_ARRIVED`）→ E10（`RESUMED`）→ E2。遅れていない部品は E1→E2（第7.1節・第7.2節）。run 末尾の日足だけ E14（`RUN_END_CLOSED`。第20.3節） | 待機に入った `entry_trigger` は S6（変化なし）、再開して決着すると S8（`Evaluated` なので S2 と同じ）（第14節 #20）。run 末尾の日足の待機要求は S10（`Skipped` で決着。変化なし） | 取引機会は生まれない（第7.3節、決定 Q23） | 通らない | 通らない | 通らない | 第7節・第20.3節 |
| 経路6: 待機期限を超え `Skipped` で決着する（ケース3） | E1→E5（`WAIT_STARTED`）→ E11（`DEADLINE_REACHED`、`Skipped`）。連鎖の要求も同じ判断時点で E11。同じ判断時点の新しい要求は E1→E5（第8節） | 待機に入った要求は S6、期限で `Skipped` になると S8（S4 と同じ。変化なし） | 取引機会は生まれない（第20.3節） | 通らない | 通らない | 通らない | 第8節 |
| 経路7: 1時間足の待機要求が追い越される（ケース4） | E1→E5（`WAIT_STARTED`）→ E12（`SUPERSEDED`、`Superseded`）。押しのけた新しい要求は E1→E5（第9節） | 待機に入った `entry_trigger` は S6、追い越しで S7（変化なし）（第14節 #20） | 追い越し以後、経路9 の発火は起きない（第20.3節） | 通らない | 通らない | 通らない | 第9節 |
| 経路8: 建玉の保有中のトレーリング | `trailing` の要求は開いている建玉1件につき E1→E2（D05 §6.11。第10節の 10:00Z・11:00Z は出力なし、12:00Z は `UpdateStop`） | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 通らない | 損切りの到達による決済: 生成前×出来事3（エンジン生成の即時決済。遷移1・2 を1つの確定単位で） | 決済約定は `TRANSFERRED`×出来事5（変化なし。建玉割当を解放する） | `OPEN`×出来事7（`UpdateStop`。変化なし、`version=2`、rank 10 の直前に適用。第10節）。`OPEN`×出来事2 → `CLOSED`（`STOP_LOSS`、`13:15Z`） | 第10節 |
| 経路9: 市場状態が許さない発火 | E1→E2 | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 12 | 通らない | 通らない | 通らない | 第11節 |
