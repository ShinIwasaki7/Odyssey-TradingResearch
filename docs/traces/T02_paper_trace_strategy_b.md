...
| **v1.3**（2026-09-25、PR #40） | 設計文書の必須表（R1。全体計画書 §8.5）に合わせ、経路ごとに通る状態機械のマスの対応表を第21節に足した。経路・検算値・結論は変えない | R1 の承認（**済**。PR #26） |
...
全体計画書 §8.5 は、状態機械や実行時の値の伝播を定める設計文書に3つの必須表を求め、紙上トレースには**表2（状態×出来事表）の代わりに経路ごとに通るマスの対応表**を、**表3（値の伝播表）の代わりに既存の「各表に残る行の一覧」**を置くとしている。本書は状態機械を定めず、D05・D06 の状態機械を1判断時点ずつ追う文書だからである。レビュー対象範囲は、本節が表を参照する設計文書の §1.2（D03 §1.2・D05 §1.2・D06 §1.2・D07 §1.2）に従う。

### 21.1 経路ごとに通るマス

列は、本書の経路が通る設計文書の状態機械ごとに1つ置く。

- **評価要求**: D05 §6.10.1 の出来事 E1〜E15（「E1→E2」は、要求を作り `Evaluated` で決着したことを表す）。待機の出来事（`WaitEventKind`、D05 §3）の並びも併記する。
- **取引機会**: D05 §7.2 の遷移1〜12（対応するマスは D05 §7.2.1）。
- **注文**: D06 §5.1 の遷移1〜6（対応するマスは D06 §5.1.1。出来事の番号は同表の列）。
- **予約**: D06 §8.1.1 の出来事1〜7。
- **建玉**: D06 §8.3.1 の出来事1〜12。
- **部品状態**: D05 §6.5.1 の出来事 S1〜S11。本書は状態の値そのものを一部（`entry_trigger` の `ConditionState`）しか追っていないので、評価の結末から決まるマスを書く。待機・追い越しで閉じた評価で状態を更新しないこと（第14節 #20）は経路5・経路7 で確かめた。

- **出力の保持**: D05 §6.12.1 の出来事 H1〜H8。保持する出力参照は第12節の表の6つで、本数はすべて 1 である。
- **run**: D06 §10.6 の出来事（行）。

列の集合は、設計文書にある状態×出来事表のすべて（D03 §3.7.2・§3.8.1、D05 §6.5.1・§6.10.1・§6.12.1・§7.2.1、D06 §5.1.1・§8.1.1・§8.3.1・§10.6、D07 §10.1.1）と照合した。どの経路も通らない表は列を置かず、表の直後の一覧に理由とともに挙げる。

| 経路 | 評価要求（D05 §6.10.1） | 部品状態（D05 §6.5.1） | 取引機会（D05 §7.2） | 注文（D06 §5.1） | 予約（D06 §8.1.1） | 建玉（D06 §8.3.1） | 出力の保持（D05 §6.12.1） | run（D06 §10.6） | 本書の節 |
|---|---|---|---|---|---|---|---|---|---|
| `01-06 15:00Z` の発火（v1.2 で追記） | E1→E2 | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 12 | 通らない | 通らない | 通らない | 保持する6つの出力参照（第12節）は、足の確定で起動した評価が `VALUE` 出力を出すたびに H2（最初は空 → 1件で上限ちょうど、以後は上限ちょうどのまま古い1件を落とす）。H6 は通らない（読み方はすべて `LatestAvailable`。第12節） | 実行中×通常の判断時点（変化なし） | 第1.6節・第20.2節 |
| 経路1: 遅延なしで確認成立 → 発注 → 約定 | 足の確定ごとに E1→E2。確認評価の要求 E1→E2（`confirmed=True`。第3.3節）。注文意図と保護水準は確認結果の配送で E1→E2（第3.4節）。利確を出す部品が無いので管理要求は出ない（第3.5節） | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 1 → 10 → 4 → 5（表3 の4行） | エントリー注文: 生成前×出来事1 → 遷移1、`PENDING`×出来事4 → 遷移2 | 生成前×出来事1 → `HELD`、`HELD`×出来事3 → `TRANSFERRED`（第3.5節・第13節の表10） | 生成前×出来事1 → `OPEN`（初期の利確なし。第3.5節） | `01-06 15:00Z` の行と同じ | 同上 | 第3節・第13節 |
| 経路2: 開始足で未確認、次の15分足で成立 | 開始足の確認評価は E1→E2（`confirmed=False`。`ConfirmationAttempt` は `NOT_CONFIRMED`、第4節）。次の15分足で E1→E2（`confirmed=True`） | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 1 → （開始足で未確認。遷移なし＝`OPEN`×O9）→ 10 → 4 → 5 | 経路1 と同じ（本節は差分だけを追う） | 経路1 と同じ | 経路1 と同じ | `01-06 15:00Z` の行と同じ | 同上 | 第4節 |
| 経路3: 確認期限に到達 | 確認評価は4回とも E1→E2（`confirmed=False`。第5節） | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 1 → 11（`EXPIRED`） | 通らない | 通らない | 通らない | `01-06 15:00Z` の行と同じ | 同上 | 第5節 |
| 経路4: 確認待ちのあいだに日足の条件が崩れる | 通らない（経路そのものが到達しない） | 通らない | `OPEN` / `CONFIRMED` からの 8 は**到達しない**（期限が先に勝つ。決定 Q24） | 通らない | 通らない | 通らない | 通らない（経路そのものが到達しない） | 通らない（経路そのものが到達しない） | 第6節 |
| 経路5: 日足が2秒遅れ、待機して再開する（ケース2） | 日足の連鎖の要求は E1→E5（`WAIT_STARTED`）→ E9（`INPUT_ARRIVED`）→ E10（`RESUMED`）→ E2。遅れていない部品は E1→E2（第7.1節・第7.2節）。run 末尾の日足だけ E14（`RUN_END_CLOSED`。第20.3節） | 待機に入った `entry_trigger` は S6（変化なし）、再開して決着すると S8（`Evaluated` なので S2 と同じ）（第14節 #20）。run 末尾の日足の待機要求は S10（`Skipped` で決着。変化なし） | 取引機会は生まれない（第7.3節、決定 Q23） | 通らない | 通らない | 通らない | 待機中の評価は出力を出さないので H7（変化なし）。再開して決着した評価の出力は H2。run 末尾の待機要求の決着は H8（変化なし）。遅れていない部品の出力は `01-06 15:00Z` の行と同じ | 実行中×通常の判断時点（変化なし）。run 末尾は「実行中 × 通常の終了 → `COMPLETED`」（D06 §10.6）。ケース2 の取引の結果は遅延なしと同じで、評価の結果区分の集計も出る（第20.3節）ので、最終評価価格のある正常な終了である | 第7節・第20.3節 |
| 経路6: 待機期限を超え `Skipped` で決着する（ケース3） | E1→E5（`WAIT_STARTED`）→ E11（`DEADLINE_REACHED`、`Skipped`）。連鎖の要求も同じ判断時点で E11。同じ判断時点の新しい要求は E1→E5（第8節） | 待機に入った要求は S6、期限で `Skipped` になると S8（S4 と同じ。変化なし） | 取引機会は生まれない（第20.3節） | 通らない | 通らない | 通らない | 待機と期限で `Skipped` になった評価は H7（変化なし）。それ以外の出力は `01-06 15:00Z` の行と同じ | 実行中×通常の判断時点（変化なし） | 第8節 |
| 経路7: 1時間足の待機要求が追い越される（ケース4） | E1→E5（`WAIT_STARTED`）→ E12（`SUPERSEDED`、`Superseded`）。押しのけた新しい要求は E1→E5（第9節） | 待機に入った `entry_trigger` は S6、追い越しで S7（変化なし）（第14節 #20） | 追い越し以後、経路9 の発火は起きない（第20.3節） | 通らない | 通らない | 通らない | 待機と追い越しで閉じた評価は H7（変化なし）。押しのけた新しい要求が出す出力は H2 | 同上 | 第9節 |
| 経路8: 建玉の保有中のトレーリング | `trailing` の要求は開いている建玉1件につき E1→E2（D05 §6.11。第10節の 10:00Z・11:00Z は出力なし、12:00Z は `UpdateStop`） | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 通らない | 損切りの到達による決済: 生成前×出来事3（エンジン生成の即時決済。遷移1・2 を1つの確定単位で） | 決済約定は `TRANSFERRED`×出来事5（変化なし。建玉割当を解放する） | `OPEN`×出来事7（`UpdateStop`。変化なし、`version=2`、rank 10 の直前に適用。第10節）。`OPEN`×出来事2 → `CLOSED`（`STOP_LOSS`、`13:15Z`） | `01-06 15:00Z` の行と同じ（`trailing` の出力は保持の計画に載らない。第12節の表） | 同上 | 第10節 |
| 経路9: 市場状態が許さない発火 | E1→E2 | S1 の後、状態を持つ部品（`entry_trigger` の `ConditionState` など）の評価が `Evaluated` で決着するたびに S2 | 12 | 通らない | 通らない | 通らない | `01-06 15:00Z` の行と同じ | 同上 | 第11節 |

**本書のどの経路も通らないもの**:

- 評価要求の E3（欠損による見送り。評価記録には現れるが経路としては追っていない、第13節の表2）、E4・E7（評価の失敗）、E6（待てない理由の待機）、E8（系列の違う2つの参照の待機）、E12'（押しのける側の無い公開）、E13（取引機会の終端による決着。待機の出来事 `OPPORTUNITY_ENDED` は本書 v1.0 の後に D05 v2.5 が足した種類）、E15（run の失敗）。
- 取引機会の遷移2（`CONCURRENCY_LIMIT_REACHED`）: 許可されない発火は同時保持の枠を消費しないと述べるが（第11節の要点1）、遷移2 そのものを通る経路は無い。
- 取引機会の遷移3・6・7・9: 本書に経路が無い（理由の記述も無い）。
- 取引機会の遷移8: `OPEN` / `CONFIRMED` からの市場状態の失効は段階3 の宣言では到達しない（第6節、決定 Q24。再検査そのものは D08 の意味論テストで検証する）。
- 注文の遷移3〜6: 本書に経路が無い（理由の記述も無い）。
- 予約の出来事2（決済の受付）・4（エントリー注文の終端）・6（run_end）・7（再配送）: 本書に経路が無い。
- 建玉の出来事3〜6、8〜12: 本書に経路が無い（検証戦略 B は利確を出さず、建玉は run 末尾より前に閉じる）。
- 部品状態の S3（戻り値の検査違反・例外）、S5（欠損による失敗）、S9（同じ `step` の前の評価の失敗）、S11（run の失敗）: 本書に経路として追っていない。
- 出力の保持の H3（同じ足の2件目による置き換え）・H4（対象区間を持たない出力）: 本書に経路が無い。H5: 到達しない（`EVENT` の出力は保持しない。第12節）。H6（履歴窓での読み取り）: 検証戦略 B では1度も起きない（第12節の結論）。
- run の「通常の開始」（能力検査の合格）: 本書は能力検査を追わず、合格を前提として判断時点から始める。能力検査の不合格・停止判定・データの途切れ・保存先が既にある場合（D06 §10.6 の残りの行）: 本書に経路が無い。
- **表ごと通らないもの**:
  - snapshot の段階（D03 §3.7.2）: 本書は人工データから作った snapshot を前提とし（第16節）、受入れ・分類・承認の流れを追わない。
  - 封印期間の状態 `HoldoutState`（D03 §3.8.1）: 人工データの partition は研究履歴として作る（第16節、D08 §9.5 の規則1）。
  - 評価の段階（D07 §10.1.1）: 本書は判断履歴を作るまでを追い、評価の手順は追わない。各節の「D07 への影響」は指標・集計の値の変化を述べるだけである。

### 21.2 値の伝播の実例

値の伝播の実例は第13節（各表に残る行の一覧と ID 連鎖の確認）である。規則の正本は D05 §7.8・D06 §9.5 の値の伝播表（R1）であり、本書はそれを追加も変更もしない。
