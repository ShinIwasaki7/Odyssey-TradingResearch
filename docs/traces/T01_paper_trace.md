...
分類の内訳: 規則の欠落8件（#1・#4・#7・#8・#9・#10・#14・#20）、型の不足・不整合・表現5件（#5・#11・#16・#17・#21）、規則の重複1件（#2）、順序・保存形式の欠落2件（#6・#18）、到達可否の明示2件（#3・#15）、差し戻して決定済み2件（Q10・Q11＝#1 の一部と #12）、阻害要因で解消済み1件（#19）、仮置き1件（#13）。合計21件。**差し戻した2件は 2026-09-21 に決定され、阻害要因1件は D05 v1.2 の改訂で解消した。本書の時点で残る未解決はない。**
...
| v1.2 | 単一実行評価（D07）までの引き渡し（指標・診断・実験 manifest） | D07 の承認 |
