...
全体計画書 §8.5 は、状態機械や実行時の値の伝播を定める設計文書に3つの必須表を求め、紙上トレースには**表2（状態×出来事表）の代わりに経路ごとに通るマスの対応表**を、**表3（値の伝播表）の代わりに既存の「各表に残る行の一覧」**を置くとしている。本書は状態機械を定めず、D05・D06 の状態機械を1判断時点ずつ追う文書だからである。レビュー対象範囲は、本節が表を参照する設計文書の §1.2（D03 §1.2・D05 §1.2・D06 §1.2・D07 §1.2）に従う。本節の対応が誤っていれば、参照先の表を持つ文書の範囲として扱う。

### 15.1 経路ごとに通るマス

列は、本書の経路が通る設計文書の状態機械ごとに1つ置く。

- **評価要求**: D05 §6.10.1 の出来事 E1〜E15（「E1→E2」は、要求を作り `Evaluated` で決着したことを表す）。
...
- **部品状態**: D05 §6.5.1 の出来事 S1〜S11。検証戦略 A で状態を持つのは Trigger の再武装だけである（D04 §9.1）。本書は状態の値そのものを追っていないので、評価の結末から決まるマスだけを書く。
- **出力の保持**: D05 §6.12.1 の出来事 H1〜H8。検証戦略 A の保持計画は `breakout_level.level` と `stop_level.level` を各1件保持する（コンパイル結果の `output_retention`。D05 §6.12）。
- **run**: D06 §10.6 の出来事（行）。
...
|---|---|---|---|---|---|---|---|---|---|
| 経路1: 正常エントリー → 利確 | 1時間足の確定で E1→E2（第1回の `step`、第2.2節）。`POSITION_OPENED` で `take_profit` の要求 E1→E2（第2回の `step`、第2.5節） | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 1 → 4 → 5（表3 の3行） | エントリー注文: 生成前×出来事1 → 遷移1、`PENDING`×出来事4 → 遷移2（`EXECUTION_OPEN`）。利確: 生成前×出来事3（エンジン生成の即時決済。遷移1・2 を1つの確定単位で。`EXECUTION_BAR_COMPLETE`） | 生成前×出来事1 → `HELD`、`HELD`×出来事3 → `TRANSFERRED`（第2.3節・第2.4節・第10節の表10）。決済約定は `TRANSFERRED`×出来事5（変化なし） | 生成前×出来事1 → `OPEN`（第2.4節）。`OPEN`×出来事6（`SetTakeProfit`。変化なし、`version=2`。第2.5節）。`OPEN`×出来事2 → `CLOSED`（`TAKE_PROFIT`。第2.6節） | H1（run 開始で空）の後、保持本数の計画に載る2つの出力（`breakout_level.level`・`stop_level.level`。各1件。検証戦略 A の保持計画）が `VALUE` を出すたびに H2（1件目で上限ちょうど、以後は古い足を落として積む）。下流は最新1件だけを読むので H6 は通らない | 実行中×通常の判断時点（変化なし） | 第2節・第10節 |
| 経路2: 正常エントリー → 損切り | 経路1 と同じ | 経路1 と同じ | 経路1 と同じ | 経路1 と同じ（決済は損切りの到達） | 経路1 と同じ | 経路1 と同じ。最後は `OPEN`×出来事2 → `CLOSED`（`STOP_LOSS`） | 経路1 と同じ | 経路1 と同じ | 第3節 |
| 経路3a・3b: 足内で損切りと利確の両方に触れる | 経路1 と同じ | 経路1 と同じ | 経路1 と同じ | 経路1 と同じ（決済は足内競合の解決） | 経路1 と同じ | 経路1 と同じ。最後は `OPEN`×出来事2（両側の到達。3a は下位足で `TAKE_PROFIT`、3b は `UNRESOLVED_SL_PRIORITY` で `STOP_LOSS`） | 経路1 と同じ | 経路1 と同じ | 第4節 |
| 経路4: 受付前拒否（`PROTECTION_INVALID`） | E1→E2 | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 1 → 4 → 6 | 生成前×出来事2（変化なし。注文を作らない） | 通らない（受付前拒否は予約を作らない。D06 §4.4） | 通らない | 経路1 と同じ | 経路1 と同じ | 第5節 |
| 経路5a: 取引機会の同時保持上限 | T_a で `breakout_level` は E1→E2、`stop_level` は E1→E3（`WARMUP_INSUFFICIENT` で `Skipped`）。役割出力が揃わず `EntryProposal` は作られない（第6.1節） | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 機会1 は 1 のあと `OPEN` のまま残り、経路8 で 9。機会2 は 2 | 通らない | 通らない | 通らない | `breakout_level.level` は H2。`stop_level` の評価は `Skipped` で出力を出さないので H7 | 経路1 と同じ | 第6.1節・第9.2節 |
| 経路5b: 建玉枠による受付前拒否（`RISK`） | E1→E2 | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 1 → 4 → 6 | 生成前×出来事2 | 通らない（新しい予約は作らない。既存建玉の割当は変わらない） | 開いている建玉はあるが、本表の出来事は起きない（受付前拒否は建玉に作用しない） | 経路1 と同じ | 経路1 と同じ | 第6.2節 |
| 経路6: 有効時間切れ | 通らない | 通らない | 通らない | 遷移3 は**到達しない**（受付時に期限内の候補を固定するため）。候補足が実際に無ければ遷移5（`DATA_ERROR`）になると述べるが、経路としては追っていない | 通らない | 通らない | 通らない（経路として追っていない。第7節） | 通らない（経路として追っていない。第7節） | 第7節 |
| 経路7: 週末持ち越し禁止（`CARRY_NOT_ALLOWED`） | E1→E2 | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 1 → 4 → 6 | 生成前×出来事2 | 通らない | 通らない | 経路1 と同じ | 経路1 と同じ | 第8節 |
| 経路8: run 末尾の残存処理 | 手順3 で E1→E2。末尾の `step` は `evaluations=()` を返し（第9.2節）、待機中の要求が無いので E14 のマスは通らない | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない。末尾の `step` は S10（変化なし） | run_end で出た `EntryProposal` は 4 → 6（手順4・4'）。残存機会（経路5a の機会1）は 9（手順6） | `21:45Z` の rank 13 で受け付けた決済注文: 生成前×出来事1 → 遷移1、`PENDING`（戦略の決済注文）×出来事8 → 遷移4（`CANCELED`、理由 `RUN_END`）（第9.1節）。手順4・4'' の要求は生成前×出来事2 | 決済の受付は `TRANSFERRED`×出来事2（変化なし。決済は予約を作らない、第9.1節）。末尾は `TRANSFERRED`×出来事6（変化なし。建玉割当を解放しない） | `OPEN`×出来事8（`ClosePosition()`。変化なし、決済要求を受付へ回す。第9.1節）。末尾は `OPEN`×出来事11（変化なし。未決済のまま MTM 評価、第9.3節） | 経路1 と同じ。末尾の `step` は H8（変化なし） | 実行中×通常の判断時点の後、run_end で実行中×通常の終了 → `COMPLETED`（手順1〜7 と末尾3集計を作る。第9.3節の `FinalSummaries`） | 第9節 |

**本書のどの経路も通らないもの**:

- 評価要求の E4〜E15（待機・追い越し・失敗の出来事）: 待機と追い越しは段階3 の範囲で、本書は追えない範囲として第11.2節に挙げた（[T02](T02_paper_trace_strategy_b.md) が追う）。評価の失敗（E4・E7）と run の失敗（E15）は経路として追っていない。
...
- 予約の出来事4（エントリー注文の終端による解放）: 段階2 の設定ではエントリー注文が判断時点をまたいで残らない（第9.1節）。出来事7（再配送）: 経路として追っていない。
- 建玉の出来事3・4（始値の gap・約定ずれ超過）、5（戦略の決済注文の約定）、7（`UpdateStop`。段階3）、9・10（同じ判断時点の競合・2件目の更新）、12（実行中の失敗）: 本書に経路が無い。
- 部品状態の S3・S5〜S9・S11（例外・欠損の失敗・待機・追い越し・再開・run の失敗）: 本書に経路が無い。待機と追い越しは段階3 の範囲である。
- 出力の保持の H3（同じ足の2件目）・H4（対象区間の無い出力）・H5（保持計画に載らない種類）・H6（履歴窓での読み取り）: 本書の2つの保持対象は判断時点ごとに `VALUE` を1件ずつ出し、下流は最新1件だけを読む（第1.3節の宣言）。H8 のうち run の失敗は経路に無い。
- run の「通常の開始」（能力検査の合格）: 本書は能力検査を追わず、合格を前提として判断時点から始める。能力検査の不合格・停止判定・データの途切れ・最終評価価格の無い終了・保存先が既にある場合（D06 §10.6 の残りの行）: 本書に経路が無い。
- **表ごと通らないもの**:
  - snapshot の段階（D03 §3.7.2）: 本書は受入れ・分類・承認を経た snapshot を前提とし（人工データ。第0節・第1.1節）、その流れを追わない。
  - 封印期間の状態 `HoldoutState`（D03 §3.8.1）: 本書が読むのは研究履歴の期間だけである（段階2 は研究履歴だけを読む。D03 §3.8）。
  - 評価の段階（D07 §10.1.1）: 本書は判断履歴を作るまでを追い、評価の手順は追わない。第9.4節は D07 の指標の検算値を与えるだけである。
