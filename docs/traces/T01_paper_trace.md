...
| v1.2 | 単一実行評価（D07）までの引き渡しの残り（指標の一覧・診断・実験 manifest） | D07 の承認 |

v1.1（2026-09-21、PR #17）では、上表の v1.2 のうち**含み損益の評価価格と `equity` の全値だけ**を前倒しした（第2.4節・第9.4節）。D07 の最大ドローダウンの検算値がこの2点に依存しており、D07 の承認と同じ PR で必要になったためである。
