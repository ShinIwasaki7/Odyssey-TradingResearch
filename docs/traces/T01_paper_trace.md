...
|---|---|---|---|---|---|---|---|
| 経路1: 正常エントリー → 利確 | 1時間足の確定で E1→E2（第1回の `step`、第2.2節）。`POSITION_OPENED` で `take_profit` の要求 E1→E2（第2回の `step`、第2.5節） | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 1 → 4 → 5（表3 の3行） | エントリー注文: 生成前×出来事1 → 遷移1、`PENDING`×出来事4 → 遷移2（`EXECUTION_OPEN`）。利確: 生成前×出来事3（エンジン生成の即時決済。遷移1・2 を1つの確定単位で。`EXECUTION_BAR_COMPLETE`） | 生成前×出来事1 → `HELD`、`HELD`×出来事3 → `TRANSFERRED`（第2.3節・第2.4節・第10節の表10）。決済約定は `TRANSFERRED`×出来事5（変化なし） | 生成前×出来事1 → `OPEN`（第2.4節）。`OPEN`×出来事6（`SetTakeProfit`。変化なし、`version=2`。第2.5節）。`OPEN`×出来事2 → `CLOSED`（`TAKE_PROFIT`。第2.6節） | 第2節・第10節 |
| 経路2: 正常エントリー → 損切り | 経路1 と同じ | 経路1 と同じ | 経路1 と同じ | 経路1 と同じ（決済は損切りの到達） | 経路1 と同じ | 経路1 と同じ。最後は `OPEN`×出来事2 → `CLOSED`（`STOP_LOSS`） | 第3節 |
| 経路3a・3b: 足内で損切りと利確の両方に触れる | 経路1 と同じ | 経路1 と同じ | 経路1 と同じ | 経路1 と同じ（決済は足内競合の解決） | 経路1 と同じ | 経路1 と同じ。最後は `OPEN`×出来事2（両側の到達。3a は下位足で `TAKE_PROFIT`、3b は `UNRESOLVED_SL_PRIORITY` で `STOP_LOSS`） | 第4節 |
| 経路4: 受付前拒否（`PROTECTION_INVALID`） | E1→E2 | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 1 → 4 → 6 | 生成前×出来事2（変化なし。注文を作らない） | 通らない（受付前拒否は予約を作らない。D06 §4.4） | 通らない | 第5節 |
| 経路5a: 取引機会の同時保持上限 | T_a で `breakout_level` は E1→E2、`stop_level` は E1→E3（`WARMUP_INSUFFICIENT` で `Skipped`）。役割出力が揃わず `EntryProposal` は作られない（第6.1節） | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 機会1 は 1 のあと `OPEN` のまま残り、経路8 で 9。機会2 は 2 | 通らない | 通らない | 通らない | 第6.1節・第9.2節 |
| 経路5b: 建玉枠による受付前拒否（`RISK`） | E1→E2 | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 1 → 4 → 6 | 生成前×出来事2 | 通らない（新しい予約は作らない。既存建玉の割当は変わらない） | 開いている建玉はあるが、本表の出来事は起きない（受付前拒否は建玉に作用しない） | 第6.2節 |
| 経路6: 有効時間切れ | 通らない | 通らない | 通らない | 遷移3 は**到達しない**（受付時に期限内の候補を固定するため）。候補足が実際に無ければ遷移5（`DATA_ERROR`）になると述べるが、経路としては追っていない | 通らない | 通らない | 第7節 |
| 経路7: 週末持ち越し禁止（`CARRY_NOT_ALLOWED`） | E1→E2 | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 1 → 4 → 6 | 生成前×出来事2 | 通らない | 通らない | 第8節 |
| 経路8: run 末尾の残存処理 | 手順3 で E1→E2。末尾の `step` は `evaluations=()` を返し（第9.2節）、待機中の要求が無いので E14 のマスは通らない | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない。末尾の `step` は S10（変化なし） | run_end で出た `EntryProposal` は 4 → 6（手順4・4'）。残存機会（経路5a の機会1）は 9（手順6） | `21:45Z` の rank 13 で受け付けた決済注文: 生成前×出来事1 → 遷移1、`PENDING`（戦略の決済注文）×出来事8 → 遷移4（`CANCELED`、理由 `RUN_END`）（第9.1節）。手順4・4'' の要求は生成前×出来事2 | 決済の受付は `TRANSFERRED`×出来事2（変化なし。決済は予約を作らない、第9.1節）。末尾は `TRANSFERRED`×出来事6（変化なし。建玉割当を解放しない） | `OPEN`×出来事8（`ClosePosition()`。変化なし、決済要求を受付へ回す。第9.1節）。末尾は `OPEN`×出来事11（変化なし。未決済のまま MTM 評価、第9.3節） | 第9節 |
