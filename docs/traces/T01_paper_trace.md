...
|---|---|---|---|---|---|---|---------|
| 経路1: 正常エントリー → 利確 | 1時間足の確定で E1→E2（第1回の `step`、第2.2節）。`POSITION_OPENED` で `take_profit` の要求 E1→E2（第2回の `step`、第2.5節） | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 1 → 4 → 5（表3 の3行） | エントリー注文: 生成前×出来事1 → 遷移1、`PENDING`×出来事4 → 遷移2（`EXECUTION_OPEN`）。利確: 生成前×出来事3（エンジン生成の即時決済。遷移1・2 を1つの確定単位で。`EXECUTION_BAR_COMPLETE`） | 生成前×出来事1 → `HELD`、`HELD`×出来事3 → `TRANSFERRED`（第2.3節・第2.4節・第10節の表10）。決済約定は `TRANSFERRED`×出来事5（変化なし） | 生成前×出来事1 → `OPEN`（第2.4節）。`OPEN`×出来事6（`SetTakeProfit`。変化なし、`version=2`。第2.5節）。`OPEN`×出来事2 → `CLOSED`（`TAKE_PROFIT`。第2.6節） | 本書では追っていない。本書は段階2 の設計（D05 v1.x）で書いており、出力の保持は段階3 で足した仕組み（D05 v2.0 §6.12、Q18 決定）である。検証戦略 B での扱いは [T02](T02_paper_trace_strategy_b.md) 第12節・第21.1節 | 実行中×通常の判断時点（変化なし） | 第2節・第10節 |
...
| 経路5a: 取引機会の同時保持上限 | T_a で `breakout_level` は E1→E2、`stop_level` は E1→E3（`WARMUP_INSUFFICIENT` で `Skipped`）。役割出力が揃わず `EntryProposal` は作られない（第6.1節） | S1（run 開始で初期値）の後、状態を持つ Trigger の評価が `Evaluated` で決着するたびに S2。値そのものは本書で追っていない | 機会1 は 1 のあと `OPEN` のまま残り、経路8 で 9。機会2 は 2 | 通らない | 通らない | 通らない | 経路1 と同じ | 経路1 と同じ | 第6.1節・第9.2節 |
...
| 経路6: 有効時間切れ | 通らない | 通らない | 通らない | 遷移3 は**到達しない**（受付時に期限内の候補を固定するため）。候補足が実際に無ければ遷移5（`DATA_ERROR`）になると述べるが、経路としては追っていない | 通らない | 通らない | 経路1 と同じ | 通らない（経路として追っていない。第7節） | 第7節 |
