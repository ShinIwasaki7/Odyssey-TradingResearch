...
|---|---|---|---|---|---|---|
| 1 | 2026-01-06T09:00Z `LEDGER_UPDATE` | 1,000,000 | なし | — | 0 | **1,000,000** |
...
| 4 | 2026-01-06T11:15Z `LEDGER_UPDATE` | 1,036,736 | なし（P1 決済済み） | — | 0 | **1,036,736** |
| 5 | P2 入場から 21:45Z までの各 snapshot | 1,036,706 | P2 買 30,000 @151.000 | 各足の終値（第9節の設定で 151.000 以上） | 0 以上 | 1,036,706 以上 |
| 6 | 2026-01-16T22:00Z `RUN_END`（最終） | 1,036,706 | P2 | `[21:45,22:00)` の終値 151.500 | +15,000 | **1,051,706** |
...
| 版 | 追記する内容 | 前提 |
|---|---|---|
| v1.1 | 検証戦略 B の時刻表（MarketState・後続確認・待機・再判定・トレーリング） | D05 v0.2 の承認 |
| v1.1 | 遅延シナリオ4ケース別の処理順の差分 | D08 |
| v1.3 | 単一実行評価（D07）までの引き渡しの残り（指標の一覧・診断・実験 manifest） | D07 の承認 |

v1.1（2026-09-21、PR #17）では、上表の最終行（D07 への引き渡しの残り）のうち**含み損益の評価価格と `equity` の全値だけ**を前倒しした（第2.4節・第9.4節）。D07 の最大ドローダウンの検算値がこの2点に依存しており、D07 の承認と同じ PR で必要になったためである。v1.2（2026-09-21、PR #18）は上表のどの行でもなく、段階2 の実装に合わせた通し番号の訂正である（第2.2節）。そのため上表の残りは v1.3 へ繰り下げた。
