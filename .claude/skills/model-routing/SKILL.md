...
ただし **実装の変更（`src/` の振る舞いを持つコードの編集）は、差分が小さくても Opus が行う**。
Sonnet は調査・分類・突き合わせまでで、実装には手を入れない。
唯一の例外は Haiku が行う機械的な修正（lint / format、命名・スタイルのみの P2 対応）で、
これは判断を含まないものに限る。
...
| 既存実装に、設計文書で確定した1フィールドを追加する（差分が明確） | Opus（**差分が小さくても実装の変更は Opus**） |
...
4. **1つの作業役に1つの設計文書（または1つの PR）**。終わったら捨てる。続きは `SendMessage` で同じ作業役を再開するより、新しい作業役に進捗ファイルを読ませる方を優先する（古い文脈を引きずらない）。
   ただし**未完了の PR を引き継ぐときは、進捗ファイルを読ませるだけでは足りない**。新しい作業役の worktree は `origin/main` の新しい基点から作られるため、その PR の未 merge のコミットが入っておらず、古い木を編集したり2本目のブランチを作ったりしてしまう。引き継ぐときは次のどちらかにする。
   - 既存の worktree がまだ残っているなら、**その作業ディレクトリと既存ブランチ名を指示に明記して**（新規に隔離させず）作業役を起動する。
   - 残っていないなら、**`EnterWorktree` の基点を PR の head ブランチにする**か、作業役の最初の手順として既存ブランチの checkout を指示する。どちらの場合も「新しいブランチを作らない・PR 番号は既存のものを使う」と明記する。
5. **並列は最大2**。設計承認と merge が人間待ちなので、それ以上は待ち行列が伸びるだけ。
6. **同じモデルで2回失敗したら1段上へ**。3回目を同じモデルで回さない。
   **Opus で2回失敗した場合は上がない**（Fable はコードを読まない・実装しない）。この場合はモデルを上げずに打ち切り、**失敗の原因の仮説・試したこと・止まっている箇所を添えて人間に報告する**（§4 の「失敗」様式）。設計が曖昧なことが原因であることが多いので、設計文書への差し戻しも選択肢に含める。
