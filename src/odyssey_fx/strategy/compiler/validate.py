...
def _check_required_inputs(
    ...
    **差異**: D04 §8 は「`required_inputs` のキー集合は使用箇所の起動条件名の集合と一致
    する」と書いているが、`required_inputs` を持つのは契約であり、起動条件名を選ぶのは
    使用箇所である（D04 §3.1）。文字どおり一致を要求すると、D05 §4.3 が `required_inputs`
    を書いていない段階2の5部品では、検証戦略 A（D05 §9）がコンパイルできない。そこで
    **キーはすべて宣言済みの起動条件を指していなければならない**（余計なキーを許さない）
    という形で実装している。
    """
    errors: list[CompileError] = []
