...
**制限**: 正規化エンコード（D02 §9.3 v1.6）は `timedelta` を符号化しない。したがって期間値
（`max_age`・`DurationWindow`・`DurationDeadline`）を含む宣言は、このダイジェストを計算できず
`KernelValueError` になる。これは確定した設計であり（D04 §13.2、D05 §5.5）、符号化規則を足す
かどうかは D02 の次回改訂へ引き渡してある。検証戦略 A（段階2）の宣言はいずれも期間値を含まない
ため、実行経路には現れない。
"""
