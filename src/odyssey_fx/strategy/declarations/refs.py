...
| `RuntimeInputRef` | エンジンが評価時点に供給する現在の建玉・口座 |

`RuntimeInputRef` は「共有された可変台帳への直接アクセス」ではなく、**エンジンが許可した
時点情報**である（上位設計書 §4.3.5）。初版の対象は建玉と口座の2つだけで、
...
class RuntimeTarget(Enum):
    ...
    初版は建玉（`POSITION`）と口座（`ACCOUNT`）の2つだけ。未約定注文
    （`PENDING_ORDER`）は**列挙に含めない**。列挙に置いてから能力検査で拒否する形にすると、
    段階2で供給しない情報を宣言でき、設定ファイルが通る範囲が型から読めなくなる。
    """
