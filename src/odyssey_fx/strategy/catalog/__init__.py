...
- `initial`: 部品テーブル `INITIAL_CATALOG`（段階2 の5部品と段階3 の登録）
- 段階3 で足した部品（D05 §2）: `features/ema`・`features/atr`（指標）、`conditions/compare`・
  ...
  D05 §9.2）を各モジュールに置く。段階3 の登録は、コンパイラが能力検査を外して段階3 の
  検査（D05 §5.6 の検査 a〜h）を備えた変更で `INITIAL_CATALOG` に載せた。実行時に動かす
  仕組み（待機・遡り・確認の経路など）はランタイム側の後続の変更で入る。
