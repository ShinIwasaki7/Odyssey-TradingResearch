...
def declared_implementation_ref(implementation_id: str, revision: int) -> ImplementationRef:
    """実装の同一性を表す参照を作る（D02 §9.2）。

    **仮置き**: 「登録済み実装の内容ハッシュ」をどう算出するかは D02 §9.2 も D05 §5.5 も
    定めていない。run 全体のソース内容は `CodeDigest`（D02 §9.4）が識別するが、部品ごとの
    指紋の作り方は未確定である。`strategy` に I/O を置けない（D01 §5）ため、ここでは
    **宣言した実装識別子と改訂番号**の正規化ダイジェストを使う。計算規則を変えたときは
    `revision` を上げる。これにより、解決済み設定の指紋（D05 §5.5 の3）が部品単位の改訂を
    識別できる。算出方法の確定は人間の決定を要する。
    """
