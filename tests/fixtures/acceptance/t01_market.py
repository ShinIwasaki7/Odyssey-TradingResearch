...
日付は T01 v1.3 の run 区間（`2015-01-04T22:00Z` から `2015-01-16T22:00Z`）と同じである。
T01 の起草時の日付は 2026 年1月だったが、2026 年以降は未分類の隔離期間であり**いかなる
経路でも読めない**（D03 §3.8、ADR-0014）ため、承認済み snapshot を経由する通しの検証を
その期間では構造的に行えなかった。2026-09-22 の人間の決定により、曜日の並びと冬時間の
条件が同じ 2015 年へ T01 の側を改めてある（`2015-01-04` は日曜、`2015-01-16` は金曜で、
どちらの年も1月は米国東部標準時。run 区間の長さ 12 日 = 1,036,800 秒も週の開閉の位置も
変わらず、価格・数量・損益・資産は日付に依存しないため、T01 の数値は1つも変わらない）。
